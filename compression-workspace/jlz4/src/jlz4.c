/*
  lz4_archive.c - LZ4-style compressor/decompressor with archive and listing
  - Single-file, no external libs
  - Supports:
      ./lz4tool -c <input> <output>    (output can be directory or single archive file ending .lz4)
      ./lz4tool -d <input> <output-root>
      ./lz4tool -l <archive.lz4>       (list archive contents)
  - Archive format: sequence of entries:
      [uint32 name_len][name bytes][uint64 orig_size][uint32 comp_size][comp_size bytes]
    where comp_size bytes are the same block-framed compressed bytes used previously.
  Compile:
    gcc -O3 -std=c11 -Wall -Wextra lz4_archive.c -o lz4tool
  Notes:
    - POSIX only (opendir/readdir). Adjust for Windows.
    - This is a compact educational implementation; not byte-for-byte compatible with official LZ4 frames.
*/
...
static void join_path(char *out, size_t outlen, const char *a, const char *b) {
    if (!a || a[0] == '\0') { strncpy(out, b, outlen-1); out[outlen-1] = '\0'; return; }
    size_t la = strlen(a);
    if (la + 1 + strlen(b) + 1 > outlen) { snprintf(out, outlen, "%s/%s", a, b); return; }
    strcpy(out, a);
    if (out[la-1] != '/') { out[la] = '/'; out[la+1] = '\0'; }
    strcat(out, b);
}
...
static void progress_update(uint64_t processed, uint64_t total) {
    ...
    printf("] %s / %s Elapsed: %s ETA: %s", hproc, htot, elapsed_s, eta_s);
    fflush(stdout);
...
static inline uint32_t hash4(const uint8_t *p) {
    uint32_t v; memcpy(&v, p, 4); v = v * 2654435761u;
...
static size_t compress_block(const uint8_t *in, size_t in_size, uint8_t *out, size_t out_capacity) {
    ...
    uint32_t orig = (uint32_t)in_size; memcpy(out, &orig, 4); out += 4;
    ...
    const uint8_t *ip = in; const uint8_t *in_end = in + in_size; const uint8_t *anchor = ip;

    while (ip + MIN_MATCH <= in_end) {
        uint32_t h = hash4(ip);
        uint32_t ref = hash_table[h];
        hash_table[h] = (uint32_t)(ip - in);
        size_t match_len = 0; size_t match_off = 0;
        if (ref != 0xFFFFFFFFu) {
            const uint8_t *rp = in + ref;
            if (ip > rp && (ip - rp) <= WINDOW_SIZE) {
                const uint8_t *p1 = ip; const uint8_t *p2 = rp; const uint8_t *mend = in_end;
                while (p1 < mend && *p1 == *p2) { p1++; p2++; }
                match_len = (size_t)(p1 - ip);
                if (match_len >= MIN_MATCH) match_off = (size_t)(ip - rp); else match_len = 0;
            }
        }
        if (match_len >= MIN_MATCH) {
            ...
            *out++ = (uint8_t)(off16 & 0xFF); *out++ = (uint8_t)((off16 >> 8) & 0xFF);
            if (ml >= 15) {
                size_t rem = ml - 15;
                while (rem >= 255) { *out++ = 255; rem -= 255; }
                *out++ = (uint8_t)rem;
            }
            ip += match_len; anchor = ip;
            const uint8_t *p = ip - (match_len - 1);
            while (p + MIN_MATCH <= in_end && p < ip) {
                uint32_t hh = hash4(p); hash_table[hh] = (uint32_t)(p - in); p++;
            }
        } else ip++;
    }
    ...
    *out++ = token;
    ...
    uint32_t comp_size = (uint32_t)(out - comp_size_ptr - 4);
    memcpy(comp_size_ptr, &comp_size, 4);
...
static size_t decompress_block(const uint8_t *in, size_t in_size, uint8_t *out, size_t out_capacity) {
    ...
    uint32_t orig; memcpy(&orig, in + 4, 4);
    uint32_t comp_size; memcpy(&comp_size, in + 8, 4);
    if (comp_size + 12 != in_size) return 0;
    size_t out_pos = 0;
    const uint8_t *ip = in + 12; const uint8_t *iend = in + in_size;
    while (ip < iend) {
        uint8_t token = *ip++;
        size_t literal_len = (token >> 4);
        if (literal_len == 15) {
            uint8_t s;
            do { if (ip >= iend) return 0; s = *ip++; literal_len += s; } while (s == 255);
        }
        if (ip + literal_len > iend) return 0;
        if (out_pos + literal_len > out_capacity) return 0;
        memcpy(out + out_pos, ip, literal_len); ip += literal_len; out_pos += literal_len;
        if (ip >= iend) break;
        if (ip + 2 > iend) return 0;
        uint16_t offset = (uint16_t)ip[0] | ((uint16_t)ip[1] << 8); ip += 2;
        ...
        for (size_t i = 0; i < match_len; ++i) out[out_pos + i] = out[ref + i];
        out_pos += match_len;
    }
...
/* Write a 32-bit little-endian integer to FILE* */
...
static int fread_u32(FILE *f, uint32_t *pv) {
    ...
    *pv = (uint32_t)b[0] | ((uint32_t)b[1]<<8) | ((uint32_t)b[2]<<16) | ((uint32_t)b[3]<<24); return 0;
}
static int fread_u64(FILE *f, uint64_t *pv) {
    uint8_t b[8]; if (fread(b,1,8,f)!=8) return -1;
    uint64_t v=0; for (int i=0;i<8;i++) v |= ((uint64_t)b[i]) << (8*i); *pv = v; return 0;
}

/* Compress a single file and write compressed blocks to FILE* archive.
   Writes per-file header with name_len, name, orig_size, comp_size (placeholder), then compressed bytes.
...
static int compress_file_to_archive(FILE *archive, const char *relname, const char *inpath, uint64_t *out_comp_size) {
    ...
    if (fseek(archive, 0, SEEK_CUR) != 0) { fclose(fin); return -1; } /* ensure writable */

    uint32_t name_len = (uint32_t)strlen(relname);
    if (fwrite(&name_len, 4, 1, archive) != 1) { fclose(fin); return -1; } /* little-endian raw write is fine on LE */
    if (fwrite(relname, 1, name_len, archive) != name_len) { fclose(fin); return -1; }
    /* orig size placeholder (uint64) and comp_size placeholder (uint32) */
    uint64_t orig_size = 0;
    /* compute orig_size by stat */
    struct stat st; if (stat(inpath, &st) == 0) orig_size = (uint64_t)st.st_size;
    ...
    if (fwrite_u32(archive, 0) != 0) { fclose(fin); return -1; } /* placeholder */
    ...
    while ((r = fread_all(fin, inbuf, BLOCK_SIZE)) > 0) {
        ...
        if (fwrite(&block_total, 4, 1, archive) != 1) { free(inbuf); free(outbuf); fclose(fin); return -1; }
...
/* Decompress a single file entry from archive (archive FILE* is positioned after header).
   comp_size bytes follow; this function reads them and writes decompressed output to outpath.
*/
static int decompress_file_from_archive(FILE *archive, const char *outpath, uint32_t comp_size) {
    ...
    while (remaining > 0) {
        ...
        if (fread(&block_total, 4, 1, archive) != 1) { fprintf(stderr, "archive corrupt (block header)\n"); free(inbuf); free(outbuf); close(fout); return -1; }
        remaining -= 4;
    ...
    free(inbuf); free(outbuf); close(fout);
    return 0;
}
...
static int compress_file_path(const char *inpath, const char *outpath) {
    ...
    free(inbuf); free(outbuf); fclose(fin); close(fout);
    return 0;
}
...
static int decompress_file_path(const char *inpath, const char *outpath) {
    ...
    while (1) {
        ...
        if (r == 0) break;
        if (r != 4) { fprintf(stderr, "corrupt input (block header)\n"); free(inbuf); free(outbuf); fclose(fin); close(fout); return -1; }
        if (block_total > BLOCK_SIZE + 64*1024) { fprintf(stderr, "block too large\n"); free(inbuf); free(outbuf); fclose(fin); close(fout); return -1; }
...
static int compute_total_size_recursive(const char *path, uint64_t *accum) {
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        *accum += (uint64_t)st.st_size; return 0;
    } else return 0;
}

static int compute_total_compressed_input_recursive(const char *path, uint64_t *accum) {
    /* For decompression progress when input is archive file(s), we sum input file sizes */
    return compute_total_size_recursive(path, accum);
}

/* Compute base root */
static int compute_base_root(const char *input, char *base_root, size_t blen) {
    struct stat st; if (lstat(input, &st) != 0) return -1;
    if (S_ISDIR(st.st_mode)) {
        strncpy(base_root, input, blen-1); base_root[blen-1] = '\0';
        size_t l = strlen(base_root); if (l > 1 && base_root[l-1] == '/') base_root[l-1] = '\0';
        return 0;
    } else {
        char tmp[PATH_MAX]; strncpy(tmp, input, sizeof(tmp)-1); tmp[sizeof(tmp)-1] = '\0';
        char *d = dirname(tmp); strncpy(base_root, d, blen-1); base_root[blen-1] = '\0'; return 0;
    }
}

static int make_relative(const char *base, const char *target, char *out, size_t outlen) {
    size_t bl = strlen(base); if (bl == 0) return -1;
    if (strncmp(base, target, bl) == 0) {
        const char *p = target + bl; if (*p == '/') p++;
        strncpy(out, p, outlen-1); out[outlen-1] = '\0'; return 0;
    }
    return -1;
}

/* Recursively compress into archive FILE* (single archive) */
static int process_compress_to_archive(FILE *archive, const char *inpath, const char *base_root) {
    struct stat st; if (lstat(inpath, &st) != 0) { fprintf(stderr, "stat '%s' failed: %s\n", inpath, strerror(errno)); return -1; }
    if (S_ISDIR(st.st_mode)) {
        DIR *d = opendir(inpath); if (!d) { fprintf(stderr, "opendir '%s' failed: %s\n", inpath, strerror(errno)); return -1; }
        struct dirent *ent;
        while ((ent = readdir(d)) != NULL) {
            if (strcmp(ent->d_name,".")==0 || strcmp(ent->d_name,"..")==0) continue;
            char child[PATH_MAX]; join_path(child, sizeof(child), inpath, ent->d_name);
        ...
        if (make_relative(base_root, inpath, rel, sizeof(rel)) != 0) {
            const char *base = strrchr(inpath, '/'); const char *fname = base ? base+1 : inpath; strncpy(rel, fname, sizeof(rel)-1); rel[sizeof(rel)-1]='\0';
        }
        printf("Archiving: %s\n", rel);
        uint64_t comp_size = 0;
        if (compress_file_to_archive(archive, rel, inpath, &comp_size) != 0) return -1;
        (void)comp_size;
        return 0;
    } else {
...
/* Recursively compress into separate .lz4 files (legacy) */
static int process_compress_recursive(const char *inpath, const char *base_root, const char *outroot) {
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        if (outroot && outroot[0] != '\0') {
            struct stat ost; if (stat(outroot, &ost) == 0 && S_ISDIR(ost.st_mode)) {
                char rel[PATH_MAX]; if (make_relative(base_root, inpath, rel, sizeof(rel)) == 0 && rel[0] != '\0') join_path(outpath, sizeof(outpath), outroot, rel);
                else { const char *base = strrchr(inpath, '/'); const char *fname = base ? base+1 : inpath; join_path(outpath, sizeof(outpath), outroot, fname); }
            } else { strncpy(outpath, outroot, sizeof(outpath)-1); outpath[sizeof(outpath)-1]='\0'; }
        } else { strncpy(outpath, inpath, sizeof(outpath)-1); outpath[sizeof(outpath)-1]='\0'; }
        size_t len = strlen(outpath);
        if (len + 5 >= sizeof(outpath)) { fprintf(stderr, "path too long\n"); return -1; }
        strcat(outpath, ".lz4");
        printf("Compress: %s -> %s\n", inpath, outpath);
        return compress_file_path(inpath, outpath);
    } else { fprintf(stderr, "Skipping non-regular: %s\n", inpath); return 0; }
}

/* Recursively decompress archive file entries into outroot */
static int process_decompress_archive(FILE *archive, const char *outroot) {
    while (1) {
        uint32_t name_len;
        if (fread(&name_len, 4, 1, archive) != 1) {
        ...
        char name[PATH_MAX]; if (fread(name,1,name_len,archive) != name_len) { fprintf(stderr, "archive corrupt (name)\n"); return -1; }
        name[name_len] = '\0';
        uint64_t orig_size; if (fread_u64(archive, &orig_size) != 0) { fprintf(stderr, "archive corrupt (orig_size)\n"); return -1; }
        uint32_t comp_size; if (fread_u32(archive, &comp_size) != 0) { fprintf(stderr, "archive corrupt (comp_size)\n"); return -1; }
        char outpath[PATH_MAX];
        if (outroot && outroot[0] != '\0') join_path(outpath, sizeof(outpath), outroot, name);
        else strncpy(outpath, name, sizeof(outpath)-1);
        printf("Extract: %s -> %s (orig %" PRIu64 ", comp %u)\n", name, outpath, orig_size, comp_size);
        if (decompress_file_from_archive(archive, outpath, comp_size) != 0) return -1;
    }
    return 0;
}
...
static int list_archive(const char *archive_path) {
    ...
    while (1) {
        ...
        if (name_len == 0 || name_len > PATH_MAX-1) { fprintf(stderr, "archive corrupt (name_len invalid)\n"); fclose(f); return -1; }
        char name[PATH_MAX]; if (fread(name,1,name_len,f) != name_len) { fprintf(stderr, "archive corrupt (name)\n"); fclose(f); return -1; }
        name[name_len] = '\0';
        uint64_t orig_size; if (fread_u64(f, &orig_size) != 0) { fprintf(stderr, "archive corrupt (orig_size)\n"); fclose(f); return -1; }
        uint32_t comp_size; if (fread_u32(f, &comp_size) != 0) { fprintf(stderr, "archive corrupt (comp_size)\n"); fclose(f); return -1; }
        /* skip comp_size bytes */
...
static void print_usage(const char *p) {
    ...
    fprintf(stderr, "  %s -c <input> <output>    # compress (output dir or single archive .lz4)\n", p);
...
int main(int argc, char **argv) {
    ...
    int do_c = 0, do_d = 0, do_list = 0;
    int argi = 1;
    if (strcmp(argv[argi], "-c") == 0) do_c = 1;
    else if (strcmp(argv[argi], "-d") == 0) do_d = 1;
    else if (strcmp(argv[argi], "-l") == 0) { do_list = 1; argi++; }
    else { print_usage(argv[0]); return 1; }

    if (do_list) {
        if (argi >= argc) { print_usage(argv[0]); return 1; }
        const char *archive = argv[argi];
    ...
    if (argi < argc) outroot = argv[argi];

    struct stat st;
    if (lstat(inpath, &st) != 0) { fprintf(stderr, "stat '%s' failed: %s\n", inpath, strerror(errno)); return 1; }

    char base_root[PATH_MAX];
    if (compute_base_root(inpath, base_root, sizeof(base_root)) != 0) { fprintf(stderr, "compute base root failed\n"); return 1; }

    /* compute total bytes for progress (sum of input bytes for compression; sum of input compressed bytes for decompression) */
    uint64_t total = 0;
    if (do_c) {
        if (S_ISDIR(st.st_mode)) {
        ...
        else if (S_ISDIR(st.st_mode)) { if (compute_total_compressed_input_recursive(inpath, &total) != 0) { fprintf(stderr, "failed to compute total size\n"); return 1; } }
        else { fprintf(stderr, "Input not file or directory\n"); return 1; }
    }

    g_total_bytes = total; g_processed_bytes = 0; gettimeofday(&g_start_time, NULL);
    progress_update(0, g_total_bytes);

    int rc = 0;
    if (do_c) {
        /* If output is a single .lz4 file and input is directory -> create archive */
        int out_is_file = 0;
        if (outroot && strlen(outroot) > 4 && strcmp(outroot + strlen(outroot) - 4, ".lz4") == 0) out_is_file = 1;
        if (out_is_file && S_ISDIR(st.st_mode)) {
            ...
            char outpath[PATH_MAX];
            if (outroot && outroot[0] != '\0') {
                struct stat ost; if (stat(outroot, &ost) == 0 && S_ISDIR(ost.st_mode)) { const char *base = strrchr(inpath, '/'); const char *fname = base ? base+1 : inpath; join_path(outpath, sizeof(outpath), outroot, fname); }
                else strncpy(outpath, outroot, sizeof(outpath)-1);
            } else strncpy(outpath, inpath, sizeof(outpath)-1);
            strcat(outpath, ".lz4");
            printf("Compress: %s -> %s\n", inpath, outpath);
        ...
        /* decompress */
        /* If input is an archive (we detect by trying to read headers), extract entries */
        if (S_ISREG(st.st_mode)) {
            /* try to open and peek: read first 4 bytes of first entry name_len */
            FILE *f = fopen(inpath, "rb");
            if (!f) { fprintf(stderr, "open '%s' failed: %s\n", inpath, strerror(errno)); return 2; }
            /* We attempt to parse as archive: read name_len (4 bytes) and check plausibility */
            uint32_t name_len;
            int is_archive = 0;
            if (fread(&name_len, 4, 1, f) == 1) {
            ...
            if (is_archive) {
                ...
                /* legacy single-file decompress */
                char outpath[PATH_MAX];
                size_t inlen = strlen(inpath);
                const char *suf = ".lz4";
                if (inlen > strlen(suf) && strcmp(inpath + inlen - strlen(suf), suf) == 0) {
                    strncpy(outpath, inpath, inlen - strlen(suf)); outpath[inlen - strlen(suf)] = '\0';
                } else snprintf(outpath, sizeof(outpath), "%s.decompressed", inpath);
                if (outroot && outroot[0] != '\0') {
                    struct stat ost; if (stat(outroot, &ost) == 0 && S_ISDIR(ost.st_mode)) {
                        const char *base = strrchr(outpath, '/'); const char *fname = base ? base+1 : outpath;
                        char tmp[PATH_MAX]; join_path(tmp, sizeof(tmp), outroot, fname); strncpy(outpath, tmp, sizeof(outpath)-1);
                    } else strncpy(outpath, outroot, sizeof(outpath)-1);
                }
