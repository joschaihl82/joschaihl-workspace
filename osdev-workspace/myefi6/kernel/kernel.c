...
int nvme_init(nvme_ctrl_t *ctrl) {
    ...
    ctrl->admin_q.sq_doorbell = (uint32_t*)(db_base);
    ctrl->admin_q.cq_doorbell = (uint32_t*)(db_base + d);
    ctrl->io_q.sq_doorbell    = (uint32_t*)(db_base + 2*d);
    ctrl->io_q.cq_doorbell    = (uint32_t*)(db_base + 3*d);
    ctrl->regs->cc.en = 1;
    while (!ctrl->regs->csts.rdy) { __asm__("pause"); }
    ...
    sqe->cdw10 = (NVME_QUEUE_ENTRY_COUNT-1)<<16|0x01; sqe->cdw11 = 0x01;
    *ctrl->admin_q.sq_doorbell = 1; nvme_wait(&ctrl->admin_q);
    ...
    sqe->cdw10 = (NVME_QUEUE_ENTRY_COUNT-1)<<16|0x01; sqe->cdw11 = 0x01|(0x01<<16);
