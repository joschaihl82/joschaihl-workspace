...
// On General Protection Fault (vector 13) the kernel will attempt to reboot.
...
/* Error-code stub macro (reads error code from stack into RSI) */
#define ISR_ERR_STUB(n) \
__attribute__((naked)) void isr_stub_##n(void) { \
    __asm__ volatile ( \
        "movq (%rsp), %rsi\n\t" /* error code -> rsi (stack still contains it) */ \
        "pushfq\n\t" \
        "pushq %rax\n\t" "pushq %rbx\n\t" "pushq %rcx\n\t" "pushq %rdx\n\t" \
        "pushq %rsi\n\t" "pushq %rdi\n\t" "pushq %rbp\n\t" \
        "pushq %r8\n\t" "pushq %r9\n\t" "pushq %r10\n\t" "pushq %r11\n\t" \
...
/* --- 8. Reboot helper --- */
/* Attempt multiple reset methods:
   1) Keyboard controller pulse (outb 0x64, 0xFE)
   2) Port 0xCF9 (reset control) write
   3) Force triple fault by loading a null IDT and issuing an interrupt
*/
static void reboot_system(void) {
    ...
    /* Try PCI/legacy reset via port 0xCF9 (some platforms) */
    outb(0xCF9, 0x06);

    /* Small delay loop to give hardware a moment */
    for (volatile int i = 0; i < 1000000; i++) { __asm__ volatile ("nop"); }

    /* If still running, force a triple fault: load an empty IDT and trigger an interrupt */
    struct { uint16_t limit; uint64_t base; } zero_idt;
    zero_idt.limit = 0;
    zero_idt.base = 0;
    __asm__ volatile ("lidt %0" :: "m"(zero_idt) : "memory");

    /* Trigger an exception; with a null IDT this should cause a triple fault and reset */
...
/* --- 9. C-level ISR/IRQ handler --- */
/* isr_handler_c: called with RDI=vector, RSI=errcode (0 if none) */
void isr_handler_c(uint64_t vector, uint64_t errcode) {
    if (vector < 32) {
        /* Exception: handle specially for General Protection Fault (13) */
        extern void printf(const char*, ...);
        const char *msg = exception_messages[vector];
        if (!msg) msg = "Unknown";

        /* If General Protection Fault, attempt reboot */
        if ((int)vector == 13) {
            /* Print a short message then reboot */
            printf("Exception %d: %s -- rebooting...\n", (int)vector, msg);
            /* Ensure buffers are pushed to VRAM before reboot attempt */
            swap_buffers();
            reboot_system();
            /* reboot_system should not return; if it does, halt */
            while (1) { __asm__("hlt"); }
        }

        /* For other exceptions, print and halt */
        printf("Exception %d: %s\n", (int)vector, msg);
        if ((int)vector == 14) { /* page fault: read CR2 */
            uint64_t cr2;
...
/* --- 13. Keyboard processing in main context --- */
