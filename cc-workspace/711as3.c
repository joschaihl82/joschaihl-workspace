 ...
 *           with GNU-as-like error messages (filename:line:col: error: ...)
 *
 ...
 * - Registers use AT&T style (e.g., %rax, %eax, %al, %xmm0, %cl).
 *
 * Build:
...
/* ---------------------------
   Comment stripping
   --------------------------- */
/* Remove C-style block and line comments in-place.
   Maintains state across lines via in_block pointer.
*/
static void strip_comments_inplace(char *buf, int *in_block) {
    ...
    while (*r) {
        ...
        if (r[0] == '/' && r[1] == '/') break; /* rest of line is comment */
        *w++ = *r++;
    }
    *w = '\0';
}
...
static const Insn insn_table[] = {
    ...
    { "movabs", 2, { K_IMM|K_REG64, K_REG64 }, 1, {0xB8,0x00,0,0}, 0 },
...
static Node *read_program(FILE *f){
    ...
    int lineno = 0;
    int in_block = 0; /* state for comments across lines */
...
static int handle_directive(Node *n){
    ...
    if(strcasecmp(n->mn,"quad")==0){
        ...
        char *ident=NULL; size_t ci=0;
        if(!parse_string_identifier(p, &ident, &ci)){ asm_error_loc(n->line, 1, ".quad expects identifier string"); return -1; }
        p += ci; while(isspace((unsigned char)*p)) p++;
        int64_t add=0;
        if(*p=='-'){ p++; Value v; if(!parse_immediate(p,&v,NULL)){ asm_error_loc(n->line, 1, ".quad offset parse error"); free(ident); return -1; } add = -v.num; }
    ...
    if(strcasecmp(n->mn,"file")==0){
        const char *p = n->args; Value v; if(!parse_immediate(p,&v,NULL)){ asm_error_loc(n->line, 1, ".file expects immediate"); return -1; }
        const char *q = p; while(*q && !isspace((unsigned char)*q)) q++; while(isspace((unsigned char)*q)) q++;
        char *s = decode_string_literal(q);
        if(!s){ asm_error_loc(n->line, 1, ".file expects string"); return -1; }
        free(s);
        return 0;
    }
...
static int parse_and_emit_instruction(Node *n, Buf *out){
    ...
    if((strcasecmp(mn,"movz")==0 || strcasecmp(mn,"movs")==0)){
        ...
        char sizech = *q;
        if(sizech!='b' && sizech!='w' && sizech!='l' && sizech!='q'){ asm_error_loc(n->line, 1, "movz/movs expects size specifier b/w/l/q"); return -1; }
        q++;
        while(isspace((unsigned char)*q)) q++;
        Operand memop; size_t cm=0;
        if(!parse_operand(q, &memop, &cm) || memop.type!=OT_MEM){ asm_error_loc(n->line, (int)(q - args) + 1, "movz/movs expects memory operand after size"); return -1; }
        q += cm; while(isspace((unsigned char)*q)) q++;
        if(*q!=','){ asm_error_loc(n->line, (int)(q - args) + 1, "movz/movs expects comma before register"); return -1; }
        q++; while(isspace((unsigned char)*q)) q++;
        Operand regop; size_t cr=0;
        if(!parse_operand(q, &regop, &cr) || regop.type!=OT_REG){ asm_error_loc(n->line, (int)(q - args) + 1, "movz/movs expects register after comma"); return -1; }
        Operand emit_ops[2]; emit_ops[0] = memop; emit_ops[1] = regop;
        const Insn *ins = find_insn(mn, 2, emit_ops);
        if(!ins){ asm_error_loc(n->line, 1, "No encoding for %s", mn); return -1; }
    ...
    if(!ins){ asm_error_loc(n->line, 1, "No matching BNF instruction or unsupported operand shapes for: %s %s", mn, args); return -1; }
    ...
    if((strcasecmp(mn,"jmp")==0 || strcasecmp(mn,"je")==0 || strcasecmp(mn,"jne")==0 || strcasecmp(mn,"jl")==0 || strcasecmp(mn,"jle")==0) && argc==1){
        if(ops[0].type==OT_IMM && ops[0].imm.is_label){
            ...
            asm_error_loc(n->line, 1, "Jump target must be a label per BNF: %s", args); return -1;
        }
    }
...
static Node *read_program_with_comments(FILE *f){
    return read_program(f); /* wrapper kept for compatibility */
}
...
int main(int argc, char **argv){
    ...
    if(resolve_relocs(&outbuf)!=0){ fprintf(stderr, "%s: relocation resolution failed\n", g_filename); return 1; }
