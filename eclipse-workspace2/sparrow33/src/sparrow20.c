 ...
 * x86-64 AT&T assembly with dynamic memory allocation for arrays.
 *
 * Build:
...
/* --- KONSTANTEN FÜR DYNAMISCHE ARRAYS --- */
#define ARRAY_INIT_CAPACITY 8 // Startgröße für neue dynamische Arrays
#define ARRAY_GROW_FACTOR 2  // Faktor, um den die Kapazität vergrößert wird (typischerweise 2)
...
static void collect_vardecls_r(Node *n, Node *fn) {
    ...
    if (n->kind == ND_VARDECL) {
        if (!find_local(fn, n->var_name)) {
            ...
            fn->local_count++;
            fn->locals[fn->local_count - 1] = lv;
        }
        // ND_VARDECL selbst ist eine Blatt-Operation, aber es könnte init-Ausdrücke haben,
        // die hier ignoriert werden, da wir nur Deklarationen sammeln.
        return;
    }
    ...
    switch (n->kind) {
        ...
        case ND_PROGRAM:
            ...
            break;
        case ND_IF:
            // Wir müssen auch die Bedingung prüfen, falls sie VARDECLs enthalten könnte (was sie im Moment nicht tut)
        ...
        case ND_WHILE:
            ...
            break;
        case ND_FOR:
            // KORREKTUR: Wir müssen den Initialisierungs-Statement (der ND_VARDECL sein kann) prüfen!
            collect_vardecls_r(n->init_stmt, fn);
            // Nur den Body durchsuchen
            collect_vardecls_r(n->then_branch, fn);
            break;
        default:
            // Alle anderen Knoten (ND_EXPR_STMT, ND_RETURN, ND_ASSIGN usw.) haben keine verschachtelten Deklarationen
            break;
    }
}
...
static void cg_emit_lvalue_addr(CodeGen *cg, Node *n) {
    if (n->kind == ND_IDENT) {
        ...
        // Objekt-Access: Lade die Adresse der geflatterten globalen Variable
        char buf[256];
        ...
        // 1. Berechne Index * 8 in %rax
        cg_emit_expr(cg, n->index);
        cg_emit(cg, "    shl $3, %%rax"); // Index * 8 (da 8-byte/64-bit Werte)
        cg_emit(cg, "    push %%rax");    // Offset speichern

        // 2. Basisadresse des Arrays in %rax holen (Der Wert der Array-Variablen ist der Heap-Pointer)
        if (n->target->kind != ND_IDENT) {
            die("array base must be a simple identifier in this simple compiler");
        }
        cg_emit_expr(cg, n->target); // Lädt den Wert (Heap-Adresse) des Arrays in %rax
...
static void cg_emit_expr(CodeGen *cg, Node *n) {
    ...
    case ND_CALL: {
        ...
        // --- DYNAMISCHE PUSH-OPERATION ---
        if (call->call_target->kind == ND_MEMBER_ACCESS &&
            strcmp(call->call_target->member_name, "push") == 0 &&
            call->arg_count == 1) {

            Node *array_ident = call->call_target->target;
            if (array_ident->kind != ND_IDENT) {
                die("push must be called on a simple array identifier: line %d", n->line);
            }

            char size_name[256], capacity_name[256];
            snprintf(size_name, sizeof(size_name), "%s_SIZE", array_ident->name);
            snprintf(capacity_name, sizeof(capacity_name), "%s_CAPACITY", array_ident->name);

            int size_gi = add_global(size_name);
            int capacity_gi = add_global(capacity_name);
            int array_gi = add_global(array_ident->name);
            int lid = ++cg->label_id;

            // 1. Lese aktuelle Größe (%r10) und Kapazität (%r11)
            cg_emit(cg, "    mov Lglob_%d(%%rip), %%r10", size_gi);     // %r10 = size
            cg_emit(cg, "    mov Lglob_%d(%%rip), %%r11", capacity_gi);  // %r11 = capacity

            // 2. Wert zum Pushen berechnen und sichern
            cg_emit_expr(cg, call->args[0]);
            cg_emit(cg, "    push %%rax"); // Wert auf Stack sichern

            // 3. Prüfe, ob Kapazität erreicht ist: if (%r10 == %r11)
            cg_emit(cg, "    cmp %%r11, %%r10");
            cg_emit(cg, "    jne .Lpush_store_%d", lid); // Wenn nicht gleich, springe zum Speichern

            // --- REALLOC WIRD BENÖTIGT ---
            cg_emit(cg, ".Lpush_realloc_%d:", lid);

            // a) Berechne neue Kapazität: %r11 = %r11 * ARRAY_GROW_FACTOR
            cg_emit(cg, "    mov $%d, %%r8", ARRAY_GROW_FACTOR);
            cg_emit(cg, "    imul %%r8, %%r11");

            // b) Lade alten Pointer in %rdi (Argument 1 für realloc)
            // KORREKTUR: %rdi = old_ptr
            cg_emit(cg, "    mov Lglob_%d(%%rip), %%rdi", array_gi);

            // c) Berechne neue Gesamtgröße in Bytes und speichere in %rsi (Argument 2 für realloc)
            // KORREKTUR: %rsi = new_size_in_bytes
            cg_emit(cg, "    mov %%r11, %%rsi");
            cg_emit(cg, "    shl $3, %%rsi");    // new_capacity * 8

            // d) realloc(old_ptr in %rdi, new_size in %rsi) aufrufen. Result (new_ptr) in %rax
            cg_emit(cg, "    call realloc");

            // e) Fehlerprüfung (optional, aber gut)
            cg_emit(cg, "    cmp $0, %%rax");
            cg_emit(cg, "    je .Lrealloc_fail_%d", lid);

            // f) Pointer und Kapazität aktualisieren
            cg_emit(cg, "    mov %%rax, Lglob_%d(%%rip)", array_gi);    // Basisadresse des Arrays aktualisieren
            cg_emit(cg, "    mov %%r11, Lglob_%d(%%rip)", capacity_gi); // Neue Kapazität speichern

            cg_emit(cg, "    jmp .Lpush_store_%d", lid);

            // --- SPEICHERN DES WERTES ---
            cg_emit(cg, ".Lpush_store_%d:", lid);

            // 4. Basisadresse laden (%r8 = Heap-Pointer)
            cg_emit(cg, "    mov Lglob_%d(%%rip), %%r8", array_gi);

            // 5. Adresse des neuen Slots berechnen: %r9 = base + size * 8
            cg_emit(cg, "    mov %%r10, %%r9"); // %r9 = size (Index des neuen Elements)
            cg_emit(cg, "    shl $3, %%r9");    // size * 8
            cg_emit(cg, "    add %%r8, %%r9");  // %r9 = Adresse des neuen Slots

            // 6. Wert schreiben und Größe inkrementieren
            cg_emit(cg, "    pop %%rbx");        // %rbx = Wert zum Pushen
            cg_emit(cg, "    mov %%rbx, (%%r9)"); // Wert in den Slot schreiben

            cg_emit(cg, "    inc %%r10"); // Größe inkrementieren
            cg_emit(cg, "    mov %%r10, Lglob_%d(%%rip)", size_gi); // Neue Größe speichern

            cg_emit(cg, "    mov $0, %%rax"); // Rückgabewert 0 (void)
            cg_emit(cg, "    jmp .Lpush_end_%d", lid);

            cg_emit(cg, ".Lrealloc_fail_%d:", lid);
            // Nutze den Standard-Exit-Call, um einen Fehler zu signalisieren (Out of memory).
            cg_emit(cg, "    mov $255, %%edi"); // Exit code 255 (Error)
            cg_emit(cg, "    call exit");

            cg_emit(cg, ".Lpush_end_%d:", lid);
            return;
        }

        // --- (Bestehende Generische Funktion Call Logik) ---

        // 1. Argumente auf den Stack legen (reverse order)
        for (int i = call->arg_count - 1; i >= 0; --i) {
            cg_emit_expr(cg, call->args[i]);
            cg_emit(cg, "    push %%rax");
        }

        // 2. Argumente in Register verschieben
        ...
        // 3. Funktion aufrufen (Result is in %rax)
        if (call->name) {
             cg_emit(cg, "    call %s", call->name);
        } else {
            // Wenn es kein einfacher ND_IDENT ist (z.B. obj.fn()), ist dies nicht implementiert
            die("nested call targets are not supported: line %d", n->line);
        }
        return;
    }
    case ND_OBJ_LITERAL:
    case ND_ARRAY_LITERAL: {
        // Literale werden nur als Teil von ND_VARDECL (Initialisierung) unterstützt.
        die("anonymous object or array literals are not supported in expressions: line %d", n->line);
        return;
    }
    default:
...
static void cg_emit_stmt(CodeGen *cg, Node *n) {
    ...
    case ND_EXPR_STMT:
        if (n->lhs && n->lhs->kind == ND_CALL) {
            ...
            // FIX: Implementiere den printf-Test-Harness-Code hier
            if (call->call_target->kind == ND_IDENT && call->name && strcmp(call->name, "printf") == 0) {
                 if (call->arg_count != 2) die("printf in test harness expects 2 args");

                // Arg 2 (The value to print)
            ...
            /* Generic function call (e.g., calling foo(1, 2) or arr.push(1)) */
            cg_emit_expr(cg, n->lhs);
            return;
        } else {
            cg_emit_expr(cg, n->lhs);
    ...
    case ND_VARDECL: {
        if (g_current_func_scope) {
            // LOKALE Variable: Initialisierung auf dem Stack (unverändert)
            ...
            if (n->init) {
                if (n->init->kind == ND_OBJ_LITERAL || n->init->kind == ND_ARRAY_LITERAL) {
                    die("Local arrays/objects not supported in this simple compiler: line %d", n->line);
                }
                cg_emit_expr(cg, n->init);
                cg_emit(cg, "    mov %%rax, %d(%%rbp)", lv->offset);
            } else {
                // Lokale Variable mit 0 initialisieren
                cg_emit(cg, "    movq $0, %d(%%rbp)", lv->offset);
            }
            return;
        } else {
            // GLOBALE Variable:
            ...
            if (init_kind == ND_ARRAY_LITERAL) {
                // --- DYNAMISCHE ARRAY INITIALISIERUNG ---
                int array_len = n->init->kv_count;
                int capacity = (array_len > 0) ? array_len : ARRAY_INIT_CAPACITY;
                int array_gi = add_global(n->var_name);

                // 1. Array-Kapazität initialisieren
                char capacity_name[256];
                snprintf(capacity_name, sizeof(capacity_name), "%s_CAPACITY", n->var_name);
                int capacity_gi = add_global(capacity_name);
                cg_emit(cg, "    movq $%d, Lglob_%d(%%rip)", capacity, capacity_gi);

                // 2. Größe initialisieren
                char size_name[256];
                snprintf(size_name, sizeof(size_name), "%s_SIZE", n->var_name);
                int size_gi = add_global(size_name);
                cg_emit(cg, "    movq $%d, Lglob_%d(%%rip)", array_len, size_gi);

                // 3. Speicher mit malloc zuweisen: malloc(capacity * 8)
                cg_emit(cg, "    mov $%d, %%rdi", capacity); // capacity
                cg_emit(cg, "    shl $3, %%rdi"); // capacity * 8
                cg_emit(cg, "    call malloc"); // Ergebnis (Pointer) in %rax

                // 4. Basisadresse speichern
                cg_emit(cg, "    mov %%rax, Lglob_%d(%%rip)", array_gi); // Speichere Heap-Pointer in Lglob_a

                // 5. Initiale Werte in den zugewiesenen Speicher kopieren
                for (int i = 0; i < array_len; ++i) {
                    cg_emit(cg, "    push %%rax"); // Heap-Pointer sichern
                    cg_emit_expr(cg, n->init->values[i]); // Wert in %rax
                    cg_emit(cg, "    mov %%rax, %%rbx"); // Wert sichern
                    cg_emit(cg, "    pop %%rax"); // Heap-Pointer wiederherstellen

                    // Speichern: *(ptr + i*8) = value
                    cg_emit(cg, "    mov %%rax, %%r8"); // %r8 = Heap-Pointer
                    cg_emit(cg, "    mov $%d, %%r9", i);
                    cg_emit(cg, "    shl $3, %%r9"); // i * 8
                    cg_emit(cg, "    add %%r8, %%r9"); // Adresse des Elements
                    cg_emit(cg, "    mov %%rbx, (%%r9)"); // Wert schreiben
                }
            ...
            if (init_kind == ND_OBJ_LITERAL) {
                // Objekt-Initialisierung (noch immer geflatteter statischer Speicher)
                ...
                // Basisvariable auf die Adresse des ersten geflatteten Elements setzen (Pointer)
                ...
                if (n->init->kv_count > 0) {
                    ...
                    int gmain = add_global(n->var_name);
                    cg_emit(cg, "    movq $0, Lglob_%d(%%rip)", gmain);
                }
            ...
            if (n->init) {
                ...
                cg_emit(cg, "    mov %%rax, Lglob_%d(%%rip)", gi);
                 ...
                 int gi = add_global(n->var_name);
                 cg_emit(cg, "    movq $0, Lglob_%d(%%rip)", gi);
            }
            return;
        }
    }
    ...
    case ND_FUNC: {
        // Stack-Analyse und Prolog/Epilog (unverändert)
        analyze_function_scope(n);
        ...
        // EPILOGUE (Rückgabewert ist bereits in %rax durch ND_RETURN)
    ...
    case ND_PROGRAM: {
        ...
        // Generiere zuerst alle Funktionen
        ...
        // Stack Alignment (8 Bytes Padding)
        cg_emit(cg, "    sub $8, %%rsp");

        // PROGRAM BODY (Nur ausführbare Anweisungen)
        g_current_func_scope = NULL;
        for (Node *s = n->stmts; s; s = s->next) {
            // Nur Anweisungen ausführen, die KEINE Funktionsdeklarationen sind.
            if (s->kind != ND_FUNC) {
                cg_emit_stmt(cg, s);
            }
        }

        // EPILOGUE (Stack restaurieren VOR exit)
        cg_emit(cg, ".Lfunc_end_main:");

        // Stack restaurieren
        cg_emit(cg, "    add $8, %%rsp"); // Stack-Padding entfernen
        cg_emit(cg, "    pop %%rbp");     // Base Pointer Frame restaurieren

        // Programm beenden
        cg_emit(cg, "    mov $0, %%edi"); // Exit-Code 0
        cg_emit(cg, "    call exit");
...
static void cg_emit_data(CodeGen *cg) {
    ...
    // Wir benötigen keine Initialisierung für die dynamischen Array-Elemente mehr,
    // nur für die Pointer und Metadaten (SIZE, CAPACITY).
    cg_emit(cg, ".section .data");
    for (int i = 0; i < g_globals.count; ++i) {
        // Alle globalen Variablen (auch die Heap-Pointer, SIZE und CAPACITY) werden mit 0 initialisiert.
...
static void run_tests(void) {
    struct Test { const char *name; const char *src; const char *expect; } tests[] = {
        ...
        // --- 1. DYNAMIC ARRAY PUSH TEST (Dynamisches Array) ---
        { "arr-push",
          "var a = [10, 20]; a.push(30); a.push(40); a.push(50); printf(\"%ld\\n\", a_SIZE); printf(\"%ld\\n\", a[2]); printf(\"%ld\\n\", a[4]);",
          "5\n30\n50\n" },

        // --- 2. VAR & ASSIGNMENT ---
        { "var-assign",
          "var a = 10; a = a + 5; printf(\"%ld\\n\", a);",
          "15\n" },

        // --- 3. ARRAY LITERAL & INDEX ACCESS ---
        { "arr-index",
          "var a = [100, 200, 300]; var i=1; printf(\"%ld\\n\", a[i]); a[0] = a[1] + a[2]; printf(\"%ld\\n\", a[0]);",
          "200\n500\n" },

        // --- 4. FUNCTION, LOCAL VARS, RETURN, GLOBAL SCOPE (COMPLEX) ---
        { "local-vars",
          "var g=100; function foo(a, b) { var x=5; x=x+a; var y=1; y=y+b; g=g-1; return x*10 + y + g; } printf(\"%ld\\n\", foo(1, 2)); printf(\"%ld\\n\", g);",
          "162\n99\n" },

        // --- 5. IF/ELSE & COMPARISON OPERATORS (>, <, >=, <=) ---
        { "if-else-stmt",
          "var x=10; var y=5; if (x > y) { y = x + 1; } else { y = x - 1; } printf(\"%ld\\n\", y); if (y <= 10) { y = 0; } else { y = 20; } printf(\"%ld\\n\", y);",
          "11\n20\n" },

        // --- 6. OBJECT LITERAL & MEMBER ACCESS (FLATTENED) ---
        { "obj-access",
        ...
        // --- 8. WHILE LOOP ---
        { "while-loop",
          "var i=0; var total=0; while (i < 5) { total = total + i; i = i + 1; } printf(\"%ld\\n\", total);",
          "10\n" },

        // --- 9. LARGE DYNAMIC ARRAY TEST ---
        { "arr-large-push",
          "var a = []; for (var i=0; i<10000; i=i+1) { a.push(i + 100); } printf(\"%ld\\n\", a_SIZE); printf(\"%ld\\n\", a[1]); printf(\"%ld\\n\", a[9999]);",
          "10000\n101\n10099\n" },
    ...
    for (int i = 0; tests[i].name; ++i) {
        ...
        /* assemble and link with C library functions (malloc, realloc, exit) */
        char cmd[1024];
        ...
        if (rc != 0) {
             ...
             // Array Out of Memory (255 * 256 for a typical shell)
             if (rc == 65280) {
                 printf("ERROR DETECTED: Array realloc failed (Out of memory).\n");
             }
        } else {
            printf("--- Output ---\n%s", buf);
            printf("--- Expect ---\n%s", tests[i].expect);
