...
//
// Modifiziert: Der rote Balken bewegt sich nun zeitbasiert 
// (1 Durchlauf pro Sekunde) statt framebasiert.
...
// Konfiguration des wandernden Strichs
#define LINE_WIDTH 10         // Breite des wandernden Strichs (10px)
#define CYCLE_DURATION_MS 1000 // Dauer eines Durchlaufs (1 Sekunde)
...
/**
 * @brief Zeichnet einen sich bewegenden vertikalen Strich (10px breit, Rot) 
 * * NEUE LOGIK: Verwendet SDL_GetTicks() für eine zeitbasierte Animation.
 * Der Balken benötigt exakt CYCLE_DURATION_MS (z.B. 1000ms) für einen 
 * kompletten Durchlauf, unabhängig von der FPS.
 *
 * @param out_rgb Der RGB888-Puffer, in den gezeichnet wird.
 */
static void draw_moving_line(uint8_t *out_rgb) {
  // Holt die aktuelle Systemzeit in Millisekunden
  uint32_t current_time_ms = SDL_GetTicks();
  
  // Berechne die Zeit innerhalb des 1-Sekunden-Zyklus
  uint32_t ms_in_cycle = current_time_ms % CYCLE_DURATION_MS;
  
  // Mappe die Zeit (0-999ms) auf eine Position, die von -LINE_WIDTH bis WIDTH reicht
  // Dies sorgt für einen sauberen Ein- und Auslauf des Balkens.
  float progress = (float)ms_in_cycle / (float)CYCLE_DURATION_MS;
  
  // Gesamtstrecke = WIDTH + LINE_WIDTH
  // Startposition = -LINE_WIDTH
  int line_x_start = (int)(progress * (WIDTH + LINE_WIDTH)) - LINE_WIDTH;
  int line_x_end = line_x_start + LINE_WIDTH;

  // Clipping: Nur Pixel zeichnen, die tatsächlich im Bild sichtbar sind
  int draw_start_x = (line_x_start < 0) ? 0 : line_x_start;
  int draw_end_x = (line_x_end > WIDTH) ? WIDTH : line_x_end;

  for (int y = 0; y < HEIGHT; y++) {
    for (int x = draw_start_x; x < draw_end_x; x++) {
      // Strichfarbe: Rot (255, 0, 0)
      int o = (y * WIDTH + x) * 3;
      out_rgb[o + 0] = 255; // R
      out_rgb[o + 1] = 0;   // G
      out_rgb[o + 2] = 0;   // B
...
    
    // Schritt 2: Wandernden Strich zeichnen (ZEITBASIERT)
    draw_moving_line(rgb); // <--- HIER IST DIE GEWÜNSCHTE FUNKTION
    
