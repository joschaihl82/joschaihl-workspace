 ...
 * main.c
 *
 * GLUT/OpenGL CAD demo — komplette Datei (ohne GPU-Shader; CPU-Grid mit horizontalen
 * und vertikalen Linien, Grid standardmäßig an, Grid-Toggle-Icon farblich hervorgehoben)
 *
 * Compile:
 *   gcc main.c -o cad -lGLEW -lGL -lGLU -lglut -lm
 *
 * Hinweis: Diese Version verwendet keinen GLSL-Shader für das Grid.
 *
 * Neue Features in dieser Version:
 * - Vektor-Icons in der Toolbar (wie zuvor)
 * - Linke Maustaste: Klick = Punkt setzen, Drag = Pan (mit Threshold)
 * - Erste Zeichenfunktion: Linienwerkzeug
 *   * Neuer Button ganz links in der Toolbar
 *   * Auswahl des Werkzeugs durch Klick auf den Button
 *   * Linksklick auf Zeichenblatt: erster Punkt = Start; nächster Linksklick = Endpunkt -> erzeugt Linie
 *   * Endpunkt wird zum neuen Startpunkt (Fortsetzen der Polyline)
 *   * Rechtsklick beendet das Linienzeichnen (Werkzeug bleibt ausgewählt)
 */
...
/* Pending line start (when drawing polyline) */
static int line_active = 0; /* 0 = no pending start, 1 = have pending start */
...
/* Toolbar buttons
   NOTE: BTN_LINE is the new first button (line drawing tool).
*/
enum { BTN_LINE = 0, BTN_CENTER = 1, BTN_PLUS = 2, BTN_MINUS = 3, BTN_RESET = 4, BTN_GRID = 5, BTN_SNAP = 6, BTN_COUNT = 7 };
static int hover_button = -1;
static int pressed_button = -1;

/* Tools */
typedef enum { TOOL_NONE = 0, TOOL_LINE = 1 } Tool;
...
static void iconMagnet(float size) {
    ...
    /* draw white separators at poles, preserve previous color */
    GLfloat prev_col[4];
    glGetFloatv(GL_CURRENT_COLOR, prev_col);
    glColor3f(1.0f, 1.0f, 1.0f);
    float cutH = size * 0.04f;
    drawRectRotated(-w + thick/2.0f, h * 0.6f, thick + 2.0f, cutH, 0);
    drawRectRotated( w - thick/2.0f, h * 0.6f, thick + 2.0f, cutH, 0);
    glColor4fv(prev_col);
}

/* Icon: Line (simple straight line symbol) */
static void iconLine(float size) {
    /* Draw a diagonal line with small endpoints */
    float len = size * 0.7f;
    float half = len * 0.5f;
    glLineWidth(2.0f);
    glBegin(GL_LINES);
    ...
    glBegin(GL_TRIANGLE_FAN);
      glVertex2f(-half, -half);
      for (int a=0;a<=12;++a) {
        float ang = a * (2.0f * PI / 12.0f);
        glVertex2f(-half + cosf(ang)*2.5f, -half + sinf(ang)*2.5f);
      }
    glEnd();
    glBegin(GL_TRIANGLE_FAN);
      glVertex2f(half, half);
...
static void draw_toolbar() {
    ...
    for (int i=0;i<BTN_COUNT;++i) {
        ...
        /* Icon color: highlight grid/snap when active; highlight line button when selected */
        if (i == BTN_GRID) {
            if (grid_visible) glColor3ub(10, 90, 200); else glColor3ub(0,0,0);
        } else if (i == BTN_SNAP) {
            if (snap_enabled) glColor3ub(10, 90, 200); else glColor3ub(0,0,0);
        } else if (i == BTN_LINE) {
            if (selected_tool == TOOL_LINE) glColor3ub(10,90,200); else glColor3ub(0,0,0);
        } else {
            glColor3ub(0,0,0);
        }
...
static void display_cb(void) {
    ...
    /* If there's a pending start, draw a small marker at it */
    if (line_active) {
        int sx, sy;
    ...
    char status[256];
    const char *toolname = (selected_tool == TOOL_LINE) ? "Line" : "None";
    snprintf(status, sizeof(status), "Tool: %s  Zoom: %.3fx  Pan: (%.2f, %.2f)  Grid: %s  Snap: %s  Points: %d  Lines: %d",
             toolname, zoom_factor, pan_x, pan_y, grid_visible ? "on" : "off", snap_enabled ? "on" : "off", points_count, lines_count);
    ...
    if (button == GLUT_LEFT_BUTTON) {
        if (state == GLUT_DOWN) {
            ...
            if (y >= 0 && y <= TOOLBAR_HEIGHT) {
                ...
                /* left down in content area: start potential drag for panning */
                left_dragging = 1;
                left_drag_start_x = x;
                left_drag_start_y = y;
                left_drag_moved = 0;
            ...
            if (pressed_button >= 0) {
                ...
                if (!left_drag_moved) {
                    ...
                    if (selected_tool == TOOL_LINE) {
                        /* Line tool: if no pending start, set it; otherwise create a line segment */
                        if (!line_active) {
                            /* set pending start */
                            line_pending_x = wx;
                            line_pending_y = wy;
                            ...
                            /* create new line from pending start to this click */
        ...
        if (state == GLUT_DOWN) {
            /* Right-click: if line tool active, finish the current polyline (clear pending start) */
            if (selected_tool == TOOL_LINE && line_active) {
                line_active = 0;
                /* keep selected_tool as TOOL_LINE (user may start new polyline later) */
                glutPostRedisplay();
            }
        }
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("CAD Demo with Vector Toolbar Icons and Line Tool");
