...
// Dummy-IPs für Middle/Exit (müssten per Directory-Fetch ermittelt werden)
...
// Statische Liste der zwei festen Guard IPs in Little-Endian Host-Order.
// 0.4.8.8 -> 0x08080400u
// 0.4.8.16 -> 0x10080400u
...
static void utl_u32_to_ip_str(uint32_t host_ip, char *ip_str) {
    struct in_addr addr;
    // Durch Zuweisung eines Little-Endian Werts zu einem struct in_addr.s_addr
    // welches Network Byte Order (Big-Endian) erwartet, wird die Reihenfolge 
    // auf Little-Endian Systemen für inet_ntop() indirekt korrigiert.
    addr.s_addr = host_ip; 
...
static void crypto_kdf_ntor(Hop_t* hop) {
    // SIMULATION: Dummy-Key-Derivation
...
static int cell_decrypt_and_unwrap(u8* cell_buf, Circuit_t* circ) {
    ...
    if (cmd == CMD_RELAY) {
        ...
        if (memcmp(hmac_received, hmac_digest, 4) != 0) {
            utl_logf("TOR", "ERROR", "HMAC-Prüfung fehlgeschlagen.");
            return 0;
        }
...
static int tls_connect_and_cert_verify(Circuit_t* circ, const char* host) {
    ...
    utl_logf("SSL", "SUCCESS", "TLS-Verbindung etabliert.");
    return 1;
}
...
static int socks_handshake(int client_fd) {
    ...
    if (atype == 0x03) { 
        ...
        utl_logf("SOCKS", "ERROR", "Nur Domain-Namen (ATYP 0x03) werden unterstützt.");
        return 0; 
    }
...
int main(void) {
    ...
    // 1. Wähle zufällig einen der beiden festen Guard ORs
    int random_index = rand() % HARDCODED_IPS_COUNT;
    uint32_t guard_ip_host = HARDCODED_IPS_NET[random_index];
    utl_u32_to_ip_str(guard_ip_host, guard_ip_str);
    
    utl_logf("MAIN", "INFO", "Starte Tor-Client. Ausgewählte Guard OR IP: %s (von 0.4.8.8 und 0.4.8.16)", guard_ip_str);
