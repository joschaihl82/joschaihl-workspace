/*
  ...
  - Uses SDL_TEXTINPUT for printable characters; KEYDOWN only for control/navigation keys
  - Avoids local echo to prevent duplicate characters on long key press
...
static void update_pty_echo_flag(void)
{
  ...
  if (tcgetattr(master_fd, &tio) == 0)
  {
    ...
    /* We keep this info but we no longer locally echo characters to avoid duplicates. */
    (void)tio;
  }
}
...
int main(int argc, char *argv[])
{
  ...
  /* initial update of PTY echo flag (kept for info only) */
  ...
  while (!quitting)
  {
    /* keep PTY echo flag up to date (not used for local echo now) */
    update_pty_echo_flag();
    ...
    while (SDL_PollEvent(&ev))
    {
      ...
      {
        /* KEYDOWN now only handles control/navigation keys and special keys
           (arrows, backspace, return, tab, escape). Printable characters are
           handled via SDL_TEXTINPUT to avoid duplication. */
        ...
        switch (ev.key.keysym.sym)
        {
          ...
          case SDLK_BACKSPACE: seq = "\x7f"; break; /* send DEL/backspace to PTY */
          case SDLK_RETURN: seq = "\r"; break;
          case SDLK_TAB: seq = "\t"; break;
          case SDLK_ESCAPE: seq = "\x1b"; break;
          default:
            /* For Ctrl-letter combos, send control codes directly */
            if (ctrl && ev.key.keysym.sym >= SDLK_a && ev.key.keysym.sym <= SDLK_z)
            {
              char c = (char)(ev.key.keysym.sym - SDLK_a + 1);
              send_data_to_pty(&c, 1);
            }
            /* otherwise ignore printable keys here; SDL_TEXTINPUT will handle them */
            break;
        }
      ...
      {
        /* All printable text comes here. Forward to PTY only.
           We do NOT locally echo characters to avoid duplicates and race conditions. */
        const char *s = ev.text.text;
        size_t slen = strlen(s);
        if (slen > 0) send_data_to_pty(s, slen);
      }
