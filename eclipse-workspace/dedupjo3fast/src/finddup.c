/*
 * dedup_loghash_varjump.c
 *
 * O(log n) sampling-based approximate signature.
 * Positions: exponential towards Dateiende.
 * Vor jedem Lesen wird ein deterministischer Sprung in [1 .. 256 * size_in_MB] angewendet.
 * Dadurch sind die Sprünge proportional zur Dateigröße; sehr große Dateien werden
 * noch sparsamer eingelesen.
 *
 * Compile:
 *   gcc -O3 -std=c11 -Wall -Wextra -o dedup_loghash_varjump dedup_loghash_varjump.c
 *
 * WARNING:
 * - Nicht kryptographisch sicher. Vor Löschaktionen mit Vollhash bestätigen.
 */
...
/* Deterministic 32-bit LCG for pseudo-random offsets */
static inline uint32_t lcg_next(uint32_t s) {
    return (uint32_t)(s * 1103515245u + 12345u);
}

/*
 * compute_log_approx_varjump
 *
 * - Samples ~log2(N/WINDOW) windows at exponential positions towards file end.
 * - Vor jedem Lesen: deterministischer Sprung in [1 .. 256 * size_in_MB].
 * - Kombiniert Fenster mit zwei 64-bit Akkumulatoren.
 * - Rückgabe 0 bei Erfolg, -1 bei Fehler.
 */
int compute_log_approx_varjump(FileEntry *entry) {
    if (!entry) return -1;
    ...
    /* Anzahl Samples: floor(log2(N/WINDOW)), mindestens 1, begrenzt auf MAX_SAMPLES */
    size_t samples = 0;
    off_t tmp = N / WINDOW;
    ...
    /* Dateigröße in MiB (mindestens 1) */
    uint64_t size_mb = (uint64_t)N / (1024ULL * 1024ULL);
    if (size_mb == 0) size_mb = 1;

    /* maximale Sprungweite = 256 * size_mb; wir verwenden Bereich [1 .. max_jump] */
    uint64_t max_jump = 256ULL * size_mb;

    /* seed deterministisch aus Größe und Pfad */
    ...
    /* Für i = 1..samples: pos = N - (N >> i), dann Sprung s in [1..max_jump] */
    for (size_t i = 1; i <= samples; ++i) {
        off_t pos = N - (N >> i);
        if (pos < 0) pos = 0;
        if (pos > N - WINDOW) pos = N - WINDOW;

        /* deterministischer Sprung 1..max_jump */
        seed = lcg_next(seed);
        uint64_t small = (uint64_t)(seed) % max_jump + 1ULL; /* 1..max_jump */

        off_t pos2;
        if (small >= (uint64_t)(N - WINDOW - pos)) {
            /* wenn Sprung zu groß, clamp ans Ende */
            pos2 = N - WINDOW;
        } else {
            pos2 = pos + (off_t)small;
        }
...
int main(void) {
    ...
    printf("Phase 2: Berechne Log-sampling Hash (O(log n)) mit variabler Sprungweite und suche Duplikate...\n");
    time_t start_time = time(NULL);

    for (size_t i = 0; i < file_count; i++) {
        if (compute_log_approx_varjump(&file_list[i]) != 0) {
