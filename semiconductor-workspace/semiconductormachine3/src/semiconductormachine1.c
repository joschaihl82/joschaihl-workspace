        ...
        while (SDL_PollEvent(&e)) {
            if (e.type == SDL_QUIT) {
                ...
                if (mx < GRID_W * app.cell_size) {
                    ...
                    // Klick in Toolbox/Buttons: handle wie bisher
                    for (int i = 0; i < 5; ++i) {
                        SDL_Rect r = { GRID_W * app.cell_size + 16, 16 + i * 56, 40, 40 };
                ...
                if (left_pressed && mx >= 0 && mx < GRID_W * app.cell_size && my >= 0 && my < GRID_H * app.cell_size) {
                    int gx = mx / app.cell_size;
                    int gy = my / app.cell_size;
                    if (gx >= 0 && gx < GRID_W && gy >= 0 && gy < GRID_H) {
