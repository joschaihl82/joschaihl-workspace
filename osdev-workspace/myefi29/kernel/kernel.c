...
// - double-buffered rendering with fallback to single-buffer
// - partial dirty-rectangle presents for performance
...
/* Present dirty rectangle from backbuf to fbb (or if no backbuf, nothing) */
static void fb_present_dirty(void) {
    ...
    for (int y = y0; y <= y1; ++y) {
        ...
        /* copy 64-bit chunks when possible */
        uint64_t *s64 = (uint64_t*)src;
        volatile uint64_t *d64 = (volatile uint64_t*)dst;
        int pairs = cols / 2;
        for (int i = 0; i < pairs; ++i) d64[i] = s64[i];
        if (cols & 1) dst[cols - 1] = src[cols - 1];
    }
