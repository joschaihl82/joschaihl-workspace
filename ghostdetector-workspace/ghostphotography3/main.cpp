...
// Live-Video-Applikation: Geisterfilter, Radioaktiver Farbraum (JET) und Gesichtserkennung
...
// Suchen Sie diesen Block im Code:

// Ändern Sie ihn auf den absoluten Pfad, z.B. unter Linux:
...
static QImage matToQImage(const cv::Mat &mat) {
    ...
    // BGR (für OpenCV Output) zu RGB konvertieren
    if (mat.type() == CV_8UC3) {
        cv::Mat rgb;
...
// ====================================================================
// GHOST FILTER: GRADIENTEN-SEPERATOR (STAGES I & II)
// ====================================================================
...
// ====================================================================
// STAGE III & VISUALISIERUNG: JET COLOUR MAP
// ====================================================================
...
Mat visualizeSceneGradient(const Mat& sceneGradient) {
    ...
    // Konvertiere X-Gradienten in 8-Bit Graustufen (skaliert)
    channels[0].convertTo(Gx_scene_float, CV_8UC1, 0.5, 127);

    // Wende den "Radioaktivitätsfarbraum" (COLORMAP_JET) an
    Mat colored_output;
...
// ====================================================================
// ZUSÄTZLICHE INTERPRETATION: GESICHTSERKENNUNG
// ====================================================================

void detectFacesAndHighlight(const Mat& inputFrame, Mat& outputFrame, CascadeClassifier& face_cascade) {
    Mat frame_gray;
    vector<Rect> faces;

    // 1. Eingabe für Haar Cascade vorbereiten
    ...
    // 3. Rechtecke auf den farbigen Output-Frame zeichnen
    for (const auto& face : faces) {
        // Zeichne einen leuchtend grünen Rahmen um das erkannte "Geist"-Objekt (Gesicht)
        rectangle(outputFrame, face, Scalar(0, 255, 0), 3); // BGR: Grün
...
public:
    ...
    {
        setWindowTitle("Live Geisterfilter: Live-Interpretation mit Gesichtserkennung");
        ...
        outputLabel = new QLabel("Geisterfilter (JET) + Gesichtserkennung");
        ...
        statusBar()->showMessage("Kamera verbunden. Warte auf Gesichtssignal...");
...
private slots:
    void processFrame() {
        ...
        // Kopiere das Eingabebild, falls es manipuliert werden muss (z.B. für Text-Overlay)
        inputFrame.copyTo(displayFrame);

        QSize labelSize = inputLabel->size();

        // --- 1. RUN GHOST FILTER (Stages I & II) ---
        Mat sceneGradient;
        Mat weightMap;
        ...
        // --- 3. INTERPRETATION (Gesichtserkennung) ---
        // Das Ergebnis der Gesichtserkennung (Rechtecke) wird auf den farbigen Output gezeichnet.
        detectFacesAndHighlight(inputFrame, filterOutputVis, faceCascade);
...
// =========================================================================
// MOC-Include (Wichtig für Qt)
// Bitte die folgende Zeile wieder einfügen (oder durch das Build-System generieren lassen)
// =========================================================================
// #include "live_ghost_filter.moc"
