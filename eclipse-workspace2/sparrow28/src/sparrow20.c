    ...
    case ND_ARRAY_LITERAL: {
        ...
        if (n->kv_count > 0) {
            ...
            if (n->kind == ND_OBJ_LITERAL) {
                 ...
                 die("Cannot use anonymous object literals in expressions: line %d", n->line);
            } else {
                 // Die Basisadresse des Arrays ist die Adresse des ersten geflatteten Elements.
                 die("Cannot use anonymous array literals in expressions: line %d", n->line);
            }
            // Da wir hier nicht zu anonymen Literalen springen, werden wir den folgenden Code nie erreichen,
            // aber wir lassen ihn zur Konsistenz (kann entfernt werden, wenn anonyme Literale nicht erlaubt sind).
            int gi = add_global(buf);
            cg_emit(cg, "    lea Lglob_%d(%%rip), %%rax", gi);
        } else {
...
static void cg_emit_stmt(CodeGen *cg, Node *n) {
    ...
    case ND_FUNC: {
        ...
        // KORREKTUR: Die Zeile 'mov $0, %%rax' wurde entfernt,
        // um den korrekten Rückgabewert nicht zu überschreiben.
