...
parse_base_type(compiler_context_t *ctx)
{
	...
	for (;;) {
		...
		/* struct/union/typedef-name fallback: treat as int for now */
		if (t->kind == TK_STRUCT || t->kind == TK_UNION || t->kind == TK_TYPEDEF) {
			/* very small handling: consume the keyword and any identifier, but return int */
			next_token(ctx);
