...
// and mapping of the physical framebuffer into a virtual address.
...
#define FB_MAP_SIZE (0x2000000ULL) /* 32 MiB mapping window (same as earlier) */
...
/* Keyboard ISR wrapper: save registers, align stack, call C handler, restore */
...
/* Map framebuffer physical range to virtual VFB_BASE using 2MiB pages.
   fb_phys_base: physical base address of framebuffer
   fb_size: size in bytes to map (rounded up to 2MiB)
*/
static void map_framebuffer_virtual(uint64_t fb_phys_base, uint64_t fb_size) {
    ...
    /* indices for the virtual base */
    uint64_t pml4_idx = (vaddr >> 39) & 0x1FF;
    uint64_t pdpt_idx = (vaddr >> 30) & 0x1FF;
    uint64_t pd_idx   = (vaddr >> 21) & 0x1FF;

    /* Ensure PML4 entry exists for pml4_idx */
    ...
    for (uint64_t i = 0; i < pages; ++i) {
        if (cur_pd_index >= 512) {
            ...
            if (cur_pdpt_idx >= 512) {
                cur_pdpt_idx = 0;
                cur_pml4_idx++;
                if (cur_pml4_idx >= 512) return; /* out of virtual space */
...
void init_paging(KernelGOPInfo *kgi) {
    ...
    /* Map framebuffer physical region into virtual address space at VFB_BASE */
...
void kmain(KernelGOPInfo *kgi) {
    ...
    /* Set fbb to the virtual framebuffer address mapped at VFB_BASE */
    /* Use the same offset into the mapped window as the physical base */
