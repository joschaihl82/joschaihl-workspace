// File: semiconductor_ca_bitmap_1d_float_final_patched.c
// Compile: gcc -o semiconductor_ca_bitmap_1d_float_final_patched semiconductor_ca_bitmap_1d_float_final_patched.c `sdl2-config --cflags --libs` -lSDL2_ttf -lm
// Single-file amalgamation: 1D uint8_t bitmap, float-based continuous brush, robust painting state.
// Changes from previous version:
// - Standardhintergrund ist jetzt Isolator (BT_I).
// - Robustes Painting: Flag 'painting' sorgt dafür, dass nach erneutem Drücken wieder kontinuierlich gemalt wird.
// - MOUSEBUTTONDOWN setzt painting=true wenn im Gitter; MOUSEBUTTONUP setzt painting=false.
...
static void init_bitmap(AppState* app) {
    ...
    // zufällige Halbleiter/Isolatoren Mischung: setze einige Halbleiter (optional)
    srand((unsigned)time(NULL));
    for (int i = 0; i < 2000; ++i) {
        int x = rand() % GRID_W;
        int y = rand() % GRID_H;
        // setze einige Zellen als Halbleiter, damit das Feld nicht komplett leer ist
        app->bitmap[idx(x, y)] = (rand() % 3 == 0) ? BT_S : BT_I;
    }
}
...
// simplified simulation step on 1D bitmap
...
// paint a filled circle at continuous cell coords (cx,cy) with radius in cell units
static void paint_circle_at_cell(AppState* app, float cx, float cy) {
    float r = app->brush_radius;
    ...
    for (int y = y0; y <= y1; ++y) {
        for (int x = x0; x <= x1; ++x) {
            ...
            float dy = (y + 0.5f) - cy;
            if (dx*dx + dy*dy <= r2) {
                app->bitmap[idx(x, y)] = (uint8_t)app->current_tool;
            }
        }
    }
}

// paint along a float line using overlapping circles (brush)
static void paint_line_float_brush(AppState* app, float x0, float y0, float x1, float y1) {
    float dx = x1 - x0;
    float dy = y1 - y0;
    float dist = sqrtf(dx*dx + dy*dy);
    float step = fmaxf(0.25f, app->brush_radius * 0.5f);
    ...
    for (int i = 0; i <= steps; ++i) {
        ...
        float cx = x0 + dx * t;
        float cy = y0 + dy * t;
        paint_circle_at_cell(app, cx, cy);
    }
}

// map window coords to logical pixel coords (float) and continuous cell coords (float)
static void window_to_logical_and_cell_coords(AppState* app, int mx, int my,
    ...
    if (cell_x) *cell_x = lx / (float)app->cell_size;
    if (cell_y) *cell_y = ly / (float)app->cell_size;
}
...
static void render(AppState* app) {
    ...
    // brush preview and info
    {
        char buf[64];
        snprintf(buf, sizeof(buf), "Brush: %.2f cells", app->brush_radius);
        draw_text(app, buf, GRID_W * app->cell_size + 16, app->window_h - 220, white);
        // small preview circle
        int px = GRID_W * app->cell_size + 40;
        int py = app->window_h - 200;
        int pr = (int)roundf(app->brush_radius * 4.0f);
        SDL_SetRenderDrawColor(app->ren, 200, 200, 200, 255);
        for (int yy = -pr; yy <= pr; ++yy) {
            for (int xx = -pr; xx <= pr; ++xx) {
                if (xx*xx + yy*yy <= pr*pr) {
                    SDL_RenderDrawPoint(app->ren, px + xx, py + yy);
                }
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    app.win = SDL_CreateWindow("Zellularautomat Halbleiter (float brush, 1D bitmap)",
    ...
    bool quit = false;
    float last_cx = NAN, last_cy = NAN;
    bool painting = false; // true while mouse is pressed in grid and painting should continue

    Uint32 last_step = SDL_GetTicks();
    const Uint32 step_interval = 100; // ms per simulation step
    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&e)) {
            if (e.type == SDL_QUIT) {
                ...
                float cell_x = -1.0f, cell_y = -1.0f;
                window_to_logical_and_cell_coords(&app, mx, my, &logical_x, &logical_y, &cell_x, &cell_y);

                // Klick im Gitterbereich?
                if (logical_x >= 0.0f && logical_x < (float)(GRID_W * app.cell_size) &&
                    logical_y >= 0.0f && logical_y < (float)(GRID_H * app.cell_size)) {
                    // Start painting mode
                    painting = true;
                    paint_circle_at_cell(&app, cell_x, cell_y);
                    last_cx = cell_x;
                    last_cy = cell_y;
                } else {
                    // Klick in Toolbox/Buttons: handle selection and actions
                    int lx = (int)logical_x;
                    int ly = (int)logical_y;
                    for (int i = 0; i < 5; ++i) {
                        SDL_Rect r = { GRID_W * app.cell_size + 16, 16 + i * 56, 40, 40 };
                ...
                int mx = e.motion.x;
                int my = e.motion.y;
                // robust mouse state via SDL_GetMouseState
                ...
                // Only paint when painting mode is active and left button is down and mouse over grid
                if (painting && left_pressed &&
                    logical_x >= 0.0f && logical_x < (float)(GRID_W * app.cell_size) &&
                    logical_y >= 0.0f && logical_y < (float)(GRID_H * app.cell_size)) {

                    if (!isnan(last_cx) && !isnan(last_cy)) {
                        // always connect last and current with a straight line (float brush)
                        paint_line_float_brush(&app, last_cx, last_cy, cell_x, cell_y);
                    } else {
                        paint_circle_at_cell(&app, cell_x, cell_y);
                    }
                    last_cx = cell_x;
                    ...
                    // If left not pressed or moved outside grid, stop painting (but keep painting=false only on release)
                    if (!left_pressed) {
                        painting = false;
                        last_cx = last_cy = NAN;
                    }
                ...
                if (e.key.keysym.sym == SDLK_SPACE) {
                    ...
                    app.brush_radius = fmaxf(0.1f, app.brush_radius - 0.1f);
                } else if (e.key.keysym.sym == SDLK_RIGHTBRACKET) {
                    app.brush_radius = fminf(10.0f, app.brush_radius + 0.1f);
