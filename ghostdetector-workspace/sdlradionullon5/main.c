...
// NEUE FARBWERTE FÜR HAUTARTIGEN VERLAUF:
...
void process_radionullon(unsigned int *output_pixels, const unsigned char* gray, float globalAvg) {
    ...
    // Zeichne das Viertelkreis-Tortenstück (unten rechts)
    // Winkel 0 .. PI/2 entspricht dem unteren rechten Quadranten in Bildkoordinaten (y nach unten)
    float start_angle = 0.0f;         // 0° (rechts)
    float end_angle = M_PI / 2.0f;    // +90° (unten)
    ...
    for (int y_offset = -g_AUDIT_CIRCLE_RADIUS; y_offset <= g_AUDIT_CIRCLE_RADIUS; y_offset++) {
        for (int x_offset = -g_AUDIT_CIRCLE_RADIUS; x_offset <= g_AUDIT_CIRCLE_RADIUS; x_offset++) {
            ...
            if (dist <= g_AUDIT_CIRCLE_RADIUS) {
                ...
                // Für unteres rechtes Viertel: ang in [0, PI/2]
                ...
                if (current_x >= 0 && current_x < IMG_W && current_y >= 0 && current_y < IMG_H) {
                    ...
                    // Weiches inneres Aufhellen: innen noch etwas heller als base
