...
public:
    explicit MainWindow(QWidget* parent=nullptr) : QMainWindow(parent) {
        ...
        setWindowTitle("Live IP Geo Map (traceroute hops)");
...
var tracerMarkers = {}; // yellow traceroute hops keyed by ip (avoid duplicates)
var homeMarker = null;
...
function addTracerHop(ip, lat, lon, label) {
  if (!lat || !lon) return;
  if (tracerMarkers[ip]) return;
  var m = L.circleMarker([lat, lon], {
  ...
  tracerMarkers[ip] = m;
    ...
    void addTracerHop(const QString& ip, double lat, double lon, const QString& label) {
        QString ipEsc = ip; ipEsc.replace("'", "\\'");
        QString labelEsc = label; labelEsc.replace("'", "\\'");
        QString js = QString("addTracerHop('%1', %2, %3, '%4');")
                         .arg(ipEsc)
                         .arg(lat, 0, 'f', 6)
                         .arg(lon, 0, 'f', 6)
                         .arg(labelEsc);
...
static std::string traceroute_probe(const std::string& dest_ip, int ttl, int probe_port, int timeout_ms) {
    ...
    // Non-blocking not necessary for send socket
    ...
    // Send a small UDP payload
    const char payload[] = "trprobe";
    ssize_t s = sendto(sendfd, payload, sizeof(payload), 0, (struct sockaddr*)&dest, sizeof(dest));
    (void)s;
...
// Stops when destination reached or max hops exceeded.
static void run_traceroute(const std::string& dest_ip,
    ...
    for (int ttl = 1; ttl <= TR_MAX_HOPS; ++ttl) {
        ...
        if (!hop.empty()) {
            ...
            // timeout: still report "*" by sending empty string (skip)
        }
            ...
            if (parse_ipv4(data,hdr->caplen,src,dst)) {
                for (const std::string& ip : {src,dst}) {
                    ...
                    std::thread([ip, win, geo, rdns]() {
                        // For each hop, resolve geo + hostname and add yellow marker
                        run_traceroute(ip, [win, geo, rdns](const std::string& hopip) {
                            ...
                            if (ok) {
                                QString qip = QString::fromStdString(hopip);
                                QString qlabel = QString::fromStdString(label);
                                QMetaObject::invokeMethod(win, [win,qip,hlat,hlon,qlabel](){
                                    win->addTracerHop(qip, hlat, hlon, qlabel);
                                }, Qt::QueuedConnection);
                            }
                        });
                    }).detach();
...
int main(int argc,char* argv[]) {
    // Ignore SIGPIPE to avoid crashes on raw sockets send
    ...
    int rc = app.exec();
    stopFlag.store(true);
    // pcap loop will exit; join thread
    th.join();
    return rc;
}
