 ...
 * main.c
 *
 * GLUT/OpenGL CAD demo — komplette Datei (mit CPU-Fallback: horizontale + vertikale Linien,
 * Grid standardmäßig an, und farblich hervorgehobenes Grid-Toggle-Icon)
 *
 * Compile:
 *   gcc main.c -o cad -lGLEW -lGL -lGLU -lglut -lm
 *
 * Hinweis: Bei Problemen mit Shadern prüfe die Konsolenausgabe (GL/GLEW/Shader-Logs).
 */
...
/* Shader program for grid rendering */
...
/* --- Robust shader compile/link wrappers with detailed logs --- */

static GLuint compile_shader_safe(GLenum type, const char *src) {
    if (!src) {
        fprintf(stderr, "compile_shader_safe: NULL source\n");
        return 0;
    }
    GLuint s = glCreateShader(type);
    if (s == 0) {
        fprintf(stderr, "compile_shader_safe: glCreateShader returned 0 (no function pointer?).\n");
        return 0;
    }
    ...
    if (!ok) {
        ...
        if (loglen > 1) {
            char *log = (char*)malloc((size_t)loglen);
            glGetShaderInfoLog(s, loglen, NULL, log);
            fprintf(stderr, "Shader compile error (type %u):\n%s\n", (unsigned)type, log);
            free(log);
        } else {
            fprintf(stderr, "Shader compile failed (type %u), no log available.\n", (unsigned)type);
        }
        glDeleteShader(s);
        return 0;
    }
...
static GLuint link_program_safe(GLuint vs, GLuint fs) {
    if (vs == 0 || fs == 0) {
        fprintf(stderr, "link_program_safe: invalid shader handles vs=%u fs=%u\n", (unsigned)vs, (unsigned)fs);
        return 0;
    }
    GLuint p = glCreateProgram();
    if (p == 0) {
        fprintf(stderr, "link_program_safe: glCreateProgram returned 0.\n");
        return 0;
    }
    glAttachShader(p, vs);
    glAttachShader(p, fs);
    ...
    if (!ok) {
        ...
        if (loglen > 1) {
            char *log = (char*)malloc((size_t)loglen);
            glGetProgramInfoLog(p, loglen, NULL, log);
            fprintf(stderr, "Program link error:\n%s\n", log);
            free(log);
        } else {
            fprintf(stderr, "Program link failed, no log available.\n");
        }
        glDeleteProgram(p);
        return 0;
    }
...
/* Simple shader sources (GLSL 1.20 for compatibility) */
...
"void main() {\n"
"  if (u_grid_visible == 0) { gl_FragColor = vec4(u_color_bg, 1.0); return; }\n"
...
static void init_grid_shader() {
    GLuint vs = compile_shader_safe(GL_VERTEX_SHADER, grid_vs_src);
    GLuint fs = compile_shader_safe(GL_FRAGMENT_SHADER, grid_fs_src);
    if (!vs || !fs) {
        fprintf(stderr, "init_grid_shader: shader compile failed, skipping GPU grid (using CPU fallback).\n");
    ...
    glDeleteShader(fs);
    if (!grid_prog) {
        fprintf(stderr, "init_grid_shader: program link failed, skipping GPU grid.\n");
        return;
    }
    ...
    fprintf(stderr, "init_grid_shader: GPU grid shader compiled and linked successfully.\n");
}

/* --- Zoom helpers: zoom_to_centered, zoom_in_centered, zoom_out_centered --- */

/* Apply a new zoom value while keeping the content center fixed.
   If animate==1, start an animated transition from current zoom to new_zoom.
   If animate==0, apply immediately (no animation).
*/
static void zoom_to_centered(double new_zoom, int animate) {
    ...
    if (animate) {
        /* start animated transition: preserve world_cx_before as anchor */
        zoom_animating = 1;
        ...
        /* immediate apply: set zoom and adjust pan so content center maps to same world point */
...
/* Timer handler for zoom animation */
static void timer_handler(int unused) {
    (void)unused;
...
/* --- UI drawing: icons, toolbar, rulers, shader invocation --- */
...
static void draw_icon_center(int cx, int cy, int w, int h) {
    int cxp = cx + w/2;
    int cyp = cy + h/2;
    glColor3ub(0,0,0);
    glLineWidth(2.0f);
    glBegin(GL_LINES);
      glVertex2i(cxp - 6, cyp); glVertex2i(cxp + 6, cyp);
      glVertex2i(cxp, cyp - 6); glVertex2i(cxp, cyp + 6);
    glEnd();
}
static void draw_icon_plus(int cx, int cy, int w, int h) { draw_icon_center(cx,cy,w,h); }
static void draw_icon_minus(int cx, int cy, int w, int h) {
    int cxp = cx + w/2;
    int cyp = cy + h/2;
    glColor3ub(0,0,0);
    glLineWidth(2.0f);
...
static void draw_icon_reset(int cx, int cy, int w, int h) {
    int cxp = cx + w/2;
    int cyp = cy + h/2;
    glColor3ub(0,0,0);
    glLineWidth(2.0f);
    ...
    for (int i=0;i<=20;++i) {
        double a = (M_PI*1.5) + (double)i/20.0 * (M_PI*1.2);
        double r = 8.0;
        glVertex2f(cxp + cos(a)*r, cyp + sin(a)*r);
    }
    glEnd();
    glBegin(GL_TRIANGLES);
      glVertex2i(cxp + 8, cyp + 2);
      glVertex2i(cxp + 12, cyp + 6);
      glVertex2i(cxp + 6, cyp + 6);
    glEnd();
}
static void draw_icon_grid(int cx, int cy, int w, int h) {
    int left = cx + (w-14)/2;
    int top = cy + (h-14)/2;
    glLineWidth(1.0f);
    for (int i=0;i<4;++i) {
        int x = left + i*4;
        glBegin(GL_LINES); glVertex2i(x, top); glVertex2i(x, top+14); glEnd();
        int y = top + i*4;
        glBegin(GL_LINES); glVertex2i(left, y); glVertex2i(left+14, y); glEnd();
    }
}
static void draw_icon_snap(int cx, int cy, int w, int h) {
    int cxp = cx + w/2;
    int cyp = cy + h/2;
    glColor3ub(0,0,0);
    glLineWidth(2.0f);
    glBegin(GL_LINES);
      glVertex2i(cxp - 6, cyp - 6); glVertex2i(cxp - 6, cyp + 2);
      glVertex2i(cxp + 6, cyp - 6); glVertex2i(cxp + 6, cyp + 2);
    glEnd();
    glBegin(GL_LINES);
      glVertex2i(cxp - 6, cyp + 2); glVertex2i(cxp + 6, cyp + 2);
    glEnd();
}
...
static void draw_toolbar() {
    ...
    for (int i=0;i<BTN_COUNT;++i) {
        ...
        /* Icon color: highlight grid/snap when active */
        if (i == BTN_GRID) {
        ...
        if (i == BTN_CENTER) draw_icon_center(bx, by, bw, bh);
        else if (i == BTN_PLUS) draw_icon_plus(bx, by, bw, bh);
        else if (i == BTN_MINUS) draw_icon_minus(bx, by, bw, bh);
        else if (i == BTN_RESET) draw_icon_reset(bx, by, bw, bh);
        else if (i == BTN_GRID) draw_icon_grid(bx, by, bw, bh);
        else if (i == BTN_SNAP) draw_icon_snap(bx, by, bw, bh);
    }
...
/* Draw rulers and invoke shader for grid */
static void draw_rulers_and_ticks_and_invoke_shader() {
    ...
    /* GPU grid if available */
    if (grid_prog && grid_visible) {
        glUseProgram(grid_prog);
        glUniform2f(uni_viewport_origin, (GLfloat)cx, (GLfloat)cy);
        glUniform2f(uni_viewport_size, (GLfloat)cw, (GLfloat)ch);
        glUniform2f(uni_pan, (GLfloat)pan_x, (GLfloat)pan_y);
        glUniform1f(uni_zoom, (GLfloat)zoom_factor);
        glUniform1f(uni_px_per_mm, (GLfloat)px_per_mm);
        glUniform1f(uni_step_mm, (GLfloat)step_mm);
        glUniform1f(uni_major_units, (GLfloat)major_units);
        glUniform1i(uni_grid_visible, grid_visible ? 1 : 0);
        glUniform3f(uni_color_bg, 1.0f, 1.0f, 1.0f);
        glUniform3f(uni_color_minor, 0.88f, 0.88f, 0.88f);
        glUniform3f(uni_color_major, 0.75f, 0.75f, 0.75f);
        ...
        glBegin(GL_QUADS);
          ...
          glVertex2f((GLfloat)(cx + cw), (GLfloat)cy);
          glVertex2f((GLfloat)(cx + cw), (GLfloat)(cy + ch));
          glVertex2f((GLfloat)cx, (GLfloat)(cy + ch));
        glEnd();
        ...
        /* CPU fallback grid (simple, coarse) - jetzt mit vertikalen UND horizontalen Linien */
        glLineWidth(1.0f);
        ...
        for (int k = -100; k <= 100; ++k) {
            double wy = k * step_mm;
            int sx0, sy0, sx1, sy1;
            world_to_screen(-1e6, wy, &sx0, &sy0);
            world_to_screen( 1e6, wy, &sx1, &sy1);
            glBegin(GL_LINES);
                glVertex2i(sx0, sy0);
                glVertex2i(sx1, sy1);
            glEnd();
...
static void display_cb(void) {
    ...
    draw_toolbar();
    draw_rulers_and_ticks_and_invoke_shader();
    ...
    char status[256];
    snprintf(status, sizeof(status), "Zoom: %.3fx  Pan: (%.2f, %.2f)  Grid: %s  Snap: %s  Points: %d",
             zoom_factor, pan_x, pan_y, grid_visible ? "on" : "off", snap_enabled ? "on" : "off", points_count);
...
static void mouse_cb(int button, int state, int x, int y) {
    mouse_x = x; mouse_y = y;
    /* GLUT y is from top; our UI uses same coordinates */
    if (button == GLUT_LEFT_BUTTON) {
        if (state == GLUT_DOWN) {
            ...
            if (y >= 0 && y <= TOOLBAR_HEIGHT) {
                ...
                /* left click in content area: add point (with optional snap) */
                double wx, wy;
            ...
            if (pressed_button >= 0) {
                ...
                if (clicked == BTN_CENTER) {
                    ...
                    /* reset pan/zoom and ensure grid on */
                    pan_x = 0.0; pan_y = 0.0;
                    zoom_factor = 1.0;
                    ensure_zoom_safe();
                    grid_visible = 1; /* sicherstellen: grid standardmäßig an */
...
static void motion_cb(int x, int y) {
    ...
    if (panning) {
        ...
        /* update hover for toolbar while dragging */
        int rects[BTN_COUNT][4];
        compute_button_rects(rects);
        hover_button = -1;
...
static void passive_motion_cb(int x, int y) {
    ...
    for (int i=0;i<BTN_COUNT;++i) {
        if (point_in_rect(x,y,rects[i])) {
            hover_button = i;
            tooltip_visible = 1;
            if (i == BTN_CENTER) snprintf(tooltip_text, sizeof(tooltip_text), "Center view");
...
static void keyboard_cb(unsigned char key, int x, int y) {
    (void)x; (void)y;
    if (key == '+' || key == '=') zoom_in_centered();
    else if (key == '-') zoom_out_centered();
    else if (key == 'c' || key == 'C') { pan_x = 0; pan_y = 0; zoom_to_centered(1.0,1); }
    else if (key == 'g' || key == 'G') { grid_visible = !grid_visible; glutPostRedisplay(); }
    else if (key == 's' || key == 'S') { snap_enabled = !snap_enabled; glutPostRedisplay(); }
...
int main(int argc, char **argv) {
    ...
    glutCreateWindow("GLUT CAD Demo - main.c");
    ...
    /* ensure grid default on (redundant but explicit) */
    grid_visible = 1;
