/* tiny_email_wget_mt.c
   Multithreaded email-only crawler (no site files saved).
   Edit macros below for configuration.
   Build: gcc -O2 -pthread -o tiny_email_wget_mt tiny_email_wget_mt.c -lcurl -lxml2
*/
...
#define START_URL "https://cia.gov/"   /* change to your start URL */
#define VISITED_FILE "visited.txt"
#define EMAILS_FILE "emails.txt"
#define MAX_DEPTH 5      /* 0 = only start page */
    ...
    if (nodes) {
        for (int i = 0; i < nodes->nodeNr; i++) {
            ...
            if (strncasecmp(href, "mailto:", 7) == 0) {
                /* capture mailto email addresses */
...
static int download_url_to_memory(CURL *curl, const char *url, Memory *mem, long *http_code, char **content_type) {
    ...
    if (res != CURLE_OK) {
        /* do not treat as fatal; just skip */
...
static void *worker_main(void *arg) {
    ...
    while (1) {
        Task *t = dequeue_task();
        if (!t) break;
        /* extract emails and links from downloaded memory; do NOT save page to disk */
        Memory mem = {0};
        long http_code = 0;
        char *ctype = NULL;
