...
// --------------------------- Raymarch Fragment Shader (GLSL 330) ---------------------------
// Integrated variant adapted from the provided raymarch shader; uses host uniforms and audio textures.
static const char *raymarch_fs =
		"#version 330 core\n"
				"uniform float iTime;\n"
				...
				"out vec4 fragColor;\n"
				"const float FOV = 0.4;\n"
				"const float MarchDumping = 0.95;\n"
				"const float Far = 20.0;\n"
				"const int MaxSteps = 24;\n"
				"const float CameraSpeedBase = 14.51;\n"
				"const float TunnelSmoothFactor = 1.0;\n"
				"const float TunnelRadiusBase = 0.8566;\n"
				"const float TunnelFreqABase = 0.18003;\n"
				"const float TunnelFreqBBase = 0.25;\n"
				"const float TunnelAmpABase = 3.6231;\n"
				"const float TunnelAmpBBase = 2.4324;\n"
				"const float NoiseIsolineBase = 0.319;\n"
				"const float NoiseScaleBase = 5.7;\n"
				"const vec3 ColorBase = vec3(0.085, 0.658, 1.0);\n"
				"#define M_NONE -1.0\n"
				"#define M_NOISE 1.0\n"
				"float hash(float h) { return fract(sin(h) * 43758.5453); }\n"
				"float noise(vec3 x) {\n"
				"    vec3 p = floor(x);\n"
				"    vec3 f = fract(x);\n"
				"    f = f * f * (3.0 - 2.0 * f);\n"
				...
				"}\n"
				"float fbm(vec3 p) { return noise(p); }\n"
				"float smax(float a, float b, float k) {\n"
				"    float h = clamp(0.5 + 0.5 * (b - a) / k, 0.0, 1.0);\n"
				"    return mix(a, b, h) + k * h * (1.0 - h);\n"
				"}\n"
				"vec2 path(float z, float ampA, float ampB, float freqA, float freqB) {\n"
				"    return vec2(ampA * sin(z * freqA), ampB * cos(z * freqB));\n"
				"}\n"
				"float noiseDist(vec3 p, float noiseScale, float noiseIso) {\n"
				"    p = p / noiseScale;\n"
				"    return (fbm(p) - noiseIso) * noiseScale;\n"
				"}\n"
				"vec2 map(vec3 p, float noiseScale, float noiseIso, float tunnelRadius, float tunnelSmooth) {\n"
				"    float d = noiseDist(p, noiseScale, noiseIso);\n"
				"    float d2 = length(p.xy - path(p.z, TunnelAmpABase, TunnelAmpBBase, TunnelFreqABase, TunnelFreqBBase)) - tunnelRadius;\n"
				"    d = smax(d, -d2, tunnelSmooth);\n"
				"    return vec2(d, M_NOISE);\n"
				"}\n"
				"vec2 castRay(vec3 ro, vec3 rd, float noiseScale, float noiseIso, float tunnelRadius) {\n"
				"    float tmin = 0.0;\n"
				"    float tmax = Far;\n"
				"    float precis = 0.008;\n"
				"    float t = tmin;\n"
				...
				"        vec2 res = map(ro + rd * t, noiseScale, noiseIso, tunnelRadius, TunnelSmoothFactor);\n"
				...
				"    return vec2(t, m);\n"
				"}\n"
				"float softshadow(vec3 ro, vec3 rd, float mint, float tmax, float noiseScale, float noiseIso, float tunnelRadius) {\n"
				"    float res = 1.0;\n"
				"    float t = mint;\n"
				"    for (int i = 0; i < 4; i++) {\n"
				"        float h = map(ro + rd * t, noiseScale, noiseIso, tunnelRadius, TunnelSmoothFactor).x;\n"
				...
				"}\n"
				"vec3 render(vec3 ro, vec3 rd, float noiseScale, float noiseIso, float tunnelRadius) {\n"
				"    vec3 col = vec3(0.0);\n"
				"    vec2 res = castRay(ro, rd, noiseScale, noiseIso, tunnelRadius);\n"
				"    float t = res.x;\n"
				"    float m = res.y;\n"
				"    if (m > -0.5) {\n"
				"        vec3 pos = ro + t * rd;\n"
				"        vec3 nor = normalize(vec3(\n"
				"            map(pos + vec3(0.004,0,0), noiseScale, noiseIso, tunnelRadius, TunnelSmoothFactor).x - map(pos - vec3(0.004,0,0), noiseScale, noiseIso, tunnelRadius, TunnelSmoothFactor).x,\n"
				"            map(pos + vec3(0,0.004,0), noiseScale, noiseIso, tunnelRadius, TunnelSmoothFactor).x - map(pos - vec3(0,0.004,0), noiseScale, noiseIso, tunnelRadius, TunnelSmoothFactor).x,\n"
				"            map(pos + vec3(0,0,0.004), noiseScale, noiseIso, tunnelRadius, TunnelSmoothFactor).x - map(pos - vec3(0,0,0.004), noiseScale, noiseIso, tunnelRadius, TunnelSmoothFactor).x\n"
				"        ));\n"
				"        float bandIdx = fract(t * 0.1) * float(u_nbands);\n"
				"        vec3 bandcol = texture(u_bandcols, (bandIdx + 0.5) / float(u_nbands)).rgb;\n"
				"        vec3 base = mix(ColorBase, bandcol, 0.6);\n"
				"        col = base + sin(t * 0.5) * 0.2;\n"
				"        col += 0.15 * sin(vec3(0.1, 0.012, 0.07) * iTime * 3.0);\n"
				"        vec3 lig = -rd;\n"
				"        float dif = clamp(dot(nor, lig), 0.0, 1.0);\n"
				"        dif *= softshadow(pos, lig, 0.03, 1.8, noiseScale, noiseIso, tunnelRadius);\n"
				"        vec3 brdf = vec3(0.0);\n"
				...
				"    coord *= FOV;\n"
				"    float bass = texture(u_samples, 0.02).r;\n"
				"    float mid  = texture(u_samples, 0.25).r;\n"
				"    float treb = texture(u_samples, 0.75).r;\n"
				"    float gain = clamp(u_gain_display, 0.0, 2.0);\n"
				"    float hue = clamp(u_hue, 0.0, 1.0);\n"
				"    float kick = float(u_kick);\n"
				"    float timeMod = iTime * (CameraSpeedBase * (1.0 + 2.0 * bass * gain));\n"
				"    float tunnelRadius = TunnelRadiusBase * (1.0 - 0.25 * bass);\n"
				"    float noiseScale = NoiseScaleBase * (1.0 + 2.0 * (1.0 - mid));\n"
				"    float noiseIso = NoiseIsolineBase * (1.0 - 0.5 * treb);\n"
				"    vec3 ro = vec3(path(timeMod, TunnelAmpABase, TunnelAmpBBase, TunnelFreqABase, TunnelFreqBBase).xy, timeMod);\n"
				"    vec3 target = vec3(path(timeMod + 0.5, TunnelAmpABase, TunnelAmpBBase, TunnelFreqABase, TunnelFreqBBase).xy, timeMod + 0.5);\n"
				...
				"    vec3 col = render(ro, rd, noiseScale, noiseIso, tunnelRadius);\n"
				"    vec3 bandAvg = texture(u_bandcols, 0.5).rgb;\n"
				"    col = mix(col, bandAvg, 0.15 * gain);\n"
				"    if (kick > 0.5) col += vec3(0.6 * bass);\n"
...
int main(void) {
	...
	// compile raymarch shader (use waterfall_vs as full-screen VS)
	GLuint ray_fs = compile_shader(GL_FRAGMENT_SHADER, raymarch_fs);
	GLuint ray_prog = 0;
	if (ray_fs && wvs)
		ray_prog = link_program(wvs, ray_fs);
	...
	// raymarch uniform locations
	GLint loc_ray_iTime = -1, loc_ray_iResolution = -1;
	GLint loc_ray_u_samples = -1, loc_ray_u_bandcols = -1;
	GLint loc_ray_u_vscale = -1, loc_ray_u_gain_display = -1;
	GLint loc_ray_u_hue = -1, loc_ray_u_kick = -1, loc_ray_u_nbands = -1;
	GLint loc_ray_u_flash_brightness = -1, loc_ray_u_flash_color = -1;
	if (ray_prog) {
		loc_ray_iTime = glGetUniformLocation(ray_prog, "iTime");
		loc_ray_iResolution = glGetUniformLocation(ray_prog, "iResolution");
		loc_ray_u_samples = glGetUniformLocation(ray_prog, "u_samples");
		loc_ray_u_bandcols = glGetUniformLocation(ray_prog, "u_bandcols");
		loc_ray_u_vscale = glGetUniformLocation(ray_prog, "u_vscale");
		loc_ray_u_gain_display = glGetUniformLocation(ray_prog,
				"u_gain_display");
		loc_ray_u_hue = glGetUniformLocation(ray_prog, "u_hue");
		loc_ray_u_kick = glGetUniformLocation(ray_prog, "u_kick");
		loc_ray_u_nbands = glGetUniformLocation(ray_prog, "u_nbands");
		loc_ray_u_flash_brightness = glGetUniformLocation(ray_prog,
				"u_flash_brightness");
		loc_ray_u_flash_color = glGetUniformLocation(ray_prog, "u_flash_color");
	}
	...
	GL_DYNAMIC_DRAW);
	// Use attribute 0 for waveform positions as well (vertex shader expects a_pos at location 0)
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, (void*) 0);
	glBindVertexArray(0);
	...
	while (running) {
		...
		// 1) Raymarch background pass (uses audio textures and uniforms)
		if (ray_prog) {
			glUseProgram(ray_prog);
			float tsec = (float) SDL_GetTicks() * 0.001f;
			glUniform1f(loc_ray_iTime, tsec);
			glUniform2f(loc_ray_iResolution, (float) win_w, (float) win_h);

			// bind audio sample texture -> unit 0
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_1D, tex);
			glUniform1i(loc_ray_u_samples, 0);

			// bind band color texture -> unit 1
			glActiveTexture(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_1D, bandtex);
			glUniform1i(loc_ray_u_bandcols, 1);

			glUniform1f(loc_ray_u_vscale, 0.4f);
			glUniform1f(loc_ray_u_gain_display, fminf(1.0f, g_limiter_gain));
			// map contrast_step hue into 0..1
			float hue_norm = (float) contrast_step
					/ (float) TOTAL_CONTRAST_STEPS;
			glUniform1f(loc_ray_u_hue, hue_norm);
			glUniform1i(loc_ray_u_kick, kick_flag);
			glUniform1i(loc_ray_u_nbands, N_BANDS);
			glUniform1f(loc_ray_u_flash_brightness, flash_brightness);
			glUniform3f(loc_ray_u_flash_color, 1.0f, 1.0f, 1.0f);
		...
		// 2) Waterfall overlay (semi-transparent)
		if (wprog) {
			glUseProgram(wprog);
			glActiveTexture(GL_TEXTURE2);
			glBindTexture(GL_TEXTURE_2D, waterfall_tex);
			glUniform1i(loc_w_u_waterfall, 2);
		...
		// 3) Waveform / VU overlay (top)
		glUseProgram(prog);
		// bind sample texture unit 0 and bandcols unit 1 for waveform shader
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_1D, tex);
		...
		glUniform1f(loc_u_hue,
				(float) contrast_step / (float) TOTAL_CONTRAST_STEPS);
		glUniform1i(loc_u_kick, kick_flag);
		glUniform1i(loc_u_nbands, N_BANDS);
		glUniform1f(loc_u_flash_brightness, flash_brightness);
		glUniform3f(loc_u_flash_color, 1.0f, 1.0f, 1.0f);

		// draw waveform as line strip (approx)
		glBindVertexArray(wave_vao);
