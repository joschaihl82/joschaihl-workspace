...
// Run: ./face
// Note: Requires a V4L2 camera at /dev/video0 supporting 800x600 YUYV.
...
#define ALPHA 0.001f           // EWMA learning rate for mu/var updates (Skin Model)
#define ALPHA_COM 0.015f       // NEU: EWMA learning rate for Center-of-Mass (Tracking)
#define KH 2.0f
#define S_BASE_MIN 0.20f
#define S_MIN_ABS 0.08f
#define BETA 0.5f
#define V_REF 0.5f
#define AREA_MIN 1500
#define STABLE_COM_MAX_MOVE 6
...
// centroid stability tracking (jetzt als geglättete EWMA Werte)
static float prev_cx_f = (float)W/2.0f; // Initialisiert mit float Werten
static float prev_cy_f = (float)H/2.0f; 
...
///////////////////// ROI & PCA ////////////////////////
// compute center-of-mass of mask (uneglättet/roh)
...
int main(){
    ...
    while(running){
        ...
        // COM
        int cx_raw, cy_raw; // NEU: Rohdaten des Schwerpunkts
        mask_center_of_mass(mask, &cx_raw, &cy_raw);
        
        // ONLINE-LEARNING ADAPTION FÜR CENTROID: EWMA Glättung (Stabilisierung des Trackings)
        float old_cx_f = prev_cx_f;
        float old_cy_f = prev_cy_f;
        
        // Formel: c_t = (1 - alpha) * c_{t-1} + alpha * c_{neu}
        if (area >= AREA_MIN) { // Nur aktualisieren, wenn ein signifikantes Gesicht erkannt wurde
            prev_cx_f = (1.0f - ALPHA_COM) * prev_cx_f + ALPHA_COM * (float)cx_raw;
            prev_cy_f = (1.0f - ALPHA_COM) * prev_cy_f + ALPHA_COM * (float)cy_raw;
        }
        
        // Verwende den geglätteten Zentroid für alle weiteren Schritte
        int cx = (int)roundf(prev_cx_f); 
        int cy = (int)roundf(prev_cy_f);

        // stability check (Vergleiche geglätteten Zentroid von frame-1 mit frame-2)
        int move = (old_cx_f < 0.0f) ? 0 : (int)roundf(fabsf(prev_cx_f - old_cx_f) + fabsf(prev_cy_f - old_cy_f));
        ...
        if(variance_gray(gray_res) > VAR_GRAY_MIN){
            ...
            // update PCA always (we allow PCA to adapt continuously)
        ...
        // Adaptive skin model update: ONLINE-LEARNING ADAPTION (EWMA für HSV Mu/Var)
        if(stable_count >= STABLE_FRAMES_REQ){
            stable_update_counter++;
            if(stable_update_counter % UPDATE_EVERY_N == 0){
                // Online mean/var update (Bereits EWMA, wie gewünscht)
                float xh = Hmean_mask;
                float xs = Smean_mask;
