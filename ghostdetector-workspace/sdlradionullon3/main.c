...
float g_MINIMUM_BG_LEARN_RATE = 0.00005f;   // EXTREM niedrig
float g_BG_RATE_SLOW = 0.00005f;           
float g_BG_RATE_FAST = 0.0001f;            

float g_INITIAL_MAX_LEARN_RATE = 0.005f;    
int g_DAMPING_DURATION_FRAMES = 1800;     // 60 Sekunden
...
int g_BLUR_KERNEL_RADIUS = 2;        // Bleibt int
...
// Hautfarben (RGB-Werte, und der extrem dunkle Rand)
int g_SKIN_COLOR_R = 210;
int g_SKIN_COLOR_G = 180;
int g_SKIN_COLOR_B = 140;

int g_CONTOUR_COLOR_R = 10;   
int g_CONTOUR_COLOR_G = 5;
int g_CONTOUR_COLOR_B = 0;
...
int dynamic_radionullon_thresh = 30; // Initialwert
double baseline_score_ema = 0.0;
int frame_counter = 0;
unsigned int frame_counter_total = 0; 
float audit_target_val = 200.0f; // Initialwert
...
void calculate_gaussian_kernel(float** kernel_ptr, int *kernel_size) {
    ...
    *kernel_size = 2 * r + 1;
    ...
    // Speicher allozieren für das quadratische Kernel-Array
...
void process_radionullon(unsigned int *output_pixels, const unsigned char* gray, float globalAvg) {
    ...
    // 1. DYNAMISCHE BG_LEARN_RATE
    float dynamic_bg_learn_rate_base;

    // Normale, ultra-langsame dynamische Rate (nahe MINIMUM_BG_LEARN_RATE)
    dynamic_bg_learn_rate_base = g_BG_RATE_SLOW + (g_BG_RATE_FAST - g_BG_RATE_SLOW) * normalized_avg;
    
    // Berechnung des Dämpfungsfaktors (60 Sekunden)
    float damping_factor = 0.0f;
    
    // Initialer Boost fällt innerhalb von 60s auf 0 ab (Dämpfung)
    if (frame_counter_total < g_DAMPING_DURATION_FRAMES) { 
        
        // Cosinus-Funktion von 1 (t=0) bis 0 (t=1800)
    ...
    for (i = 0; i < size; i++) {
        ...
        // Koordinaten des aktuellen Pixels
        int px = i % IMG_W;
        int py = i / IMG_W;
        
        // Prüfen, ob das Pixel im Audit-Kreis liegt
        ...
        if (dist_from_center_sq <= (g_AUDIT_CIRCLE_RADIUS * g_AUDIT_CIRCLE_RADIUS)) {
            continue; // Pixel im Audit-Kreis wird später separat behandelt
        }
    ...
    // --- 4.5 ANZEIGE DES MORPHING-MINIMUMS und ROSA RAUSCHEN im KREIS ---
    
    // Verwende den Mittelpunkt des Kreises für die Berechnungsreferenz
    ...
    if (center_pixel_index < 0 || center_pixel_index >= PIXEL_SIZE) {
        center_pixel_index = PIXEL_SIZE / 2; // Fallback zum Bildschirmzentrum
    }
    
    float current_bg_val = bg[center_pixel_index]; 
    float min_morphing_residue = fabsf(audit_target_val - current_bg_val);
    
    // Visuelles Feedback-Farbe (Rot/Grün-Indikator)
    unsigned int circle_feedback_color;
    if (min_morphing_residue > 10.0f) {
        unsigned char r_val = (unsigned char)(fminf(255.0f, min_morphing_residue * 2.0f));
        circle_feedback_color = (0xFF << 24) | (r_val << 16) | (0x00 << 8) | 0x00; // Rot (Hohe Restdifferenz)
    } else {
        unsigned char g_val = (unsigned char)(fminf(255.0f, (10.0f - min_morphing_residue) * 25.0f));
        circle_feedback_color = (0xFF << 24) | (0x00 << 16) | (g_val << 8) | 0x00; // Grün (Niedrige Restdifferenz)
    }
    
    // Zeichne den Kreis mit Farbverlauf, Rauschen und Feedback-Overlay
    for (int y_offset = -g_AUDIT_CIRCLE_RADIUS; y_offset <= g_AUDIT_CIRCLE_RADIUS; y_offset++) {
        for (int x_offset = -g_AUDIT_CIRCLE_RADIUS; x_offset <= g_AUDIT_CIRCLE_RADIUS; x_offset++) {
            ...
            if (dist <= g_AUDIT_CIRCLE_RADIUS) {
                ...
                if (current_x >= 0 && current_x < IMG_W && current_y >= 0 && current_y < IMG_H) {
                    ...
                    // 1. Hautkontur-Basis (Verlauf)
                    float normalized_dist = dist / g_AUDIT_CIRCLE_RADIUS; 
                    unsigned char r_base = (unsigned char)(g_SKIN_COLOR_R * (1.0f - normalized_dist) + g_CONTOUR_COLOR_R * normalized_dist);
                    unsigned char g_base = (unsigned char)(g_SKIN_COLOR_G * (1.0f - normalized_dist) + g_CONTOUR_COLOR_G * normalized_dist);
                    unsigned char b_base = (unsigned char)(g_SKIN_COLOR_B * (1.0f - normalized_dist) + g_CONTOUR_COLOR_B * normalized_dist);
                    ...
                    // Zufällige H-S-V Werte im Rosa-Bereich und hoher Helligkeit
                    ...
                    unsigned int noise_color = (0xFF << 24) | (r_noise << 16) | (g_noise << 8) | b_noise;

                    // 3. Rauschen über Basis blenden (Alpha-Wert g_NOISE_ALPHA)
                    unsigned int combined_color = blend_color(base_color, r_noise, g_noise, b_noise, g_NOISE_ALPHA);

                    // 4. Rot/Grün-Feedback darüber blenden (200 Alpha)
                    output_pixels[current_pixel_index] = blend_color(combined_color, 
                                                                      (circle_feedback_color >> 16) & 0xFF,
                                                                      (circle_feedback_color >> 8) & 0xFF,
                                                                      circle_feedback_color & 0xFF,
                                                                      200); 
                }
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    g_AUDIT_CIRCLE_CENTER_X = IMG_W - g_AUDIT_CIRCLE_RADIUS - 10; // 10 Pixel vom rechten Rand
    g_AUDIT_CIRCLE_CENTER_Y = IMG_H - g_AUDIT_CIRCLE_RADIUS - 10; // 10 Pixel vom unteren Rand
    dynamic_radionullon_thresh = g_RADIONULLON_THRESH_MIN;
    audit_target_val = g_AUDIT_HIGH_VAL;
    srand(time(NULL)); // Initialisiert den Zufallsgenerator für das Rauschen
    // ----------------------------------------------
    ...
    window = SDL_CreateWindow("Radionullon Ghost Filter (SDL2) - Variable Config with HSV Noise Audit Circle",
