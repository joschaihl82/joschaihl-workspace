...
//--------------------------------------------------------------
// Helpers for interpretation & HTML
//--------------------------------------------------------------
...
void html_add_hdb(FILE *html, const char *line, unsigned long long *counter) {
    fprintf(html,"<h1>Virus %llu (hdb)</h1>\n",(*counter)++);
    fprintf(html,"<h2>"); html_escape(html,line); fprintf(html,"</h2>\n");
    fprintf(html,"<p>Description not available</p>\n");
}

void html_add_ndb(FILE *html, const char *line, unsigned long long *counter) {
    fprintf(html,"<h1>Virus %llu (ndb)</h1>\n",(*counter)++);
    fprintf(html,"<h2>"); html_escape(html,line); fprintf(html,"</h2>\n");
    fprintf(html,"<p>Description not available</p>\n");
}

void html_add_ldb(FILE *html, const unsigned char *buf, size_t len, unsigned long long *counter) {
    fprintf(html,"<h1>Virus %llu (ldb)</h1>\n",(*counter)++);
    fprintf(html,"<h2>Binary LDB entry</h2>\n");
    fprintf(html,"<p>Binary content (not displayed)</p>\n");
}
...
int main(void) {
    ...
    while ((de = readdir(d))) {
        ...
        if(strstr(de->d_name,".hdb")) {
            ...
            while(line){ if(*line) html_add_hdb(html,line,&virus_counter); line=strtok_r(NULL,"\n",&saveptr);}
        } else if(strstr(de->d_name,".ndb")) {
            char *saveptr=NULL, *line=strtok_r((char*)buf,"\n",&saveptr);
            while(line){ if(*line) html_add_ndb(html,line,&virus_counter); line=strtok_r(NULL,"\n",&saveptr);}
        } else if(strstr(de->d_name,".ldb")) {
            html_add_ldb(html,buf,flen,&virus_counter);
        }
