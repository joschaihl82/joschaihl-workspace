...
// This is a naive parser that scans for 16-byte blob at the end as in your original,
// but with safer checks. For production, use libXau; here we keep it minimal.
static int read_xauth_cookie(uint8_t out[16]) {
    ...
    // Naively assume last 16 bytes are cookie data (as in original code).
...
void create_window(state_t *state, uint16_t w, uint16_t h, uint32_t window_parent) {
    ...
    const int len = 8; // words, including header
    uint32_t packet[len];
...
void put_image(state_t *state) {
    ...
    enum { BITMAP_SIZE_BYTES = W * H * BYTES_PER_PIXEL };

    static uint32_t *packet = NULL;
    if (!packet) {
        packet = (uint32_t *)malloc(24 + BITMAP_SIZE_BYTES);
        if (!packet) FATAL_ERROR("malloc failed for PutImage packet");
    }

    uint32_t *bmp = packet + 6; // bitmap data starts after 6 words
    for (int y = 0; y < H; y++) {
        uint32_t *row = bmp + y * W;
        for (int x = 0; x < W; x++) {
            row[x] = ((uint32_t)x << 8) + (uint32_t)y; // simple gradient
        }
    }

    // First word: opcode (byte 0), format (byte 1), length (bytes 2-3)
    // format=2 (ZPixmap)
    ...
    uint32_t req_len_units = (uint32_t)(W * H + 5);
