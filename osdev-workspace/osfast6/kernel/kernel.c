...
/* --- 7. HSV Conversion (float, H in degrees 0..360, S/V 0..1) --- */
...
void kmain(KernelGOPInfo *kgi) {
    ...
    init_paging(kgi);
    init_gdt_tss();
    enable_sse(); /* SSE must be enabled before using floating point in the loop */
    ...
    /* Fixed saturation and value at maximum (full hue, bright) */
    const float sat = 1.0f;   /* volle Sättigung */
    const float val = 1.0f;   /* volle Helligkeit */

    /* Hue cycles through full 0..360 degrees */
    float hue = 0.0f;
    const float hue_step = 1.0f; /* Grad pro Frame, anpassen für Geschwindigkeit */
    ...
    while (1) {
        /* compute RGB from current HSV (only hue changes) */
        uint32_t rgb = hsv_to_rgb(hue, sat, val);

        /* fill backbuffer with the color */
        for (uint32_t i = 0; i < screen_height * pps; i++) backbuffer[i] = rgb;
        ...
        /* small busy-wait to slow down the visual change (tune as needed) */
