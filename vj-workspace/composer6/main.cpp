...
// Added: delete button 'x' in the top-right corner of every node to remove it.
// Added: automatic slot synchronization — when GLSL editor text changes, 'in' declarations create input slots
//        and 'out' declarations create output slots with the exact names from the code.
...
public:
        ...
        : QGraphicsObject(parent), m_id(id), m_title(title)
    {
        ...
        m_deleteProxy = nullptr;
    }

    ~NodeItem() override {
        // Proxy widgets are children of this QGraphicsItem; deleting proxies deletes widgets.
        if (m_editorProxy) {
    ...
    void setEditorCodeAndSlots(const QString& glslCode, bool isShaderNode) {
        ...
        // parse 'in' and 'out' and update slots
        parseSlotsFromCode();
        ...
        // ensure delete button exists for every node
        createDeleteButton();
...
Q_SIGNALS:
    ...
    void requestDelete(NodeItem*); // emitted when delete button is pressed
...
private:
    ...
    void adjustSizeForEditor() {
        if (!m_isShaderNode) {
            ...
            if (m_deleteProxy) updateDeleteButtonGeometry();
            return;
        }
    ...
    void createEmbeddedEditorWidget() {
        ...
        // connect text changes back to node state and parse slots automatically
        connect(m_editorWidget, &QPlainTextEdit::textChanged, this, [this]() {
            QString newText = m_editorWidget->toPlainText();
            if (newText == m_editorCode) return;
            m_editorCode = newText;
            // parse and update slots to match 'in'/'out' declarations in the code
            parseSlotsFromCode();
            // update geometry and notify edges
            adjustSizeForEditor();
            update();
            emit positionChanged(this);
        });
    ...
    // Create a small delete button in the top-right corner (one per node)
    void createDeleteButton() {
        if (m_deleteProxy) return; // already created

        m_deleteButton = new QPushButton("x");
        m_deleteButton->setFixedSize(18, 18);
        m_deleteButton->setToolTip("Delete node");
        // minimal styling to fit the dark UI
        m_deleteButton->setStyleSheet(
            "QPushButton { background: #b04040; color: white; border: none; border-radius: 9px; font-weight: bold; }"
            "QPushButton:hover { background: #d05050; }"
        );

        m_deleteProxy = new QGraphicsProxyWidget(this);
        m_deleteProxy->setWidget(m_deleteButton);
        m_deleteProxy->setZValue(2); // above editor and node
        m_deleteProxy->setAcceptedMouseButtons(Qt::LeftButton);
        m_deleteProxy->setFlag(QGraphicsItem::ItemIsSelectable, false);
        m_deleteProxy->setFlag(QGraphicsItem::ItemIsMovable, false);

        connect(m_deleteButton, &QPushButton::clicked, this, [this]() {
            emit requestDelete(this);
        });

        updateDeleteButtonGeometry();
    }

    void updateDeleteButtonGeometry() {
        if (!m_deleteProxy) return;
        const qreal margin = 6.0;
        QSizeF btnSize = m_deleteProxy->size();
        // If size is not set yet, use the widget's fixed size
        if (btnSize.width() <= 0 || btnSize.height() <= 0) {
            if (m_deleteButton) btnSize = QSizeF(m_deleteButton->width(), m_deleteButton->height());
            else btnSize = QSizeF(18, 18);
        }
        qreal x = m_size.width() - btnSize.width() - margin;
        qreal y = margin;
        m_deleteProxy->setPos(x, y);
        m_deleteProxy->setGeometry(QRectF(x, y, btnSize.width(), btnSize.height()));
    }

    // Parse the current m_editorCode for 'in' and 'out' declarations and update m_slots
    void parseSlotsFromCode() {
        QRegularExpression re(R"(^\s*(in|out)\s+\w+\s+([A-Za-z_]\w*)\s*;)", QRegularExpression::MultilineOption);
        QRegularExpressionMatchIterator it = re.globalMatch(m_editorCode);
        ...
        // If no declarations found, do not clear existing slots automatically unless desired.
        // Here we replace slots only if we found at least one declaration OR if the code is empty.
        bool shouldReplace = !newSlots.empty() || m_editorCode.trimmed().isEmpty();

        if (!shouldReplace) return;

        // compare newSlots with existing m_slots
        bool equal = (newSlots.size() == m_slots.size());
        if (equal) {
            for (size_t i = 0; i < newSlots.size() && equal; ++i) {
                if (newSlots[i].name != m_slots[i].name || newSlots[i].isOutput != m_slots[i].isOutput) equal = false;
            }
        }
    ...
    QGraphicsProxyWidget* m_deleteProxy = nullptr;
};
...
public Q_SLOTS:
    ...
    NodeItem* createMicrophoneNode(const QPointF& scenePos) {
        ...
        connect(n, &NodeItem::requestDelete, this, &NodeView::onRequestDeleteNode);
        return n;
    }
...
private Q_SLOTS:
    void onRequestDeleteNode(NodeItem* node) {
        if (!node) return;
