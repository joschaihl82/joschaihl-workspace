...
// Updated: shader nodes render an embedded fixed-width GLSL editor area inside the node.
// - Editor uses a fixed-width font at 12pt
// - Editor area shows up to 10 lines and 80 characters per line
...
public:
    ...
    void paint(QPainter* p, const QStyleOptionGraphicsItem* option, QWidget*) override {
        ...
        // If this node is a shader node and has editor content, draw the editor box centered
        if (m_isShaderNode) {
            drawEmbeddedEditor(p);
        }
...
private:
    ...
    // Compute editor metrics and adjust node size so editor area (10 lines x 80 chars) fits
    ...
    // Draw the embedded editor rectangle and clipped text inside the node
    void drawEmbeddedEditor(QPainter* p) {
        ...
        qreal editorWidth = charWidth * 40.0;
        qreal editorHeight = lineHeight * 10.0;

        const qreal marginX = 12.0;
        const qreal marginY = 8.0;

        // ensure editor fits inside node width (it should because we adjusted size)
        ...
        // set monospace font and clip to editorRect
        p->save();
        p->setFont(fmFont);
        p->setPen(QColor(200,200,200));
        p->setClipRect(editorRect.adjusted(2,2,-2,-2));

        // split editor text into lines, clamp to 10 lines and 80 chars
        QStringList lines = m_editorCode.split('\n');
        // ensure at least 10 lines for display (pad with empty)
        while (lines.size() < 10) lines.append(QString());
        // draw up to 10 lines
        int maxLines = qMin(10, lines.size());

        for (int i = 0; i < maxLines; ++i) {
            QString line = lines.at(i);
            if (line.length() > 80) line = line.left(80);
            QPointF pos(editorRect.left() + 4.0, editorRect.top() + (i+1) * lineHeight - (lineHeight - fm.ascent()));
            p->drawText(pos, line);
        }
