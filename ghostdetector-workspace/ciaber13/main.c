...
// ***************************************************************
// KONSTANTEN FÜR DOSIMETRIE & KONVERTIERUNG (NEU)
// ***************************************************************
// Konvertierungsfaktor: 1 Sv/h = 10^6 µSv / 3600 s ≈ 277.777778 µSv/s
#define SVH_TO_USVS 277.777777778

// --- Audio-Konstanten (Unverändert) ---
...
// ***************************************************************
// FARB-KONVERTIERUNG (Unverändert)
// ***************************************************************
...
// ***************************************************************
// ALSA / SOUND FUNKTIONEN (Unverändert)
// ***************************************************************
...
void init_click_buffers() {
    srand(time(NULL)); const double base_decay = 800.0; 
...
void play_startup_clicks() {
    ...
    for (int i = 0; i < NUM_SCALE_NOTES; i++) {
        play_alsa_click(i); usleep(100000); 
    }
...
static inline uint8_t clamp_u8(int val) {
    if (val < 0) return 0; if (val > 255) return 255; return (uint8_t)val;
}
...
void process_radionullon(GeigerLogicState *state, const uint32_t *img_in, uint32_t *img_out) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        // Heatmap-Farbberechnung mit HSV 3.0
        float H_norm = 1.0f - ((float)s_clamped / 255.0f); 
        float H = H_norm * 240.0f; 
        float S = 1.0f; 
        float V = 1.0f; 
        
        float R_float, G_float, B_float;
        hsv_to_rgb(H, S, V, &R_float, &G_float, &B_float);

        uint8_t r_hsv = clamp_u8((int)(R_float * 255.0f));
        uint8_t g_hsv = clamp_u8((int)(G_float * 255.0f));
        uint8_t b_hsv = clamp_u8((int)(B_float * 255.0f));

        // Blending
        uint32_t original_rgb = img_in[i]; 
        ...
        uint8_t R_blend = (uint8_t)((R0 * (255 - alpha) + r_hsv * alpha) / 255);
        uint8_t G_blend = (uint8_t)((G0 * (255 - alpha) + g_hsv * alpha) / 255);
        uint8_t B_blend = (uint8_t)((B0 * (255 - alpha) + b_hsv * alpha) / 255);
    ...
    // Die akkumulierte Dosis (totalDoseAcc) bleibt proportional, aber wir skalierten sie für die Anzeige.
...
// ***************************************************************
// X11 UND MAIN FUNKTIONEN (ANGEPASST)
// ***************************************************************

int x11_init() {
    // ... (X11-Initialisierung unverändert)
    int screen;
    ...
    while (running) {
        ...
        if (r > 0 && FD_ISSET(v4l2_fd, &fds)) {
            if (v4l2_read_frame(original_frame_buffer) == 0) {
                ...
                // ***************************************************************
                // NEUE ANZEIGE: µSv/s und akkumulierte µSv
                // ***************************************************************
                char stats_text[128];
                char dose_text[128];
                
                // Konvertiere die Rate (logischer Wert, skaliert als Sv/h) nach µSv/s
                double current_usvs = logic_state.stats.currentRad * SVH_TO_USVS;
                double avg_usvs = logic_state.stats.avgRad * SVH_TO_USVS;
                
                // Konvertiere die akkumulierte Dosis (logischer Wert, skaliert als Sv * 0.001) nach µSv
                // Da 1 Sv = 10^6 µSv, multiplizieren wir mit 1000, um den logischen Faktor 0.001 zu kompensieren
                double total_usv = logic_state.stats.totalDose * 1000.0 * 1000.0; 

                // UTF-8 Zeichen für µ (Micro)
                sprintf(stats_text, "AKTUELL: %.2f µSv/s | Ø MITTEL: %.2f µSv/s", current_usvs, avg_usvs);
                sprintf(dose_text, "TOTALDOSIS: %.3f µSv", total_usv);
                
                
                // Textbereich vor dem Zeichnen löschen und Rate anzeigen
                ...
                // Gesamtdosis anzeigen
                XDrawString(display, window, gc, video_x_start, video_y_start + IMG_H + 55, dose_text, strlen(dose_text));
