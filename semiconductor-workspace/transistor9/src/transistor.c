 ...
 * Zeichenwerkzeugen: Line, Circle, Triangle (outline + filled),
 * Rectangle (outline + filled), Polygon (outline + filled) und Farbpalette.
 *
 * Compile:
 ...
 * - Polygon tools: Linksklick fügt Scheitelpunkt hinzu; Rechtsklick schließt das Polygon (>=3).
 * - Es gibt Outline- und Filled-Varianten für Triangle, Rectangle und Polygon.
...
/* Polygons (variable vertex count per polygon) */
static int polys_count = 0;
static int polys_vert_count[MAX_POLYS];
static double polys_x[MAX_POLYS][MAX_POLY_VERTS];
static double polys_y[MAX_POLYS][MAX_POLY_VERTS];
static unsigned int polys_color[MAX_POLYS];
static int polys_filled[MAX_POLYS];
...
/* Polygon pending buffer */
static int poly_pending_count = 0;
static double poly_pending_x[MAX_POLY_VERTS], poly_pending_y[MAX_POLY_VERTS];
static int poly_pending_filled = 0; /* set depending on selected tool */
...
/* Toolbar buttons
   Added outline + filled variants for triangle, rectangle and polygon.
*/
enum {
    BTN_LINE = 0,
    BTN_CIRCLE = 1,
...
/* Icon: Polygon (outline or filled) */
static void iconPolygon(float size, int filled) {
    /* simple pentagon */
    float r = size * 0.32f;
    int n = 5;
    if (filled) {
        glBegin(GL_POLYGON);
        for (int i=0;i<n;++i) {
            float a = (float)i / n * 2.0f * PI - PI/2.0f;
            glVertex2f(cosf(a)*r, sinf(a)*r);
        }
        glEnd();
    } else {
...
static void compute_button_rects(int rects[BTN_COUNT][4]) {
    ...
    x += 36 + spacing;
    rects[BTN_POLY_OUT][0] = x; rects[BTN_POLY_OUT][1] = top; rects[BTN_POLY_OUT][2] = x + 36; rects[BTN_POLY_OUT][3] = top + btn_h;
    x += 36 + spacing;
    rects[BTN_POLY_FILLED][0] = x; rects[BTN_POLY_FILLED][1] = top; rects[BTN_POLY_FILLED][2] = x + 36; rects[BTN_POLY_FILLED][3] = top + btn_h;
    x += 36 + spacing;
    ...
    for (int i=0;i<BTN_COUNT;++i) {
        ...
        if (i == BTN_GRID) {
            ...
            if ((i == BTN_LINE && selected_tool == TOOL_LINE) ||
                ...
                (i == BTN_POLY_OUT && selected_tool == TOOL_POLY_OUT) ||
                (i == BTN_POLY_FILLED && selected_tool == TOOL_POLY_FILLED)) {
        ...
        else if (i == BTN_POLY_OUT) iconPolygon(size, 0);
        else if (i == BTN_POLY_FILLED) iconPolygon(size, 1);
...
static void display_cb(void) {
    ...
    /* Draw polygons (filled or outline) */
    for (int i=0;i<polys_count;++i) {
        unsigned int hex = polys_color[i];
        float rcol = ((hex>>16)&0xFF)/255.0f;
        float gcol = ((hex>>8)&0xFF)/255.0f;
        float bcol = (hex&0xFF)/255.0f;
        glColor3f(rcol,gcol,bcol);
        int n = polys_vert_count[i];
        if (n < 3) continue;
        if (polys_filled[i]) {
            glBegin(GL_POLYGON);
            for (int v=0; v<n; ++v) {
                int sx, sy;
                world_to_screen(polys_x[i][v], polys_y[i][v], &sx, &sy);
                glVertex2i(sx, sy);
            }
            glEnd();
        } else {
    ...
    /* If polygon pending vertices, draw preview (markers + lines) */
    if (poly_pending_count > 0) {
        glColor3ub(80, 80, 200);
        for (int i=0;i<poly_pending_count;++i) {
            int sx, sy;
            world_to_screen(poly_pending_x[i], poly_pending_y[i], &sx, &sy);
            int r = 4;
            glBegin(GL_TRIANGLE_FAN);
              glVertex2i(sx, sy);
              for (int a=0;a<=20;++a) { double ang = a*(2.0*M_PI/20.0); glVertex2f(sx+cos(ang)*r, sy+sin(ang)*r); }
    ...
    else if (selected_tool == TOOL_POLY_OUT) toolname = "Polygon (outline)";
    else if (selected_tool == TOOL_POLY_FILLED) toolname = "Polygon (filled)";
    snprintf(status, sizeof(status), "Tool: %s  Color: #%06X  Zoom: %.3fx  Pan: (%.2f, %.2f)  Grid: %s  Snap: %s  Lines: %d  Circles: %d  Tris: %d  Rects: %d  Polys: %d",
             toolname, current_color, zoom_factor, pan_x, pan_y, grid_visible ? "on" : "off", snap_enabled ? "on" : "off",
             lines_count, circles_count, tris_count, rects_count, polys_count);
...
/* Helper to finalize pending polygon (store if >=3 vertices) */
static void finalize_pending_polygon() {
    if (poly_pending_count >= 3 && polys_count < MAX_POLYS) {
        int idx = polys_count++;
        polys_vert_count[idx] = poly_pending_count;
        for (int v=0; v<poly_pending_count && v<MAX_POLY_VERTS; ++v) {
            polys_x[idx][v] = poly_pending_x[v];
            polys_y[idx][v] = poly_pending_y[v];
        }
        polys_color[idx] = current_color;
        polys_filled[idx] = poly_pending_filled ? 1 : 0;
    }
    poly_pending_count = 0;
}
    ...
    if (button == GLUT_LEFT_BUTTON) {
        if (state == GLUT_DOWN) {
            ...
            if (pressed_button >= 0) {
                ...
                } else if (clicked == BTN_POLY_OUT) {
                    if (selected_tool == TOOL_POLY_OUT) selected_tool = TOOL_NONE;
                    else {
                        selected_tool = TOOL_POLY_OUT;
                        poly_pending_filled = 0;
                    }
                } else if (clicked == BTN_POLY_FILLED) {
                    if (selected_tool == TOOL_POLY_FILLED) selected_tool = TOOL_NONE;
                    else {
                        selected_tool = TOOL_POLY_FILLED;
                        poly_pending_filled = 1;
                    }
                ...
                if (!left_drag_moved) {
                    ...
                    } else if (selected_tool == TOOL_POLY_OUT || selected_tool == TOOL_POLY_FILLED) {
                        /* add vertex to pending polygon */
                        if (poly_pending_count < MAX_POLY_VERTS) {
                            poly_pending_x[poly_pending_count] = wx;
                            poly_pending_y[poly_pending_count] = wy;
                            poly_pending_count++;
                        }
                    } else {
        ...
        if (state == GLUT_DOWN) {
            /* Right-click: cancel pending for current tool or finish polyline/polygon */
            if (selected_tool == TOOL_LINE && line_active) {
                ...
                /* finalize polygon if enough vertices, otherwise cancel */
                if (poly_pending_count >= 3) {
                    /* store polygon */
                    poly_pending_filled = (selected_tool == TOOL_POLY_FILLED) ? 1 : 0;
                    finalize_pending_polygon();
                } else {
                    poly_pending_count = 0;
                }
            }
    ...
    if (panning) {
        ...
        for (int i=0;i<BTN_COUNT;++i) {
            if (point_in_rect(x,y,rects[i])) {
                ...
                else if (i == BTN_POLY_OUT) snprintf(tooltip_text, sizeof(tooltip_text), "Polygon outline");
                else if (i == BTN_POLY_FILLED) snprintf(tooltip_text, sizeof(tooltip_text), "Polygon filled");
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("CAD Demo with Tools: Line, Circle, Triangle, Rect, Polygon (filled/outline) and Palette");
