            ...
            for (int x = 0; x < Config::FRAME_WIDTH; x += 2) {
                ...
                auto calculateNeonColor = [&](quint8 Y_val, int x_pos) {
                    if (Y_val >= Config::PIXEL_THRESHOLD) {
                        ...
                        // 1. Grundhelligkeit erhöhen, um Glüheffekt zu simulieren
                        double glowFactor = 1.0 + (t * 0.5); // Bis zu 50% heller als 255
                        
                        // 2. Farb-Mapping basierend auf Intensität (t)
                        // Blau/Violett -> Magenta -> Weiß/Gelb
                        if (t < 0.3) {
                            ...
                            r = (int)(255 * glowFactor);
                            g = (int)(255 * (t - 0.7) / 0.3 * glowFactor); // Grün steigt schnell an
                            b = (int)(255 * t * glowFactor); // Blau ist auch hoch für Weiß
                        }
                        
                        // Sicherstellen, dass die Farbwerte im gültigen Bereich [0, 255] liegen
                        r = qMin(255, qMax(0, r));
                        g = qMin(255, qMax(0, g));
                        b = qMin(255, qMax(0, b));
...
private slots:
    ...
    void updateDosageDisplay(double dosage) { 
        // Anpassung aus dem vorherigen Schritt beibehalten (Schriftgröße im b-Tag etwas kleiner)
