// editor_big2_refactored.c
// gcc -O2 -o editor_big2_refactored editor_big2_refactored.c -lSDL2 -lSDL2_ttf
// Refaktorierte Version: main() schlank, Logik in Unterfunktionen ausgelagert.
...
/* Binary search over codepoint offsets to find best byte offset for pixel x */
...
static int create_ui(UI *ui, const char *font_path) {
    ui->win = SDL_CreateWindow("Editor Big 2x Refactored", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WINW, WINH, SDL_WINDOW_RESIZABLE);
    if (!ui->win) return 0;
...
static int handle_events(Doc *doc, UI *ui, int *cx, int *cy, int *scroll_y, int *dragging_thumb, int *thumb_drag_offset) {
    ...
    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_QUIT) { running = 0; break; }
            ...
            // nothing here; render recalculates viewport
        }
...
int main(int argc, char **argv) {
    ...
    while (running) {
        ...
        // Render (Caret nur anzeigen wenn show_cursor)
        // Temporär: wenn show_cursor==0, render_frame zeichnet trotzdem Caret; wir können Caret-Ausgabe steuern:
        if (show_cursor) render_frame(&doc, &ui, cx, cy, scroll_y);
        else {
            // Render ohne Caret: render lines but skip caret draw by passing cx=-1
            render_frame(&doc, &ui, -1, cy, scroll_y);
        }
        SDL_Delay(10);
    }
