...
//
// NOTE: This is a complete source file. Replace your current gitpush1.cpp with this file.
...
public slots:
    void run() {
        ...
        if (error) {
            ...
            // try HEAD:refs/heads/main as fallback
            ...
            if (error) {
                ...
                emit message(QStringLiteral("[%1] ERROR push fallback failed: %2 (code %3)").arg(timestamp()).arg(errMsg2).arg(QString::number(error)));
                git_remote_free(remote);
                git_repository_free(repo);
                git_libgit2_shutdown();
                emit finished(12);
...
public slots:
    void onLibgit2Progress(qint64 received, qint64 total) {
        ...
        // keep logging of progress messages, but do not log per-second upload speed here
...
private slots:
    void updateSpeed() {
        ...
        // Do NOT log speed into the text box; show it in the dedicated label instead
