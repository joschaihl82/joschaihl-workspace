...
/* New main: write assembly to input.s (or input.s if input not .c), set tempfile,
   emit assembly, and call assemble_file() in-process. */
int main(int argc, char **argv) {
    ...
    /* Set global tempfile so println and other emitters write to the .s file.
       This guarantees tempfile is initialized before any codegen uses it. */
    tempfile = asm_out;
    ...
    /* Emit assembly into asm_out via codegen (println writes to tempfile). */
    ...
    /* Flush and close the assembly file before assembling. */
    fflush(asm_out);
    fclose(asm_out);
    tempfile = NULL; /* avoid accidental further writes to closed FILE* */

    /* If -S is given, assembly text is the final output: copy asm_path -> output_path. */
    if (opt_S) {
        FILE *in = fopen(asm_path, "r");
        if (!in) {
            error("cannot open assembly file for copying: %s: %s", asm_path, strerror(errno));
        }
        FILE *out;
        ...
        fclose(in);
        /* remove the .s file if you don't want to keep it */
        free(asm_path);
        exit(0);
    }

    /* Otherwise, call assembler in-process */
    uint32_t load_addr = 0; /* set if you parse a -a option earlier */
