// alpha_overlay_combined_full.c
// Vollbild-ARGB-Overlay mit ~85% Alpha, Input-Passthrough, robust gegen BadMatch
// Deckt komplette Root-Fläche ab (Multi-Monitor übliches Root-Boundingbox-Verhalten)
// Kompilieren: gcc -O2 -Wall -o alpha_overlay_combined_full alpha_overlay_combined_full.c -lX11 -lXrender -lXext
...
int main(void) {
    ...
    /* Bestimme Größe der Root-Window direkt (kombinierte Desktop-Größe) */
    int sw = 0, sh = 0;
    {
        Window root_ret;
        int x, y, bw, depth;
        unsigned int uw, uh;
        if (XGetGeometry(dpy, root, &root_ret, &x, &y, &uw, &uh, (unsigned int *)&bw, &depth)) {
            sw = (int)uw;
            sh = (int)uh;
        } else {
            /* Fallback */
    ...
    XStoreName(dpy, win, "alpha_overlay_combined_full");
    XMapRaised(dpy, win);

    /* Stelle sicher, dass das Fenster die gesamte Root-Fläche belegt */
    XMoveResizeWindow(dpy, win, 0, 0, (unsigned int)sw, (unsigned int)sh);
    XFlush(dpy);
    ...
    if (pat_pic) {
        ...
        for (int y = 0; y < pat_h; y += 32) {
            for (int x = 0; x < pat_w; x += 32) {
                int idx = ((x + y) / 32) & 3;
                unsigned long col1 = ((cols[idx].red & 0xFF00) << 8) | ((cols[idx].green & 0xFF00)) | ((cols[idx].blue & 0xFF00) >> 8);
                unsigned long col2 = ((cols[(idx+1)&3].red & 0xFF00) << 8) | ((cols[(idx+1)&3].green & 0xFF00)) | ((cols[(idx+1)&3].blue & 0xFF00) >> 8);
                XSetForeground(dpy, gc, col1);
                XFillRectangle(dpy, pat_pix, gc, x, y, 32, 32);
                XSetForeground(dpy, gc, col2);
        ...
        while (XPending(dpy)) {
            ...
            if (ev.type == ConfigureNotify) {
                ...
                if (ce.width != sw || ce.height != sh) {
                    sw = ce.width;
                    sh = ce.height;
                    /* Fenster explizit an Root-Bounding-Box ausrichten */
                    XMoveResizeWindow(dpy, win, 0, 0, (unsigned int)sw, (unsigned int)sh);
                }
            }
        }
