/*
 * main.cpp - qgeister1 "CIA Modell Berlin 6" (CSV only)
 *
 * Minimaler Amalgamat-Build: Berlin6 Preset immer aktiv, CSV-Logging standardmäßig aktiviert.
 * Dieses Programm läuft ausschließlich im CIA "Berlin 6" Modus und schreibt fortlaufend
 * berlin6_log.csv (ts_ms, Sv/s, avg Sv/s, total Sv, alert).
 *
 * Änderungen gegenüber Berlin5:
 * - Sensitivität leicht reduziert gegenüber Berlin5 (weniger false positives)
 * - Klick-Mapping dichter bei mittlerer Aktivität, sanfter bei Extremwerten (softcap)
 * - Alert-Hysterese: längere Bestätigungszeit, kürzere Nachhaltezeit
 * - CSV-Dateiname angepasst
 *
 * Build: Linux, link with -lasound and Qt Widgets
...
// --- Helper: Formatierung Sv/s mit SI‑Präfixen (milli, micro, nano, pico, femto)
static QString formatSvPerS(double value) {
    if (qIsNaN(value)) return QString("NaN Sv/s");
    double v = value;
    double absv = qAbs(v);
    const char *prefix = "";
    double scaled = v;
    if (absv >= 1.0) {
        ...
        prefix = "m"; scaled = v * 1e3;
    } else if (absv >= 1e-6) {
        prefix = "µ"; scaled = v * 1e6;
    } else if (absv >= 1e-9) {
        prefix = "n"; scaled = v * 1e9;
    } else if (absv >= 1e-12) {
        prefix = "p"; scaled = v * 1e12;
    } else {
        prefix = "f"; scaled = v * 1e15;
    }
    return QString("%1 %2Sv/s").arg(scaled, 0, 'f', 3).arg(prefix);
}

static QString formatSv(double sv) {
    if (qIsNaN(sv)) return QString("NaN Sv");
    double absv = qAbs(sv);
    const char *prefix = "";
    double scaled = sv;
    if (absv >= 1.0) {
        ...
        prefix = "m"; scaled = sv * 1e3;
    } else if (absv >= 1e-6) {
        prefix = "µ"; scaled = sv * 1e6;
    } else if (absv >= 1e-9) {
        prefix = "n"; scaled = sv * 1e9;
    } else if (absv >= 1e-12) {
        prefix = "p"; scaled = sv * 1e12;
    } else {
        prefix = "f"; scaled = sv * 1e15;
    }
    return QString("%1 %2Sv").arg(scaled, 0, 'f', 3).arg(prefix);
}

// --- Configuration (fixed Berlin6 + CSV) ---
...
// Berlin6 tweaks vs Berlin5:
// - Sensitivity slightly lower (less aggressive): scale 1.5 vs 1.8
// - Click gain uses softcap (log-like) to avoid runaway click explosion
// - Alert confirmation longer, hold shorter
static const bool CIA_MODE_BERLIN6 = true;
static const bool CIA_ENABLE_CSV_LOG = true;
static const double CIA_BERLIN6_SENS_SCALE = 1.5;     // hotPixel->Sv/h scale (slightly lower)
static const double CIA_BERLIN6_CLICK_GAIN = 1.4;     // click mapping gain (moderate)
static const double CIA_BERLIN6_CLICK_MAX = 400.0;    // hard cap clicks/s
static const double CIA_BERLIN6_ALERT_CONFIRM_SEC = 3.0; // need 3s over threshold to trigger
static const double CIA_BERLIN6_ALERT_HOLD_SEC = 0.8;    // keep alert briefly after drop
static const double CIA_BERLIN6_ALERT_THRESHOLD_S_PER_S = (GEIGER_ALERT_THRESH / 3600.0) * 0.7; // 70% of old

// --- Data structures ---
struct GeigerStats {
    double currentRad = 0.0; // Sv/s
    double avgRad = 0.0;     // Sv/s
    double totalDose = 0.0;  // Sv
...
public:
    explicit GeigerLogic(QObject *parent = nullptr) : QObject(parent) {
        ...
        // Open CSV for Berlin6 logging immediately
        if (CIA_ENABLE_CSV_LOG) {
            berlinCsvFile.setFileName("berlin6_log.csv");
            if (berlinCsvFile.open(QIODevice::Append | QIODevice::Text)) {
                QTextStream ts(&berlinCsvFile);
                if (berlinCsvFile.size() == 0) ts << "ts_ms,raw_Sv_per_s,avg_Sv_per_s,total_Sv,alert\n";
                berlinCsvFile.flush();
            } else {
                qWarning() << "Konnte berlin6_log.csv nicht öffnen zum Schreiben";
            }
        }
    }

    ~GeigerLogic() {
        if (berlinCsvFile.isOpen()) berlinCsvFile.close();
    }
    ...
    QImage processRadionullon(const QImage &img) {
        ...
        // long exposure EMA (double)
        for (int i = 0; i < size; ++i) {
            longExp[i] = (1.0 - LONG_EXP_LEARN) * longExp[i] + LONG_EXP_LEARN * (double)gray[i];
        }

        // bg + residue
        double globalAvg = 0.0;
        for (int i = 0; i < size; ++i) {
            bg[i] = (1.0 - BG_LEARN_RATE) * bg[i] + BG_LEARN_RATE * (double)gray[i];
            double combinedBg = 0.7 * bg[i] + 0.3 * longExp[i];
            int diff = qAbs((int)gray[i] - (int)qRound(combinedBg));
            residue[i] = (diff > RESIDUE_THRESH) ? (quint8)qMin(diff, 255) : 0;
            globalAvg += (double)gray[i];
        }
        globalAvg /= (double)size;

        // flicker
        double currentAvg = qBound(0.0, globalAvg, 255.0);
        int delta = (int)qAbs(currentAvg - prevGlobalAvg);
        prevGlobalAvg = currentAvg;
        int boost = (delta > FLICKER_THRESH) ? 20 : 1;
        for (int i = 0; i < size; i++) {
            flicker[i] = (quint8)qBound(0, (int)flicker[i] + boost, 255);
            if (flicker[i] > 0) flicker[i] = (quint8)qMax(0, (int)flicker[i] - 1);
        }
        ...
        for (int y = 1; y < IMG_H-1; y++) {
            for (int x = 1; x < IMG_W-1; x++) {
                ...
                int mag = (int)(EDGE_GAIN * qSqrt((double)(gx*gx + gy*gy)));
        ...
        for (int y = 0; y < IMG_H; ++y) {
            ...
            for (int x = 0; x < IMG_W; ++x) {
                ...
                if (score < RADIONULLON_THRESH) {
                    ...
                    int mixed = (int)((0.7 * (double)gray[i]) + (0.3 * (double)le));
        ...
        // --- Geiger values (Berlin6 scaling applied) ---
        ...
        // frame interval measurement for dose accumulation
        double frameSec = 0.1;
        ...
        // Build stats and Berlin6 alert/hysteresis
        GeigerStats stats;
        stats.currentRad = rawVal;
        stats.avgRad = currentRadSmooth;
        stats.totalDose = totalDoseAcc;

        // Berlin6 short-term hysteresis logic (confirm longer, hold shorter)
        double nowSec = QDateTime::currentDateTime().toMSecsSinceEpoch() * 0.001;
        if (currentRadSmooth > CIA_BERLIN6_ALERT_THRESHOLD_S_PER_S) {
            if (berlin_peak_start_sec < 0.0) berlin_peak_start_sec = nowSec;
            if ((nowSec - berlin_peak_start_sec) >= CIA_BERLIN6_ALERT_CONFIRM_SEC) {
                berlin_alert_active = true;
            }
        } else {
            if (berlin_alert_active && (nowSec - berlin_peak_start_sec) < (CIA_BERLIN6_ALERT_CONFIRM_SEC + CIA_BERLIN6_ALERT_HOLD_SEC)) {
                // keep alert briefly
            } else {
                berlin_alert_active = false;
                berlin_peak_start_sec = -1.0;
            }
        }
        stats.alertState = berlin_alert_active;
        ...
        if (berlinCsvFile.isOpen()) {
            QTextStream ts(&berlinCsvFile);
            ts << QString::number(QDateTime::currentMSecsSinceEpoch()) << ",";
            ts << QString::number(rawVal, 'g', 8) << ",";
            ts << QString::number(currentRadSmooth, 'g', 8) << ",";
            ts << QString::number(totalDoseAcc, 'g', 12) << ",";
            ts << (stats.alertState ? "1" : "0") << "\n";
            berlinCsvFile.flush();
        }

        // send activity in Sv/s to audio engine (Berlin6 clicks)
...
signals:
    ...
    void requestClick(double activity);
...
private:
    ...
    double prevGlobalAvg = 128.0;
    QVector<double> longExp;
    ...
    // Berlin6 state
    double berlin_peak_start_sec = -1.0;
    bool berlin_alert_active = false;
    QFile berlinCsvFile;
...
// --- AudioEngine (Berlin6 click mapping, event-driven with softcap) ---
        ...
        while (running.load()) {
            {
                ...
                while (!clickQueue.empty()) {
                    ...
                    // Map activity (Sv/s) -> expected clicks/s with softcap
                    ...
                    // Softcap: use sqrt to compress very high values, then cap
                    double soft = sqrt(scaled);
                    double clicksPerSecond_est = qMin(CIA_BERLIN6_CLICK_MAX, soft * soft); // effectively scaled but compressed
                    if (clicksPerSecond_est < 1.0) clicksPerSecond_est = 1.0;

                    // Expectation per enqueue cycle window (0.02s)
                    double expectation = clicksPerSecond_est * 0.02;
                    int injectCount = (int)qFloor(expectation);
                    ...
                    for (int i = 0; i < injectCount; ++i) {
                        ActiveClick ac;
                        ac.pos = rand() % qMax(1, (int)(clickLenMs * 0.001 * sampleRate));
                        ...
                        ac.pitchVar = 1.0 + ((((seed >> 8) & 0xFF) / 255.0) - 0.5) * 0.06;
                        ac.id = nextClickId++;
...
public:
    GeigerWidget(QWidget *parent = nullptr) : QWidget(parent) {
        setWindowTitle("qrad Radionullon Viewer | CIA Modell Berlin 6");
...
protected:
    void paintEvent(QPaintEvent *) override {
        ...
        p.drawText(textX, textY, QString("AKTUELL: %1").arg(formatSvPerS(m_stats.currentRad)));
        p.setFont(QFont("Monospace", 12));
        p.setPen(Qt::white);
        p.drawText(textX, textY + 22, QString("Ø MITTEL: %1").arg(formatSvPerS(m_stats.avgRad)));
        p.drawText(textX, textY + 42, QString("GESAMT DOSE: %1").arg(formatSv(m_stats.totalDose)));

        // Berlin6 visual alert (blinking red frame)
        if (m_stats.alertState) {
            QColor blinkCol = QColor(255,0,0, (int)(128 + 127 * qSin(QDateTime::currentMSecsSinceEpoch()/200.0)));
            QPen pen(blinkCol, 6);
            p.setPen(pen);
        ...
        QString avgLabel = QString("Ø %1").arg(formatSvPerS(m_stats.avgRad));
...
public slots:
    void updateIcon(GeigerStats stats) {
        ...
        tray->setToolTip(QString("Rad: %1 | Avg: %2").arg(formatSvPerS(stats.currentRad)).arg(formatSvPerS(stats.avgRad)));
    }
