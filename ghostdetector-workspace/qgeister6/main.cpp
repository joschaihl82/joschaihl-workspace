...
public:
    explicit GeigerLogic(QObject *parent = nullptr) : QObject(parent) {
        ...
        // Langzeit-Integrationsbild initialisieren
    ...
    QImage processRadionullon(const QImage &img) {
        ...
        // --- Langzeit-Mittelwertintegration (Langzeitbelichtung) ---
        // longExp[i] = (1 - LONG_EXP_LEARN) * longExp[i] + LONG_EXP_LEARN * gray[i]
        ...
        // Wir kombinieren kurzes BG-EMA (bg) mit dem Langzeitbild (longExp) für robustere Residue-Erkennung.
        float globalAvg = 0.0f;
        for (int i = 0; i < size; i++) {
            // Update BG (kurzfristiges EMA)
            bg[i] = (1.0f - BG_LEARN_RATE) * bg[i] + BG_LEARN_RATE * (float)gray[i];

            // Kombiniertes Hintergrundsignal: gewichteter Mix aus kurzzeitigem BG und Langzeit-Integration
            float combinedBg = 0.7f * bg[i] + 0.3f * longExp[i];

            // Residue relativ zum kombinierten Hintergrund
        ...
        for (int y = 0; y < IMG_H; ++y) {
            ...
            for (int x = 0; x < IMG_W; ++x) {
                ...
                if (score < RADIONULLON_THRESH) {
                    // Mischung aus aktuellem Frame und Langzeit-Integrationsbild zur Darstellung
                    int le = qBound(0, (int)qRound(longExp[i]), 255);
                    int mixed = (int)((0.7f * gray[i]) + (0.3f * le)); // 70% aktuell, 30% Langzeit
...
private:
    ...
    // Langzeit-Integrationsbild (float pro Pixel)
...
// --- UI Widget (Für Dual-Display angepasst, skaliert bei Größenänderung) ---
...
public:
    GeigerWidget(QWidget *parent = nullptr) : QWidget(parent) {
        ...
        // Basissize (logical coordinate system) — darauf basiert das Seitenverhältnis
        baseW = 2 * IMG_W + 15;
        baseH = IMG_H + 110;
        resize(baseW, baseH);
...
public slots:
    ...
    void updateOriginalImage(const QImage &img) { m_lastFrame_original = img; update(); } // Originalbild von V4L2Capture
    void updateProcessedImage(const QImage &img) { m_lastFrame_processed = img; update(); } // Verarbeitetes Bild von GeigerLogic

protected:
    void paintEvent(QPaintEvent *) override {
        QPainter p(this);
        p.fillRect(rect(), Qt::black); // Sicherstellen, dass der Hintergrund schwarz ist

        // Apply uniform scale so the logical layout keeps the original ratio
        p.save();
        p.scale(scaleFactor, scaleFactor);

        int gap = 5;

        // --- 1. Linkes Bild: Original Video ---
        if (!m_lastFrame_original.isNull()) {
            p.drawImage(QRect(gap, gap, IMG_W, IMG_H), m_lastFrame_original);
        } else {
            p.setPen(Qt::white);
            p.drawText(QRect(gap, gap, IMG_W, IMG_H), Qt::AlignCenter, "Original (Warte auf Stream)");
        }
        p.setPen(Qt::white);
        p.drawText(QRect(gap, IMG_H + gap + 5, IMG_W, 20), Qt::AlignCenter, "UNVERÄNDERTES ORIGINAL");

        // --- 2. Rechtes Bild: Radionullon Heatmap ---
        if (!m_lastFrame_processed.isNull()) {
            p.drawImage(QRect(IMG_W + gap*2, gap, IMG_W, IMG_H), m_lastFrame_processed);
        } else {
            p.setPen(Qt::white);
            p.drawText(QRect(IMG_W + gap*2, gap, IMG_W, IMG_H), Qt::AlignCenter, "Radionullon (Warte auf Stream)");
        }
        p.setPen(QColor(255, 150, 0)); // Orange
        p.drawText(QRect(IMG_W + gap*2, IMG_H + gap + 5, IMG_W, 20), Qt::AlignCenter, "RADIONULLON HEATMAP (ALGORITHMUS)");


        // --- 3. Overlay Text und Geiger-Werte (Unten) ---
        int textY = IMG_H + 40;
        int textX = 20;
        ...
        p.setPen(Qt::white);
        p.drawText(textX, textY + 30, QString("Ø MITTEL: %1 Sv/h").arg(m_stats.avgRad, 0, 'f', 2));
        p.drawText(textX, textY + 50, QString("GESAMT DOSE: %1 Sv").arg(m_stats.totalDose, 0, 'f', 4));


        // 4. Die 3 transparenten Graphen (Unter den Bildern)
        int graphH = 80;
        int margin = 10;
        int totalW = baseW - (4 * margin);
        int singleGraphW = (totalW - 20) / 3; // Platz lassen für Abstand
        int yPos = baseH - graphH - margin;
    ...
    void resizeEvent(QResizeEvent *event) override {
        Q_UNUSED(event);
        // Berechne eine uniforme Skalierung basierend auf dem Basissize (baseW/baseH)
        float sx = (float)width() / (float)baseW;
        float sy = (float)height() / (float)baseH;
        scaleFactor = qMin(sx, sy);
        update();
    }
...
private:
    void drawGraph(QPainter &p, QRect r, const QVector<double> &data, QColor col, QString title, QString labelL, QString labelR) {
        ...
        maxVal = qMax(maxVal, 3.0); // Mindesthöhe für Graphen
        ...
        for (int i = 0; i < data.size(); ++i) {
            double valNorm = qMin(data[i] / maxVal, 1.0); // Clamping
