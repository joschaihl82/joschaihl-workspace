...
public slots:
    void run() {
        ...
        // 1) create repo on GitHub (use robust helper)
        emit message(QStringLiteral("[%1] Creating repository on GitHub: %2").arg(timestamp()).arg(QFileInfo(m_dirPath).fileName()));
        ...
        QByteArray resp;
        QString netErr;
        if (!postJsonSyncWithDebug(mgr, url, json, authHeader, 30000, httpStatus, resp, netErr)) {
        ...
        emit message(QStringLiteral("[%1] Repository created on GitHub: %2 (HTTP %3)").arg(timestamp()).arg(QFileInfo(m_dirPath).fileName()).arg(QString::number(httpStatus)));
        ...
        emit message(QStringLiteral("[%1] Remote origin set to %2").arg(timestamp()).arg(remoteUrl));
        ...
        if ((error = git_remote_lookup(&r, repo, "origin")) != 0) {
            emit message(QStringLiteral("[%1] ERROR remote lookup: %2").arg(timestamp()).arg(QString::number(error)));
            git_repository_free(repo);
            git_libgit2_shutdown();
            emit finished(11);
            return;
...
// ---------- Small GUI (auto-start, logging text area) ----------
...
public slots:
    ...
    void onProgress(qint64 transferred, qint64 total, double bytesPerSec) {
        ...
        appendLog(QStringLiteral("[%1] Progress: %2/%3 — %4").arg(timestamp()).arg(QString::number(transferred)).arg(QString::number(total)).arg(speedStr));
    }
