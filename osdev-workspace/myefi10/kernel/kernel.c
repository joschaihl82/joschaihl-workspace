// kernel.c
// Kernel entry for the provided UEFI bootloader.
// - Exports `_start` so the bootloader's e_entry points to valid code.
// - `printf` prints to serial and, when available, also to the framebuffer.
// - Minimal serial + framebuffer output, simple page allocator, NVMe init/read (best-effort).
//
// Build and link as before; the UEFI bootloader will call the ELF e_entry which should point to `_start`.
