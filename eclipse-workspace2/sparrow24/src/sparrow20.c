 ...
 * x86-64 AT&T assembly. Adds support for function calls, local variables,
 * object/array literals, and index/member access.
 *
 * Build:
...
static Node *parse_primary(Parser *p) {
    ...
    // Arrayliteral: [ 1, 2, 3 ]
    if (tk.kind == TK_OP && tk.op == '[') {
        // FIX: Erwarte ']' (93) als schließendes Zeichen
...
static void analyze_function_scope(Node *fn) {
    ...
    // 1. Parameter (werden vom Register auf den Stack gespeichert)
    for (int i = 0; i < fn->param_count; ++i) {
        LVar *lv = calloc(1, sizeof(LVar));
        if (!lv) die("out of memory");
        lv->name = fn->params[i];

        offset -= 8; // Stack wächst nach unten
        lv->offset = offset;
    ...
    // 2. Lokale Variablen (ND_VARDECL)
    Node *s = fn->body->stmts;
    // Vereinfachung: Wir finden nur var-Deklarationen im äußersten Block.
    while (s) {
        if (s->kind == ND_VARDECL) {
            if (find_local(fn, s->var_name)) {
                die("variable '%s' redefined in function '%s'", s->var_name, fn->name);
            }
    ...
    // 3. Stack-Größe auf 16 Bytes ausrichten (ABI)
    fn->stack_size = (abs(offset) + 15) & ~15;
}
...
static void cg_emit_stmt(CodeGen *cg, Node *n) {
    ...
    case ND_EXPR_STMT:
        if (n->lhs && n->lhs->kind == ND_CALL) {
            ...
            /* Jetzt behandeln wir alle anderen Aufrufe über cg_emit_expr */
            cg_emit_expr(cg, n->lhs);
            return;
        } else {
            cg_emit_expr(cg, n->lhs);
...
static void run_tests(void) {
    struct Test { const char *name; const char *src; const char *expect; } tests[] = {
        ...
        /* Lokale Variablen, Funktionsaufruf (mit ABI-Fix) */
        { "local-vars",
