...
#include <QPen> 
#include <QListWidget> // Hinzugefügt für die Listbox
#include <QListWidgetItem> // Hinzugefügt für Listeneinträge
...
#include <cstring>      
#include <cstdlib>
#include <ctime> // Hinzugefügt für std::time
...
namespace Config {
    ...
    const int PU239_CHANCE_PERCENT = 3;  // Hohe Gefahr (ID 2)
    const int CS137_CHANCE_PERCENT = 10; // Mittlere Gefahr (ID 1)
    // K-40 (ID 0) erhält den Rest
}
...
public slots:
    void playClick(double intensity, int isotopeId) {
        ...
        // Anpassung der Klick-Frequenz basierend auf Isotop
        if (isotopeId == 2) {
            // Pu-239: Sehr hoher Ton, kürzere Dauer (Alpha-Teilchen Simulation)
            dynamicFreq = Config::MAX_PITCH_FREQ * 0.95 + (freqRange * normalizedIntensity * 0.5); 
            clickDuration = 0.003; 
        } else if (isotopeId == 1) {
            // Cs-137: Mittlerer Ton
            dynamicFreq = Config::MIN_PITCH_FREQ * 1.5 + (freqRange * normalizedIntensity * 0.7);
            clickDuration = 0.008;
        } else {
            // K-40: Niedriger Ton (Standard)
            dynamicFreq = Config::MIN_PITCH_FREQ + (freqRange * normalizedIntensity * 0.5);
            clickDuration = 0.01;
        }
...
class CameraCapturer : public QThread {
	Q_OBJECT
// ... (CameraCapturer bleibt unverändert) ...
public:
...
class ImageProcessor : public QObject {
	Q_OBJECT
// ... (ImageProcessor bleibt unverändert) ...
public:
...
public:
    explicit DosimeterLogic(QObject *parent = nullptr) : QObject(parent) {
        ...
        m_lastAlertTime = 0; 
        m_isRunning = false;
        m_dangerousIsotopeCount = 0; // Zähler initialisieren
    }
signals:
    ...
    // Signatur des Play-Click-Signals
    void playClick(double intensity, int isotopeId); 
    
    // Signal für die Isotopen-Protokollierung (totalCount zählt nur ID 2)
    void isotopeDetected(int isotopeId, double radValue, int totalCount); 
...
public slots:
    void processCameraData(int hotPixelCount) {
        ...
        // --- Mehrstufige Isotopen-Simulation ---
        int isotopeId = 0; // Standard: K-40
        int chance = std::rand() % 100;

        if (chance < Config::PU239_CHANCE_PERCENT) { 
             isotopeId = 2; // Pu-239 (Hohe Gefahr)
        } else if (chance < (Config::PU239_CHANCE_PERCENT + Config::CS137_CHANCE_PERCENT)) {
             isotopeId = 1; // Cs-137 (Mittlere Gefahr)
        }
        // Rest (100% - 13%) ist isotopeId = 0 (K-40)

        // Zählung nur für das Isotop der höchsten Gefahr (ID 2)
        if (isotopeId == 2) {
            m_dangerousIsotopeCount++;
        }
        
        // Protokoll-Signal senden (alle Klicks, wenn hotPixelCount > 0)
        emit isotopeDetected(isotopeId, radValue, m_dangerousIsotopeCount); 
        emit playClick(radValue, isotopeId); // Klick-Sound mit Isotopen-ID
        // ------------------------------------
...
private:
    ...
    int m_dangerousIsotopeCount; // Zählt nur Pu-239 (ID 2)
};
...
private slots:
    ...
    void addHistoryPoint(double rad) {
        ...
        if (m_series->points().count() > 1) {
            // ... (Y-Achsen-Anpassung bleibt gleich) ...
    ...
    // Slot für die Protokollierung des gefährlichen Isotops
    void logIsotopeHit(int isotopeId, double radValue, int totalCount) {
        QString isotopeType;
        QColor itemColor;
        
        // --- Logik für die Anzeige und Farbe ---
        if (isotopeId == 2) {
            isotopeType = "☢️ Pu-239 (Hohe Gefahr)";
            itemColor = QColor("#FF00FF"); // Neon-Pink/Rot für höchste Gefahr
        } else if (isotopeId == 1) {
            isotopeType = "⚠️ Cs-137 (Mittlere Gefahr)";
            itemColor = QColor("#FF9900"); // Orange für mittlere Gefahr
        } else {
            isotopeType = "🟢 K-40 (Standard)";
            itemColor = QColor("#CCCC00"); // Gelb-Grün für Standard
        }
        
        QString timestamp = QDateTime::currentDateTime().toString("hh:mm:ss.zzz");
        QString countInfo = (isotopeId == 2) ? QString("Gesamt Pu-239 Hits: **%1**").arg(totalCount) : "Zähler: N/A";
        
        QString message = QString(
            "[%1] | %2 | Int: %3 rad/m² | %4"
        ).arg(timestamp).arg(isotopeType).arg(QString::number(radValue, 'f', Config::LCD_DECIMALS)).arg(countInfo);
        
        QListWidgetItem *item = new QListWidgetItem(message);
        item->setForeground(itemColor); 
        
        // Fügt den Eintrag am Anfang der Liste ein
        m_isotopeLogList->insertItem(0, item);

        // Limitiere die Listengröße, um RAM zu sparen
        if (m_isotopeLogList->count() > 500) {
            m_isotopeLogList->takeItem(500); 
        }
...
private:
    ...
    QListWidget *m_isotopeLogList; // HINZUGEFÜGT
    ...
    void setupUI() {
        ...
        // --- TOP LAYOUT (LCD, Info und Listbox) ---
        ...
        // ListWidget für radioaktive Isotope HINZUGEFÜGT
        m_isotopeLogList = new QListWidget;
        m_isotopeLogList->setWindowTitle("Isotopen-Protokoll");
        m_isotopeLogList->setMinimumHeight(200);
        m_isotopeLogList->setMaximumWidth(400); 
        m_isotopeLogList->setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::Expanding);
        
        // Transparentes und stilisiertes Aussehen
        m_isotopeLogList->setStyleSheet(
            "QListWidget {"
        ...
        topLayout->addWidget(m_isotopeLogList, 2); 
    ...
    void setupClassesAndConnections() {
        ...
        // Verbindung für die Isotopen-Protokollierung (Listbox)
        connect(m_logic, &DosimeterLogic::isotopeDetected, this, &GeigerMainWindow::logIsotopeHit);
