...
typedef struct Node {
    ...
    /* call */
    // Für Aufrufe von ND_IDENT: name ist der Funktionsname
    // Für verschachtelte Aufrufe: name ist "nested_call" (wird nicht verwendet)
    // Das aufgerufene Ziel ist im lhs/rhs, da wir ND_CALL nicht als binären Knoten verwenden.
    struct Node *call_target; // Der eigentliche Ausdruck, der die Funktion ergibt (meist ND_IDENT)
    struct Node **args;
    int arg_count;
...
static void cg_emit_expr(CodeGen *cg, Node *n) {
    ...
    case ND_ARRAY_LITERAL: {
        ...
        // Sie dürfen nicht als eigenständige Ausdrücke zur Laufzeit erscheinen,
        // da sie keine anonyme Speicherallokation implementieren.
...
static void cg_emit_stmt(CodeGen *cg, Node *n) {
    ...
    case ND_EXPR_STMT:
        if (n->lhs && n->lhs->kind == ND_CALL) {
            ...
            // FIX: Implementiere den printf-Test-Harness-Code hier, da dies ein ND_CALL innerhalb eines ND_EXPR_STMT ist
            if (call->name && strcmp(call->name, "printf") == 0) {
    ...
    case ND_FUNC: {
        ...
        // Die Zeile 'mov $0, %%rax' wurde entfernt.
    ...
    case ND_PROGRAM: {
        ...
        // KORREKTUR TEIL 1: Generiere zuerst alle Funktionen
        ...
        // KORREKTUR TEIL 2: PROGRAM BODY (Nur ausführbare Anweisungen)
        ...
        // KORREKTUR TEIL 3: EPILOGUE (Stack restaurieren VOR exit)
...
static void run_tests(void) {
    struct Test { const char *name; const char *src; const char *expect; } tests[] = {
        ...
        // --- 8. FUNCTION CALL WITH RETURN VALUE ---
        { "func-call-return",
          "function get_val() { return 100; } var x = get_val(); printf(\"%ld\\n\", x);",
          "100\n" },

        // --- 9. COMPLEX ASSIGNMENT (ARRAY/OBJ) ---
        { "complex-assign",
          "var arr=[10, 20]; var obj={x:1}; arr[1] = obj.x + 5; printf(\"%ld\\n\", arr[1]); obj.x = 99; arr[0] = arr[1] + obj.x; printf(\"%ld\\n\", arr[0]);",
          "6\n105\n" },
