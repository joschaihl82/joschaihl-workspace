...
static void detect_eyes_mouth(uint8_t *rgb, int cx, int cy){
    ...
    // Precompute grayscale and gradient magnitude inside ROI
    ...
    // Precompute symmetry line (face vertical midline = ROI center)
    ...
    for(int y=eyes_y0+1; y<=eyes_y1-1; y++){
        ...
        for(int x=roi_x0+1; x<=roi_x1-1; x++){
            ...
            // update bests, enforce some separation
            if(score < best1.score){
                // shift best1 to best2 if far enough
                if(best1.x>=0){
                    best2 = best1;
                }
...
int main(){
    ...
    while(running){
        ...
        // Augen & Mund innerhalb der ROI erkennen und einzeichnen (präziser)
