// main.cpp
// Qt5 GUI proxy checker with auto-detection of proxy type for lines without prefix.
// Includes: progress, file watch, CSV export, "Launch Chromium with best proxy",
// and HTTP GET check to http://www.google.com expecting HTTP 200 OK.
//
// Build with qmake (qproxyperf.pro) or CMake. Requires Qt5 Widgets and pthread on Unix.
...
enum ProtoMode { MODE_AUTO, MODE_HTTP, MODE_HTTPS, MODE_SOCKS4, MODE_SOCKS5 };
...
struct Result {
    ...
    long tcp_ms;
    long proto_ms;
    long http_ms;        // ms for the HTTP GET to google.com
    QString statusText;  // e.g., "OK 200", "HTTP_ERR", "PROTO_ERR", "TCP_ERR ..."
};
...
// connect_nonblocking_addr and tcp_connect_to connect to a target host:port (used for both proxy and probe)
...
/* Protocol actions (HTTP CONNECT, SOCKS4, SOCKS5) */
...
static int perform_http_get_check(int sock, const char *host, const char *path, int timeout_ms, long *http_ms_out) {
    ...
    *line_end = '\0';
    ...
    if (strstr(buf, " 200 ") != nullptr || strstr(buf, " 200\r") != nullptr || strstr(buf, " 200\n") != nullptr) return 0;
    if (strstr(buf, "200") != nullptr) return 0;
    return -1;
}

// Probe functions: open fresh TCP connection to proxy and try a single probe.
// Return 0 on success for that probe, -1 on failure.

static int probe_socks5(const char *proxy_host, const char *proxy_port, int timeout_ms) {
    struct timespec t1, t2;
    int s = -1;
    if (tcp_connect_to(proxy_host, proxy_port, timeout_ms, &s, &t1, &t2) != 0) return -1;
    // send greeting
    unsigned char greet[3] = {0x05, 0x01, 0x00};
    int ok = -1;
    if (send_all_timeout(s, greet, 3, timeout_ms) >= 0) {
        unsigned char sel[2];
        ssize_t r = recv_some_timeout(s, sel, 2, timeout_ms);
        if (r == 2 && sel[0] == 0x05 && sel[1] != 0xFF) ok = 0;
    }
    close(s);
    return ok;
}

static int probe_socks4(const char *proxy_host, const char *proxy_port, int timeout_ms) {
    struct timespec t1, t2;
    int s = -1;
    if (tcp_connect_to(proxy_host, proxy_port, timeout_ms, &s, &t1, &t2) != 0) return -1;
    // build socks4 connect to example.com:80
    struct addrinfo hints, *res;
    memset(&hints, 0, sizeof(hints));
    hints.ai_family = AF_INET;
    hints.ai_socktype = SOCK_STREAM;
    if (getaddrinfo("example.com", "80", &hints, &res) != 0) { close(s); return -1; }
    struct sockaddr_in *sin = (struct sockaddr_in *)res->ai_addr;
    uint16_t portn = sin->sin_port; // network order
    uint32_t ipn = sin->sin_addr.s_addr;
    freeaddrinfo(res);
    ...
    memcpy(&req[2], &portn, 2);
    memcpy(&req[4], &ipn, 4);
    // send 8 bytes + null userid
    int ok = -1;
    if (send_all_timeout(s, req, 8, timeout_ms) >= 0) {
        unsigned char zero = 0x00;
        if (send_all_timeout(s, &zero, 1, timeout_ms) >= 0) {
            unsigned char resp[8];
            ssize_t r = recv_some_timeout(s, resp, sizeof(resp), timeout_ms);
            if (r >= 2 && resp[1] == 0x5A) ok = 0;
        }
    }
    close(s);
    return ok;
...
static int probe_http_connect(const char *proxy_host, const char *proxy_port, int timeout_ms) {
    struct timespec t1, t2;
    int s = -1;
    if (tcp_connect_to(proxy_host, proxy_port, timeout_ms, &s, &t1, &t2) != 0) return -1;
    int ok = -1;
    if (do_http_connect(s, "example.com", "80", timeout_ms) == 0) ok = 0;
    close(s);
    return ok;
}

static int probe_http_get(const char *proxy_host, const char *proxy_port, int timeout_ms) {
    // Some HTTP proxies accept direct GET with full URL for port 80.
    struct timespec t1, t2;
    int s = -1;
    if (tcp_connect_to(proxy_host, proxy_port, timeout_ms, &s, &t1, &t2) != 0) return -1;
    int ok = -1;
    // send GET http://www.google.com/ ...
    char req[512];
    snprintf(req, sizeof(req),
             "GET http://www.google.com/ HTTP/1.1\r\nHost: www.google.com\r\nUser-Agent: qproxyperf/1.0\r\nConnection: close\r\n\r\n");
    if (send_all_timeout(s, req, strlen(req), timeout_ms) >= 0) {
        char buf[2048];
        ssize_t r = recv_some_timeout(s, buf, sizeof(buf) - 1, timeout_ms);
        if (r > 0) {
            buf[r] = '\0';
            if (strstr(buf, "HTTP/1.1 200") || strstr(buf, "HTTP/1.0 200") || strstr(buf, " 200 ")) ok = 0;
        }
    }
    close(s);
    return ok;
...
// Detect proxy type by probing. Returns detected ProtoMode (MODE_SOCKS5, MODE_SOCKS4, MODE_HTTP) or MODE_HTTP as fallback.
static ProtoMode detect_proxy_type(const QString &host, const QString &port, int timeout_ms) {
    const char *ph = host.toUtf8().constData();
    const char *pp = port.toUtf8().constData();

    // Try SOCKS5
    if (probe_socks5(ph, pp, timeout_ms) == 0) return MODE_SOCKS5;
    // Try SOCKS4
    if (probe_socks4(ph, pp, timeout_ms) == 0) return MODE_SOCKS4;
    // Try HTTP CONNECT
    if (probe_http_connect(ph, pp, timeout_ms) == 0) return MODE_HTTP;
    // Try HTTP GET (some proxies accept direct GET)
    if (probe_http_get(ph, pp, timeout_ms) == 0) return MODE_HTTP;
    // fallback to HTTP
    return MODE_HTTP;
}
...
private:
    ...
    void run() {
        ...
        while (q->pop(job)) {
            ...
            // If mode is AUTO, detect it first (this will open several short-lived TCP connections)
            if (job.mode == MODE_AUTO) {
                ProtoMode detected = detect_proxy_type(job.host, job.port, timeout);
                job.mode = detected;
            }
            ...
            if (job.mode == MODE_HTTP || job.mode == MODE_HTTPS) {
                if (do_http_connect(sock, t_host, t_port, timeout) == 0) {
                    ...
                    // For HTTP proxies, if CONNECT failed, we may still try direct GET through proxy (some proxies don't allow CONNECT to example.com:80)
                    // Try a direct GET to www.google.com via proxy (full URL)
                    long http_ms_try = -1;
                    int http_try = perform_http_get_check(sock, "www.google.com", "/", timeout, &http_ms_try);
                    if (http_try == 0) {
                        clock_gettime(CLOCK_MONOTONIC, &t_proto_end);
                        proto_ok = true;
                    } else {
...
private slots:
    void onResultReady(const Result &r) {
        ...
        // store http_ms in a secondary role on column 3
...
private:
    ...
    static int parse_line_to_job(const QString &line, Job &out) {
        ...
        ProtoMode mode = MODE_AUTO; // default to AUTO when no prefix
        QString p = s;
        ...
        // If no prefix, mode remains MODE_AUTO and p == s
