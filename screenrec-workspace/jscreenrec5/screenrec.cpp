...
private slots:
    // Patched: capture screen and draw current mouse cursor into the frame before pushing
    void captureScreenFrame() {
        ...
        // Global cursor position and screen geometry
        ...
        // Try to obtain the current cursor pixmap and hotspot
        QCursor currentCursor = QCursor();
        QPixmap cursorPixmap = currentCursor.pixmap();
        QPoint hotspot = currentCursor.hotSpot();

        if (!cursorPixmap.isNull()) {
            // Draw the cursor pixmap at the correct hotspot position
            // If device pixel ratio differs, scale the cursor pixmap accordingly
