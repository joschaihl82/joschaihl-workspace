/* sparrow.c
 *
 * A compact compiler for a tiny JavaScript-like language that emits
 * x86-64 AT&T assembly. Single-file implementation in Plain C.
 *
 * Features:
 *  - Lexer: keywords, identifiers, numbers, strings, operators
 *  - Parser: recursive descent, AST for expressions and statements
 *  - Codegen: x86-64 AT&T assembly, printf integration, global flattening
 *  - Runtime stubs appended to assembly
 *  - Simple test harness that compiles and runs sample programs
 *
 * Usage:
 *  gcc -std=c11 -O2 -o sparrow sparrow.c
 *  ./sparrow            # runs built-in tests
 *  ./sparrow -o out.s file.sparrow
 *
 * Limitations:
 *  - Minimal type system: everything is a 64-bit integer or pointer
 *  - Strings are emitted as .asciz and referenced by label
 *  - No closures or heap-managed objects beyond flattened globals
 */
...
static const char *keywords[] = {
    "var", "function", "if", "else", "while", "for", "return", "print", NULL
};
...
static void expect_op(Parser *p, int ch) {
    Token tk = parser_peektok(p);
    if (!(tk.kind == TK_OP && tk.op == ch)) {
        die("expected '%c' at line %d", ch, tk.line);
    }
    parser_nexttok(p);
}
...
static Node *parse_primary(Parser *p) {
    ...
    die("unexpected token at line %d", tk.line);
    return NULL;
}
...
static char *escape_asm_string(const char *s) {
    /* escape backslashes and double quotes and percent signs for printf */
    size_t len = 0;
...
static void cg_emit_node(CodeGen *cg, Node *n);
...
static void cg_emit_expr(CodeGen *cg, Node *n) {
    ...
    case ND_ASSIGN: {
        /* not used in this simplified generator */
        return;
    }
    case ND_CALL: {
        /* special-case print */
        if (strcmp(n->name, "print") == 0 && n->arg_count == 1) {
            cg_emit_expr(cg, n->args[0]);
            /* if arg is string pointer, call js_print_value */
            cg_emit(cg, "    mov %%rax, %%rdi");
            cg_emit(cg, "    call js_print_value");
            return;
        }
        /* general call: push args right-to-left, call, result in rax */
        for (int i = n->arg_count - 1; i >= 0; --i) {
            cg_emit_expr(cg, n->args[i]);
            cg_emit(cg, "    push %%rax");
        }
        /* move up to 6 args into registers */
        const char *argregs[] = {"%rdi","%rsi","%rdx","%rcx","%r8","%r9"};
        for (int i = 0; i < n->arg_count && i < 6; ++i) {
            cg_emit(cg, "    pop %s", argregs[i]);
        }
        if (n->arg_count > 6) {
            /* not supported in this simple compiler */
            die("more than 6 args not supported");
        }
        cg_emit(cg, "    call %s", n->name);
        return;
    }
    default:
...
static void cg_emit_stmt(CodeGen *cg, Node *n) {
    ...
    case ND_VARDECL: {
        ...
        if (n->init && n->init->kind == ND_OBJ_LITERAL) {
            /* flatten keys into globals: var x = {a:1} -> Lglob_x_a = 1 */
            /* We'll create globals named x_key */
            for (int i = 0; i < n->init->kv_count; ++i) {
                char buf[256];
                snprintf(buf, sizeof(buf), "%s_%s", n->var_name, n->init->keys[i]);
                add_global(buf);
            }
            /* top-level var itself can be pointer to first key or 0 */
            add_global(n->var_name);
            return;
        }
        /* otherwise create a global symbol and initialize in .data */
        add_global(n->var_name);
        if (n->init) {
            /* emit code to set global at runtime in main */
...
/* append runtime assembly stubs for printing */
static void cg_emit_runtime(CodeGen *cg) {
    cg_emit(cg, ".section .text");
    cg_emit(cg, ".globl js_print_value");
    cg_emit(cg, "js_print_value:");
    cg_emit(cg, "    push %%rbp");
    cg_emit(cg, "    mov %%rsp, %%rbp");
    /* argument in rdi: value or pointer */
    /* We'll assume if pointer points to rodata label, it's a string; otherwise print as integer */
    /* For simplicity, call printf with %%s and %%ld heuristics: try to print as string */
    cg_emit(cg, "    mov %%rdi, %%rsi"); /* arg */
    cg_emit(cg, "    mov $fmt_try_string, %%rdi");
    cg_emit(cg, "    call printf");
    cg_emit(cg, "    pop %%rbp");
    cg_emit(cg, "    ret");
    cg_emit(cg, "fmt_try_string: .asciz \"%s\"");
    cg_emit(cg, ".globl js_print_newline");
    cg_emit(cg, "js_print_newline:");
    cg_emit(cg, "    push %%rbp");
    cg_emit(cg, "    mov %%rsp, %%rbp");
    cg_emit(cg, "    mov $fmt_nl, %%rdi");
    cg_emit(cg, "    call printf");
    cg_emit(cg, "    pop %%rbp");
    cg_emit(cg, "    ret");
    cg_emit(cg, "fmt_nl: .asciz \"\\n\"");
}
...
static void compile_to_asm(const char *src, const char *outpath) {
    ...
    /* emit header */
    cg_emit(&cg, ".intel_syntax noprefix"); /* optional; but we will use AT&T style below; keep for assembler compatibility */
    cg_emit(&cg, ".section .text");
...
static void run_tests(void) {
    struct Test { const char *name; const char *src; const char *expect; } tests[] = {
        ...
        { "add", "print(1+2*3);", "7" },
        { "if-else", "if (1) { print(10); } else { print(20); }", "10" },
        { "while", "var i=0; while (i<3) { print(i); i = i + 1; }", "0\n1\n2" },
        { "obj-flatten", "var obj = {a:1, b:2}; print(obj_a); print(obj_b);", "1\n2" },
        { NULL, NULL, NULL }
    };
    ...
    for (int i = 0; tests[i].name; ++i) {
        ...
        int rc = system(cmd);
        if (rc != 0) { fprintf(stderr, "compile failed for %s\n", tests[i].name); continue; }
        /* run and capture output */
        snprintf(cmd, sizeof(cmd), "./%s > out.txt 2>&1", exe);
        rc = system(cmd);
        if (rc != 0) { fprintf(stderr, "run failed for %s\n", tests[i].name); continue; }
        /* show output */
        FILE *f = fopen("out.txt", "r");
        if (!f) { fprintf(stderr, "cannot open out.txt\n"); continue; }
