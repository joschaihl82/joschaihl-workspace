...
uint8_t heap[0x1000000] __attribute__((aligned(4096))); // 16MB Heap for buffers
...
void debug_hexdump(void *ptr, int len) {
...
static void pci_write(uint32_t b, uint32_t s, uint32_t f, uint32_t o, uint32_t v) {
    outl(0xCF8, (uint32_t)((b << 16) | (s << 11) | (f << 8) | (o & 0xFC) | 0x80000000));
    outl(0xCFC, v);
}
...
static void nvme_wait(nvme_queue_t *q) {
    ...
    while (cqe->cid != q->last_cid) { __asm__("pause"); }
    q->cq_head = (q->cq_head + 1) % NVME_QUEUE_ENTRY_COUNT;
    *q->cq_doorbell = q->cq_head;
}

void nvme_read_sector(nvme_ctrl_t *ctrl, uint64_t lba, void* buffer) {
...
int nvme_init(nvme_ctrl_t *ctrl) {
    uint32_t bar0 = 0;
    for (int b = 0; b < 256; b++) {
        for (int s = 0; s < 32; s++) {
    ...
    ctrl->admin_q.cq_doorbell = (uint32_t*)((uint8_t*)ctrl->regs + 0x1000 + d);
    ctrl->io_q.sq_doorbell    = (uint32_t*)((uint8_t*)ctrl->regs + 0x1000 + 2*d);
    ctrl->io_q.cq_doorbell    = (uint32_t*)((uint8_t*)ctrl->regs + 0x1000 + 3*d);
    ...
    // Create IO Queues using admin commands
    NVME_SQE *sqe = &ctrl->admin_q.sq_entries[0];
    sqe->opc = NVME_CREATE_IO_COMPLETION_QUEUE; sqe->cid = ++ctrl->admin_q.last_cid;
    sqe->prp[0] = (uintptr_t)ctrl->io_q.cq_entries; sqe->cdw10 = (NVME_QUEUE_ENTRY_COUNT-1)<<16|0x01; sqe->cdw11 = 0x01;
    *ctrl->admin_q.sq_doorbell = 1; nvme_wait(&ctrl->admin_q);

    sqe = &ctrl->admin_q.sq_entries[1];
    sqe->opc = NVME_CREATE_IO_SUBMISSION_QUEUE; sqe->cid = ++ctrl->admin_q.last_cid;
    sqe->prp[0] = (uintptr_t)ctrl->io_q.sq_entries; sqe->cdw10 = (NVME_QUEUE_ENTRY_COUNT-1)<<16|0x01; sqe->cdw11 = 0x01|(0x01<<16);
    *ctrl->admin_q.sq_doorbell = 2; nvme_wait(&ctrl->admin_q);
...
void kmain(KernelGOPInfo *kgi) {
    ...
    if (nvme_init(&ctrl) == 0) {
        ...
        k_printf("NVMe Ready. Reading first 512 bytes (LBA 0)...\n");
