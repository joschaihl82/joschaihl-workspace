...
/* =====================================================================================
   TAR + GZIP helpers (kept if you unpack upstream)
   ===================================================================================== */
struct tar_header {
...
/* =====================================================================================
   Robin Hood Hash Map (open addressing) for fast grouping (average O(1))
   ===================================================================================== */
typedef struct {
    group *slots;
    size_t cap;
...
void progress_bar_update(size_t i, size_t total) {
    const int width = 40;
...
/* =====================================================================================
   ...
   ===================================================================================== */
int main(void) {
    printf(ANSI_BOLD "Generating Ultra ASCII Catalog (ANSI + Progressbar) ..." ANSI_RESET "\n");
    ...
    while ((de = readdir(d))) {
        ...
        if (!f) { processed++; progress_bar_update(processed, total_files); continue; }
        ...
        if (flen) {
            ...
            if (!buf) { fclose(f); processed++; progress_bar_update(processed, total_files); continue; }
            fread(buf, 1, flen, f);
        }
        fclose(f);
        ...
        processed++;
        progress_bar_update(processed, total_files);
    }
