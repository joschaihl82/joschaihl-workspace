...
// offsets (must match x64 offsets relative to the cpustate struct)
#define RA_OFF       0x00
#define RB_OFF       0x08
#define RC_OFF       0x10
#define RD_OFF       0x18
...
// x64 instruction emitters (targeting rdi, which holds the cpustate* 'this' pointer)
...
static JIT_Func jit_compile(const uint8_t* rom, size_t rom_size) {
    ...
    while (pc < (int)rom_size) {
        ...
        switch (op) {
            ...
            case OP_MOV_IMM: {
                // mov [rdi+off], arg (low 32-bit), mov [rdi+off+4], 0 (high 32-bit)
            ...
            case OP_SHL_REG: {
                ...
                emit_load_reg64_rcx(&cp, reg_b); // shift count must be in cl/rcx
                *cp++=0x48;*cp++=0xD3;*cp++=0xE0; // shl rax, cl
                emit_store_reg64_from_rax(&cp,reg_a); pc+=3; break;
            }
            case OP_SHR_REG: {
                emit_load_reg64_rax(&cp, reg_a);
                emit_load_reg64_rcx(&cp, reg_b); // shift count must be in cl/rcx
                *cp++=0x48;*cp++=0xD3;*cp++=0xE8; // shr rax, cl
                emit_store_reg64_from_rax(&cp,reg_a); pc+=3; break;
            }
            ...
            case OP_CMP_REG: {
                ...
                *cp++=0x48;*cp++=0x3B; emit_modrm_disp32_rdi(&cp,0); emit_disp32(&cp,(uint32_t)offb);
                // setz al (zf in al, bit 0)
                *cp++=0x0F;*cp++=0x94;*cp++=0xC0;
                // mov al to rflags byte
            ...
            case OP_JNE_REL:
            case OP_JE_REL: {
                // movzx eax, byte [rdi+rflags_off]
                *cp++=0x0F;*cp++=0xB6;*cp++=0x87; emit_disp32(&cp,(uint32_t)RFLAGS_OFF);
                // test eax, eax
                *cp++=0x85;*cp++=0xC0;
                // jnz/jz (jump if zf=0 / jump if zf=1)
            ...
            case OP_PUSH_REG: {
                ...
                // mov [ram+rsp], reg[a]
                emit_load_reg64_rax(&cp,reg_a);
                // mov [rdi+ram_off+rcx], rax (rsp is already loaded in rcx from the sub instruction)
            ...
            case OP_WRITE_FB_A: {
                // mov ecx, [rdi+rb_off] (index)
                *cp++=0x8B;*cp++=0x8F; emit_disp32(&cp,(uint32_t)RB_OFF);
                // mov eax, [rdi+ra_off] (color)
                *cp++=0x8B;*cp++=0x87; emit_disp32(&cp,(uint32_t)RA_OFF);
                // mov [rdi+fb_off+rcx*4], eax
                *cp++=0x89;*cp++=0x84;*cp++=0x8F; emit_disp32(&cp,(uint32_t)FB_OFF);
                pc+=3; break;
            }

            default: {
                // --- error handling for undefined instruction ---
                fprintf(stderr,"\n[jit error] undefined opcode 0x%02x at rom pc 0x%x. aborting compilation.\n", op, pc);
...
// -----------------------------------------------------------------------------
// demo rom: rainbow effect based on pixel index
// -----------------------------------------------------------------------------

static const uint8_t* create_rom(size_t* out_size) {
    // this rom calculates color = 0xffrrggbb based on the framebuffer index (rb).
    // r = (rb >> 16) & 0xff
    // g = (rb >> 8) & 0xff
    // b = (rb >> 0) & 0xff
    ...
    // registers used:
    // rA (0): final color
    // rB (1): index (input/output)
    // rC (2): temp for components/alpha
    // rI (8): const 8 (for shifts)
    // rJ (9): const 16 (for shifts)
    // rK (10): const 0xff (for mask)

    uint8_t rom[] = {
        // --- setup constants ---
        // pc 0: ri = 8
        OP_MOV_IMM, 0x08, 0x08, 0x00,
        // pc 3: rj = 16
        OP_MOV_IMM, 0x09, 0x10, 0x00,
        // pc 6: rk = 0xff
        OP_MOV_IMM, 0x0a, 0xff, 0x00,

        // --- blue component (b = rb & 0xff) ---
        // pc 9: rc = rb (index)
        OP_MOV_REG, 0x21, 0x00, // corrected: was op_mov_reg
        // pc 12: rc &= 0xff (rc = b)
        OP_AND_REG, 0x2a, 0x00, // corrected: was op_and_reg
        // pc 15: ra = rc (ra = 0x000000bb)
        OP_MOV_REG, 0x02, 0x00, // corrected: was op_mov_reg

        // --- green component (g = (rb >> 8) & 0xff) ---
        // pc 18: rc = rb
        OP_MOV_REG, 0x21, 0x00, // corrected: was op_mov_reg
        // pc 21: rc >>= 8
        OP_SHR_REG, 0x28, 0x00, // corrected: was op_shr_reg
        // pc 24: rc &= 0xff (rc = g)
        OP_AND_REG, 0x2a, 0x00, // corrected: was op_and_reg
        // pc 27: rc <<= 8 (shift g to bit 8)
        OP_SHL_REG, 0x28, 0x00, // corrected: was op_shl_reg
        // pc 30: ra |= rc (ra = 0x0000ggbb)
        OP_OR_REG, 0x02, 0x00, // corrected: was op_or_reg

        // --- red component (r = (rb >> 16) & 0xff) ---
        // pc 33: rc = rb
        OP_MOV_REG, 0x21, 0x00, // corrected: was op_mov_reg
        // pc 36: rc >>= 16
        OP_SHR_REG, 0x29, 0x00, // corrected: was op_shr_reg
        // pc 39: rc &= 0xff (rc = r)
        OP_AND_REG, 0x2a, 0x00, // corrected: was op_and_reg
        // pc 42: rc <<= 16 (shift r to bit 16)
        OP_SHL_REG, 0x29, 0x00, // corrected: was op_shl_reg
        // pc 45: ra |= rc (ra = 0x00rrggbb)
        OP_OR_REG, 0x02, 0x00, // corrected: was op_or_reg

        // --- alpha component (a = 0xff) ---
        // pc 48: rc = 0xff00 (low 16 bits of alpha)
        OP_MOV_IMM, 0x02, 0x00, 0xff, // corrected: was op_mov_imm
        // pc 51: rc <<= 16 (rc = 0xff000000)
        OP_SHL_REG, 0x29, 0x00, // corrected: was op_shl_reg
        // pc 54: ra |= rc (ra = 0xffrrggbb)
        OP_OR_REG, 0x02, 0x00, // corrected: was op_or_reg

        // --- write and halt ---
        // pc 57: fb[rb] = ra
        OP_WRITE_FB_A, 0x01, 0x00, // corrected: was op_write_fb_a
        // pc 60: rb++
        OP_INC_REG, 0x10, 0x00, // corrected: was op_inc_reg
        // pc 63: halt
        OP_HALT, 0x00, 0x00 // corrected: was op_halt
    };
...
static void* worker_thread(void* arg) {
    ...
    const uint32_t chunk = 4096; // pixels to process per scheduling block
    while (s->running) {
        // run jit chunk: increments rb by 1 per halt; run it chunk times
        for (uint32_t i = 0; i < chunk && s->running; i++) {
            // check for wrap outside of jit to keep the rom simple
...
int main(void) {
    ...
    // check if jit compilation failed and exit immediately
    if (!fn) {
        free((void*)rom);
        fprintf(stderr, "[emu] compilation failed. exiting.\n");
        return 1;
    }
    ...
    SDL_Window* win = SDL_CreateWindow(
        "jit x64 emulator (threaded, rainbow)",
    ...
    while (running) {
        ...
        // the worker thread is continuously writing to state.framebuffer
        ...
        if (now - last >= 1000) {
            char title[128];
            snprintf(title, sizeof(title), "jit x64 emulator (threaded, rainbow) - %llu fps",
