// main.cpp
// Qt6 C++ single-file: Download all Google Drive files serially and exit when done.
// Requires Qt6 (Core, Network, NetworkAuth)
...
public slots:
    void startDownloads(const QJsonArray &queue, const QString &token, const QString &syncPath) {
        ...
        for (const QJsonValue &v : queue) {
            ...
            safeRel.replace('\\', '_');
            // If metadata contains parents and we want full path, we expect caller to have provided full path in "path" field
            if (meta.contains("path") && meta.value("path").isString()) {
                safeRel = meta.value("path").toString();
            }
            ...
            emit overallProgress( qBound(0, int((downloadedBytes * 100) / qMax<qint64>(1, totalBytes)), 100) );
...
public slots:
    void startSync() {
        ...
        // Fetch metadata in worker thread
...
private slots:
    ...
    void onDownloadStatus(const QString &s) { qDebug() << s; }
    void onDownloadFileProgress(const QString &name, qint64 rec, qint64 tot) {
        qDebug() << "[file]" << name << rec << "/" << tot;
    }
    void onDownloadOverall(int p) { qDebug() << "[overall]" << p << "%"; }
    void onDownloadEta(const QString &eta) { qDebug() << "[eta]" << eta; }
...
private:
    ...
    QHash<QString, QJsonObject> pathIndex; // path -> meta (if multiple with same path, last wins)
    ...
    void setupOAuth() {
        ...
        google->setClientIdentifier(QStringLiteral("YOUR_CLIENT_ID.apps.googleusercontent.com"));
        google->setClientIdentifierSharedKey(QStringLiteral("YOUR_CLIENT_SECRET"));
        ...
        connect(google, &QOAuth2AuthorizationCodeFlow::authorizeWithBrowser, [](const QUrl &url) {
            qDebug() << "Öffne Browser für Autorisierung:" << url.toString();
...
#endif
            // Save access token to tokenFilePath (simple)
    ...
    void buildPathIndexAndQueue() {
        ...
        QStringList entries = root.entryList(QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot, QDir::Name);
        // recursive scan
