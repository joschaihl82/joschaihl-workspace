// main.c — Berlin6 continuous, memoryless auto-calibration with warm-up seeding, adaptive thresholding, drift-triggered recalibration, and audio guardrails
...
#define BPP 4 // Bytes per Pixel: RGBA
...
double get_current_time_sec() { return (double)get_current_time_ms() / 1000.0; }
...
// Percentile-based threshold from grayscale histogram.
static int compute_hot_threshold(const uint8_t *gray, int size, double percentile) {
    int hist[256]; memset(hist, 0, sizeof(hist));
    for (int i = 0; i < size; ++i) hist[gray[i]]++;
    int target = (int)(percentile * size);
...
static int v4l2_set_ctrl_int(int fd, __u32 id, int value, const char *name) {
    struct v4l2_control ctrl = { .id = id, .value = value };
    if (ioctl(fd, VIDIOC_S_CTRL, &ctrl) < 0) {
        if (errno != EINVAL) {
            fprintf(stderr, "[V4L2] WARN: Set ctrl %s (0x%x) to %d failed: %s\n",
                    name, id, value, strerror(errno));
        }
        return 0;
    } else {
        fprintf(stderr, "[V4L2] Set ctrl %s (0x%x) = %d\n", name, id, value);
        return 1;
    }
}

int v4l2_init_c(V4L2Ctx **ctx_out) {
    V4L2Ctx *ctx = (V4L2Ctx*)calloc(1, sizeof(V4L2Ctx));
    if (!ctx) return 0;
    ctx->fd = open(DEVICE_NODE, O_RDWR | O_NONBLOCK, 0);
    if (ctx->fd < 0) { free(ctx); return 0; }

    // Manual camera sanity setup (best-effort)
    v4l2_set_ctrl_int(ctx->fd, V4L2_CID_EXPOSURE_AUTO, V4L2_EXPOSURE_MANUAL, "EXPOSURE_AUTO");
    v4l2_set_ctrl_int(ctx->fd, V4L2_CID_EXPOSURE_ABSOLUTE, 200, "EXPOSURE_ABSOLUTE");
    v4l2_set_ctrl_int(ctx->fd, V4L2_CID_AUTOGAIN, 0, "AUTOGAIN");
    v4l2_set_ctrl_int(ctx->fd, V4L2_CID_GAIN, 10, "GAIN");
    v4l2_set_ctrl_int(ctx->fd, V4L2_CID_AUTO_WHITE_BALANCE, 0, "AUTO_WHITE_BALANCE");
    v4l2_set_ctrl_int(ctx->fd, V4L2_CID_BRIGHTNESS, 128, "BRIGHTNESS");
    v4l2_set_ctrl_int(ctx->fd, V4L2_CID_CONTRAST, 64, "CONTRAST");
...
uint8_t *v4l2_grab_frame_c(V4L2Ctx *ctx) {
...
// --- VI. Continuous calibration (memoryless) ---

typedef enum { CALIB_IDLE = 0, CALIB_RUNNING = 1 } CalibPhase;
...
typedef struct {
    ...
    double emaHotAbs;
    double emaHotRel;
    uint64_t lastCompleteMs;
} RadCalib;

static RadCalib g_calib = {
    .frames=0,.sumHotAbs=0,.sumHotRel=0,.calibrated=0,.normFactor=20000.0,
    .startMs=0,.targetSec=30.0,.phase=CALIB_RUNNING,
    .emaGlobalAvg=128.0,.emaHotAbs=0.0,.emaHotRel=0.0,.lastCompleteMs=0
};

// Tunables
static const double CALIB_BASELINE_uSv_h = 0.2;
static const double EMA_ALPHA_FAST = 0.15;
static const double EMA_ALPHA_SLOW = 0.05;
static const double TRIG_GLOBAL_SHIFT = 15.0;
static const double TRIG_HOT_ABS_MULT = 2.5;
static const double TRIG_HOT_REL_MULT = 2.5;
static const double MIN_RECALIB_GAP_SEC = 20.0;
static const double POST_FINISH_GRACE_SEC = 5.0;
static const double MIN_REL_SIGNAL_RATIO = 0.001; // 0.1%
static const int    MIN_FRAMES_FOR_DECISION = 60;
static const double NORMF_MIN = 5000.0, NORMF_MAX = 500000.0;
static const double GLOBAL_SAT_THRESH = 245.0;
static const double REL_SURGE_RESET_MULT = 6.0;

static inline double ema_update(double prev, double x, double alpha) { return prev + alpha * (x - prev); }

static inline void calib_begin(double targetSec) {
    g_calib.phase = CALIB_RUNNING;
    g_calib.targetSec = targetSec;
    g_calib.startMs = get_current_time_ms();
    g_calib.frames = 0;
    g_calib.sumHotAbs = 0;
    g_calib.sumHotRel = 0;
    fprintf(stderr, "[Calib] begin (%.0fs window)\n", targetSec);
}

static inline void calib_finish(double avgAbs, double avgRel) {
    double baselineHot = (avgRel > 1.0) ? avgRel : avgAbs;
    double target_Sv_h = CALIB_BASELINE_uSv_h * 1e-6;
    double nf = (baselineHot * CIA_BERLIN6_SENS_SCALE) / target_Sv_h;
    nf = fmax(NORMF_MIN, fmin(NORMF_MAX, nf));
    g_calib.normFactor = ema_update(g_calib.normFactor, nf, EMA_ALPHA_SLOW);
    g_calib.calibrated = 1;
    g_calib.phase = CALIB_IDLE;
    g_calib.lastCompleteMs = get_current_time_ms();
    // Reset EMAs to measured averages to prevent instant surge
    g_calib.emaHotAbs = avgAbs;
    g_calib.emaHotRel = avgRel;
    fprintf(stderr, "[Calib] finish: avgAbs=%.2f avgRel=%.2f -> normF(EMA)=%.0f\n", avgAbs, avgRel, g_calib.normFactor);
}

static inline int recalib_cooldown_elapsed(void) {
    if (g_calib.lastCompleteMs == 0) return 1;
    double gap = (double)(get_current_time_ms() - g_calib.lastCompleteMs)/1000.0;
    return gap >= fmax(MIN_RECALIB_GAP_SEC, POST_FINISH_GRACE_SEC);
}

static void calib_maybe_update(double globalAvg, int hotAbs, int hotRel) {
    // Update EMAs (brightness EMA will settle as frames pass)
    g_calib.emaGlobalAvg = ema_update(g_calib.emaGlobalAvg, globalAvg, EMA_ALPHA_FAST);
    g_calib.emaHotAbs    = ema_update(g_calib.emaHotAbs,    (double)hotAbs, EMA_ALPHA_FAST);
    g_calib.emaHotRel    = ema_update(g_calib.emaHotRel,    (double)hotRel, EMA_ALPHA_FAST);

    // Grace: avoid triggers immediately after finish
    double sinceFinish = g_calib.lastCompleteMs ? (double)(get_current_time_ms() - g_calib.lastCompleteMs) / 1000.0 : 1e9;
    if (g_calib.phase == CALIB_IDLE && sinceFinish < POST_FINISH_GRACE_SEC) return;

    // Decide if we should start calibration
    if (g_calib.phase == CALIB_IDLE) {
        int relMeaningful = hotRel > (int)(MIN_REL_SIGNAL_RATIO * (IMG_W * IMG_H));
        int surgeAbs = hotAbs > (int)(TRIG_HOT_ABS_MULT * g_calib.emaHotAbs);
        int surgeRel = relMeaningful && (hotRel > (int)(TRIG_HOT_REL_MULT * g_calib.emaHotRel));
        double shiftGlobal = fabs(globalAvg - g_calib.emaGlobalAvg);
        int hugeSurgeRel = hotRel > (int)(REL_SURGE_RESET_MULT * g_calib.emaHotRel);
        int nearSaturation = (globalAvg > GLOBAL_SAT_THRESH);

        if (recalib_cooldown_elapsed() && (nearSaturation || hugeSurgeRel)) {
            calib_begin(30.0);
            return;
        }
        if (recalib_cooldown_elapsed() && (surgeAbs || surgeRel || (shiftGlobal > TRIG_GLOBAL_SHIFT))) {
            double winSec = (surgeAbs || surgeRel) ? 15.0 : 30.0;
            calib_begin(winSec);
        }
    }

    // Accumulate if running
    if (g_calib.phase == CALIB_RUNNING) {
        g_calib.sumHotAbs += hotAbs;
        g_calib.sumHotRel += hotRel;
        g_calib.frames++;
        double elapsed = (double)(get_current_time_ms() - g_calib.startMs) / 1000.0;
        if (elapsed >= g_calib.targetSec) {
            double avgAbs = (double)g_calib.sumHotAbs / (double)g_calib.frames;
            double avgRel = (double)g_calib.sumHotRel / (double)g_calib.frames;
            calib_finish(avgAbs, avgRel);
        }
    }
}
...
void geiger_logic_process(uint32_t *src_rgb32, uint32_t *dst_rgb32, GeigerLogicState *state, double frame_sec, GeigerStats *stats) {
    ...
    // Seed background and long exposure on first frame to avoid startup explosion
    if (!state->bg_init_done) {
        for (int i = 0; i < size; ++i) {
            state->bg[i] = (float)state->gray[i];
            state->longExp[i] = (double)state->gray[i];
        }
        state->bg_init_done = true;
    }

    // 1b. Adaptive threshold (top 0.5%), with moderated floor
    int hot_thresh = compute_hot_threshold(state->gray, size, 0.005);
    if (hot_thresh < 215) hot_thresh = 215; // previous 225 was too aggressive in many scenes

    // 1c. Absolute hot pixels
    int hotPixelsAbs = 0;
    for (int i = 0; i < size; ++i) {
        if (state->gray[i] >= hot_thresh) hotPixelsAbs++;
        if ((i & (YIELD_PIXEL_STRIDE - 1)) == 0) sched_yield();
    }
    ...
    // 3. BG und Residue + global avg (faster BG during warm-up)
    double globalAvg = 0.0;
    double bg_rate = (state->frames_seen < 60) ? 0.2 : BG_LEARN_RATE; // ~1s fast settle, then normal
    for (int i = 0; i < size; ++i) {
        state->bg[i] = (1.0 - bg_rate) * state->bg[i] + bg_rate * (double)state->gray[i];
    ...
    // Relative-to-background hot pixels (only after warm-up)
    int hotPixelsRel = 0;
    if (state->frames_seen >= 60) {
        for (int i = 0; i < size; ++i) {
            double combinedBg = 0.7 * state->bg[i] + 0.3 * state->longExp[i];
            int diff = (int)state->gray[i] - (int)round(combinedBg);
            if (diff > (RESIDUE_THRESH + 10)) hotPixelsRel++;
            if ((i & (YIELD_PIXEL_STRIDE - 1)) == 0) sched_yield();
        }
    }
    ...
    // 7. Continuous calibration and smoothing
    calib_maybe_update(globalAvg, hotPixelsAbs, hotPixelsRel);

    // Prefer relative hotness if meaningful (~0.1% of pixels) and warm-up done; else absolute
    int rel_ready = (state->frames_seen >= 60);
    int hotPixels = (rel_ready && hotPixelsRel > (int)(IMG_W * IMG_H * MIN_REL_SIGNAL_RATIO)) ? hotPixelsRel : hotPixelsAbs;

    // Use current norm factor (EMA-smoothed)
    double normF = g_calib.normFactor;
    double rawVal_per_h = ((double)hotPixels / normF) * CIA_BERLIN6_SENS_SCALE;
    ...
    stats->hotPixelsAbs = hotPixelsAbs;
    stats->hotPixelsRel = hotPixelsRel;
    stats->hotThreshold = hot_thresh;
    stats->normFactor = g_calib.normFactor;
    stats->calibrated = g_calib.calibrated;
...
void log_to_csv(const GeigerStats *stats) {
    ...
    if (!header_written) {
        ...
        if (ftell(csv_file) == 0) {
            fprintf(csv_file, "ts_ms,raw_Sv_per_s,avg_Sv_per_s,total_Sv,alert,hot_abs,hot_rel,hot_thr,normF,calib_phase\n");
        }
        header_written = true;
    }
    fprintf(csv_file, "%" PRIu64 ",%.8g,%.8g,%.12g,%d,%d,%d,%d,%.0f,%d\n",
            stats->currentTimeMs,
            stats->currentRad,
            stats->avgRad,
            stats->totalDose,
...
void csv_cleanup() {
    // file closed via static pointer in log_to_csv if necessary
}

// --- IX. Audio Engine (ALSA Thread) ---

static inline int is_calibrated(void) { return g_calib.calibrated; }
...
static int audio_thread(void *data) {
    ...
    uint64_t audioStartMs = get_current_time_ms();

    while (atomic_load_explicit(&g_running_atomic, memory_order_acquire)) {
        // Pop all queued click events
        ClickEvent ev;
        while (click_queue_pop(&ev)) {
            ...
            // Slow-start during calibration or first 5s of app start
            double elapsedAudio = (double)(get_current_time_ms() - audioStartMs) / 1000.0;
            if (g_calib.phase == CALIB_RUNNING || (!is_calibrated() || elapsedAudio < 5.0)) {
                clicksPerSecond_est = fmin(clicksPerSecond_est, 5.0);
            }
            clicksPerSecond_est = fmin(clicksPerSecond_est, CIA_BERLIN6_CLICK_MAX);

            // Expectation per 20 ms block
            ...
            // Burst cap
            const int MAX_PER_BLOCK = 8;
            if (injectCount > MAX_PER_BLOCK) injectCount = MAX_PER_BLOCK;
            ...
            for (int i = 0; i < injectCount; ++i) {
                if (num_active_clicks < 64) {
                    ...
                    // Amplitude taper when very hot
                    double hot_scaler = 1.0;
                    if (clicksPerSecond_est > 50.0) hot_scaler = 0.85;
                    if (clicksPerSecond_est > 100.0) hot_scaler = 0.7;

                    ac->ampVar = hot_scaler * (0.80 + (double)(seed & 0xFF) / 255.0 * 0.6);
        ...
        for (int n = 0; n < AUDIO_PERIOD_FRAMES; ++n) {
            ...
            double lpPrev = lpState;
            lpState = lpState + alphaLp * (s - lpState);
            double banded = lpState - hpCoef * prevOut;
            prevOut = lpPrev;

            // Global fade-in over first 2 seconds
            double fade = 1.0;
            double sinceStart = (double)(get_current_time_ms() - audioStartMs) / 1000.0;
            if (sinceStart < 2.0) fade = sinceStart / 2.0;

            double sample = banded * fade;
...
void sdl_render_loop() {
    ...
    // Debug overlay for calibration progress and thresholds
    char dbg[200];
    uint64_t nowMs = get_current_time_ms();
    double calibElapsed = 0.0;
    const char *phaseStr = (g_calib.phase == CALIB_RUNNING) ? "running" : "idle";
    if (g_calib.phase == CALIB_RUNNING && g_calib.startMs) {
        calibElapsed = (double)(nowMs - g_calib.startMs) / 1000.0;
    } else if (g_calib.phase == CALIB_IDLE && g_calib.lastCompleteMs) {
        calibElapsed = (double)(nowMs - g_calib.lastCompleteMs) / 1000.0;
    }
    snprintf(dbg, sizeof(dbg), "HOT_THR:%d ABS:%d REL:%d NORM:%.0f  CALIB:%s %.0fs",
             stats_copy.hotThreshold, stats_copy.hotPixelsAbs, stats_copy.hotPixelsRel,
             stats_copy.normFactor, phaseStr, calibElapsed);
    SDL_Texture *tex_dbg = render_text(dbg, (SDL_Color){200,200,255,255}, g_font_small, g_renderer);
    if (tex_dbg) { int w,h; SDL_QueryTexture(tex_dbg,NULL,NULL,&w,&h);
        SDL_Rect dst={textX, textY+40, w, h}; SDL_RenderCopy(g_renderer, tex_dbg, NULL, &dst); SDL_DestroyTexture(tex_dbg); }
