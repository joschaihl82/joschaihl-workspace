// editor_padding.c
// gcc -O2 -o editor_padding editor_padding.c -lSDL2 -lSDL2_ttf
// Refaktorierter Editor mit Bottom Padding: Caret bleibt beim Scrollen nicht direkt am Rand.
...
// Bottom padding in pixels: Abstand zwischen Caret und unterem Fensterrand
...
static int create_ui(UI *ui, const char *font_path) {
    ui->win = SDL_CreateWindow("Editor Padding", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WINW, WINH, SDL_WINDOW_RESIZABLE);
    if (!ui->win) return 0;
...
/* ---------------------------
   Scroll-Hilfsfunktionen mit Padding
   --------------------------- */
...
// View height that is used for caret visibility calculations (same as compute_view_height)
static int compute_view_height_with_padding(UI *ui) {
    return compute_view_height(ui);
}

// Clamp scroll so that you can scroll until the last line is visible with bottom padding
static void clamp_scroll_to_content_with_padding(Doc *doc, UI *ui, int *scroll_y) {
    int content_h = compute_content_height(doc);
    int view_h = compute_view_height_with_padding(ui);
    // allow extra scroll so last line can be positioned above bottom padding
...
/* ---------------------------
   Sichtbarkeit des Carets mit Padding
   --------------------------- */
static void ensure_caret_visible_with_padding(Doc *doc, UI *ui, int cy, int *scroll_y) {
    int view_h = compute_view_height_with_padding(ui);
    if (view_h <= 0) return;
    int caret_ypix = cy * (FONT_SIZE + LINE_SPACING);
    // gewünschter unterer Bereich: view_h - (line height) - BOTTOM_PADDING
    ...
    clamp_scroll_to_content_with_padding(doc, ui, scroll_y);
}

/* ---------------------------
   Event handling (ausgelagert)
   --------------------------- */
static int handle_events(Doc *doc, UI *ui, int *cx, int *cy, int *scroll_y, int *dragging_thumb, int *thumb_drag_offset) {
    SDL_Event e;
    int running = 1;
    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_QUIT) { running = 0; break; }
        else if (e.type == SDL_WINDOWEVENT && e.window.event == SDL_WINDOWEVENT_SIZE_CHANGED) { clamp_scroll_to_content_with_padding(doc, ui, scroll_y); }
        else if (e.type == SDL_KEYDOWN) {
            SDL_Keycode k = e.key.keysym.sym;
            if (k == SDLK_ESCAPE) { running = 0; break; }
            else if (k == SDLK_BACKSPACE) {
                if (*cx > 0) { delete_back(&doc->a[*cy], *cx); (*cx)--; }
                else if (*cy > 0) { int prevlen = doc->a[*cy - 1].len; doc_join_line(doc, *cy - 1); (*cy)--; *cx = prevlen; clamp_scroll_to_content_with_padding(doc, ui, scroll_y); }
            } else if (k == SDLK_RETURN || k == SDLK_KP_ENTER) {
                doc_split_line(doc, *cy, *cx);
                (*cy)++; *cx = 0;
                ensure_caret_visible_with_padding(doc, ui, *cy, scroll_y);
            } else if (k == SDLK_LEFT) {
                if (*cx > 0) (*cx)--;
                ...
                if (*cy > 0) { (*cy)--; if (*cx > doc->a[*cy].len) *cx = doc->a[*cy].len; ensure_caret_visible_with_padding(doc, ui, *cy, scroll_y); }
            } else if (k == SDLK_DOWN) {
                if (*cy < doc->n - 1) { (*cy)++; if (*cx > doc->a[*cy].len) *cx = doc->a[*cy].len; ensure_caret_visible_with_padding(doc, ui, *cy, scroll_y); }
            }
        }
            ...
            if (mx >= vbg_x && my >= vbg_y && my <= vbg_y + vbg_h) {
                ...
                *cy = clicked_line;
                ...
                if (byte_len <= 0) { *cx = 0; }
                    ...
                    *cx = find_offset_binary(ui->font, doc->a[*cy].text, byte_len, target_x);
                    if (*cx < 0) *cx = 0;
...
/* ---------------------------
   Render-Funktion
   --------------------------- */
static void render_frame(Doc *doc, UI *ui, int cx, int cy, int scroll_y) {
...
int main(int argc, char **argv) {
    ...
    while (running) {
        running = handle_events(&doc, &ui, &cx, &cy, &scroll_y, &dragging_thumb, &thumb_drag_offset);
        Uint32 now = SDL_GetTicks();
        if (now - last_blink >= BLINK_MS) { show_cursor = !show_cursor; last_blink = now; }
        if (show_cursor) render_frame(&doc, &ui, cx, cy, scroll_y);
        else render_frame(&doc, &ui, -1, cy, scroll_y);
        SDL_Delay(10);
    }
