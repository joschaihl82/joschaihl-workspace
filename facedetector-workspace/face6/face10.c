...
// ROI / resize
#define ROI_SIZE 160          // gut für ein Gesicht bei 640x480
#define RES 64
...
///////////////////// Augen- und Mund-Erkennung //////////////////////
// Zwei nahe kreise: schwarzer Kern, heller Ring, starker Konturrand
static void detect_eye_circles(uint8_t *rgb, int cx, int cy){
    ...
    // Augen liegen im oberen Drittel der ROI
    int eyes_y0 = roi_y0;
    int eyes_y1 = roi_y0 + ROI_SIZE/3;

    // globale Mittelwerte -> adaptive Schwellen
    double sumY=0; int cntY=0;
    for(int i=0;i<Rw*Rh;i++){ sumY += Y[i]; cntY++; }
    float meanY = (float)(sumY/cntY);
    float core_dark_thr  = meanY * 0.48f;           // Kern: strenger dunkel
    float ring_bright_thr = fminf(255.0f, meanY * 1.18f); // Ring: deutlich heller

    // Radien (für 640x480/ROI 160 gut passend)
    int r_core = 3;          // schwarzer Kern
    int r_ring_in = 5;       // Ring innen
    int r_ring_out = 8;      // Ring außen
    int r_contour = 6;       // Konturprobe
    ...
    for(int y=eyes_y0 + r_ring_out; y<=eyes_y1 - r_ring_out; y++){
        for(int x=roi_x0 + r_ring_out; x<=roi_x1 - r_ring_out; x++){
            ...
            if(core_mean > core_dark_thr) continue;
            ...
            if(ring_mean < ring_bright_thr) continue;

            // Konturprüfung: Gradient entlang Ringradius
            ...
            // Score: dunkel (klein), starke Kontur (negativ), heller Ring (negativ)
            float score = core_mean - 0.8f*cont_mean + (ring_bright_thr - ring_mean);
    ...
    // Plausibilität: ähnliche y-Position, sinnvoller horizontaler Abstand (nah beieinander, aber nicht zu nah)
    if(best1.x>=0 && best2.x>=0){
        int dy = abs(best1.y - best2.y);
        int dx = abs(best1.x - best2.x);
        int min_dx = r_ring_out*3;   // mindestens ~3*Ringradius
        int max_dx = ROI_SIZE/2;     // nicht weiter als halbe ROI
        if(dy <= r_ring_out*2 && dx >= min_dx && dx <= max_dx){
            // Visualisierung: schwarzer Kern + weißer Ring + dünne Konturfarbe
            draw_circle_rgb(rgb, best1.x, best1.y, r_core, 0, 0, 0);
            draw_ring_rgb(rgb, best1.x, best1.y, r_ring_in, r_ring_out, 255, 255, 255);
            draw_ring_rgb(rgb, best1.x, best1.y, r_ring_out+1, r_ring_out+2, 0, 128, 255);
...
int v4l2_init(){
    ...
    fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV; // bei Bedarf: V4L2_PIX_FMT_MJPEG
...
int main(){
    ...
    // Fenster: resizable + maximiert; Texture rendert in Fenstergröße
