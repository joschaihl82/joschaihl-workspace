...
/* Safe bounded find_define: avoids infinite traversal by limiting steps */
Define* find_define(char* p, int len) {
    Define* cur = defines;
    const unsigned long MAX_STEPS = 100000UL; /* large but finite */
    unsigned long steps = 0;
    while (cur) {
        if (cur->name_len == len && strncmp(cur->name, p, len) == 0) return cur;
        cur = cur->next;
        steps++;
        if (steps >= MAX_STEPS) {
            /* Defensive failure: list too long or corrupted (possible cycle) */
            error("internal compiler error: defines list traversal exceeded %lu steps (possible corruption)", MAX_STEPS);
            return NULL; /* unreachable because error() exits */
        }
    }
    return NULL;
}
