...
//
// Modifiziert:
// - Fenster ist resizable und startet maximiert.
// - Inhalt skaliert automatisch mit der Fenstergröße.
 ...
 * zeitbasiert über SDL_GetTicks(). Dauer pro Durchlauf = CYCLE_DURATION_MS.
 */
...
static int init_sdl() {
  ...
  // Logische Größe festlegen: Renderer skaliert Inhalt automatisch auf Fenstergröße
...
int main(int argc, char *argv[]) {
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      // Optional: auf Größenänderung reagieren (falls du Status loggen willst)
      if (event.type == SDL_WINDOWEVENT && event.window.event == SDL_WINDOWEVENT_SIZE_CHANGED) {
        // Der Renderer skaliert bereits via LogicalSize; kein weiterer Code nötig.
      }
    }
    ...
    // Dank SDL_RenderSetLogicalSize skaliert der Renderer das Bild automatisch.
