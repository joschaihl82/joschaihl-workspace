// main.cpp
// Qt6 single-file fullscreen visualizer + editor (patched)
// - Left: GLSL editor (vertex + fragment) as central widget
// - Right: Preview: dockable QDockWidget containing fullscreen quad visualizer
// - Bottom: dockable shader log
...
int main(int argc, char **argv) {
	...
	// toolbar: compile, toggle preview, fullscreen
	...
	if (settings.contains("geometry"))
		mainWin->restoreGeometry(settings.value("geometry").toByteArray());
	if (settings.contains("windowState"))
		mainWin->restoreState(settings.value("windowState").toByteArray());
	...
	QObject::connect(actCompile, &QAction::triggered, [&]() {
		compileNow(true);
	});

	QObject::connect(actTogglePreview, &QAction::triggered, [&]() {
		dockPreview->setVisible(!dockPreview->isVisible());
	});

	QObject::connect(actToggleLog, &QAction::triggered, [&]() {
		dockLog->setVisible(!dockLog->isVisible());
	});

	QObject::connect(actFullscreen, &QAction::triggered, [&]() {
		// If floating, maximize; otherwise float and maximize
		if (dockPreview->isFloating()) {
			dockPreview->showMaximized();
		} else {
	...
	// connect GL widget fullscreen requests to dock behavior
	QObject::connect(glw, &GLWaveWidget::requestToggleFullscreen, [&]() {
		if (dockPreview->isFullScreen() || dockPreview->isFloating() && dockPreview->isVisible() && dockPreview->window()->isFullScreen()) {
			// restore floating state
			dockPreview->window()->showNormal();
		} else {
			// float and fullscreen
			dockPreview->setFloating(true);
			dockPreview->window()->showFullScreen();
		}
	});
	QObject::connect(glw, &GLWaveWidget::requestExitFullscreen, [&]() {
		if (dockPreview->isFloating() && dockPreview->window()->isFullScreen()) {
			dockPreview->window()->showNormal();
		}
	});
