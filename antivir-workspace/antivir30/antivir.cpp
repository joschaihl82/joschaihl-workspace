...
private:
    ...
    // Angepasst: Erzeugt eine temporäre Liste mit genau einem Eintrag "/"
    bool buildOpenResourcesList(QString &outPath, int &outCount) {
        ...
        logMessageQt(QString("Prepared open resources list with single entry '/': %1 (count=%2)").arg(outPath).arg(outCount));
        return true;
    }
