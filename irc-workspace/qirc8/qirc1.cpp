...
private slots:
    ...
    void onChannelTableCellDoubleClicked(int row, int /*column*/) {
        ...
        // Nur Channels mit führendem '#' joinen, sonst ignorieren
        if (!chn.startsWith("#")) {
            // Falls der Server andere Channel-Formate erlaubt, entferne diese Prüfung
            return;
        }
        ...
        // Wechsel zum Channel-Tab (nicht zur Channel-Liste)
        int idx = tabCtr->indexOf(mapTxt.contains(chn) ? mapTxt[chn]->parentWidget() : nullptr);
        // Fallback: suche Tab-Index anhand des Tab-Textes
        if (idx == -1) {
            for (int i = 0; i < tabCtr->count(); ++i) {
                if (tabCtr->tabText(i) == chn) { idx = i; break; }
            }
        }
