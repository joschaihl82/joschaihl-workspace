// screenrec.cpp
// Single-file Qt6 screen recorder with ALSA microphone capture and libav encoding/muxing.
// Build with qmake using jscreenrec.pro linking Qt6 Widgets Multimedia and libav* + alsa.
...
// ---------------------------
// Helper: map channel count to channel layout mask
// (portable: avoid av_get_default_channel_layout if not available)
// ---------------------------
...
public:
    ...
    // Pop one audio buffer (interleaved S16_LE)
...
// ---------------------------
// Recorder thread: encodes video/audio and muxes to MP4 using libav*
// ---------------------------
...
protected:
    void run() override {
        ...
        if (!videoCodec) {
            emit errorOccurred("H.264 encoder not found");
        ...
        video_enc_ctx->width = m_screenW;
        video_enc_ctx->height = m_screenH;
        ...
        video_enc_ctx->bit_rate = 2000 * 1000;

        if (fmt_ctx->oformat->flags & AVFMT_GLOBALHEADER)
            video_enc_ctx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;

        if (avcodec_open2(video_enc_ctx, videoCodec, nullptr) < 0) {
            emit errorOccurred("Could not open video encoder");
            avcodec_free_context(&video_enc_ctx);
            avformat_free_context(fmt_ctx);
            return;
        }

        if (avcodec_parameters_from_context(video_st->codecpar, video_enc_ctx) < 0) {
        ...
        if (!audioCodec) {
            emit errorOccurred("AAC encoder not found");
            avcodec_free_context(&video_enc_ctx);
            avformat_free_context(fmt_ctx);
            return;
        }
        ...
        if (!audio_st) {
            emit errorOccurred("Failed to create audio stream");
            avcodec_free_context(&video_enc_ctx);
            avformat_free_context(fmt_ctx);
            return;
        }
        ...
        audio_enc_ctx->sample_fmt = AV_SAMPLE_FMT_FLTP; // prefer float planar for AAC
        ...
        if (avcodec_open2(audio_enc_ctx, audioCodec, nullptr) < 0) {
            emit errorOccurred("Could not open audio encoder");
        ...
        if (avcodec_parameters_from_context(audio_st->codecpar, audio_enc_ctx) < 0) {
            emit errorOccurred("Failed to copy audio codec parameters");
            avcodec_free_context(&audio_enc_ctx);
            avcodec_free_context(&video_enc_ctx);
            avformat_free_context(fmt_ctx);
            return;
        ...
        if (!(fmt_ctx->oformat->flags & AVFMT_NOFILE)) {
            if (avio_open(&fmt_ctx->pb, m_outFile.toUtf8().constData(), AVIO_FLAG_WRITE) < 0) {
        ...
        if (avformat_write_header(fmt_ctx, nullptr) < 0) {
            emit errorOccurred("Error writing header to output file");
        ...
        SwsContext *sws_ctx = sws_getContext(
            m_screenW, m_screenH, AV_PIX_FMT_BGRA,
            video_enc_ctx->width, video_enc_ctx->height, video_enc_ctx->pix_fmt,
            SWS_BILINEAR, nullptr, nullptr, nullptr);

        // Configure resampler robustly using swr_alloc + av_opt_set_int / av_opt_set_sample_fmt
        SwrContext *swr = swr_alloc();
        if (!swr) {
            emit errorOccurred("Could not allocate SwrContext");
            avio_closep(&fmt_ctx->pb);
            avcodec_free_context(&audio_enc_ctx);
            avcodec_free_context(&video_enc_ctx);
            avformat_free_context(fmt_ctx);
        ...
        // Build channel masks from channel counts (portable)
        uint64_t in_ch_mask = channelMaskForCount(m_alsa->channels());
        int out_nb_channels = audio_enc_ctx->ch_layout.nb_channels;
        uint64_t out_ch_mask = channelMaskForCount(out_nb_channels);

        // Set options on swr using masks and sample formats
        if (av_opt_set_int(swr, "in_channel_layout",  (int64_t)in_ch_mask, 0) < 0 ||
            av_opt_set_int(swr, "out_channel_layout", (int64_t)out_ch_mask, 0) < 0 ||
            av_opt_set_int(swr, "in_sample_rate",    m_alsa->sampleRate(), 0) < 0 ||
            av_opt_set_int(swr, "out_sample_rate",   audio_enc_ctx->sample_rate, 0) < 0 ||
            av_opt_set_sample_fmt(swr, "in_sample_fmt",  AV_SAMPLE_FMT_S16, 0) < 0 ||
            av_opt_set_sample_fmt(swr, "out_sample_fmt", audio_enc_ctx->sample_fmt, 0) < 0) {
            emit errorOccurred("Failed to set swr options via av_opt_set_*");
            swr_free(&swr);
        ...
        if (ret < 0) {
            ...
            emit errorOccurred(QString("Failed to initialize resampler: %1").arg(errbuf));
            swr_free(&swr);
            sws_freeContext(sws_ctx);
            avio_closep(&fmt_ctx->pb);
            avcodec_free_context(&audio_enc_ctx);
        ...
        // Start ALSA capture if needed
        if (!m_alsa->isRunning()) {
            if (!m_alsa->startCapture()) {
                emit errorOccurred("Failed to start ALSA capture thread");
            }
        }
        ...
        // Main loop: encode video frames and audio buffers
        bool running = true;
        while (!m_stop && running) {
            ...
            while (m_alsa->popAudio(audioBuf)) {
                ...
                if (outSamples > aframe->nb_samples) {
                    ...
                    if (av_frame_get_buffer(aframe, 0) < 0) {
                        emit errorOccurred("Failed to reallocate audio frame buffer");
                        running = false;
                        break;
                    }
                }
                ...
                if (converted < 0) {
                    char errbuf[256]; av_strerror(converted, errbuf, sizeof(errbuf));
                    emit errorOccurred(QString("swr_convert failed: %1").arg(errbuf));
                    running = false;
                    break;
                }
...
// ---------------------------
// Webcam preview (small top-right window)
// ---------------------------
...
// ---------------------------
// Main application: tray + capture loop
// ---------------------------
class RecorderApp : public QObject {
    Q_OBJECT
public:
    RecorderApp(QApplication &app)
        ...
        m_overlayW = qMax(64, (int)std::round(m_screenW * 0.075));
...
public slots:
    void startRecording() {
        ...
        if (!m_recorder->startRecording(m_outFile)) {
            onError("Failed to start recorder thread");
            return;
        }
    ...
    void togglePause() {
        ...
        if (!m_isPaused) {
            m_captureTimer->stop();
            m_isPaused = true;
            m_actionPause->setText("Resume");
            m_tray->showMessage("Screen Recorder", "Paused");
        } else {
            m_captureTimer->start(33);
            m_isPaused = false;
            m_actionPause->setText("Pause");
            m_tray->showMessage("Screen Recorder", "Resumed");
        }
    }
...
private slots:
    void captureScreenFrame() {
        ...
        VideoFrame vf;
        vf.image = img;
        vf.pts = QDateTime::currentMSecsSinceEpoch();
        m_frameQueue.push(std::move(vf));
    }

    void onError(const QString &msg) {
        LOG << "Error:" << msg;
        m_tray->showMessage("Screen Recorder Error", msg);
    }

    void onFinished() {
        LOG << "Recording finished";
        m_tray->showMessage("Screen Recorder", "Recording finished");
    }
...
int main(int argc, char **argv) {
    ...
    // modern FFmpeg doesn't require av_register_all(); initialize network if needed
