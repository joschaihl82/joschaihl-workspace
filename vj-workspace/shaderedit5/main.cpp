// main.cpp - Qt6 amalgamated single-file application
// Left: GLSL editor, Right: OpenGL preview. 'f' toggles fullscreen, Esc to exit.
// Build with CMake (Qt6 Widgets, OpenGL, OpenGLWidgets) and link with asound (ALSA).
...
constexpr int BUFFERSIZE = 512, FFT_SIZE = 2048, KICK_DEBOUNCE_MS = 200;
constexpr unsigned DEFAULT_SR = 48000;
constexpr float TARGET_DB = 48.0f, MAX_GAIN = 4.0f, ATTACK_FAST = 0.6f,
		RELEASE_SLOW = 0.02f;
...
static double fft_re[FFT_SIZE], fft_im[FFT_SIZE], tw_re[FFT_SIZE / 2],
		tw_im[FFT_SIZE / 2], window_hann[FFT_SIZE];

const char *DEFAULT_SHADER =
		R"(// Fragment shader hier bearbeiten
// Beispiel: Standard-Shader
#version 120
uniform sampler1D u_samples; uniform float u_vscale, u_gain_display, u_hue; uniform int u_kick; varying vec2 v_uv;
vec3 hsv2rgb(float h, float s, float v) {
  float c=v*s, hh=h*6.0, x=c*(1.0-abs(mod(hh,2.0)-1.0)); vec3 col;
  if(hh<1.0) col=vec3(c,x,0.0); else if(hh<2.0) col=vec3(x,c,0.0); else if(hh<3.0) col=vec3(0.0,c,x);
  else if(hh<4.0) col=vec3(0.0,x,c); else if(hh<5.0) col=vec3(x,0.0,c); else col=vec3(c,0.0,x); return col+vec3(v-c);
}
void main() {
  if (u_kick==1) { gl_FragColor=vec4(1.0); return; }
  float s=texture1D(u_samples, v_uv.x).r, dist=abs(v_uv.y-(0.5-s*u_vscale));
  float alpha=smoothstep(1.0/600.0, 0.0, dist), c_alpha=smoothstep(0.002, 0.0, abs(v_uv.y-0.5));
  vec3 wcol=hsv2rgb(u_hue, 1.0, 1.0), col=mix(wcol*0.5, wcol, alpha); float f_alpha=max(alpha, c_alpha);
  if (v_uv.x<0.15 && v_uv.y<0.03 && (v_uv.x/0.15)<u_gain_display) { col=vec3(0.0, 1.0, 0.0); f_alpha=1.0; }
...
static bool alsa_find_capture_control(const char *name) {
	...
	if (!(g_selem = snd_mixer_find_selem(g_mixer, sid)))
		return false;
...
static void alsa_set_capture_all(long v) {
	if (g_selem)
		snd_mixer_selem_set_capture_volume_all(g_selem,
				std::max(g_vol_min, std::min(g_vol_max, v)));
}
...
static void bit_reverse_permute(double *re, double *im) {
	for (unsigned i = 0, j = 0; i < FFT_SIZE; ++i) {
		if (i < j) {
			std::swap(re[i], re[j]);
			std::swap(im[i], im[j]);
		}
	...
	for (int len = 2; len <= FFT_SIZE; len <<= 1) {
		...
		for (int i = 0; i < FFT_SIZE; i += len) {
			for (int j = 0; j < half; ++j) {
				...
				double tre = tw_re[tw] * re[k + half]
						- tw_im[tw] * im[k + half], tim = tw_re[tw]
						* im[k + half] + tw_im[tw] * re[k + half];
				re[k] = ur + tre;
				im[k] = ui + tim;
				re[k + half] = ur - tre;
				im[k + half] = ui - tim;
...
static double fft_compute_bass_energy(unsigned sr, double f_lo, double f_hi) {
	...
	int b_lo = std::max(0, (int) floor(f_lo / ((double) sr / FFT_SIZE))), b_hi =
			std::min(FFT_SIZE / 2 - 1,
					(int) ceil(f_hi / ((double) sr / FFT_SIZE)));
	double sum = 0.0;
	for (int b = b_lo; b <= b_hi; ++b)
		sum += sqrt(fft_re[b] * fft_re[b] + fft_im[b] * fft_im[b]);
	return sum;
}
...
class GLWaveWidget: public QOpenGLWidget, protected QOpenGLFunctions {
	Q_OBJECT
public:
	explicit GLWaveWidget(unsigned sr, QWidget *p = nullptr) :
			QOpenGLWidget(p), rate(sr), tex_size(FFT_SIZE), limiter_gain(1.f), kick(
					false), hue(0.f) {
	...
	void updateTextureFromRing() {
		...
		for (int i = 0; i < tex_size; ++i)
			tex_samples[i] = g_ring[(st + i) % g_ring_cap];
	}
	bool setFragmentShaderSource(const QString &fsrc, QString &errOut) {
		...
		if (!ok) {
			...
			errOut = QString("VS error: %1").arg(log);
			glDeleteShader(vs);
			doneCurrent();
			return false;
		}
		...
		if (!ok) {
			...
			errOut = QString("FS error: %1").arg(log);
			glDeleteShader(vs);
			glDeleteShader(fs);
			doneCurrent();
			return false;
		...
		if (!ok) {
			...
			errOut = QString("Link error: %1").arg(log);
...
protected:
	void initializeGL() override {
		...
		if (!setFragmentShaderSource(DEFAULT_SHADER, err))
			qWarning() << "Default shader error:" << err;
	}
	...
	signals: void requestToggleFullscreen();
...
private:
	...
	GLint loc_u_samples = -1, loc_u_vscale = -1, loc_u_gain_display = -1,
			loc_u_hue = -1, loc_u_kick = -1;
};

class AlsaCaptureThread: public QThread {
	Q_OBJECT
public:
	explicit AlsaCaptureThread(unsigned sr, QObject *p = nullptr) :
			QThread(p), rate(sr) {
	}
	void run() override {
		snd_pcm_t *pcm;
		...
		snd_pcm_uframes_t period = BUFFERSIZE, buffer = period * 4;
		...
		float g_limit = 1.0f;
		const float TGT_LVL = powf(10.0f, -TARGET_DB / 20.0f);
		alsa_set_capture_all(g_vol_min + (g_vol_max - g_vol_min) / 2);
		double run_avg = 1e-8;
		quint32 last_kick = 0;

		while (!isInterruptionRequested()) {
			snd_pcm_sframes_t f = snd_pcm_readi(pcm, read_buf.data(),
					BUFFERSIZE);
			if (f == -EPIPE) {
				snd_pcm_prepare(pcm);
				continue;
			...
			float chunk_rms = compute_rms_s16(read_buf.data(), f), des =
					std::min(MAX_GAIN, TGT_LVL / std::max(chunk_rms, 1e-12f));
			float factor = des > g_limit ? ATTACK_FAST : RELEASE_SLOW;
			g_limit = factor * des + (1.0f - factor) * g_limit;
			alsa_set_capture_all(
					g_limit >= 1.f ?
							g_vol_max :
							g_vol_min
									+ (long) ((g_vol_max - g_vol_min)
											* std::max(0.f, g_limit) + 0.5f));
			push_to_ring_from_s16(read_buf.data(), f);
			...
			if (energy > run_avg * 3.0 && energy > 1e-6
					&& (now - last_kick) > KICK_DEBOUNCE_MS) {
				is_kick = true;
				last_kick = now;
			}

			emit audioFrameAvailable(g_limit, is_kick,
					std::min(0.7f,
							compute_rms_from_ring((rate * 500) / 1000) * 5.0f));
			msleep(1);
		}
		snd_pcm_close(pcm);
	}
	signals: void audioFrameAvailable(float lg, bool k, float h);
private:
	unsigned rate;
};
...
int main(int argc, char **argv) {
	...
	if (!alsa_find_capture_control("Capture")
		...
		qWarning("No ALSA capture found. Exiting.");
		return 2;
	}

	g_ring_cap = (size_t) DEFAULT_SR * 2;
	...
	QPlainTextEdit *editor = new QPlainTextEdit(
			QString::fromUtf8(DEFAULT_SHADER));
