...
struct llama_file::impl {
    ...
    impl(const char * fname, const char * mode) {
        fp = ggml_fopen(fname, mode);
        if (fp == NULL) {
            //throw std::runtime_error(("failed to open %s: %s", fname, strerror(errno)));
        }
        seek(0, SEEK_END);
        size = tell();
        seek(0, SEEK_SET);
...
#endif
        if (ret == -1) {
            //throw std::runtime_error(("ftell error: %s", strerror(errno)));
        }
...
#endif
        if (ret != 0) {
            //throw std::runtime_error(("seek error: %s", strerror(errno)));
        }
    }
