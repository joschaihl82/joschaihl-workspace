...
int main(){
    ...
    while(running){
        ...
        // ROI / resize
        #define ROI_SIZE 192          // etwas größer für 800x600
        #define RES 64
        #define PIXELS (RES*RES)
        ...
        #define BETA 0.5f
        #define V_REF 0.5f
        #define AREA_MIN 5000         // höher für 800x600
        #define STABLE_COM_MAX_MOVE 8
        #define STABLE_FRAMES_REQ 3
        #define UPDATE_EVERY_N 3
        ...
        // Nose detector: find strong vertical gradient (bridge) and nostril dark blobs.
        // Returns 1 if nose found; sets tip_x, tip_y
        static int detect_nose(uint8_t *rgb, int face_cx, int face_cy, int *tip_x, int *tip_y){
            int sx = face_cx - ROI_SIZE/6; if(sx<0) sx=0;
            int ex = face_cx + ROI_SIZE/6; if(ex>=W) ex=W-1;
            int sy = face_cy - ROI_SIZE/8; if(sy<0) sy=0;
            int ey = face_cy + ROI_SIZE/3; if(ey>=H) ey=H-1;
        ...
        ///////////////////// Eyes & mouth detection (improved + iris/nose/mouth refine) //////////////////////
        // Robustere Augenheuristik:
        // - Arbeitsbereich: oberes ROI-Halbdrittel
        // - Kandidaten: lokale Minima in Grauwert (dunkel), mit starkem Gradienten-Ring (Sobel) um den Punkt
        // - Symmetrie-Score: Spiegelpunkt zur Gesichtsmittellinie (zwischen den Augen) ist ähnlich dunkel
        // - Auswahl: zwei beste Kandidaten mit plausibler horizontaler Trennung und ähnlicher y-Position
        static void detect_eyes_mouth(uint8_t *rgb, int cx, int cy){
            ...
            for(int yy=0; yy<Rh; yy++)
                for(int xx=0; xx<Rw; xx++){ sumY += Y[yy*Rw+xx]; cntY++; }
            float meanY = (float)(sumY / (float)cntY);
            float eye_dark_thr = meanY * 0.55f; // strenger als vorher
            int ring_r = 3;
        ...
        int v4l2_init(){
            ...
            fmt.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
            fmt.fmt.pix.width = W; fmt.fmt.pix.height = H;
            fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV; // falls 800x600 nicht unterstützt: probiere MJPEG
            fmt.fmt.pix.field = V4L2_FIELD_NONE;
            if(ioctl(fd, VIDIOC_S_FMT, &fmt) < 0){ perror("VIDIOC_S_FMT"); return -1; }
            struct v4l2_requestbuffers req = {0};
            req.count = 4; req.type = V4L2_BUF_TYPE_VIDEO_CAPTURE; req.memory = V4L2_MEMORY_MMAP;
        ...
        int main(){
            ...
            while(running){
                ...
                // Augen, Iris, Nase & Mund innerhalb der ROI erkennen und einzeichnen
                detect_eyes_mouth(frame_rgb, cx, cy);
