// cc.c - single-file small C compiler (amalgamated, self-hosted, with fixes)
// - Handles long/short/unsigned types
// - Simple preprocessor with #include, #pragma once, #define (fixed)
...
// Use:   ./cc input.c > out.s
//        gcc -no-pie -o a.out out.s

// -------------------- Minimal standard declarations --------------------
...
Token *tokenize(char *p, bool eof) {
    ...
    while (*p) {
        // Handle backslash line-continuation and lone backslash
        if (*p == '\\') {
            // If backslash followed by newline -> line continuation: skip both
            if (p[1] == '\n') {
                p += 2;
                continue;
            }
            // Otherwise treat backslash as a single-char reserved token
        ...
        if (isdigit((int)(unsigned char)*p)) {
            ...
            if (startswith(p, "0x") || startswith(p, "0X")) {
                base = 16;
                p += 2;
                while (isxdigit((int)(unsigned char)*p)) p++;
            } else {
...
// --- Preprocessor (simple, fixed #define parsing) ---
    ...
    for (Token *t = tok; t; t = t->next) {
        if (t->kind == TK_PREPROCESSOR) {
            ...
            if (str_starts_with_str(t->str, "#define")) {
                // The entire directive may be inside the TK_PREPROCESSOR token,
                // e.g. "#define FOO 1\n". Parse the token string to extract the
                // identifier and the replacement text, then tokenize the replacement.
                char *line = t->str->ptr;
                ...
                // Extract identifier
                int id_start = pos;
                while (pos < line_len && (isalpha((unsigned char)line[pos]) || line[pos] == '_' || line[pos] == '$' || isalnum((unsigned char)line[pos]))) pos++;
                if (id_start == pos) error_at_here("#define expects identifier");

                // Create String for identifier (store pointer to duplicated buffer)
                ...
                // Tokenize the replacement text to produce a token list
                ...
                // free temporary buffers (ident_buf is stored inside String->ptr)
                free(repl_text);
...
int sizeof_type(Type *type) {
    ...
    case TY_ARRAY: return sizeof_type(type->ptr_to) * type->array_size;
    case TY_CHAR: return 1;
    case TY_STRUCT: return type->struct_type->size;
    case TY_BOOL: return 1;
    case TY_VA_LIST_TAG: return 24;
...
Type *consume_type_name() {
    ...
    if (consume_kind(TK_VOID)) return new_type(TY_VOID, NULL);
    if (consume_kind(TK_BOOL)) return new_type(TY_BOOL, NULL);
...
ConsumeTypeRes *expect_nested_type(Type *type) {
    ...
    for (int i = 0; i < ptr; i++) res->type = new_type(TY_PTR, res->type);
...
ConsumeTypeRes *consume_type() {
    ...
    if (!type) return NULL;
    while (consume("*")) type = new_type(TY_PTR, type);
...
Node *add() {
    ...
    for (;;) {
        if ((node->type->ty == TY_PTR || node->type->ty == TY_ARRAY) && node->type->ptr_to && node->type->ptr_to->ty != TY_VOID) {
            int size = sizeof_type(node->type->ptr_to);

            if (consume("+")) {
                if (node->type->ty == TY_ARRAY)
                    node = new_typed_node(ND_ADDR, node, NULL, new_type(TY_PTR, node->type->ptr_to));
...
void add_field(StructType *type, Type *ty, String *name) {
    ...
    field->name = name;
    int al;
    if (ty->ty == TY_ARRAY) al = sizeof_type(ty->ptr_to);
...
External *external() {
    locals = NULL;
    if (!globals) new_gvar(new_string("NULL", 4), new_type(TY_PTR, new_type(TY_VOID, NULL)));
