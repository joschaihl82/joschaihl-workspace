/**
  ...
  - Supports:
      ./jlz4 -c <input> <output>    (output can be directory or single archive file ending .lz4)
...
static int process_decompress_archive(FILE *archive, const char *outroot) {
    while (1) {
        ...
        if (decompress_file_from_archive(archive, outpath, comp_size, name, orig_size) != 0) return -1;
    }
    return 0;
}
