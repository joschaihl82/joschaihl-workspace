/*
 * main.cpp - qgeister1 "Berlin 5" (CSV only)
 *
 * Minimaler Amalgamat-Build: Berlin5 Preset immer aktiv, CSV-Logging standardmäßig aktiviert.
 * Entfernt CLI-Modi und alternative Presets — dieses Programm läuft ausschließlich im
 * CIA "Berlin 5" Modus und schreibt fortlaufend berlin5_log.csv (ts_ms, Sv/s, avg Sv/s, total Sv, alert).
 *
...
// --- Configuration (fixed Berlin5 + CSV) ---
...
// Berlin5 fixed parameters
static const bool CIA_MODE_BERLIN5 = true;          // always enabled
static const bool CIA_ENABLE_CSV_LOG = true;        // always enabled
static const double CIA_BERLIN5_SENS_SCALE = 1.8;   // hotPixel->Sv/h scale
static const double CIA_BERLIN5_CLICK_GAIN = 1.6;   // click mapping gain
static const double CIA_BERLIN5_CLICK_MAX = 500.0;  // hard cap clicks/s
static const double CIA_BERLIN5_ALERT_SHORT_SEC = 2.0; // hold time to trigger alert
static const double CIA_BERLIN5_ALERT_THRESHOLD_S_PER_S = (GEIGER_ALERT_THRESH / 3600.0) * 0.6; // 60% of old threshold
...
// --- GeigerLogic with Radionullon filter and Berlin5 CSV ---
...
public:
    explicit GeigerLogic(QObject *parent = nullptr) : QObject(parent) {
        ...
        // Open CSV for Berlin5 logging immediately
        if (CIA_ENABLE_CSV_LOG) {
            berlin5CsvFile.setFileName("berlin5_log.csv");
            if (berlin5CsvFile.open(QIODevice::Append | QIODevice::Text)) {
                QTextStream ts(&berlin5CsvFile);
                // write header only if file was empty (quick check)
                if (berlin5CsvFile.size() == 0) ts << "ts_ms,raw_Sv_per_s,avg_Sv_per_s,total_Sv,alert\n";
                berlin5CsvFile.flush();
            } else {
                qWarning() << "Konnte berlin5_log.csv nicht öffnen zum Schreiben";
            }
        }
    }

    ~GeigerLogic() {
        if (berlin5CsvFile.isOpen()) berlin5CsvFile.close();
    }
    ...
    QImage processRadionullon(const QImage &img) {
        ...
        // --- Geiger values (Berlin5 scaling applied) ---
        double rawVal_per_h = ((double)hotPixels / GEIGER_NORM_FACTOR) * CIA_BERLIN5_SENS_SCALE;
        ...
        // Build stats and Berlin5 alert/hysteresis
        GeigerStats stats;
        stats.currentRad = rawVal;
        stats.avgRad = currentRadSmooth;
        stats.totalDose = totalDoseAcc;

        // Berlin5 short-term hysteresis logic
        double nowSec = QDateTime::currentDateTime().toMSecsSinceEpoch() * 0.001;
        if (currentRadSmooth > CIA_BERLIN5_ALERT_THRESHOLD_S_PER_S) {
            if (berlin5_peak_start_sec < 0.0) berlin5_peak_start_sec = nowSec;
            if ((nowSec - berlin5_peak_start_sec) >= CIA_BERLIN5_ALERT_SHORT_SEC) {
                berlin5_alert_active = true;
            }
        } else {
            if (berlin5_alert_active && (nowSec - berlin5_peak_start_sec) < (CIA_BERLIN5_ALERT_SHORT_SEC + 1.0)) {
                // keep alert briefly
            } else {
                berlin5_alert_active = false;
                berlin5_peak_start_sec = -1.0;
            }
        }
        stats.alertState = berlin5_alert_active;
        ...
        // CSV logging (always enabled in this build)
        if (berlin5CsvFile.isOpen()) {
            QTextStream ts(&berlin5CsvFile);
        ...
        // send activity in Sv/s to audio engine (Berlin5 clicks)
...
private:
    ...
    // Berlin5 state
    double berlin5_peak_start_sec = -1.0;
    bool berlin5_alert_active = false;
    QFile berlin5CsvFile;

    QElapsedTimer frameTimer;
    qint64 lastFrameMs { -1 };
};

// --- AudioEngine (Berlin5 click mapping, event-driven) ---
...
public slots:
    // Enqueue a click event; activity in Sv/s
        ...
        while (running.load()) {
            {
                ...
                while (!clickQueue.empty()) {
                    ...
                    // Map activity (Sv/s) -> expected clicks/s (Berlin5 scaling)
                    double activityPerHour = ev.activity * 3600.0;
                    double baseClicks = 0.9 * activityPerHour + 0.5;
                    double scaled = baseClicks * CIA_BERLIN5_CLICK_GAIN;
                    double clicksPerSecond_est = qMin(CIA_BERLIN5_CLICK_MAX, scaled);
                    ...
                    for (int i = 0; i < injectCount; ++i) {
                        ...
                        ac.ampVar = 0.85 + double(seed & 0xFF) / 255.0 * 0.5;
                        ac.pitchVar = 1.0 + ((((seed >> 8) & 0xFF) / 255.0) - 0.5) * 0.05;
                        ac.id = nextClickId++;
...
public:
    GeigerWidget(QWidget *parent = nullptr) : QWidget(parent) {
        setWindowTitle("qrad Radionullon Viewer | Berlin5 CSV");
...
protected:
    void paintEvent(QPaintEvent *) override {
        ...
        // Berlin5 visual alert (blinking red frame)
