// main.cpp
// Qt6 amalgamated single-file application
// - Left: GLSL fragment shader editor (compiles on every keystroke)
...
//
// Build with the provided CMakeLists.txt (Qt6 Widgets, OpenGL, OpenGLWidgets) and link with asound (ALSA).
//
// Notes:
// - The editor compiles on every text change. If compilation fails, the preview continues
//   to use the last successfully linked program.
// - ALSA capture, ring buffer, FFT-based kick detection are ported from the original C example.
// - This file is intentionally self-contained for quick testing; for production split into headers/source.
...
public:
    ...
    bool setFragmentShaderSource(const QString &fsrc, QString &errorOut) {
        ...
        if (!ok) {
            ...
            errorOut = QString("Fragment shader compile error: %1").arg(log);
            glDeleteShader(vs);
            glDeleteShader(fs);
            doneCurrent();
            return false;
        ...
        if (!ok) {
            char log[4096]; glGetProgramInfoLog(newProg, sizeof(log), nullptr, log);
            errorOut = QString("Program link error: %1").arg(log);
...
void main() {
  ...
  gl_FragColor = vec4(col, final_alpha);
    ...
    {
        QString err;
        if (!glw->setFragmentShaderSource(editor->toPlainText(), err)) qWarning() << "Initial shader compile error:" << err;
    }
