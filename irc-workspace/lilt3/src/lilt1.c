/*
  ...
  - Always redraws all rows each frame to avoid blacking out non-dirty lines
...
/* --- Rendering --- */
/* NOTE: This version redraws all rows every frame to avoid the "only last row visible" issue.
   If you want to optimize, maintain a persistent backing texture and update only changed rows. */
static void render_dirty_lines(void)
{
  if (!renderer) return;
