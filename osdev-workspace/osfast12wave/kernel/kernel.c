...
// Quads are filled as two triangles; each quad gets a tiny random color variation
// around an ocean-blue base color.
...
/* --- Triangle rasterizer using edge functions (float) --- */
static void rasterize_triangle(float x0, float y0, float x1, float y1, float x2, float y2, uint32_t color) {
    /* bounding box */
    ...
    /* constant terms */
    float c0 = e0x * x0 + e0y * y0;
    float c1 = e1x * x1 + e1y * y1;
    float c2 = e2x * x2 + e2y * y2;

    for (int y = miny; y < maxy; ++y) {
        for (int x = minx; x < maxx; ++x) {
            float px = (float)x + 0.5f;
            float py = (float)y + 0.5f;
            float v0 = e0x * px + e0y * py - c0;
            float v1 = e1x * px + e1y * py - c1;
            float v2 = e2x * px + e2y * py - c2;
            /* allow top-left fill rule: accept pixels where all edge funcs have same sign (>=0) */
...
/* --- Fill quad as two triangles with slight per-quad color jitter --- */
static void fill_quad(float ax, float ay, float bx, float by, float cx, float cy, float dx, float dy, uint32_t base_rgb) {
    /* derive tiny color jitter from RNG */
    uint32_t r = rand32();
    /* jitter in [-0.03, +0.03] for value (brightness) */
    float jitter = ((float)(r & 0xFF) / 255.0f - 0.5f) * 0.06f;
    /* convert base_rgb (RGB) to HSV-like simple brightness adjust: scale components */
    uint8_t br = (base_rgb >> 16) & 0xFF;
    uint8_t bg = (base_rgb >> 8) & 0xFF;
    uint8_t bbv = base_rgb & 0xFF;
    /* apply jitter by mixing towards white or dark */
    int ir = (int)(br + (255 - br) * jitter);
    int ig = (int)(bg + (255 - bg) * jitter);
    int ib = (int)(bbv + (255 - bbv) * jitter);
    if (ir < 0) ir = 0; if (ir > 255) ir = 255;
    if (ig < 0) ig = 0; if (ig > 255) ig = 255;
    if (ib < 0) ib = 0; if (ib > 255) ib = 255;
    uint32_t color = ((uint32_t)ir << 16) | ((uint32_t)ig << 8) | (uint32_t)ib;

    /* triangle 1: A B C */
    rasterize_triangle(ax, ay, bx, by, cx, cy, color);
    /* triangle 2: A C D */
    rasterize_triangle(ax, ay, cx, cy, dx, dy, color);
}
...
void main(KernelGOPInfo *kgi) {
    ...
    /* Base ocean-blue color (RGB) */
    uint32_t base_ocean = hsv_to_rgb(200.0f, 0.9f, 0.45f);

    /* Main loop */
    while (1) {
        /* clear backbuffer to background */
        uint32_t bgcol = hsv_to_rgb(210.0f, 0.6f, 0.06f);
