...
protected:
    void paintEvent(QPaintEvent *ev) override {
        ...
        // selection normalization (compute a/b once)
        QPoint a = selStart;
        QPoint b = selEnd;
        if (a.x() > b.x()) std::swap(a.rx(), b.rx());
        if (a.y() > b.y()) std::swap(a.ry(), b.ry());
        ...
        for (int r = 0; r < termH; ++r) {
            for (int c = 0; c < termW; ++c) {
                ...
                if (r < showFromScrollback) {
                    ...
                    absoluteRowIndex = sbLines + liveRow;
                ...
                // selection check: selStart/selEnd are absolute row indices
                bool isSelected = false;
                if (selStart.x() >= 0 && selEnd.x() >= 0) {
                    if (absoluteRowIndex >= a.y() && absoluteRowIndex <= b.y() &&
                        c >= a.x() && c <= b.x()) {
                        isSelected = true;
                    }
                }
                ...
                if (isSelected) {
                    QColor selColor = QColor(0x33,0x66,0xCC, 160); // translucent blue
                    p.fillRect(dst, selColor);
                } else {
                ...
                if (!atlas.isNull()) {
                    ...
                    // if selected, draw glyph in inverted color for readability
                    if (isSelected) {
                        gp.fillRect(colored.rect(), Qt::white);
                    } else {
                    ...
                    p.setPen(isSelected ? Qt::white : colors[fg]);
                    p.setFont(font);
                ...
                if (cell.a.underline) {
                    p.fillRect(dst.x(), dst.y() + glyphH - 2, glyphW, 2, isSelected ? Qt::white : colors[fg]);
                }
            }
        }
    ...
    void keyPressEvent(QKeyEvent *ev) override {
        // Copy shortcuts: Ctrl+Shift+C or Ctrl+C when selection exists
        if ((ev->modifiers() & (Qt::ControlModifier | Qt::ShiftModifier)) == (Qt::ControlModifier | Qt::ShiftModifier)
            && ev->key() == Qt::Key_C) {
            copySelectionToClipboard();
            return;
        }
        if ((ev->modifiers() & Qt::ControlModifier) && ev->key() == Qt::Key_C && selStart.x() >= 0 && selEnd.x() >= 0) {
            copySelectionToClipboard();
            return;
        }
    ...
    void mousePressEvent(QMouseEvent *ev) override {
        // if terminal mouse reporting is enabled, keep existing behavior
        if (mouseMode != -1) {
            handleMouse(ev, true);
            return;
        }
        ...
        int scrollbackStartRow = sbLines - showFromScrollback;
        int absoluteRow;
        if (y < showFromScrollback) absoluteRow = scrollbackStartRow + y;
        else absoluteRow = sbLines + (y - showFromScrollback);

        selecting = true;
        selStart = QPoint(x, absoluteRow);
        selEnd = selStart;
        update();
    }
    ...
    void mouseMoveEvent(QMouseEvent *ev) override {
        if (mouseMode != -1) {
            if (mouseMotionEnabled) handleMouse(ev, true);
            return;
        }
        if (!selecting) return;
        int x = ev->position().x() / glyphW;
...
private:
    ...
    // Selection state (absolute row indices)
    bool selecting;
    QPoint selStart; // (col, absoluteRow)
    QPoint selEnd;   // (col, absoluteRow)
    ...
    void allocScreen(int cols, int rows) {
        ...
        if (colsChanged) {
            ...
            scrollOffset = 0;
            // clear selection when columns change
            selStart = QPoint(-1,-1);
            selEnd = QPoint(-1,-1);
        }
    ...
    // Copy selection to clipboard. selStart/selEnd are absolute row indices.
    void copySelectionToClipboard() {
        if (selStart.x() < 0 || selEnd.x() < 0) return;
        QPoint a = selStart, b = selEnd;
        if (a.x() > b.x()) std::swap(a.rx(), b.rx());
        if (a.y() > b.y()) std::swap(a.ry(), b.ry());
        ...
        for (int row = a.y(); row <= b.y(); ++row) {
            int startCol = (row == a.y()) ? a.x() : a.x();
            int endCol = (row == b.y()) ? b.x() : b.x();

            if (row >= 0 && row < sbLines) {
                // scrollback row
                int idx = row;
                for (int col = startCol; col <= endCol; ++col) {
                    if (col >= 0 && col < termW)
                        out.append(QChar(scrollbackRows[idx][col].c));
                }
            } else {
                // live buffer row
                int liveRow = row - sbLines;
                if (liveRow >= 0 && liveRow < termH) {
                    for (int col = startCol; col <= endCol; ++col) {
                        if (col >= 0 && col < termW)
                            out.append(QChar(screenBuf[liveRow*termW + col].c));
                    }
                }
            }
            if (row != b.y()) out.append('\n');
        }
