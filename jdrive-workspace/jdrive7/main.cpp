...
// Logfenster mit automatisch scrollendem Textfeld, Gesamt-ETA und Dateifortschritt
...
signals:
    ...
    void etaUpdated(const QString &eta); // ETA string for UI
...
private:
    ...
    qint64 lastDownloadedBytesSnapshot = 0;
    QElapsedTimer rateSampleTimer;
    ...
    void reconcileAndSync() {
        ...
        if (!downloadQueue.isEmpty()) {
            ...
            rateSampleTimer.restart();
            lastDownloadedBytesSnapshot = 0;
    ...
    void processNextDownload() {
        ...
        // progress
        connect(reply, &QNetworkReply::downloadProgress, this, [this, name, size](qint64 rec, qint64 tot) {
            emit fileProgress(name, rec, tot);
            // update ETA periodically based on bytes downloaded
            updateEta();
        });
        ...
        connect(reply, &QNetworkReply::finished, this, [this, reply, outFile, outPath, okSize, size, name]() mutable {
            ...
            if (okSize && size > 0) downloadDownloadedBytes += size;
                ...
                // If size unknown, try to add actual file size
                QFile f(finalPath);
            ...
            // update ETA after finishing a file
            updateEta();
...
public slots:
    ...
    void onStatus(const QString &s) {
        // Append with timestamp and auto-scroll
...
int main(int argc, char *argv[]) {
    ...
    // Also show brief tray notifications on important status updates (throttled)
