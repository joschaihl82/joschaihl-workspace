...
int is_type_token(token *tok) {
    ...
    if (tok->kind == tk_ident) {
        ...
        // check global scope for typedefs (not implemented, but required for full c)
    }
    return 0;
}
...
// type_specifier = ("void" | "char" | "int" | "long" | "unsigned" | "struct" ...) ("static" | "extern")?
ty *parse_type_specifier(int *is_unsigned_ptr, int *is_static_ptr, int *is_extern_ptr) {
    // track qualifiers
    int is_unsigned = 0;
    int is_static = 0;
...
ty *parse_declarator(ty *base, token **ident_tok_ptr) {
    ...
    // function declarator (simplified: only takes base type, full parameter parsing later)
    if (consume("(")) {
        ...
        // a function type is not fully supported yet, so we return a ptr to the base for now.
    }
...
sinit *parse_initializer(ty *type) {
    ...
    if (type->kind == ty_array && type->base->kind == ty_char && current_token->kind == tk_string) {
        ...
        current_token = current_token->next;
        return new_sinit_ptr(str_tok->str); // str_content is the actual string data
    }
...
node *relational() {
    ...
    for (;;) {
        ...
        else if (consume(">")) n = new_node(nd_lt, shift(), n); // swapped for convenience
...
node *stmt() {
    ...
    if (consume_keyword("return")) {
        ...
        n->lhs = cond;
        n->rhs = body;
        n->val = new_labels->continue_label; // reuse val for continue label (loop start)
        n->val = new_labels->break_label; // this is messy, need to fix

        // fix: use node structure to hold both labels (e.g., rhs for body, val/els for labels)
...
// fundef_or_global = type_specifier (declarator ("(" (param_decl ("," param_decl)*)? ")" stmt)? | declaration)
void parse_fundef_or_global() {
    token *start_tok = current_token;
    int is_static = 0;
    int is_extern = 0;

    // specifiers (int, static, etc.)
    ty *base = parse_type_specifier(NULL, &is_static, &is_extern);
    ...
    if (consume("{")) {
        // if { is consumed, it MUST be a function definition
        if (current_func) error_tok(ident_tok, "nested function definition not allowed");

        function *func = calloc(1, sizeof(function));
        func->name = strndup(ident_tok->str, ident_tok->len);
        func->ret_type = base; // simplified: not respecting the full type_decl for function pointers
        func->is_static = is_static;
        current_func = func;

        // parameter list (simplified)
        // this version skips the parameter list parsing completely and expects a block
        current_token = start_tok;
        // re-parse up to the function name
        parse_type_specifier(NULL, &is_static, &is_extern);

        ty *func_type = new_type(ty_func, 0, 1);
        func_type->base = base; // return type

        // simplified param parsing for locals only
        ty *type = NULL;
        token *name = NULL;
        ...
        if (!consume(")")) {
            do {
                ...
                if (strncmp(current_token->str, "...", 3) == 0 && current_token->len == 3) {
                    ...
                    error_tok(current_token, "expected parameter name");
                }
            } while (consume(","));
            expect(")");
        }
        ...
        func->is_variadic = func_type->is_variadic;
        ...
        // calculate stack offsets for locals
        int offset = 0;
        ...
        // global variable or function prototype declaration
        current_token = start_tok; // backtrack for declaration parsing
        declaration(1);
    }
}
...
void emit_load(ty *type) {
    if (type->kind == ty_array || type->kind == ty_struct || type->kind == ty_void) return;

    char *reg;
    if (type->size == 1) reg = (type->is_unsigned ? "al" : "al");
    else if (type->size == 4) reg = (type->is_unsigned ? "eax" : "eax");
    else if (type->size == 8) reg = "rax";
    else return; // for structs > 8 bytes (not fully supported by this simple load)
...
void codegen_program(function *funcs) {
    ...
    for (function *func = funcs; func; func = func->next) {
        ...
        for (lvar *var = func->locals; var && var->next; var = var->next) {
            if (var->offset == 0) continue; // skip last local var (no offset)

            int i = func->num_params - (var->offset / 8);
            if (i >= 0 && i < func->num_params) {
                printf("  mov [rbp - %d], %s\n", var->offset, arg_regs[i]);
            }
        }
