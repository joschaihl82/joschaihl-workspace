// ============================================================================
// jemu.c - vollständige, erweiterte x64-emulator-amalgamation (+SSE2 XMM MOV)
// - enthält: cpu struct, memory helpers, erweiterte opcode-handler
// - dispatcher erweitert um alu, jcc, mov, register-movs, sse2 movdqa/movdqu
// ============================================================================
...
#include <unistd.h>
#include <SDL2/SDL.h>
#include <emmintrin.h> /* sse2 intrinsics (optional) */
...
/* -------------------------
   0F group handler (extended: jcc/setcc + SSE2 MOVDQA/MOVDQU)
   ------------------------- */
void handle_0F_group(uint8_t op2) {
    switch (op2) {
        /* -------------------------
           SSE2 / XMM moves (integer 128-bit)
           Encoding:
             - 66 0F 6F /r  : MOVDQA xmm, xmm/m128 (aligned load)
             - 66 0F 7F /r  : MOVDQA xmm/m128, xmm (aligned store)
             - F3 0F 6F /r  : MOVDQU xmm, xmm/m128 (unaligned load)
             - F3 0F 7F /r  : MOVDQU xmm/m128, xmm (unaligned store)
           We gate by legacy prefix tracking (pref_66 / pref_f3).
           ------------------------- */
        case 0x6F: { /* MOVDQA/MOVDQU load to XMM */
            decode_modrm();
            int dst = cpu.modrm_reg | (cpu.rex_r << 3);
            int is_movdqu = cpu.pref_f3;
            int is_movdqa = cpu.pref_66 && !is_movdqu;

            if (!is_movdqa && !is_movdqu) {
                /* Without prefixes, 0F 6F is MMX MOVQ; we don't implement MMX. */
                trace_printf("Unimplemented 0F 6F without 66/F3 (MMX MOVQ) at rip=0x%016" PRIx64 "\n", cpu.rip-2);
                exit(1);
            }
            ...
            if (cpu.modrm_mod == 3) {
                ...
                cpu.xmm[dst] = xmm_load_mem(addr, is_movdqa /* aligned? */, is_movdqa ? "MOVDQA" : "MOVDQU");
            }
            break;
        }
        case 0x7F: { /* MOVDQA/MOVDQU store from XMM */
            decode_modrm();
            ...
            if (!is_movdqa && !is_movdqu) {
                /* Without prefixes, 0F 7F is MMX MOVQ store; we don't implement MMX. */
                trace_printf("Unimplemented 0F 7F without 66/F3 (MMX MOVQ store) at rip=0x%016" PRIx64 "\n", cpu.rip-2);
                exit(1);
            }
            ...
            if (cpu.modrm_mod == 3) {
                ...
                uint64_t addr = compute_ea();
                xmm_store_mem(addr, cpu.xmm[src], is_movdqa /* aligned? */, is_movdqa ? "MOVDQA" : "MOVDQU");
            }
            break;
        }
        ...
        case 0x90 ... 0x9F: { /* setcc r/m8 */
            ...
            if (cpu.modrm_mod == 3) {
                /* only al, cl, dl, bl, ah, ch, dh, bh are architecturally byte-addressable in legacy,
                   but we model GPR as 64-bit; write low 8 bits. */
...
void run_loop_optimized(void) {
    ...
    op_mov_imm_reg: { /* 0xB8 - 0xBF: MOV r64, imm64 */
        ...
        if (cpu.rex_w) {
            ...
            imm = (uint64_t)(int64_t)(int32_t)fetch32(); /* imm32 sign-extended to 64 */
        }
        cpu.gpr[reg_idx] = imm;
        goto dispatch_continue;
    }
    ...
    op_0F: { /* 2-byte opcodes */
        /* Use tracked prefixes (pref_66/pref_f2/pref_f3) inside handler */
