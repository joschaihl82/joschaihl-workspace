...
// Kick -> weißer Blitz mit logistischer Abnahme (20 ms) -> Hintergrund- und Graphfarbe wechseln (32 Schritte)
// Alle dynamischen Puffer einmalig beim Start alloziert; keine malloc/free in der Hauptschleife.
...
#define KICK_FLASH_MS 80            // kurzer VU-Overlay-Frame (nicht der weiße Blitz)
#define FLASH_MS 20                 // weiße Blitz-Dauer (20 ms Abklingzeit)
...
static const double growth_k = 12.0;   // Steilheit logistische Funktion für Bandhelligkeit
static const double growth_x0 = 0.18*12.0;  // Mitte für Bandhelligkeit

// --------------------------- Onset / Flash / Colors ---------------------------
static double running_avg_low = 1e-8;
static const double alpha_avg = 0.08;
static double onset_factor = 3.0; // Pfeil hoch/runter änderbar

static Uint32 last_kick_ts = 0;
static volatile int kick_flag = 0; // 0 none, 1 overlay, 2 flash active

// Weißer Blitz (logistische Abnahme über FLASH_MS)
static Uint32 flash_start_ts = 0;
static const double flash_k = 14.0;   // Steilheit der logistischen Abnahme (größer -> schärfer)
static const double flash_x0 = 0.5;   // Mitte der logistischen Kurve (normierte Zeit)
static const int TOTAL_CONTRAST_STEPS = 64;
...
// Berechnet die nächste Kontrastfarbe basierend auf dem aktuellen Schritt (32 Schritte)
static HSV get_next_contrast_color(int current_step) {
    const int total_steps = TOTAL_CONTRAST_STEPS;
    const double offset = 360.0 / total_steps; // 11.25 Grad
    HSV new_color;
...
"}\n"
"void main() {\n"
"  // White flash overlay (highest priority) using u_flash_brightness\n"
"  if (u_flash_brightness > 0.0001) {\n"
"    fragColor = vec4(u_flash_color, u_flash_brightness);\n"
"    return;\n"
"  }\n"
"  // VU color strip at top\n"
"  if (v_uv.y > 0.92) {\n"
...
"    return;\n"
"  }\n"
"  // Waveform uses u_graph_color (complement of background)\n"
...
// logarithmische Bandzuordnung (N_BANDS)
static void map_bins_to_bands(const double *mag, double *bands_out, unsigned sample_rate) {
    double f_min = 20.0;
...
int main(void) {
    ...
    SDL_Window *win = SDL_CreateWindow("BeatDetect Main", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 2 * tex_size, NULL, GL_DYNAMIC_DRAW);
    ...
    HSV initial_graph = initial_hsv; initial_graph.h = fmod(initial_graph.h + 180.0, 360.0);
    float graph_rgb[3];
    ...
    fprintf(stderr, "Start: 16-band FFT + color VU + white flash (logistic decay %d ms) enabled.\n", FLASH_MS);
    ...
    while (running) {
        ...
        if (low_sum > running_avg_low * onset_factor && low_sum > 1e-8) {
            if ((now - last_kick_ts) > KICK_DEBOUNCE_MS) {
                detected = true;
                last_kick_ts = now;
                // start white flash (logistic decay)
                flash_start_ts = now;
            }
        }
        ...
        if (detected) {
            ...
            glUseProgram(prog);
            glUniform3f(loc_u_graph_color, graph_rgb2[0], graph_rgb2[1], graph_rgb2[2]);
            glUseProgram(0);
        }
        ...
        if (flash_start_ts != 0) {
            ...
            if (t <= 1.0) {
                // logistic: b(t) = 1 / (1 + exp(k*(t - x0)))  -> starts near 1 at t=0, near 0 at t=1
        ...
        if (now - last_dbg > 1000) {
            fprintf(stderr, "rate=%u tex=%d limiter_gain=%.3f onset=%.2f kick=%d step=%d low=%.6f avg=%.6f flash=%.3f\n",
                    rate, tex_size, g_limiter_gain, onset_factor, kick_flag, contrast_step, low_sum, running_avg_low, flash_brightness);
            last_dbg = now;
        }
