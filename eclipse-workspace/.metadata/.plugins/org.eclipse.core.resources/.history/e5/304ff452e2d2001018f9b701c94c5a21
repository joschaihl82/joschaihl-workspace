// cc.c
// C99: Lexer, Parser, robuster x86-64 Codegenerator (REX/ModR/M/SIB, spills, SysV ABI)
// main: liest Quelltext von stdin, ruft lex(), parse(), codegen() auf (keine Tests, keine JIT-Ausführung).
// Compile: cc -std=c99 -Wall -Wextra cc.c -o cc
...
typedef struct sym_entry {
    ...
    sym_class_t cls;
    type_info_t *type;
    sym_value_t val; /* address or function addr */
...
/* ---------------------------
   Helpers: types and symbols
   --------------------------- */
static type_info_t *type_new(ty_kind_t k) {
    type_info_t *t = (type_info_t *)calloc(1, sizeof(type_info_t));
    if (!t) { fprintf(stderr, "out of memory\n"); exit(1); }
    t->kind = k; t->is_unsigned = false; t->size_in_bytes = 0; t->layout = NULL; t->base = NULL; t->array_len = 0; t->typedef_name = NULL;
    return t;
}
static sym_entry_t *sym_new(const char *name) {
    sym_entry_t *s = (sym_entry_t *)calloc(1, sizeof(sym_entry_t));
    if (!s) { fprintf(stderr, "out of memory\n"); exit(1); }
    s->name = name ? strdup(name) : NULL; s->cls = SC_NONE; s->type = NULL; s->layout = NULL; s->scope_level = 0; s->local_index = -1; s->val.i = 0;
    return s;
}
static void sym_table_add(compiler_context_t *c, sym_entry_t *s) {
    if (c->sym_count + 1 >= c->sym_capacity) {
        ...
        c->sym_table = (sym_entry_t *)realloc(c->sym_table, newcap * sizeof(sym_entry_t));
...
static void token_append(compiler_context_t *c, token_kind_t kind, const char *start, size_t len, int line) {
    if (c->token_count + 1 >= c->token_capacity) {
        ...
        c->tokens = (token_t *)realloc(c->tokens, newcap * sizeof(token_t));
...
void lex(compiler_context_t *c) {
    ...
    while (*p) {
        ...
        if (p[0] == '=' && p[1] == '=') { token_append(c, TK_EQ, p, 2, c->line); p += 2; continue; }
        if (p[0] == '!' && p[1] == '=') { token_append(c, TK_NE, p, 2, c->line); p += 2; continue; }
        if (p[0] == '<' && p[1] == '=') { token_append(c, TK_LE, p, 2, c->line); p += 2; continue; }
        if (p[0] == '>' && p[1] == '=') { token_append(c, TK_GE, p, 2, c->line); p += 2; continue; }
        switch (*p) {
        ...
        case '<': token_append(c, TK_LT, p, 1, c->line); ++p; break;
        case '>': token_append(c, TK_GT, p, 1, c->line); ++p; break;
        case '+': token_append(c, TK_PLUS, p, 1, c->line); ++p; break;
        case '-': token_append(c, TK_MINUS, p, 1, c->line); ++p; break;
        case '/': token_append(c, TK_DIV, p, 1, c->line); ++p; break;
        case '%': token_append(c, TK_MOD, p, 1, c->line); ++p; break;
...
/* ---------------------------
   Parser (simplified but functional)
   - supports: int, typedef names, pointers, arrays, function defs, local vars, return, expr-stmt, assignment, add/mul
   --------------------------- */
...
/* parse base type (int only for brevity) */
static type_info_t *parse_base_type(compiler_context_t *c) {
    token_t *t = peek_token(c);
    if (t->kind == TK_INT) { next_token((compiler_context_t *)c); type_info_t *ti = type_new(TY_INT); ti->size_in_bytes = 4; return ti; }
    if (t->kind == TK_CHAR) { next_token((compiler_context_t *)c); type_info_t *ti = type_new(TY_CHAR); ti->size_in_bytes = 1; return ti; }
    /* typedef names and struct/union omitted for brevity; default to int */
    type_info_t *ti = type_new(TY_INT); ti->size_in_bytes = 4; return ti;
}

/* parse declarator name with pointer/array/func suffixes */
...
static ast_node_t *parse_primary(compiler_context_t *c) {
    ...
    if (t->kind == TK_IDENT) {
        ...
        if (accept_tok(c, TK_LPAREN)) {
            ...
            ast_node_t *call = (ast_node_t *)calloc(1, sizeof(ast_node_t));
...
static ast_node_t *parse_muldiv(compiler_context_t *c) {
    ...
    while (1) {
        token_t *t = peek_token(c);
        if (t->kind == TK_STAR) { next_token((compiler_context_t *)c); ast_node_t *r = parse_unary(c); ast_node_t *n = (ast_node_t *)calloc(1, sizeof(ast_node_t)); n->type = AST_MUL; n->op_a = node; n->op_b = r; node = n; }
        else if (t->kind == TK_DIV) { next_token((compiler_context_t *)c); ast_node_t *r = parse_unary(c); ast_node_t *n = (ast_node_t *)calloc(1, sizeof(ast_node_t)); n->type = AST_DIV; n->op_a = node; n->op_b = r; node = n; }
        else break;
    }
    return node;
}
...
static ast_node_t *parse_addsub(compiler_context_t *c) {
    ...
    while (1) {
        token_t *t = peek_token(c);
        if (t->kind == TK_PLUS) { next_token((compiler_context_t *)c); ast_node_t *r = parse_muldiv(c); ast_node_t *n = (ast_node_t *)calloc(1, sizeof(ast_node_t)); n->type = AST_ADD; n->op_a = node; n->op_b = r; node = n; }
        else if (t->kind == TK_MINUS) { next_token((compiler_context_t *)c); ast_node_t *r = parse_muldiv(c); ast_node_t *n = (ast_node_t *)calloc(1, sizeof(ast_node_t)); n->type = AST_SUB; n->op_a = node; n->op_b = r; node = n; }
        else break;
    }
    return node;
}

static ast_node_t *parse_cmp(compiler_context_t *c) {
    ast_node_t *node = parse_addsub(c);
    token_t *t = peek_token(c);
    if (t->kind == TK_EQ || t->kind == TK_NE || t->kind == TK_LT || t->kind == TK_LE || t->kind == TK_GT || t->kind == TK_GE) {
        token_kind_t k = t->kind; next_token((compiler_context_t *)c);
...
static ast_node_t *parse_statement(compiler_context_t *c) {
    ...
    if (t->kind == TK_RETURN) {
        ...
        ast_node_t *ret = (ast_node_t *)calloc(1, sizeof(ast_node_t));
    ...
    ast_node_t *e = parse_expression(c);
    expect_tok(c, TK_SEMI, ";");
    ast_node_t *stmt = (ast_node_t *)calloc(1, sizeof(ast_node_t));
...
static ast_node_t *parse_block(compiler_context_t *c) {
    ...
    c->current_scope++;
    ast_node_t *block = (ast_node_t *)calloc(1, sizeof(ast_node_t));
    block->type = AST_BLOCK;
    ast_node_t **last = &block->op_a;
    while (!accept_tok(c, TK_RBRACE)) {
        token_t *t = peek_token(c);
        if (t->kind == TK_INT) {
            type_info_t *ty = parse_base_type(c);
            char *name = parse_declarator_name(c, NULL);
            if (!name) error_at_ctx(c, t->pos ? t->pos : c->src_buf, "expected local name");
            expect_tok(c, TK_SEMI, ";");
            sym_entry_t *s = sym_new(name);
            s->cls = SC_LOC; s->type = ty;
            s->local_index = 0; /* assigned later by function parser */
            sym_table_add(c, s);
            ast_node_t *decl = (ast_node_t *)calloc(1, sizeof(ast_node_t));
...
static ast_node_t *parse_param_list(compiler_context_t *c) {
    ...
    do {
        ...
        sym_table_add(c, s);
        ast_node_t *p = (ast_node_t *)calloc(1, sizeof(ast_node_t));
...
static ast_node_t *parse_function(compiler_context_t *c) {
    ...
    sym_table_add(c, s);
    ast_node_t *fnode = (ast_node_t *)calloc(1, sizeof(ast_node_t));
...
void parse(compiler_context_t *c) {
    c->token_idx = 0;
    c->current_scope = 0;
    ast_node_t *program = (ast_node_t *)calloc(1, sizeof(ast_node_t));
            ...
            if (after->kind == TK_LPAREN) {
                ...
                *last = f; last = &f->next;
                ...
                do {
                    ...
                    sym_table_add(c, s);
                    ast_node_t *g = (ast_node_t *)calloc(1, sizeof(ast_node_t));
...
/* ---------------------------
   Robust Codegen (REX/ModR/M/SIB, spills, SysV ABI)
   - Emits code into c->x64_text; does not execute it.
   --------------------------- */

/* Emitter helpers */
typedef struct { uint8_t *ptr; } x64_emitter_t;
static inline void em_bytes2(x64_emitter_t *em, const uint8_t *b, size_t n) { memcpy(em->ptr, b, n); em->ptr += n; }
static inline void em_byte2(x64_emitter_t *em, uint8_t v) { *em->ptr++ = v; }
static inline void em_int322(x64_emitter_t *em, int32_t v) { *(int32_t *)em->ptr = v; em->ptr += 4; }
static inline void em_ptr642(x64_emitter_t *em, int64_t v) { *(int64_t *)em->ptr = v; em->ptr += 8; }

static void emit_rex2(x64_emitter_t *em, int w, int r, int x, int b) {
    uint8_t rex = 0x40;
    if (w) rex |= 0x08;
    if (r) rex |= 0x04;
    if (x) rex |= 0x02;
...
static void emit_modrm2(x64_emitter_t *em, uint8_t mod, uint8_t reg, uint8_t rm) {
    uint8_t modrm = (uint8_t)((mod << 6) | ((reg & 7) << 3) | (rm & 7));
    em_byte2(em, modrm);
}
static void emit_sib2(x64_emitter_t *em, uint8_t scale, uint8_t index, uint8_t base) {
    uint8_t sib = (uint8_t)((scale << 6) | ((index & 7) << 3) | (base & 7));
    em_byte2(em, sib);
}

/* Encoders */
static void emit_mov_imm64_to_reg2(x64_emitter_t *em, reg_code_t rd, int64_t imm) {
    int rex_b = (rd & 8) ? 1 : 0;
    emit_rex2(em, 1, 0, 0, rex_b);
    em_byte2(em, 0xB8 + (rd & 7));
    em_ptr642(em, imm);
}
static void emit_mov_reg_reg2(x64_emitter_t *em, reg_code_t src, reg_code_t dst) {
    int rex_r = (src & 8) ? 1 : 0;
    int rex_b = (dst & 8) ? 1 : 0;
    emit_rex2(em, 1, rex_r, 0, rex_b);
    em_byte2(em, 0x89);
    emit_modrm2(em, 3, src & 7, dst & 7);
}
static void emit_mov_reg_to_mem_rbp2(x64_emitter_t *em, reg_code_t src, int32_t disp32) {
    int rex_r = (src & 8) ? 1 : 0;
    emit_rex2(em, 1, rex_r, 0, 1);
    em_byte2(em, 0x89);
    emit_modrm2(em, 2, src & 7, 5);
    em_int322(em, -disp32);
}
static void emit_mov_mem_rbp_to_reg2(x64_emitter_t *em, reg_code_t dst, int32_t disp32) {
    int rex_r = (dst & 8) ? 1 : 0;
    emit_rex2(em, 1, rex_r, 0, 1);
    em_byte2(em, 0x8B);
    emit_modrm2(em, 2, dst & 7, 5);
    em_int322(em, -disp32);
}
static void emit_mov_absaddr_to_reg2(x64_emitter_t *em, reg_code_t dst, int64_t addr) {
    emit_mov_imm64_to_reg2(em, dst, addr);
    int rex_b = (dst & 8) ? 1 : 0;
    emit_rex2(em, 1, 0, 0, rex_b);
    em_byte2(em, 0x8B);
    emit_modrm2(em, 0, dst & 7, dst & 7);
}
static void emit_mov_reg_to_absaddr2(x64_emitter_t *em, reg_code_t src, int64_t addr) {
    emit_mov_imm64_to_reg2(em, REG_RDX, addr);
    int rex_r = (src & 8) ? 1 : 0;
    int rex_b = (REG_RDX & 8) ? 1 : 0;
    emit_rex2(em, 1, rex_r, 0, rex_b);
    em_byte2(em, 0x89);
    emit_modrm2(em, 0, src & 7, REG_RDX & 7);
}
static void emit_lea_reg_mem_rbp2(x64_emitter_t *em, reg_code_t dst, int32_t disp32) {
    int rex_r = (dst & 8) ? 1 : 0;
    emit_rex2(em, 1, rex_r, 0, 1);
    em_byte2(em, 0x8D);
    emit_modrm2(em, 2, dst & 7, 5);
...
static void emit_cmp_reg_reg2(x64_emitter_t *em, reg_code_t a, reg_code_t b) {
    int rex_r = (b & 8) ? 1 : 0;
    int rex_b = (a & 8) ? 1 : 0;
    emit_rex2(em, 1, rex_r, 0, rex_b);
    em_byte2(em, 0x39);
    emit_modrm2(em, 3, b & 7, a & 7);
}
static void emit_setcc_and_movzx_rax2(x64_emitter_t *em, uint8_t setcc_low) {
    em_byte2(em, 0x0F); em_byte2(em, 0x90 | setcc_low);
    em_bytes2(em, (const uint8_t *)"\x48\x0F\xB6\xC0", 4);
}
static void emit_call_abs2(x64_emitter_t *em, int64_t addr) {
    emit_mov_imm64_to_reg2(em, REG_RAX, addr);
    em_bytes2(em, (const uint8_t *)"\xFF\xD0", 2);
}
static void emit_ret2(x64_emitter_t *em) { em_byte2(em, 0xC3); }

/* ---------------------------
   Simple register allocator with spills
   --------------------------- */
typedef struct {
    reg_code_t pool[9];
    int used[9];
...
static void ra_init(reg_alloc_t *ra, int spill_base) {
    reg_code_t p[] = { REG_RAX, REG_RCX, REG_RDX, REG_RSI, REG_RDI, REG_R8, REG_R9, REG_R10, REG_R11 };
    memcpy(ra->pool, p, sizeof(p));
    memset(ra->used, 0, sizeof(ra->used));
    ra->spill_base = spill_base;
    ra->next_spill = 0;
}
static int ra_alloc(reg_alloc_t *ra) {
    for (int i = 0; i < 9; ++i) if (!ra->used[i]) { ra->used[i] = 1; return i; }
    return -1;
}
static void ra_free(reg_alloc_t *ra, int idx) { if (idx >= 0 && idx < 9) ra->used[idx] = 0; }
static int ra_spill_slot(reg_alloc_t *ra) { return ra->spill_base + (ra->next_spill++) * 8; }
static reg_code_t ra_regcode(reg_alloc_t *ra, int idx) { return ra->pool[idx]; }

/* ---------------------------
   Codegen: AST -> machine (subset)
   - Produces code into c->x64_text; does not execute.
   --------------------------- */
static void codegen_expr_node(compiler_context_t *c, x64_emitter_t *em, reg_alloc_t *ra, ast_node_t *node);
static void codegen_stmt_node(compiler_context_t *c, x64_emitter_t *em, reg_alloc_t *ra, ast_node_t *node, int func_scope_level);

static void codegen_expr_node(compiler_context_t *c, x64_emitter_t *em, reg_alloc_t *ra, ast_node_t *node) {
    if (!node) return;
    switch (node->type) {
    case AST_IMM:
        emit_mov_imm64_to_reg2(em, REG_RAX, node->value);
        break;
    case AST_ID:
        if (!node->sym) error_at_ctx(c, node->pos ? node->pos : c->src_buf, "unknown identifier");
        if (node->sym->cls == SC_GLO) emit_mov_absaddr_to_reg2(em, REG_RAX, node->sym->val.i);
        else if (node->sym->cls == SC_LOC) {
            int32_t off = 8 + node->sym->local_index * 8;
            emit_mov_mem_rbp_to_reg2(em, REG_RAX, off);
        } else error_at_ctx(c, node->pos ? node->pos : c->src_buf, "unsupported id class");
        break;
    case AST_ASSIGN: {
        ast_node_t *lhs = node->op_a;
        ast_node_t *rhs = node->op_b;
        codegen_expr_node(c, em, ra, rhs); /* rax = rhs */
        if (lhs->type == AST_ID && lhs->sym) {
            if (lhs->sym->cls == SC_LOC) {
                int32_t off = 8 + lhs->sym->local_index * 8;
                emit_mov_reg_to_mem_rbp2(em, REG_RAX, off);
            } else if (lhs->sym->cls == SC_GLO) {
                emit_mov_reg_to_absaddr2(em, REG_RAX, lhs->sym->val.i);
    ...
    case AST_ADD: {
        codegen_expr_node(c, em, ra, node->op_a); /* rax = left */
        int spill = ra_spill_slot(ra);
        emit_mov_reg_to_mem_rbp2(em, REG_RAX, spill);
        codegen_expr_node(c, em, ra, node->op_b); /* rax = right */
        emit_mov_mem_rbp_to_reg2(em, REG_RDX, spill); /* rdx = left */
        em_bytes2(em, (const uint8_t *)"\x48\x01\xD0", 3); /* add rax, rdx */
        break;
    }
    case AST_MUL: {
        codegen_expr_node(c, em, ra, node->op_a);
        int spill = ra_spill_slot(ra);
        emit_mov_reg_to_mem_rbp2(em, REG_RAX, spill);
        codegen_expr_node(c, em, ra, node->op_b);
        emit_mov_mem_rbp_to_reg2(em, REG_RDX, spill);
        em_bytes2(em, (const uint8_t *)"\x48\x0F\xAF\xC2", 4); /* imul rax, rdx */
        break;
    }
    case AST_EQ: {
        codegen_expr_node(c, em, ra, node->op_a);
        emit_mov_reg_to_mem_rbp2(em, REG_RAX, 8);
        codegen_expr_node(c, em, ra, node->op_b);
        emit_mov_mem_rbp_to_reg2(em, REG_RDX, 8);
        emit_cmp_reg_reg2(em, REG_RDX, REG_RAX);
        emit_setcc_and_movzx_rax2(em, 0x4); /* sete -> low nibble 0x4 */
        break;
    }
    case AST_LT: {
        codegen_expr_node(c, em, ra, node->op_a);
        ...
        emit_mov_mem_rbp_to_reg2(em, REG_RDX, 8);
        emit_cmp_reg_reg2(em, REG_RDX, REG_RAX);
        emit_setcc_and_movzx_rax2(em, 0xC); /* setl -> 0xC */
        break;
    }
    case AST_FUN_CALL: {
        /* Evaluate up to 6 args into registers rdi,rsi,rdx,rcx,r8,r9 */
        reg_code_t arg_regs[6] = { REG_RDI, REG_RSI, REG_RDX, REG_RCX, REG_R8, REG_R9 };
        int idx = 0;
        ast_node_t *a = node->op_b;
        while (a && idx < 6) {
            codegen_expr_node(c, em, ra, a); /* rax = arg */
            emit_mov_reg_reg2(em, REG_RAX, arg_regs[idx]);
            a = a->next; idx++;
        }
        /* call direct if symbol address known */
        if (node->op_a && node->op_a->sym && node->op_a->sym->val.i) {
            emit_call_abs2(em, node->op_a->sym->val.i);
        } else {
            /* indirect call: evaluate callee into rax then call rax */
            codegen_expr_node(c, em, ra, node->op_a);
            em_bytes2(em, (const uint8_t *)"\xFF\xD0", 2); /* call rax */
        }
        break;
    }
    case AST_MEMBER: {
        ...
        emit_mov_absaddr_to_reg2(em, REG_RAX, base->val.i);
        if (offset) { em_bytes2(em, (const uint8_t *)"\x48\x05", 2); em_int322(em, (int32_t)offset); }
        em_bytes2(em, (const uint8_t *)"\x48\x8B\x00", 3); /* mov rax, [rax] */
        break;
    }
    default:
        em_byte2(em, 0xCC); /* int3 for unsupported nodes */
        break;
    }
}
...
static void codegen_stmt_node(compiler_context_t *c, x64_emitter_t *em, reg_alloc_t *ra, ast_node_t *node, int func_scope_level) {
    ...
    case AST_BLOCK: {
        ...
        while (it) { codegen_stmt_node(c, em, ra, it, func_scope_level); it = it->next; }
        break;
    }
    case AST_EXPR_STMT:
        codegen_expr_node(c, em, ra, node->op_a);
        break;
    case AST_RETURN:
        if (node->op_a) codegen_expr_node(c, em, ra, node->op_a);
        em_bytes2(em, (const uint8_t *)"\x48\x89\xEC\x5D\xC3", 5); /* mov rsp, rbp; pop rbp; ret */
        break;
    case AST_IF: {
        /* cond in op_a, then op_b, else op_c */
        codegen_expr_node(c, em, ra, node->op_a); /* rax = cond (0/1) */
        em_bytes2(em, (const uint8_t *)"\x48\x85\xC0", 3); /* test rax, rax */
        em_byte2(em, 0x0F); em_byte2(em, 0x84); uint8_t *patch_pos = em->ptr; em_int322(em, 0);
        codegen_stmt_node(c, em, ra, node->op_b, func_scope_level);
        if (node->op_c) {
            em_byte2(em, 0x0F); em_byte2(em, 0x85); uint8_t *patch2 = em->ptr; em_int322(em, 0);
            int32_t rel = (int32_t)(em->ptr - (patch_pos + 4));
            *(int32_t *)patch_pos = rel;
            codegen_stmt_node(c, em, ra, node->op_c, func_scope_level);
    ...
    case AST_WHILE: {
        ...
        codegen_expr_node(c, em, ra, node->op_a);
        em_bytes2(em, (const uint8_t *)"\x48\x85\xC0", 3);
        em_byte2(em, 0x0F); em_byte2(em, 0x84); uint8_t *patch_jz = em->ptr; em_int322(em, 0);
        codegen_stmt_node(c, em, ra, node->op_b, func_scope_level);
        em_byte2(em, 0xE9); int32_t rel_back = (int32_t)(loop_start - (em->ptr + 4)); em_int322(em, rel_back);
...
/* codegen: walk AST and emit functions (addresses stored in sym->val.i) */
int codegen(compiler_context_t *c) {
    ...
    while (it) {
        if (it->type == AST_FUN_DEF && it->sym) {
            ...
            int max_local = -1;
            for (size_t i = 0; i < c->sym_count; ++i) {
                if (c->sym_table[i].cls == SC_LOC && c->sym_table[i].scope_level >= it->sym->scope_level) {
                    if (c->sym_table[i].local_index > max_local) max_local = c->sym_table[i].local_index;
                }
            }
            int local_count = max_local + 1;
            int stack_size = local_count * 8;
            if (stack_size % 16) stack_size = ((stack_size + 15) / 16) * 16;
            em_bytes2(&em, (const uint8_t *)"\x55\x48\x89\xE5", 4); /* push rbp; mov rbp, rsp */
            if (stack_size > 0) { em_bytes2(&em, (const uint8_t *)"\x48\x81\xEC", 3); em_int322(&em, stack_size); }
            reg_alloc_t ra; ra_init(&ra, 8 + local_count * 8);
            /* spill args into locals (first 6) */
            reg_code_t arg_regs[6] = { REG_RDI, REG_RSI, REG_RDX, REG_RCX, REG_R8, REG_R9 };
            for (size_t i = 0; i < c->sym_count; ++i) {
                ...
                if (s->cls == SC_LOC && s->scope_level >= it->sym->scope_level && s->local_index >= 0 && s->local_index < 6) {
                    ...
                    emit_mov_reg_to_mem_rbp2(&em, arg_regs[s->local_index], off);
                }
            }
            codegen_stmt_node(c, &em, &ra, it->op_b, it->sym->scope_level);
            em_bytes2(&em, (const uint8_t *)"\x48\x89\xEC\x5D\xC3", 5); /* default epilog */
        }
        it = it->next;
    }
    c->x64_e = em.ptr;
...
/* ---------------------------
   main: read stdin, lex, parse, codegen (no tests)
   --------------------------- */
int main(int argc, char **argv) {
    compiler_context_t ctx;
    compiler_init(&ctx);

    /* read entire stdin into src_buf */
    size_t cap = 4096, len = 0;
    ctx.src_buf = (char *)malloc(cap);
    if (!ctx.src_buf) { fprintf(stderr, "out of memory\n"); return 1; }
    while (1) {
        size_t r = fread(ctx.src_buf + len, 1, cap - len - 1, stdin);
        if (r == 0) break;
        len += r;
        if (len + 256 >= cap) {
            cap *= 2;
            ctx.src_buf = (char *)realloc(ctx.src_buf, cap);
            if (!ctx.src_buf) { fprintf(stderr, "out of memory\n"); return 1; }
        }
    }
    ctx.src_buf[len] = '\0';
    ...
    /* cleanup (tokens and symbols) */
    for (int i = 0; i < ctx.token_count; ++i) free(ctx.tokens[i].text);
    free(ctx.tokens);
    for (size_t i = 0; i < ctx.sym_count; ++i) {
        if (ctx.sym_table[i].layout) {
            for (size_t f = 0; f < ctx.sym_table[i].layout->field_count; ++f) free(ctx.sym_table[i].layout->fields[f].name);
...
#ifndef _WIN32
    if (ctx.x64_text) munmap(ctx.x64_text, ctx.poolsz);
#else
    if (ctx.x64_text) free(ctx.x64_text);
#endif
