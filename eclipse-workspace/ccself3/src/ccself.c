// cc.c - small C compiler (single-file, C-only, with #include support for "..." and <...>)
...
void print_error_line(char* loc, char* fmt, va_list ap) {
    /* Use the combined preprocessed text (user_input) for mapping line numbers.
       This ensures line numbers in error messages correspond to the preprocessed
       (amalgamated) source that the compiler actually tokenized. */
...
static char* try_candidate(const char* prefix, const char* path) {
    ...
    /* allocate enough for prefix + '/' + path + '\0' */
...
char* parse_directive(char* p, char** output_ptr, char** q_ptr, size_t *output_cap_ptr, const char* current_dir) {
    ...
    if (name_len == 6 && strncmp(name_start, "define", 6) == 0) {
        ...
        *q_ptr = q;
        ...
        /* Candidate list: for quoted includes prefer current_dir, then as-given, then ./, then system dirs.
           For angle includes try as-given, then system dirs. */
        ...
        /* Determine directory of included file to pass as current_dir for nested includes */
        char* included_dir = dir_of_path(path);

        /* Preprocess included file recursively, passing included_dir as current_dir */
        char* included_preprocessed = preprocess(included_raw, included_dir ? included_dir : ".");
        free(included_raw);
        free(included_dir);

        /* Append included_preprocessed into output buffer */
...
void gen_function(Function* f) {
    ...
    for (int i = 0; i < f->node->param_len && i < 6; i++) {
        ...
        // store register into local slot
        printf("  mov [rbp-%d], %s\n", p->offset, arg_regs[i]);
    }
