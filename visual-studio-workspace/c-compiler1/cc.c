...
int vfprintf(void *stream, const char *format, ...);
...
void gen(Node *node)
{
  ...
  case ND_ASSIGN:
...

    {
      ...
      // Aggregate assignment: RHS was pushed in chunks (each pushq), dest address is below them.
      int tmp = total;
      int chunks[32];
      int nc = 0;
      while (tmp > 0)
      ...
      // Load destination address from stack without popping: it's at rsp + 8*nc
      ...
      // remove destination address from stack
      printf("  popq %%rax\n");

      // push a placeholder result (0)
      printf("  pushq $0\n");
      return;
    }
