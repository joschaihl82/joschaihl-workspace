...
// Uses libgit2 only (no CLI fallback). If push fails, retries with alternative refspec and explicit git_remote_connect + retry.
...
// ---------- HTTP helpers (synchronous wrappers with debug) ----------
...
public slots:
    void run() {
        ...
        if (git_index_entrycount(idx2) > 0) {
            ...
            if (error) {
                git_index_free(idx2);
                emit message(QStringLiteral("[%1] ERROR write_tree: %2").arg(timestamp()).arg(QString::number(error)));
                git_repository_free(repo);
                git_libgit2_shutdown();
                emit finished(7);
                return;
            ...
            if (error) {
                git_index_free(idx2);
                emit message(QStringLiteral("[%1] ERROR tree_lookup: %2").arg(timestamp()).arg(QString::number(error)));
                git_repository_free(repo);
                git_libgit2_shutdown();
                emit finished(7);
                return;
            ...
            if (error) {
                git_tree_free(tree);
                git_index_free(idx2);
                emit message(QStringLiteral("[%1] ERROR signature: %2").arg(timestamp()).arg(QString::number(error)));
                git_repository_free(repo);
                git_libgit2_shutdown();
                emit finished(7);
                return;
            ...
            const git_commit *parents_arr[1];
            int parent_count = 0;
            if (parent) { parents_arr[0] = parent; parent_count = 1; }

            // Try to create commit normally (update HEAD). If that fails, try fallback (create commit without updating refs)
            error = git_commit_create_v(&commit_oid, repo, "HEAD", sig, sig, nullptr, "first commit", tree, parent_count, parent_count ? parents_arr : nullptr);
            if (error) {
                const git_error *ge = git_error_last();
                QString gm = ge ? QString::fromUtf8(ge->message) : QStringLiteral("unknown");
                emit message(QStringLiteral("[%1] ERROR creating commit (primary): %2 (code %3)").arg(timestamp()).arg(gm).arg(QString::number(error)));

                // Fallback: create commit object without updating HEAD (update_ref = NULL), then create/move branch refs/heads/main to it.
                int error2 = git_commit_create_v(&commit_oid, repo, nullptr, sig, sig, nullptr, "first commit", tree, parent_count, parent_count ? parents_arr : nullptr);
                if (error2) {
                    const git_error *ge2 = git_error_last();
                    QString gm2 = ge2 ? QString::fromUtf8(ge2->message) : QStringLiteral("unknown");
                    emit message(QStringLiteral("[%1] ERROR creating commit (fallback): %2 (code %3)").arg(timestamp()).arg(gm2).arg(QString::number(error2)));
                    ...
                    // commit created (fallback). Now create/move refs/heads/main to this commit_oid
                    git_reference *ref = nullptr;
                    int rc = git_reference_create(&ref, repo, "refs/heads/main", &commit_oid, 1, "create main branch (fallback)");
                    if (rc == GIT_EEXISTS) {
                        git_reference *old = nullptr;
                        if (git_reference_lookup(&old, repo, "refs/heads/main") == 0) {
                            git_reference *new_ref = nullptr;
                            int rc2 = git_reference_set_target(&new_ref, old, &commit_oid, "move main to new commit (fallback)");
                            git_reference_free(old);
                            if (rc2 != 0) {
                                const git_error *ge3 = git_error_last();
                                QString gm3 = ge3 ? QString::fromUtf8(ge3->message) : QStringLiteral("unknown");
                                emit message(QStringLiteral("[%1] ERROR moving refs/heads/main (fallback): %2").arg(timestamp()).arg(gm3));
                            ...
                            emit message(QStringLiteral("[%1] WARNING: could not lookup existing refs/heads/main to move (fallback)").arg(timestamp()));
                        }
                    } else if (rc != 0) {
                        const git_error *ge3 = git_error_last();
                        QString gm3 = ge3 ? QString::fromUtf8(ge3->message) : QStringLiteral("unknown");
                        emit message(QStringLiteral("[%1] ERROR creating refs/heads/main (fallback): %2").arg(timestamp()).arg(gm3));
                    ...
                    int setHeadErr = git_repository_set_head(repo, "refs/heads/main");
                    if (setHeadErr != 0) {
                        emit message(QStringLiteral("[%1] WARNING: could not set HEAD to refs/heads/main after fallback commit (code %2)").arg(timestamp()).arg(QString::number(setHeadErr)));
                    } else {
                        emit message(QStringLiteral("[%1] Branch 'main' created (fallback) and HEAD set.").arg(timestamp()));
                    }
                    createdCommit = true;
                }
            } else {
                // primary commit succeeded
        ...
        // 4b) ensure branch main exists and HEAD points to it (if commit was created above we already tried to create/move main)
        if (!createdCommit) {
            // try to create main from existing HEAD if possible
        ...
        callbacks.credentials = [](git_cred **out, const char *url, const char *username_from_url, unsigned int allowed_types, void *payload)->int {
            CombinedPayloadLocal *cp = static_cast<CombinedPayloadLocal*>(payload);
        ...
        if (error) {
            ...
            emit message(QStringLiteral("[%1] ERROR push failed: %2 (code %3)").arg(timestamp()).arg(errMsg).arg(QString::number(error)));

            // Attempt fallback refspec HEAD:refs/heads/main
            ...
            int err2 = git_remote_push(remote, &refspecs_head, &push_opts);
            if (err2 == 0) {
                emit message(QStringLiteral("[%1] Push fallback HEAD:main succeeded.").arg(timestamp()));
            } else {
                const git_error *e2 = git_error_last();
                QString errMsg2 = e2 ? QString::fromUtf8(e2->message) : QStringLiteral("Unknown libgit2 error");
                emit message(QStringLiteral("[%1] Push fallback also failed: %2 (code %3)").arg(timestamp()).arg(errMsg2).arg(QString::number(err2)));

                // explicit connect + retry
                int rc_connect = git_remote_connect(remote, GIT_DIRECTION_PUSH, &callbacks, nullptr, nullptr);
                if (rc_connect != 0) {
                    const git_error *ec = git_error_last();
                    QString em = ec ? QString::fromUtf8(ec->message) : QStringLiteral("unknown");
                    emit message(QStringLiteral("[%1] ERROR: git_remote_connect failed: %2 (code %3)").arg(timestamp()).arg(em).arg(QString::number(rc_connect)));
                    git_remote_free(remote);
                    git_repository_free(repo);
                    git_libgit2_shutdown();
                    emit finished(12);
                ...
                // Retry push after explicit connect
                int rc_retry = git_remote_push(remote, &refspecs_main, &push_opts);
                if (rc_retry != 0) {
                    const git_error *er = git_error_last();
                    QString em = er ? QString::fromUtf8(er->message) : QStringLiteral("Unknown libgit2 error");
                    emit message(QStringLiteral("[%1] ERROR: push after git_remote_connect failed: %2 (code %3)").arg(timestamp()).arg(em).arg(QString::number(rc_retry)));
                    git_remote_free(remote);
                    git_repository_free(repo);
                    git_libgit2_shutdown();
                    emit finished(12);
                    ...
                    emit message(QStringLiteral("[%1] Push succeeded after explicit git_remote_connect.").arg(timestamp()));
                }
            }
        }
...
public slots:
    void onLibgit2Progress(qint64 received, qint64 total) {
        ...
        // log progress (but speed shown in UI label)
        emit message(QStringLiteral("[%1] Progress: %2/%3 — %.2f B/s").arg(timestamp()).arg(QString::number(received)).arg(QString::number(total)).arg(bytesPerSec));
    }
...
public:
          ...
          m_latestTransferred(0), m_latestTotal(0), m_prevTransferred(0)
    {
        ...
        // top: status label + speed label in a horizontal layout
        QHBoxLayout *topLayout = new QHBoxLayout;
        m_label = new QLabel("Bereit", this);
        m_label->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
        m_speedLabel = new QLabel("Upload: 0 B/s", this);
...
private slots:
    void updateSpeed() {
        ...
        // Show speed in the dedicated label (do not log into text box)
        m_speedLabel->setText(QStringLiteral("Upload: %1").arg(speedStr));
    }
...
private:
    ...
    QLabel *m_label;
    QLabel *m_speedLabel; // shows per-second upload speed
...
// ---------- Helper: authenticate with GitHub (GET /user) ----------
