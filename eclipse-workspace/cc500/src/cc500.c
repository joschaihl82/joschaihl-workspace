/*
 * cc1000.c x86_64 vollständige portierung (mit bugfixes)
 *
 * hinweis
 * ------
 * diese datei emittiert rohe x86_64‑maschinenbytes in eine elf64‑datei.
 * sie ist eine verbesserte, fehlerbereinigte version des ursprünglichen cc500.c.
 *
 * achtung: der compiler verwendet weiterhin eine nicht standardmäßige
 * x86_64 aufrufkonvention (stack-argumente), die mit den syscalls in
 * den stubs kollidieren kann und ein tiefgreifender architekturfehler
 * ist, der hier nicht behoben wurde, da er eine komplette umschreibung
 * des parsers und backends erfordern würde.
 */

/* unsere library functions. */
void exit(int);
int getchar(void);
void *malloc(int);
void free(void *);
...
/* hilfsfunktion zum speichern von 4 bytes */
void save_int(char *p, long n)
{
...
char *my_realloc(char *old, int oldlen, int newlen)
{
  ...
  int i = 0;
  // kopiere nur die kleinere der beiden größen, um keinen ungültigen speicher zu lesen
  int len_to_copy = oldlen < newlen ? oldlen : newlen;
  while (i <= len_to_copy - 1) {
    new[i] = old[i];
    i = i + 1;
  }
  // fehlerbehebung: alter speicher muss freigegeben werden, um speicherleck zu vermeiden
  if (old != 0)
    free(old);
  return new;
}
...
void takechar()
{
  if (token_size <= i + 1) {
    int x = (i + 10) << 1;
    // fehlerbehebung: stelle sicher, dass der null-pointer für malloc/free initialisiert ist
    if (token_size == 0)
      token = 0;
...
void get_token()
{
  ...
  while (w) {
    w = 0;
    // fehlerbehebung: verwende || anstelle von | für logische bedingungen
    while ((nextc == ' ') || (nextc == 9) || (nextc == 10))
      nextc = getchar();
    i = 0;
    // fehlerbehebung: verwende && und || anstelle von & und |
...
{
  int i = 0;
  // fehlerbehebung: verwende && anstelle von &
...
void be_pop(int n)
{
  ...
  // fehlerbehebung: 7 bytes (48 81 c4 + 4 bytes imm32)
  emit(7, "\x48\x81\xc4....");
  // fehlerbehebung: save_int verwendet, um nur 4 bytes zu schreiben, und offset korrigiert
  save_int(code + codepos - 4, (long)(n << 3));
}
...
void sym_get_value(char *s)
{
  ...
  emit(10, "\x48\xb8\x00\x00\x00\x00\x00\x00\x00\x00"); // 10 bytes
  // fehlerbehebung: offset von save_long korrigiert
  ...
  if (table[t + 1] == 'd') {
    ...
    emit(8, "\x48\x8d\x84\x24\x00\x00\x00\x00");
    // fehlerbehebung: save_int verwendet
...
void be_start()
{
  ...
  save_int(code + codepos - 4, 0); // fehlerbehebung: save_int verwendet
  ...
  emit(6, "\x0f\x85....");
  save_int(code + codepos - 4, 14); // fehlerbehebung: save_int verwendet
  ...
  // achtung: der aufrufer muss die größe in rdi laden, was er nicht tut.
  emit(5, "\xbf\x00\x00\x00\x00");
  ...
  /* free stub (hinzugefügt, um speicherlecks zu beheben) */
  sym_define_global(sym_declare_global("free"));
  /* mov eax, 11 (munmap) ; syscall ; ret (achtung: sollte rdi verwenden) */
  emit(5, "\xb8\x0b\x00\x00\x00");
  emit(2, "\x0f\x05");
  emit(1, "\xc3");
...
void be_finish()
{
  ...
  /* patch program header p_filesz and p_memsz placeholders.
      ...
      the p_filesz placeholder starts at offset 64 + 16 (approx).
  */
...
{
  int type;
  if (('0' <= token[0]) && (token[0] <= '9')) { // fehlerbehebung: &&
    // fehlerbehebung: long n verwenden, um 64-bit-überlauf zu vermeiden
    long n = 0;
    i = 0;
    while (token[i]) {
      n = (n << 1) + (n << 3) + token[i] - '0';
    ...
    /* movabs rax, imm64 */
    emit(10, "\x48\xb8\x00\x00\x00\x00\x00\x00\x00\x00");
    save_long(code + codepos - 8, n);
    type = 3;
  }
  else if (('a' <= token[0]) && (token[0] <= 'z')) { // fehlerbehebung: &&
    sym_get_value(token);
    type = 2;
  }
  else if (accept("(")) {
  ...
  // fehlerbehebung: && anstelle von &
    ...
    /* emit call rel32 */
    emit(1, "\xe8");
    /* fehlerbehebung: save_int verwendet, um nur 4 bytes für rel32 zu speichern */
    save_int(code + codepos, i + 1);
    codepos = codepos + 4; // 4 bytes übersprungen
    emit(i + 1, token);
    emit(1, "\x58"); /* pop rax */
    type = 3;
  }
...
int postfix_expr()
{
  ...
  if (accept("[")) {
    ...
    emit(8, "\x48\x8b\x84\x24\x00\x00\x00\x00");
    // fehlerbehebung: save_int verwendet
    save_int(code + codepos - 4, (long)((stack_pos - s - 1) << 3));
    /* call *%rax -> ff d0 */
    emit(2, "\xff\xd0");
    be_pop(stack_pos - s);
    stack_pos = s;
...
int relational_expr()
{
  ...
  while (accept("<=")) {
    ...
    emit(11, "\x5b\x48\x39\xd8\x0f\x9e\xc0\x0f\xb6\xc0\x48");
...
int equality_expr()
{
  ...
  while (1) {
    if (accept("==")) {
      binary1(type);
      emit(11, "\x5b\x48\x39\xd8\x0f\x94\xc0\x0f\xb6\xc0\x48");
      type = binary2(relational_expr(), 0, "");
    }
    else if (accept("!=")) {
      binary1(type);
      emit(11, "\x5b\x48\x39\xd8\x0f\x95\xc0\x0f\xb6\xc0\x48");
      type = binary2(relational_expr(), 0, "");
    }
    else
      return type;
...
int bitwise_and_expr()
{
  ...
  // fehlerbehebung: verwende && für die bedingung
  while (accept("&")) {
    binary1(type);
...
int main1()
{
  ...
  // speicher für token initialisieren
  token_size = 128;
  token = malloc(token_size);
  if (token == 0)
    exit(1);

  // speicher für table initialisieren
  table_size = 512;
