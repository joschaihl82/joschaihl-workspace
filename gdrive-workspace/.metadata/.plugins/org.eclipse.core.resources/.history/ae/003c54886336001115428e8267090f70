...
// Requires Qt6 Core, Network, NetworkAuth, Gui
...
// --- MetadataFetcher (wie zuvor, erweitert um signal totalBytes/count) ---
...
// --- DownloadWorker (seriell) with extra signals unchanged but used) ---
...
// --- SummaryWindow: zeigt Anzahl Dateien + Summe Bytes ---
...
public slots:
    void updateTotals(qint64 bytes, int files) {
        ...
        bytesLabel->setText(QStringLiteral("Gesamtgröße: %1 B").arg(bytes));
    }
...
// --- ProgressWindow: Gesamt- und Dateifortschritt ---
...
public:
    explicit ProgressWindow(QWidget *parent = nullptr) : QWidget(parent) {
        ...
        l->addWidget(new QLabel(QStringLiteral("Gesamt:"), this));
...
public slots:
    void onFileProgress(const QString &name, qint64 rec, qint64 tot) {
        ...
        int p = 0;
        if (tot > 0) p = qBound(0, int((rec * 100) / tot), 100);
        fileProgress->setValue(p);
    }
    void onOverallProgress(int p) { overallProgress->setValue(p); }
...
private slots:
    void onTotalsComputed(qint64 totalBytes, int fileCount) {
        // Update tray tooltip and summary window
        QString tooltip = QStringLiteral("DriveSync: %1 Dateien, %2 B").arg(fileCount).arg(totalBytes);
...
private:
    ...
    void setupUi() {
        // Tray icon
        trayIcon = new QSystemTrayIcon(QIcon::fromTheme("folder-download"), this);
