 ...
 * filescanner.c
 *
 * Linear raw ext4 filescanner that prints only reconstructed paths which
 * contain the inode-placeholder component "[ino:...]" and analyzes only those.
 *
 * Behavior:
 *  - Scans the device linearly (from byte 0 to device end) in fixed-size chunks
 *    equal to inode_size and treats each chunk as a candidate inode.
 *  - For candidate directory inodes, parses direct blocks (i_block[0..11])
 *    and extracts directory entries using strict validation to reduce false positives.
 *  - Reconstructs best-effort full paths from collected directory entries using
 *    '..' parent links found in the collected entries.
 *  - **Only** reconstructed paths that contain the substring "[ino:" are
 *    considered for printing/analysis. All other paths are ignored.
 *  - For each printed path the program prints a monotonic scan-progress percentage
 *    (how far the scanner had progressed through the device when the entry was discovered)
 *    followed by the reconstructed path (which contains "[ino:...]").
 *
 * Build:
 *   gcc -O2 -o filescanner filescanner.c
 *
 * Usage:
 *   sudo ./filescanner /dev/sdXN      # root: will mount if needed
 *   ./filescanner image.img           # non-root: will not mount; unverified candidates
 *
 * Note:
 *  - This program intentionally restricts output to placeholder-containing paths.
 *  - It still uses strict filename validation to avoid spurious matches.
 *  - Only direct blocks are parsed (no extents/indirect).
 */
...
/* Minimal ext4 on-disk structures */
...
/* global mountpoint for lstat checks (kept but not used for filtering here) */
...
/* require at least one alnum or dot/underscore/hyphen */
static bool name_has_filename_chars(const char *s, size_t len) {
...
static bool validate_dir_entry(const uint8_t *block, size_t block_size, size_t offset) {
    ...
    if (!name_has_filename_chars((const char*)namep, name_len)) return false;
...
/* reconstruct path best-effort; returns allocated string */
...
/* linear scan: use a monotonic processed_bytes counter for percent
 * Only print/analyze reconstructed paths that contain "[ino:".
 */
static int linear_scan_and_collect(int fd, struct dir_entry_list *dlist, struct str_list *found, uint16_t inode_size, uint32_t block_size) {
    ...
    for (off_t off = 0; off + inode_size <= end; off += inode_size) {
        ...
        if (r != (ssize_t)inode_size) {
            processed_bytes = off + inode_size;
            continue;
        }

        /* update monotonic processed_bytes now */
        processed_bytes = off;
                    ...
                    if (inode_no != 0 && name_len > 0 && name_len <= EXT4_NAME_LEN) {
                        ...
                        if (del_add(dlist, pseudo_inode, (uint64_t)inode_no, name) == 0) {
                            if (strcmp(name, ".") != 0 && strcmp(name, "..") != 0) {
                                ...
                                if (rel) {
                                    /* Only consider paths that contain the placeholder "[ino:" */
                                    if (strstr(rel, "[ino:") == NULL) {
                                        free(rel);
                                    } else {
                                        /* compute monotonic percent from processed_bytes */
                                        double percent = 0.0;
                                        if (total_bytes > 0) percent = ((double)processed_bytes / (double)total_bytes) * 100.0;

                                        /* print only placeholder-containing paths; deduplicate */
                                        if (!sl_contains(found, rel)) {
                                            printf("%.2f%% %s\n", percent, rel);
                                            fflush(stdout);
                                            sl_add(found, rel);
                                        }
...
int main(int argc, char **argv) {
    ...
    if (mountpoint[0] != '\0') strncpy(g_mountpoint, mountpoint, sizeof(g_mountpoint) - 1);
    else g_mountpoint[0] = '\0';
