/*
 * main.cpp - qgeister1 vollständiges Amalgamat (Sv/s in SI-Präfixen, double precision)
 *
 * Integriert:
 * - V4L2 Video Capture (YUYV -> RGB32)
 * - Bildverarbeitung: Background EMA, Long-exposure EMA, Sobel, Residue, Heatmap
 * - UI: Full-window dual video, overlay graphs, tray minigraph (Labels in Sv/s, using milli/micro/nano/pico/femto)
...
// returns e.g. "12.345 mSv/s", "5.678 µSv/s", "123.000 nSv/s", "0.000 fSv/s"
static QString formatSvPerS(double value) {
    ...
    if (absv >= 1.0) {
        prefix = ""; scaled = v; // Sv/s
    } else if (absv >= 1e-3) {
        prefix = "m"; scaled = v * 1e3; // milli
    } else if (absv >= 1e-6) {
        prefix = "µ"; scaled = v * 1e6; // micro
    } else if (absv >= 1e-9) {
        prefix = "n"; scaled = v * 1e9; // nano
    } else if (absv >= 1e-12) {
        prefix = "p"; scaled = v * 1e12; // pico
    } else {
        prefix = "f"; scaled = v * 1e15; // femto or smaller
    }

    // Use 3 decimal places for readability
    return QString("%1 %2Sv/s").arg(scaled, 0, 'f', 3).arg(prefix);
}

// Helper for total dose (Sv) formatting using same SI prefixes (no "/s")
...
public:
    ...
    QImage processRadionullon(const QImage &img) {
        ...
        // accumulate total dose in Sv using measured frame interval if available
