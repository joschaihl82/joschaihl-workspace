 ...
 * nanoc.c
 *
 * Teaching C compiler front-end and simple x86-64 code generator.
 * This extended version improves parsing and code generation, adds
 * more AST nodes, a proper recursive-descent parser with operator
 * precedence, a clearer symbol table, better error reporting, and
 * more robust codegen with labels, stack alignment, and simple
 * data-section emission for string literals.
 *
 * This file is intentionally long and verbose for educational clarity.
 *
 * NOTE: This is an educational project and not production-grade.
 * It focuses on clarity, correctness of structure, and incremental
 * features to help you extend it further.
 */
...
/*
 * We implement a recursive-descent parser with functions for each
 * precedence level. The grammar is simplified but supports:
 *
 * program        := (function)*
 * function       := type ident '(' params ')' compound_stmt
 * params         := (type ident (',' type ident)*)?
 * type           := 'int' | 'char' | 'long' | 'void' ('*')*
 * compound_stmt  := '{' (stmt)* '}'
 * stmt           := compound_stmt | if | while | for | return | break | continue | decl | expr ';'
 ...
 * mul            := unary (('*' | '/') unary)*
 * unary          := ('+' | '-' | '!' | '*' | '&') unary | primary
 * primary        := number | char | string | ident | '(' expr ')' | call
 * call           := ident '(' (expr (',' expr)*)? ')'
 *
 * This parser returns an AST rooted at AST_PROGRAM.
 */
...
/*
 * Semantic analysis resolves identifiers to variables, assigns types,
 * computes stack offsets for locals, and performs basic type checks.
 *
 * For simplicity:
 * - Function parameters are added to the function's locals list
 *   (so find_var_in_function can find them).
 * - All functions return int unless specified otherwise.
 * - Pointer arithmetic and casts are minimally supported.
 */
...
/*
 * We generate x86-64 System V assembly (AT&T-like but using Intel syntax
 * in printf strings). This is a simplified emitter:
 *
 * - Each expression pushes its result (64-bit) onto the stack.
 * - codegen_expr pops/pushes as needed.
 * - Function prologue/epilogue manage rbp/rsp and reserve stack for locals.
 * - String literals are emitted in a .data section at the end.
 *
 * This code generator is intentionally verbose and annotated for learning.
 */
...
static void codegen_expr(ASTNode *node, Function *f) {
    ...
    switch (node->kind) {
        ...
        case AST_CALL: {
            ...
            // Evaluate args and store temporaries on stack in reverse order
            ...
            for (int i = nargs - 1; i >= 0; i--) {
                if (i < (int)(sizeof(arg_regs)/sizeof(arg_regs[0]))) {
                    ...
                    // more args than registers: push onto stack (not implemented)
                    printf("  pop rax\n"); // fallback
                    printf("  push rax\n");
                }
            }
...
static void codegen_program(ASTNode *prog) {
    ...
    for (int i = 0; i < prog->child_count; i++) {
        ...
        // For simplicity, we assume parameters are already in locals (semantic added them)
        // But we can store incoming registers into local stack slots for consistent addressing
        for (int p = 0; p < f->num_params; p++) {
            Var *pv = f->params[p];
            if (!pv) continue;
            // store register into [rbp - offset]
            if (p < (int)(sizeof(arg_regs)/sizeof(arg_regs[0]))) {
                printf("  mov [rbp - %d], %s\n", pv->offset, arg_regs[p]);
            } else {
    ...
    if (string_table_count > 0) {
        ...
        for (int i = 0; i < string_table_count; i++) {
            printf(".LC%d:\n", i);
            printf("  .string \"%s\"\n", string_table[i].s ? string_table[i].s : "");
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    // Debug: print AST and symbol tables (uncomment to inspect)
    // print_ast(prog, 0);
