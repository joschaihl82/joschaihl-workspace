...
parse_translation_unit(compiler_context_t *ctx)
{
	while (peek_token(ctx)->kind != TK_EOF) {
		token_t *t = peek_token(ctx);
		if (t->kind == TK_INT || t->kind == TK_CHAR) {
			/* could be function or global var */
			type_info_t *base = parse_base_type(ctx);
			token_t *n = peek_token(ctx);
			if (n->kind != TK_IDENT)
				error_at(ctx, n->pos ? n->pos : ctx->src_buf, "expected identifier");
			/* peek ahead to see if function (identifier '(') or var (identifier ';' or '[') */
			char *name = strdup(n->text);
			next_token(ctx);
			if (accept_token(ctx, TK_LPAREN)) {
				/* function: we already consumed '(' so put it back by rewinding token_idx one */
				ctx->token_idx--; /* put back '(' so parse_function can parse params */
				/* rewind identifier so parse_function will re-read it */
				ctx->token_idx--; /* put back identifier */
				parse_function(ctx);
			} else {
				/* global variable declaration (very minimal) */
