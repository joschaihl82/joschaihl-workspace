// xghost.c
// Gespenster-Filter mit Radionullon-Modulation und O(n^3)-Frequenzbereichs-Optimierung
// Features: Resizable Window, Aspect-Fit, Vollbild-Start, ESC/Q beendet
// Tasten: 1/2/3 Bänder, A/Z Decay, S/X Glow
...
// Radionullon: Ghost-Blend mit Modulation
...
// Aspect-Fit-Nearest-Neighbour, RGB24 -> BGRX
...
// Fiktive Nullon-Score pro Frequenz
static double simulate_nullon_score(double f_mhz, double t_seconds) {
    double w1 = exp(-pow((f_mhz - 13.77) / 0.03, 2.0));
    double w2 = exp(-pow((f_mhz - 13.89) / 0.05, 2.0));
    double w3 = exp(-pow((f_mhz - 14.02) / 0.04, 2.0));
    double drift = 0.15 * sin(t_seconds * 0.7 + f_mhz * 0.9);
    double flicker = 0.10 * sin(t_seconds * 2.3 + f_mhz * 3.1);
    return 0.8 * (w1 + w2 + w3) + drift + flicker;
}

// O(n^3): prüft alle Intervalle (L,R), summiert Score in O(n)
static FreqRange find_best_nullon_range(FreqGrid grid, double t_seconds, double edge_penalty) {
    int n = grid.n_bins;
    double df = (grid.f_end_mhz - grid.f_start_mhz) / (double)(n - 1);
    double *scores = (double*)malloc(sizeof(double) * n);
    if (!scores) {
        FreqRange err = {0}; err.best_score = -1e18; return err;
    }
    for (int i = 0; i < n; i++) {
        double f = grid.f_start_mhz + i * df;
        scores[i] = simulate_nullon_score(f, t_seconds);
    }

    FreqRange res = {0};
    res.best_score = -1e18; res.L = 0; res.R = 0;

    for (int L = 0; L < n; L++) {
        for (int R = L; R < n; R++) {
            double sum = 0.0;
            for (int k = L; k <= R; k++) sum += scores[k];

            double left_edge = (L > 0) ? fabs(scores[L] - scores[L - 1]) : 0.0;
            double right_edge = (R + 1 < n) ? fabs(scores[R] - scores[R + 1]) : 0.0;
            double penalty = edge_penalty * (left_edge + right_edge);

            double total = sum - penalty;
            if (total > res.best_score) { res.best_score = total; res.L = L; res.R = R; }
        }
    }

    res.best_start_mhz = grid.f_start_mhz + res.L * df;
    res.best_end_mhz   = grid.f_start_mhz + res.R * df;
...
int main(void) {
    ...
    Window win = XCreateWindow(dpy, root, 100, 100, SRC_W, SRC_H, 0,
                               ...
                               CWEventMask, &swa);
    XStoreName(dpy, win, "xghost - Radionullon (O(n^3) Optimizer)");
    XMapWindow(dpy, win);
    ...
    // Nullon-State + Optimizer-Grid
    NullonState ns; set_nullon_band(&ns, BAND_WHISPER_LOW);
    FreqGrid grid = { .f_start_mhz = 13.70, .f_end_mhz = 14.10, .n_bins = 301 }; // 301 Bins (machbar)
    double t_opt_accum = 0.0;
    double edge_penalty = 0.5;
    FreqRange cur = {0};
    cur.best_start_mhz = 13.73; cur.best_end_mhz = 14.07; cur.best_score = 0.0;
    ...
    while (running) {
        ...
        // Alle ~1 s: O(n^3)-Optimierung des fiktiven Frequenzbereichs
        if (t_opt_accum >= 1.0) {
            t_opt_accum = 0.0;
            cur = find_best_nullon_range(grid, ns.t, edge_penalty);
            double span = cur.best_end_mhz - cur.best_start_mhz;
            double center = 0.5 * (cur.best_end_mhz + cur.best_start_mhz);
            // Koppeln: breite Bereiche -> mehr Glow, schmale -> mehr Decay
            ns.base_glow  = fminf(0.80f, fmaxf(0.05f, ns.base_glow  + (float)(0.02 * span)));
            ns.base_decay = fminf(0.98f, fmaxf(0.80f, ns.base_decay + (float)(-0.01 * (0.08 - span))));
            ns.drift = (float)(0.1 + 0.3 * sin(center * 0.7));
        }
        ...
        snprintf(overlay1, sizeof(overlay1),
                 "Radionullon: %s  Decay=%.3f  Glow=%.3f  [ESC/Q quit | 1/2/3 band | A/Z decay | S/X glow]",
                 band_name, ns.base_decay, ns.base_glow);
        XSetForeground(dpy, gc, WhitePixel(dpy, screen));
        XDrawString(dpy, win, gc, 16, 24, overlay1, (int)strlen(overlay1));

        // Overlay-Zeile 2: O(n^3) Ergebnis
        char overlay2[200];
        snprintf(overlay2, sizeof(overlay2),
                 "Auto Nullon (O(n^3)): %.2f–%.2f MHz  score=%.3f  bins=%d",
                 cur.best_start_mhz, cur.best_end_mhz, cur.best_score, grid.n_bins);
        XDrawString(dpy, win, gc, 16, 42, overlay2, (int)strlen(overlay2));
