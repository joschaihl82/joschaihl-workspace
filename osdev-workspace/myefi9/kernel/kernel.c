// kernel.c
// Kernel that expects to be started by the provided UEFI bootloader.
// - Defensive copy of KernelGOPInfo
// - printf writes to serial and framebuffer
// - NVMe init/read best-effort
...
/* printf that writes to both outputs */
...
/* Defensive copy storage for GOP info */
static KernelGOPInfo __kernel_gop_copy;

/* Forward declarations */
void kmain(KernelGOPInfo *kgi);

/* Exported entry symbol called by bootloader */
__attribute__((visibility("default")))
void _start(KernelGOPInfo *kgi) {
    /* Defensive copy into kernel-owned memory */
    if (kgi) {
        /* Try to copy; if pointer is invalid this may fault, but bootloader fix prevents that */
        memcpy(&__kernel_gop_copy, kgi, sizeof(__kernel_gop_copy));
        kmain(&__kernel_gop_copy);
    } else {
        kmain(NULL);
    }
    for (;;) { __asm__("hlt"); }
...
void kmain(KernelGOPInfo *kgi) {
    serial_init();
            ...
            printf("Framebuffer present: base=%p pps=%u w=%u h=%u\n",
                   (uintptr_t)kgi->FrameBufferBase, pps, screen_width, screen_height);
