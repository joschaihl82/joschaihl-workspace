// main.c
// gcc -O2 -o beatdetect_main main.c -lSDL2 -lpulse-simple -lpulse -lGLEW -lGL -lm -lpthread
// Converted: capture via PulseAudio "default.monitor" instead of ALSA PCM
...
// --------------------------- Signal handling ---------------------------
static volatile int running_flag = 1;
static void sigint_handler_local(int sig) { (void)sig; running_flag = 0; }
...
int main(int argc, char *argv[]) {
    ...
    // ---------------- PulseAudio setup ----------------
    const char *pa_device = "default.monitor";
    // allow override via argv: --device <name>
    ...
    int pa_error = 0;
    pa_simple *pa = pa_simple_new(NULL, "beatdetect_main", PA_STREAM_RECORD,
                                  pa_device, "record", &ss, NULL, NULL, &pa_error);
    if (!pa) {
    ...
    // read_buf must hold interleaved stereo samples: frames * channels
    int channels = ss.channels;
    int16_t *read_buf = (int16_t*) malloc(sizeof(int16_t) * BUFFERSIZE * channels);
    if (!read_buf) {
    ...
    fprintf(stderr,
            "Start: FFT + color VU + white flash + full-window waterfall (alpha-fade + 50%% transparency) enabled.\n");
    fprintf(stderr, "Onset factor initial = %.2f (Arrow Up/Down)\n",
            onset_factor);

    // signal handling
    signal(SIGINT, sigint_handler_local);
    signal(SIGTERM, sigint_handler_local);

    while (running && running_flag) {
        ...
        // read PulseAudio (blocking) -- read interleaved stereo frames
        size_t bytes_to_read = (size_t) BUFFERSIZE * channels * sizeof(int16_t);
        int pa_err = 0;
        if (pa_simple_read(pa, read_buf, bytes_to_read, &pa_err) < 0) {
            fprintf(stderr, "PulseAudio read error: %s\n", pa_strerror(pa_err));
            SDL_Delay(1);
            continue;
        }
        int frames = BUFFERSIZE; // frames read

        // Convert interleaved stereo -> mono (average L/R) into a small mono buffer
        int16_t mono_buf[BUFFERSIZE];
        for (int i = 0; i < frames; ++i) {
            int32_t l = read_buf[channels * i + 0];
            int32_t r = (channels > 1) ? read_buf[channels * i + 1] : l;
            mono_buf[i] = (int16_t) ((l + r) / 2);
        }

        // RMS chunk
        float chunk_rms = compute_rms_s16(mono_buf, (size_t) frames);

        // limiter (display only; no hardware volume control with Pulse here)
        ...
        // push into ring
        push_to_ring_from_s16_locked(mono_buf, (size_t) frames);
