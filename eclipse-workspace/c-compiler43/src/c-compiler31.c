// cc.c - single-file small C compiler (patched: typedefs before extern prototypes)
// - Handles long/short/unsigned types
// - Simple preprocessor with #include, #pragma once, #define
// - Tokenizer handles backslash line-continuation and lone backslash
// - Single-file: minimal external declarations instead of #include
// - Top-of-file patch applied: size_t/ssize_t typedefs appear before extern prototypes
//
// Build: gcc -O0 -g -Wall -o cc cc.c
// Use:   ./cc cc.c > cc.s
//        gcc -no-pie -o a.out cc.s

/* --- Minimal runtime/type declarations for self-hosted build --- */

/* Make sure size_t/ssize_t are defined before any extern prototypes */
typedef unsigned long size_t;
typedef long ssize_t;

/* Minimal FILE forward declaration */
typedef struct __FILE FILE;

/* builtin va_list alias */
typedef __builtin_va_list va_list;
#define va_start(ap, param) __builtin_va_start((ap), (param))
#define va_end(ap) __builtin_va_end(ap)

/* --- Minimal external function prototypes (no parameter names) --- */
/* Use only types in parameter lists to avoid introducing extra identifier tokens */
...
/* SEEK_* fallbacks for self-hosted build */
#ifndef SEEK_SET
#define SEEK_SET 0
#endif
#ifndef SEEK_CUR
...
char *user_input = NULL; // set later to source buffer
int line_of_loc(char *loc) {
	if (!user_input || !loc)
		return -1;
	int line = 1;
	for (char *p = user_input; p < loc && *p; p++)
		if (*p == '\n')
			line++;
	return line;
}
...
void error_at_loc(char *loc, char *fmt, ...) {
	int ln = line_of_loc(loc);
	va_list ap;
	va_start(ap, fmt);
	if (ln > 0)
...
static int startswith_impl(const char *p, const char *q) {
	size_t i = 0;
	while (q[i]) {
		if (p[i] != q[i])
			return 0;
...
#define startswith(p,q) startswith_impl((p),(q))

static bool is_ident1(char c) {
	return isalpha((int) (unsigned char) c) || c == '_' || c == '$';
}
static bool is_ident2(char c) {
	return isalnum((int) (unsigned char) c) || c == '_' || c == '$';
}
...
Token* tokenize(char *p, bool eof) {
	...
	while (*p) {
		...
		if (*p == '"') {
			...
			if (*p != '"')
				error_at_loc(start, "unclosed string literal");
			int len = (int) (p - start);
		...
		if (*p == '\'') {
			...
			if (*p == '\\' && p[1])
				...
				p++;
			if (*p != '\'')
				error_at_loc(start, "unclosed char constant");
			int len = (start[0] == '\\') ? 2 : 1;
		...
		if (isdigit((int) (unsigned char) *p)) {
			...
			if (startswith(p, "0x") || startswith(p, "0X")) {
				base = 16;
				p += 2;
				while (isxdigit((int) (unsigned char) *p))
					p++;
			} else {
			...
			cur->next = new_token(TK_NUM, start, (int) (p - start));
			cur = cur->next;
			cur->val = val;
			continue;
		}
		if (is_ident1(*p)) {
			...
			if (len == 2 && strncmp(start, "if", 2) == 0)
				...
				kind = TK_CHAR;
			else if (len == 4 && strncmp(start, "enum", 4) == 0)
				kind = TK_ENUM;
			else if (len == 6 && strncmp(start, "struct", 6) == 0)
				kind = TK_STRUCT;
			else if (len == 5 && strncmp(start, "union", 5) == 0)
				kind = TK_UNION;
			else if (len == 4 && strncmp(start, "void", 4) == 0)
				kind = TK_VOID;
			else if (len == 4 && strncmp(start, "bool", 4) == 0)
				kind = TK_BOOL;
			else if (len == 6 && strncmp(start, "sizeof", 6) == 0)
				kind = TK_SIZEOF;
			else if (len == 7 && strncmp(start, "typedef", 7) == 0)
				kind = TK_TYPEDEF;
			else if (len == 6 && strncmp(start, "switch", 6) == 0)
				kind = TK_SWITCH;
			else if (len == 4 && strncmp(start, "case", 4) == 0)
				kind = TK_CASE;
			else if (len == 7 && strncmp(start, "default", 7) == 0)
				kind = TK_DEFAULT;
			else if (len == 5 && strncmp(start, "break", 5) == 0)
				kind = TK_BREAK;
			else if (len == 8 && strncmp(start, "continue", 8) == 0)
				kind = TK_CONTINUE;
			else if (len == 6 && strncmp(start, "extern", 6) == 0)
				kind = TK_EXTERN;
			else if (len == 4 && strncmp(start, "long", 4) == 0)
				kind = TK_LONG;
			else if (len == 8 && strncmp(start, "unsigned", 8) == 0)
				kind = TK_UNSIGNED;
			else if (len == 5 && strncmp(start, "short", 5) == 0)
				kind = TK_SHORT;
			else if (len >= 17 && strncmp(start, "__builtin_va_list", 17) == 0)
				kind = TK_BUILTIN_VA_LIST;
		...
		error_at_loc(p, "tokenize: unknown token at: %s", p);
	}
	if (eof) {
		...
		if (t->kind == TK_PREPROCESSOR) {
			if (str_starts_with_str(t->str, "#include")) {
				...
				if (!t || t->kind != TK_STRING)
					error_at_loc(t ? t->loc : t->str->ptr,
							"#include expects string");
				...
				FILE *f = fopen(path, "r");
				if (!f)
					error_at_loc(t->loc, "include file not found: %s", path);
				fseek(f, 0, SEEK_END);
				long sz = ftell(f);
				fseek(f, 0, SEEK_SET);
			...
			if (str_starts_with_str(t->str, "#define")) {
				char *line = t->str->ptr;
				int line_len = t->str->len;
				int pos = 7;
				while (pos < line_len && (line[pos] == ' ' || line[pos] == '\t'))
					pos++;
				int id_start = pos;
				while (pos < line_len
						&& (isalpha((unsigned char) line[pos])
								|| line[pos] == '_' || line[pos] == '$'
								|| isalnum((unsigned char) line[pos])))
					pos++;
				if (id_start == pos)
					error_at_loc(t->loc, "#define expects identifier");
				char *ident_buf = strndup(line + id_start, pos - id_start);
				String *ident_str = new_string(ident_buf, pos - id_start);
				while (pos < line_len && (line[pos] == ' ' || line[pos] == '\t'))
					pos++;
				char *repl_text = NULL;
				int repl_len = 0;
				if (pos < line_len) {
					repl_len = line_len - pos;
					repl_text = calloc(1, repl_len + 1);
					memcpy(repl_text, line + pos, repl_len);
				} else {
					repl_text = calloc(1, 1);
					repl_text[0] = '\0';
					repl_len = 0;
				}
				Token *repl_tokens = NULL;
				if (repl_len > 0)
					repl_tokens = tokenize(repl_text, false);
				Macro *m = calloc(1, sizeof(Macro));
				m->ident = ident_str;
				m->replace = repl_tokens;
				m->next = macros_list;
				macros_list = m;
				free(repl_text);
				continue;
...
Type* new_struct_type(String *name, bool is_union) {
	Type *ty = new_type(TY_STRUCT, NULL);
...
void expect(char *op) {
	...
	if (token->kind == TK_RESERVED) {
		char buf[128];
		int n = token->str ? token->str->len : 0;
		if (n > 120)
			n = 120;
		if (token->str && token->str->ptr) {
			int i;
			for (i = 0; i < n; i++)
				buf[i] = token->str->ptr[i];
			buf[n] = '\0';
			error_at_loc(token->loc,
					"token mismatch: expected %s but got reserved token \"%s\"",
					op, buf);
		} else
			error_at_loc(token->loc,
					"token mismatch: expected %s but got reserved token", op);
	} else if (token->kind == TK_IDENT)
		error_at_loc(token->loc,
				"token mismatch: expected %s but got identifier %.*s", op,
				token->str->len, token->str->ptr);
	else if (token->kind == TK_NUM)
		error_at_loc(token->loc,
				"token mismatch: expected %s but got number %d", op,
				token->val);
	else if (token->kind == TK_EOF)
		error_at_loc(token->loc, "token mismatch: expected %s but got EOF", op);
	else
		error_at_loc(token->loc,
				"token mismatch: expected %s but got token kind %d", op,
				token->kind);
}
...
void expect_kind(TokenKind kind) {
	if (token->kind != kind)
		error_at_loc(token->loc, "token mismatch");
	token = token->next;
}
int expect_number() {
	if (token->kind != TK_NUM)
		error_at_loc(token->loc, "token mismatch: expected number");
	int v = token->val;
	token = token->next;
	return v;
}
...
Type* consume_type_name() {
	...
	if (consume_kind(TK_LONG)) {
		Type *t = new_type(TY_LONG, NULL);
		t->is_unsigned = saw_unsigned;
		return t;
	}
	if (consume_kind(TK_SHORT)) {
		Type *t = new_type(TY_SHORT, NULL);
		t->is_unsigned = saw_unsigned;
		return t;
	}
	...
	if (consume_kind(TK_VOID))
		return new_type(TY_VOID, NULL);
	if (consume_kind(TK_BOOL))
		return new_type(TY_BOOL, NULL);
	if (consume_kind(TK_BUILTIN_VA_LIST)) {
		Type *type = new_type(TY_ARRAY, new_type(TY_VA_LIST_TAG, NULL));
		type->array_size = 1;
		return type;
	}
	if (consume_kind(TK_ENUM)) {
		...
		if (consume("{")) {
			...
			while (!consume("}")) {
				...
				if (!name)
					error_at_loc(name ? name->loc : token->loc,
							"expected enum name");
	...
	if (check_kind(TK_STRUCT) || check_kind(TK_UNION)) {
		...
		if (consume("{")) {
			...
			while (!consume("}")) {
				...
				if (!res)
					error_at_loc(token->loc, "expected type");
			...
			if (id) {
				...
				if (s) {
					if (s->fields)
						error_at_loc(id->loc, "struct %.*s is already defined",
								id->str->len, id->str->ptr);
					else {
			...
			if (!id)
				error_at_loc(token->loc, "expected struct name");
...
ConsumeTypeRes* expect_nested_type(Type *type) {
	...
	if (consume("(")) {
		...
		if (!tok)
			error_at_loc(token->loc, "expected identifier");
	...
	if (consume("(")) {
		...
		if (!tmp)
			error_at_loc(token->loc, "expected type in parameter");
		while (consume(",")) {
			if (consume("..."))
				break;
		...
		expect(")");
		res->type = new_type(TY_FUNC, type);
	}
	for (int i = 0; i < ptr; i++)
		res->type = new_type(TY_PTR, res->type);
	while (consume("[")) {
		Node *size = NULL;
		if (token->kind == TK_NUM) {
		...
		expect("]");
		res->type = new_type(TY_ARRAY, type);
...
Node* stmt() {
	...
	if (consume_kind(TK_CASE)) {
		Node *e = expr();
		if (e->kind != ND_NUM)
			error_at_loc(token->loc, "expected constant expression");
		expect(":");
...
Node* add() {
	...
	for (;;) {
		if ((node->type->ty == TY_PTR || node->type->ty == TY_ARRAY)
				&& node->type->ptr_to && node->type->ptr_to->ty != TY_VOID) {
			int size = sizeof_type(node->type->ptr_to);
			if (consume("+")) {
				if (node->type->ty == TY_ARRAY)
					node = new_typed_node(ND_ADDR, node, NULL,
							new_type(TY_PTR, node->type->ptr_to));
				node = new_node(ND_ADD, node,
						new_node(ND_MUL, mul(), new_node_num(size)));
			} else if (consume("-")) {
				if (node->type->ty == TY_ARRAY)
					node = new_typed_node(ND_ADDR, node, NULL,
							new_type(TY_PTR, node->type->ptr_to));
				node = new_node(ND_SUB, node,
						new_node(ND_MUL, mul(), new_node_num(size)));
			}
			return node;
		}
...
Node* unary() {
	if (consume("+"))
		...
		Node *l = primary();
		if (l->type->ty != TY_PTR && l->type->ty != TY_ARRAY)
			error_at_loc(
					l->type ?
							l->type->ptr_to ?
									l->type->ptr_to->struct_type ?
											l->type->ptr_to->struct_type->name->ptr :
											NULL
									: NULL
							: NULL, "dereference failed: not a pointer");
		if (l->type->ty == TY_ARRAY)
	...
	for (;;) {
		if (consume("[")) {
			...
			if (!consume(")")) {
				n->lhs = expr();
				while (consume(","))
					n->lhs = new_node(ND_UNNAMED, n->lhs, expr());
				expect(")");
			}
			node = n;
		} else
...
Node* primary() {
	...
	if (token->kind == TK_IDENT) {
		...
		LVar *lv = find_lvar(t);
		if (lv)
			return new_node_lvar(lv);
		GVar *gv = find_gvar(t);
		if (gv)
			return new_node_gvar(gv);
		Func *f = find_func(t);
		if (f) {
			Node *n = calloc(1, sizeof(Node));
			n->kind = ND_CALL;
		...
		new_gvar(t->str, new_type(TY_INT, NULL));
		GVar *g2 = find_gvar(t);
		return new_node_gvar(g2);
	}
	error_at_loc(token ? token->loc : user_input, "primary: unexpected token");
	return NULL;
}
...
void add_field(StructType *type, Type *ty, String *name) {
	...
	if (type->is_union) {
		...
		if (type->fields) {
			field->offset = align(
					type->fields->offset + sizeof_type(type->fields->type), al);
			type->size = align(field->offset + sizeof_type(field->type),
					type->alignment);
		} else {
			field->offset = 0;
...
External* external() {
	locals = NULL;
	if (!globals)
		new_gvar(new_string("NULL", 4),
				new_type(TY_PTR, new_type(TY_VOID, NULL)));
	...
	if (check_kind(TK_STRUCT) || check_kind(TK_UNION)) {
		...
		if (!ty)
			error_at_loc(token->loc, "expected struct or union");
		expect(";");
		return external;
	}
	...
	ConsumeTypeRes *res = consume_type();
	if (!res)
		error_at_loc(token->loc, "invalid type");
	external->name = res->tok->str;
	if (res->type->ty == TY_FUNC) {
		external->kind = EXT_FUNC;
		if (!find_func(res->tok))
			new_func(res->tok->str, res->type->ptr_to);

		/* Robustly locate the '(' that starts the parameter list.
		 Search forward from the function-name token for the '(' token
		 and set token to the token after '(' so parameter parsing begins correctly. */
		if (!res->tok)
			error_at_loc(token->loc,
					"internal parser error: missing function name token");
		Token *scan = res->tok;
		while (scan
				&& !(scan->kind == TK_RESERVED && scan->str
						&& scan->str->len == 1 && scan->str->ptr[0] == '('))
			scan = scan->next;
		if (!scan) {
			Token *t = res->tok;
			fprintf(stderr, "debug: tokens after function name: ");
			for (int k = 0; k < 6 && t; k++, t = t->next) {
				if (t->str && t->str->ptr)
					fprintf(stderr, "'%.*s' ", t->str->len, t->str->ptr);
				else
					fprintf(stderr, "<tok%d> ", t->kind);
			}
			fprintf(stderr, "\n");
			error_at_loc(res->tok->loc, "expected '(' after function name");
		}
		token = scan->next;
		...
		if (!consume(")") && !no_args) {
			for (;;) {
				...
				if (!cres)
					error_at_loc(token->loc, "failed to parse argument");
				LVar *lvar = find_lvar(cres->tok);
				if (!lvar)
					lvar = new_lvar(cres->tok->str, cres->type);
		...
		if (external->code->kind != ND_BLOCK)
			error_at_loc(
					external->code ?
							external->code->lhs ?
									external->code->lhs->type ?
											external->code->lhs->type->ptr_to ?
													external->code->lhs->type->ptr_to->struct_type ?
															external->code->lhs->type->ptr_to->struct_type->name->ptr :
															NULL
													: NULL
											: NULL
									: NULL
...
int main(int argc, char **argv) {
	...
	if (last_slash) {
		size_t n = (size_t) (last_slash - filename);
		char *dn = calloc(1, n + 1);
		memcpy(dn, filename, n);
		dir_name = dn;
	} else
		dir_name = ".";
	...
	tok = preprocess(tok);
	if (!tok)
		error_at_here("preprocess returned NULL");
	token = tok;
