 ...
 *
 * Enhancements in this version (requested "Option A"):
 * - pushq / popq instructions implemented
 * - .align directive implemented
 * - addps SSE instruction implemented (and improved SSE support)
 * - Robust %rip handling: parser accepts both rip and %rip in memory operands
 * - movq $symbol, %reg now emits a 64-bit immediate with R_X86_64_64 relocation
 * - Various encoder improvements and small fixes
 *
 * Build:
 *   gcc -O2 jas.c -o jas
 *
 ...
 *
 * Limitations:
 * - Still a teaching/minimal assembler and linker; not production-grade.
 * - Not all x86-64 instructions supported; subset useful for examples.
 */
...
static int parse_register_after_percent(struct operand *op) {
    ...
    /* special-case %rip: accept it and mark rip */
...
/* mov imm->reg (32-bit immediate fallback) */
...
/* pushq reg */
static void encode_push_reg(int reg) {
    int rex_b = rex_bit(reg);
    if (rex_b) emit_rex(0,0,0,rex_b);
    emit_text_byte((unsigned char)(0x50 + (reg & 7)));
}

/* popq reg */
static void encode_pop_reg(int reg) {
    int rex_b = rex_bit(reg);
    if (rex_b) emit_rex(0,0,0,rex_b);
    emit_text_byte((unsigned char)(0x58 + (reg & 7)));
}
...
/* SSE helper: opcode is second byte after 0F */
...
/* ---------- Instruction dispatch (subset, extended) ---------- */
static void assemble_instruction(const char *mnemonic, struct operand *ops, int nops) {
    if (strcmp(mnemonic, "movq") == 0) {
        ...
        if (nops==2 && ops[0].kind==1 && ops[1].kind==0) {
            if (ops[0].sym) {
                /* emit mov imm64, reg with relocation R_X86_64_64 */
                int reg = ops[1].reg;
                int rex_b = rex_bit(reg);
                emit_rex(1,0,0,rex_b);
                emit_text_byte((unsigned char)(0xB8 + (reg & 7)));
                uint64_t off = text_len;
                emit_text_u64(0);
                push_reloc(off, 1, ops[0].sym, R_X86_64_64, 0);
                return;
            } else {
                encode_mov_imm_reg64(ops[1].reg, (uint64_t)ops[0].imm);
                return;
            }
        }
        warnf("unsupported movq form");
        return;
    }
    ...
    /* pushq / popq */
    if (strcmp(mnemonic, "pushq")==0) {
        if (nops==1 && ops[0].kind==0) { encode_push_reg(ops[0].reg); return; }
        warnf("unsupported pushq form");
        return;
    }
    if (strcmp(mnemonic, "popq")==0) {
        if (nops==1 && ops[0].kind==0) { encode_pop_reg(ops[0].reg); return; }
        warnf("unsupported popq form");
        return;
    }

    /* SSE: movups/addps/movaps */
    if (strcmp(mnemonic, "movups")==0) {
        if (nops==2 && ops[0].kind==2 && (ops[1].flags & (1<<16))) { /* mem -> xmm */
            int dst = ops[1].reg;
            int rex_r = (dst>>3)&1;
            if (rex_r) emit_rex(0,rex_r,0,0);
            emit_text_byte(0x0F); emit_text_byte(0x10); /* MOVUPS xmm, m */
            emit_modrm_sib_disp(dst, &ops[0], 1);
            return;
        }
        if (nops==2 && (ops[0].flags & (1<<16)) && ops[1].kind==2) { /* xmm -> mem */
            int src = ops[0].reg;
            int rex_r = (src>>3)&1;
            if (rex_r) emit_rex(0,rex_r,0,0);
            emit_text_byte(0x0F); emit_text_byte(0x11); /* MOVUPS m, xmm */
    ...
    if (strcmp(mnemonic, "movaps")==0) {
        if (nops==2 && ops[0].kind==2 && (ops[1].flags & (1<<16))) { int src = ops[1].reg; int rex_r = (src>>3)&1; if (rex_r) emit_rex(0,rex_r,0,0); emit_text_byte(0x0F); emit_text_byte(0x11); emit_modrm_sib_disp(src, &ops[0], 1); return; }
        if (nops==2 && (ops[0].flags & (1<<16)) && ops[1].kind==2) { int dst = ops[0].reg; int rex_r = (dst>>3)&1; if (rex_r) emit_rex(0,rex_r,0,0); emit_text_byte(0x0F); emit_text_byte(0x10); emit_modrm_sib_disp(dst, &ops[1], 1); return; }
    ...
    if (strcmp(mnemonic, "addps")==0) {
        if (nops==2 && (ops[0].flags & (1<<16)) && ops[1].kind==2) { /* xmm, mem */
            ...
            int rex_r = (dst>>3)&1;
            if (rex_r) emit_rex(0,rex_r,0,0);
            emit_text_byte(0x0F); emit_text_byte(0x58); /* ADDPS xmm, m */
            emit_modrm_sib_disp(dst, &ops[1], 1);
            return;
        }
        if (nops==2 && (ops[0].flags & (1<<16)) && (ops[1].flags & (1<<16))) { /* xmm, xmm */
...
static void do_align(int align) {
    ...
    if (current_segment == 1) {
        uint64_t pad = (align - (text_len % align)) % align;
        for (uint64_t i=0;i<pad;i++) emit_text_byte(0);
    } else if (current_segment == 2) {
        uint64_t pad = (align - (data_len % align)) % align;
        for (uint64_t i=0;i<pad;i++) emit_data_byte(0);
    } else if (current_segment == 3) {
        uint64_t pad = (align - (bss_len % align)) % align;
        bss_len += pad;
    }
}
...
static void process_line(const char *line) {
    ...
    if (tk == TK_IDENT && strcmp(tk_text, ".align") == 0) {
        next_tok();
        if (tk == TK_NUMBER) { do_align((int)tk_number); next_tok(); } else warnf(".align expects number");
        return;
    }
    if (tk == TK_IDENT && strcmp(tk_text, "align") == 0) {
        next_tok();
...
int main(int argc, char **argv) {
    ...
    /* compile as jas (avoid running as a.out) - user must compile tool separately */
