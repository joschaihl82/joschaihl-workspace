 ...
 *  - For each reconstructed path, checks lstat() against the mounted tree and
 ...
 *  - Must be run as root to mount devices and read raw devices.
 *
 * Build:
 *   gcc -O2 -o filescanner filescanner.c
 *
 * Usage:
 *   sudo ./filescanner /dev/sdXN
 *
 * Safety:
...
/* Validate that bytes form printable UTF-8 filename: no NUL, no '/', no control chars.
 * Require at least one printable character. Reject overly long names.
 */
...
/* Validate a directory entry at block+offset */
            ...
            for (int bi = 0; bi < 12; ++bi) {
                ...
                while (pos + 8 < block_size) {
                    if (!validate_dir_entry(block, block_size, pos)) {
                        /* resync: advance by 4 bytes to try to find next valid entry */
                        pos += 4;
                        continue;
                    }
                    ...
                    if (inode_no != 0 && name_len > 0 && name_len <= EXT4_NAME_LEN) {
                        ...
                        if (del_add(dlist, pseudo_inode, (uint64_t)inode_no, name) == 0) {
                            if (strcmp(name, ".") != 0 && strcmp(name, "..") != 0) {
                                ...
                                if (rel) {
                                    /* build full path: mountpoint + rel */
                                    if (g_mountpoint[0] != '\0') {
...
int main(int argc, char **argv) {
    ...
    if (geteuid() != 0) {
        fprintf(stderr, "This program requires root privileges to mount devices and read raw devices.\n");
        return 1;
    }
