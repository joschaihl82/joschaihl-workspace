...
private:
    ...
    void updateTabTitle(const QString &baseName) {
        // find tab index by tabData
        for (int i = 0; i < tabCtr->count(); ++i) {
            QVariant d = tabCtr->tabBar()->tabData(i);
            if (d.isValid() && d.toString() == baseName) {
                int cnt = unreadCount.value(baseName, 0);
                if (cnt > 0) {
                    tabCtr->setTabText(i, QString("%1 (%2)").arg(baseName).arg(cnt));
                } else {
                    tabCtr->setTabText(i, baseName);
                }
                return;
            }
        }
    ...
    void makeTab(const QString &strNam) {
        ...
        // store base name in tabData for reliable lookup
    ...
    void showTxt(const QString &strNam, const QString &strMsg) {
        ...
        int idxTab = -1;
        for (int i = 0; i < tabCtr->count(); ++i) {
            if (tabCtr->tabBar()->tabData(i).isValid() && tabCtr->tabBar()->tabData(i).toString() == strNam) {
                idxTab = i;
                break;
            }
        }
