/*
 lapd_ss2000.c
 - LAPD-CIA Sirene Sound 2000 Standard (SS2000) parameters
 - Mono DSP pipeline -> stereo output via small fractional delay
 - Pre-synthesis producer thread filling ring buffer to avoid underruns
 - Short convolution reverb (short built-in IRs)
 - Probabilistic playback: Wail 75% / Yelp 25%
 - Yelp LFOs (vibrato) made much, much slower per request
 - Build:
     gcc -O2 -Wall -pthread -o lapd_ss2000 lapd_ss2000.c -lasound -lm
 - Run:
     ./lapd_ss2000 [optional ALSA device e.g., hw:0,0 or default]
*/
...
/* ---------- Constants (SS2000 LAPD-CIA Sirene Standard) ---------- */
#define SR                  44100u
#define CHANNELS            2
...
/* SS2000 standard frequency / timing values (applied as requested) */
#define WAIL_F_LO     700.0
#define WAIL_F_HI    1700.0
#define WAIL_DUR      8.0

/* Yelp: SS2000 variant, slower segments and much slower vibrato (user request) */
#define YELP_F_MIN    900.0
#define YELP_F_MAX   2000.0
#define YELP_SEG      0.60     /* seconds per yelp segment (longer for SS2000 feel) */
#define YELP_TOTAL    6.0      /* total yelp run duration */

/* Hi-Lo kept optional (rare); not used in default probabilistic schedule */
#define HILO_LOW      600.0
#define HILO_HIGH    1400.0
#define HILO_SEG      0.40

/* Vibrato (LFO) settings */
#define VIB_RATE      5.5f     /* wail vibrato (Hz) */
#define VIB_AMT       0.006f   /* 0.6% */
 /* Yelp vibrato made MUCH slower as requested */
#define YELP_VIB      0.6f     /* 0.6 Hz (very slow LFO for yelper) */
#define YELP_VIB_AMT  0.002f   /* very shallow vibrato (0.2%) */

/* stereo fractional right delay (samples) */
#define RIGHT_DELAY_SAMPLES 1.2f

/* voices */
#define VOICE1_AMP    0.92f
#define VOICE2_AMP    0.72f
...
/* LFO sanity */
#define MIN_VIB_RATE  0.05f   /* allow very slow LFOs now */
#define MAX_VIB_RATE 50.0f

/* ---------- Short built-in IRs (vehicle/urban) ---------- */
...
static void *producer(void *arg) {
    ...
    /* Yelp segment state */
    int yelp_seg_blocks = 0;
    int yelp_seg_index = 0;
    double yelp_seg_center = YELP_F_MIN;
    ...
    for (;;) {
        ...
        /* choose next cycle at boundaries: Yelp 25% / Wail 75% (no Hi-Lo by default) */
        if (state_blocks <= 0) {
            ...
            if (!last_was_yelp && r < 0.25f) {
                ...
                last_was_yelp = 1;
                /* init yelp segments for this run */
                yelp_seg_blocks = (int)ceil((YELP_SEG * SR) / (double)frames_block);
                if (yelp_seg_blocks < 1) yelp_seg_blocks = 1;
                yelp_seg_index = 0;
                yelp_seg_center = YELP_F_MIN + (double)(xrng() % (int)(YELP_F_MAX - YELP_F_MIN + 1));
            } else {
                state = 0;
        ...
        if (state == 0) {
            ...
            double f2_center = lerpf(700.0, 1500.0, 0.5 + 0.5 * sin(global_time * 0.15));
            ...
            /* YELP: composed of longer segments, one center per segment, very slow vibrato */
            /* start-of-run initialization handled above */
            /* pick new center when advancing to a new segment */
            if (yelp_seg_index > 0 && (state_blocks % yelp_seg_blocks) == 0) {
                yelp_seg_center = YELP_F_MIN + (double)(xrng() % (int)(YELP_F_MAX - YELP_F_MIN + 1));
            }

            /* compute segment-local progress (0..1) */
            int seg_pos_in_blocks = yelp_seg_blocks - (state_blocks % yelp_seg_blocks);
            if (seg_pos_in_blocks <= 0) seg_pos_in_blocks = 1;
            double seg_prog = (double)seg_pos_in_blocks / (double)yelp_seg_blocks;
            double seg_start = yelp_seg_center * 0.85;
            double seg_end   = yelp_seg_center * 1.25;
            /* map seg_prog to sweep across the segment (so each segment is a coherent sweep) */
            double f_block_start = seg_start + (seg_end - seg_start) * (seg_prog * 0.0); /* starts near seg_start */
            double f_block_end   = seg_start + (seg_end - seg_start) * (seg_prog * 1.0); /* sweep toward seg_end across segment */
            /* make second voice proportional to primary sweep */
            double f2s = f_block_start * SECOND_RATIO * 0.95;
            double f2e = f_block_end   * SECOND_RATIO * 1.05;

            /* override vibrato to very slow Yelp LFOs */
            ...
            voice_mono_generate(&vm1, mono_v1, frames_block, f_block_start, f_block_end, global_time);
            voice_mono_generate(&vm2, mono_v2, frames_block, f2s, f2e, global_time);
            vm1.vibrato_amt = save1_vib; vm1.vibrato_rate = save1_rate;
            vm2.vibrato_amt = save2_vib; vm2.vibrato_rate = save2_rate;

            /* advance segment index if we've completed a segment's worth of blocks */
            if ((state_blocks % yelp_seg_blocks) == 1) yelp_seg_index++;
        }

        /* mix mono voices, saturate, compress */
        ...
        /* convolution reverb send (mono) -> per-channel conv */
        ...
        /* produce stereo block: left=mono+L-reverb; right=delayed-mono+R-reverb */
        for (int n=0;n<frames_block;++n) {
        ...
        state_blocks--;
        if (state_blocks <= 0) {
            /* next loop will choose next cycle probabilistically */
        }
...
int main(int argc, char **argv) {
    ...
    /* LFO sanity checks (allowing very slow Yelp LFO now) */
    if (!check_lfo_sanity(VIB_RATE, PERIOD_FRAMES, SR)) {
        fprintf(stderr, "Wail vibrato rate (%g Hz) out of sane range\n", (double)VIB_RATE);
        return 1;
    }
    ...
    fprintf(stderr, "Playing LAPD SS2000 siren (mono -> stereo). Yelp LFOs much slower per request.\n");
