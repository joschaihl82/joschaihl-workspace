// beatdetect_gl_shader_threaded_adj.c
// gcc -O2 -o beatdetect_gl_shader_threaded_adj beatdetect_gl_shader_threaded_adj.c -lSDL2 -lasound -lGLEW -lGL -lm -lpthread
//
// ALSA PCM Capture + ALSA Mixer (Hardware Gain)
// FFT-basierte Beat-Erkennung (Worker-Thread, keine FFT-Lib)
// OpenGL 3.3 Core Shader Rendering (VBO/VAO, 1D texture)
// BUFFERSIZE = 512, FFT_SIZE = 2048
// Pfeil hoch = feinere (sensitivere) Erkennung, Pfeil runter = gröbere Erkennung
...
// texture buffer (copied from ring for GPU)
...
static volatile int kick_flag = 0; // 0/1 set by FFT thread
static volatile int stop_threads = 0;

// running average for energy (in FFT thread)
static double running_avg = 1e-8;
static const double alpha_avg = 0.05;

// adjustable onset factor (user controls with arrow keys)
// energy > running_avg * onset_factor triggers a beat
...
// --- ring helpers (thread-safe wrappers) ---
static void push_to_ring_from_s16_locked(const int16_t *in, size_t n) {
...
// FFT worker: copies last FFT_SIZE samples under mutex, computes bass energy and sets kick_flag
static void *fft_worker(void *arg) {
    unsigned sample_rate = (unsigned)(uintptr_t)arg;
    float *snapshot = (float*)malloc(sizeof(float) * FFT_SIZE);
    if (!snapshot) return NULL;
    while (!stop_threads) {
        // copy last FFT_SIZE samples
        pthread_mutex_lock(&ring_mutex);
...
int main(void) {
    ...
    SDL_Window *win = SDL_CreateWindow("BeatDetect Shader Threaded Adjust", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    ...
    if (!g_ring) { fprintf(stderr, "Out of memory\n"); snd_pcm_close(g_pcm); if (g_mixer) snd_mixer_close(g_mixer); SDL_GL_DeleteContext(ctx); SDL_DestroyWindow(win); SDL_Quit(); return 1; }

    // allocate texture samples buffer
    tex_size = FFT_SIZE;
    tex_samples = (float*)calloc((size_t)tex_size, sizeof(float));
    if (!tex_samples) { free(g_ring); snd_pcm_close(g_pcm); if (g_mixer) snd_mixer_close(g_mixer); SDL_GL_DeleteContext(ctx); SDL_DestroyWindow(win); SDL_Quit(); return 1; }

    // allocate read buffer
    int16_t *read_buf = (int16_t*)malloc(sizeof(int16_t) * BUFFERSIZE);
    if (!read_buf) { free(tex_samples); free(g_ring); snd_pcm_close(g_pcm); if (g_mixer) snd_mixer_close(g_mixer); SDL_GL_DeleteContext(ctx); SDL_DestroyWindow(win); SDL_Quit(); return 1; }
    ...
    // compile shaders and program (modern pipeline)
    ...
    // create 1D texture for samples (floating point)
    GLuint tex;
    glGenTextures(1, &tex);
    ...
    fprintf(stderr, "Onset factor initial = %.2f (use Arrow Up/Down to adjust)\n", onset_factor);
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&ev)) {
            if (ev.type == SDL_QUIT) running = false;
                ...
                if (ev.key.keysym.sym == SDLK_ESCAPE) running = false;
                    ...
                    // finer detection -> reduce onset_factor (min 1.0)
                    pthread_mutex_lock(&param_mutex);
                    onset_factor -= 0.1;
                    if (onset_factor < 1.0) onset_factor = 1.0;
                    double cur = onset_factor;
                    pthread_mutex_unlock(&param_mutex);
                    fprintf(stderr, "Onset factor decreased -> finer detection: %.2f\n", cur);
                } else if (ev.key.keysym.sym == SDLK_DOWN) {
                    // coarser detection -> increase onset_factor (max 10.0)
                    pthread_mutex_lock(&param_mutex);
                    onset_factor += 0.1;
                    if (onset_factor > 10.0) onset_factor = 10.0;
                    double cur = onset_factor;
                    pthread_mutex_unlock(&param_mutex);
                    fprintf(stderr, "Onset factor increased -> coarser detection: %.2f\n", cur);
                }
        ...
        // push samples into ring (thread-safe)
        push_to_ring_from_s16_locked(read_buf, (size_t)frames);

        // prepare texture samples: copy last tex_size samples into tex_samples (under mutex)
        pthread_mutex_lock(&ring_mutex);
        ...
        // read kick flag (atomic-ish)
        int kick = kick_flag;

        // compute hue from 500ms RMS (locked)
        ...
        if (now - last_dbg > 1000) {
            ...
            pthread_mutex_unlock(&param_mutex);
            fprintf(stderr, "rate=%u tex=%d limiter_gain=%.3f onset_factor=%.2f kick=%d\n", rate, tex_size, g_limiter_gain, cur_onset, kick);
            last_dbg = now;
        }
