...
// Inline only #include "file.h" (no macro expansion) using Clang preprocessor callbacks,
...
// Inline includes recursively into a buffer string. `baseDir` is directory of the file owning `content`.
// `inlined` is a set of canonicalized paths already inlined to avoid cycles/repeats.
static std::string inlineIncludesText(const std::string &content, const std::string &baseDir, std::set<std::string> &inlined) {
  ...
  // Need FileManager/SourceManager before createDiagnostics overload that requires VFS
  CI.createFileManager();
  CI.createSourceManager(CI.getFileManager());

  // Create diagnostics (VFS-aware overload)
  ...
  for (auto &inc : rec->includes) {
    ...
    // Recursively inline includes inside the included file. baseDir for included file is its directory.
    SmallString<256> p(inc.resolved);
    SmallString<256> parent = llvm::sys::path::parent_path(p);
    std::string incBase = std::string(parent.str());
    std::string recursed = inlineIncludesText(incText, incBase, inlined);
...
// -------------------- AST-based collector and cleaner (unchanged conservative logic) --------------------
...
public:
  ...
  void HandleTranslationUnit(ASTContext &Context) override {
      ...
      for (auto &entry : S.enumConstantsUsed) {
        ...
        if (B.isValid() && E.isValid() && SM.isWrittenInSameFile(B,E)) {
          ...
          if (realEnd.isValid()) {
            ...
            llvm::outs() << "Enum used in file " << S.fileName << " :\n";
            llvm::outs() << src << "\n";
    ...
    // Get preprocessed buffer (we parsed the inlined text as main file)
    SourceManager &SM = Context.getSourceManager();
    ...
    auto addRange = [&](const RangeInfo &r){
      unsigned s=r.startOffset, e=r.endOffset;
    ...
    auto is_semicolon_only = [](StringRef line)->bool{
      line = line.trim();
      if (line.empty()) return false;
      size_t pos = line.find("//");
      if (pos!=StringRef::npos) line = line.substr(0,pos).rtrim();
      if (line.find("/*")!=StringRef::npos || line.find("*/")!=StringRef::npos) return false;
      bool hasSemi=false;
      for (char c : line) {
        if (c==';') hasSemi=true;
    ...
    std::vector<std::string> lines;
    {
      ...
      while (pos < len) {
        ...
        if (nl==StringRef::npos) { lines.emplace_back(newContent.substr(pos)); break; }
    ...
    for (size_t i=0;i<lines.size();++i) {
      if (is_semicolon_only(lines[i])) continue;
    ...
    // Write cleaned file
    std::string outFile = S.fileName + OutputSuffix;
...
int main(int argc, const char **argv) {
  ...
  for (const std::string &srcPath : OptionsParser.getSourcePathList()) {
    ...
    std::string inlined = inlineIncludesText(orig, baseDir, localInlined);
    // merge local into global
    for (auto &s : localInlined) globalInlined.insert(s);

    // Parse the inlined text with Clang so AST sees header contents (macros remain as inlined text)
    // Attempt to reuse compile commands if available
    std::vector<std::string> args;
    auto cmds = Compilations.getCompileCommands(srcPath);
    if (!cmds.empty()) {
      const auto &cmdline = cmds[0].CommandLine;
      if (cmdline.size() >= 2) args.assign(cmdline.begin()+1, cmdline.end());
      else args = {"-std=c11"};
    } else {
      args = {"-std=c11"};
    }
    ...
    bool ok = tooling::runToolOnCodeWithArgs(std::make_unique<PerFileAction>(), inlined, args, srcPath);
    if (!ok) {
