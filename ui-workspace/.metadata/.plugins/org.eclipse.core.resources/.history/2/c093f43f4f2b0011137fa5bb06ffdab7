// notepad1_fixed_font_required_realtime_highlight.c
#define UNICODE
#define _UNICODE
#include <windows.h>
#include <commdlg.h>
...
/* Font file handling (load from exe directory, register private) */
static wchar_t g_privateFontPath[MAX_PATH] = L"";
static bool g_privateFontRegistered = false;

/* Name of the font file to look for in the exe directory */
static const wchar_t *PRIVATE_FONT_FILENAME = L"SourceCodePro-Regular.ttf";
/* Face name expected inside the TTF */
static const wchar_t *PRIVATE_FACE_NAME = L"Source Code Pro";

/* Try to load TTF from the EXE directory and register it as a private font.
...
/* Unregister private font and free HFONT if created */
...
/* Setzt Format für Bereich [start, end) korrekt */
...
/* --- Set default format for a range --- */
...
/* Highlight a specific range [start,end) by tokenizing only that range */
static void HighlightRange(HWND edit, int start, int end) {
    ...
    // Set default format for the range first
...
/* Syntax Highlighting for entire document (kept for load/paste) */
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        // Create monospace font (use private face - WinMain ensured registration)
        LOGFONTW lf;
        ZeroMemory(&lf, sizeof(lf));
        HDC hdc = GetDC(hwnd);
        lf.lfHeight = -MulDiv(10, GetDeviceCaps(hdc, LOGPIXELSY), 72); // 10pt
        ReleaseDC(hwnd, hdc);
        lf.lfWeight = FW_NORMAL;
        lf.lfCharSet = DEFAULT_CHARSET;

        // Use the private face name
        wcsncpy_s(lf.lfFaceName, LF_FACESIZE, PRIVATE_FACE_NAME, _TRUNCATE);
        hMonoFont = CreateFontIndirectW(&lf);
        if (!hMonoFont) {
            // fallback to Consolas
            wcsncpy_s(lf.lfFaceName, LF_FACESIZE, L"Consolas", _TRUNCATE);
            hMonoFont = CreateFontIndirectW(&lf);
            if (!hMonoFont) {
                wcsncpy_s(lf.lfFaceName, LF_FACESIZE, L"Courier New", _TRUNCATE);
                hMonoFont = CreateFontIndirectW(&lf);
            }
        }
        ...
        // Set default CHARFORMAT2W to force monospace face and default color
        {
            CHARFORMAT2W cf;
            ZeroMemory(&cf, sizeof(cf));
            cf.cbSize = sizeof(cf);
            cf.dwMask = CFM_FACE | CFM_SIZE | CFM_COLOR;
            cf.yHeight = 10 * 20; // 10pt in twips
            cf.crTextColor = COLOR_DEFAULT;
            wcscpy_s(cf.szFaceName, LF_FACESIZE, PRIVATE_FACE_NAME);
    ...
    case WM_COMMAND:
        if (HIWORD(wParam) == EN_CHANGE && (HWND)lParam == hEdit) {
            SetWindowTextW(hLineNums, L"");
            UpdateLineNumbers();
            HighlightLineAtCaret(hEdit);   // real-time per-line highlighting
            UpdateStatusBar();
            break;
        }
