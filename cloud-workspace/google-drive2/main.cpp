...
public:
    OAuthHelper(QObject *parent = nullptr) : QObject(parent) {
        ...
        // Ensure reply handler uses configured port
        auto *handler = new QOAuthHttpServerReplyHandler(replyPort, this);
        oauth->setReplyHandler(handler);

        // IMPORTANT: ensure scope is present in the authorization request.
        // Use a lambda that captures 'this' and injects the scope string explicitly.
        oauth->setModifyParametersFunction([this](QAbstractOAuth::Stage stage, QMultiMap<QString,QVariant> *params){
            if (stage == QAbstractOAuth::Stage::RequestingAuthorization && params) {
                ...
                // Build scope string from configured scopes and ensure it's present
                QString scopeStr;
                for (int i = 0; i < scopes.size(); ++i) {
                    if (i) scopeStr += ' ';
                    scopeStr += scopes.at(i);
                }
                if (!scopeStr.isEmpty()) {
                    params->insert("scope", scopeStr);
                }
            }
        });
    ...
    void start() {
        ...
        // Set requested scope tokens (Qt6 API)
        ...
        // Debug: log the authorization URL parameters (for troubleshooting)
        // Note: authorizationUrl() may not include all params until grant() is called in some Qt versions,
        // but we still call grant() to start the flow.
        oauth->grant();
    }
...
private:
    bool loadClientSecrets() {
        ...
        // Configure scopes here (must match Google Cloud Console)
...
public slots:
    ...
    void enqueueUpload(const QString &filePath) {
        ...
        m_retry->performWithRetry(creator, [this, filePath, fileData](QNetworkReply *r) {
            ...
            if (total <= CHUNK_SIZE) {
                QNetworkRequest putReq{QUrl(uploadUrl)};
...
int main(int argc, char **argv) {
    ...
    QAction *actOpenFolder = new QAction("Open Sync Folder", menu);
    QAction *actShowActivity = new QAction("Show Activity", menu);
    QAction *actQuit = new QAction("Quit", menu);
    ...
    // Objects
    OAuthHelper *oauth = new OAuthHelper(&app);
    NotifyWindow *notify = new NotifyWindow();
    SyncManager *sync = nullptr; // created after oauth ready
    ...
    QObject::connect(actShowActivity, &QAction::triggered, [notify]() {
        if (notify->isVisible()) notify->hide(); else notify->show();
    });
    QObject::connect(actQuit, &QAction::triggered, [&]() {
        logMsg("User requested quit");
        app.quit();
    });

    // Start OAuth flow and create SyncManager when ready
    QObject::connect(oauth, &OAuthHelper::ready, [&]() {
        logMsg("OAuth ready; starting sync manager");
        if (!sync) {
    ...
    // Start OAuth (this will either use saved refresh token or open browser)
    oauth->start();
