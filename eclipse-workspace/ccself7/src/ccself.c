...
void error(char* fmt, ...);
void error_at_t(Token* tok, char* fmt, ...);
...
void error_at_t(Token* tok, char* fmt, ...) {
    ...
    token = tok;
    print_error_line(tok->str, fmt, ap);
    token = old_token;
    va_end(ap);
    exit(1);
}
...
/* Standard C helper to construct candidate path and read file (replaces C++ lambda) */
static char* read_from_candidate_path(const char* prefix, const char* path_to_append) {
    size_t prefix_len = prefix ? strlen(prefix) : 0;
    size_t cand_len = prefix_len + strlen(path_to_append) + 1;
    char* cand = calloc(1, cand_len);
    if (!cand) return NULL;
    cand[0] = '\0';
    if (prefix_len) strcpy(cand, prefix);
    strcat(cand, path_to_append);
...
char* parse_directive(char* p, char** output_ptr, char** q_ptr, size_t *output_cap_ptr, const char* current_dir) {
    ...
    if (name_len == 6 && strncmp(name_start, "define", 6) == 0) {
        ...
        *q_ptr = q;
        ...
        /* System directories to search for angle-bracket includes */
        const char* system_dirs[] = { "/usr/include/", "/usr/local/include/" };
        size_t num_system_dirs = sizeof(system_dirs) / sizeof(system_dirs[0]);

        if (is_quoted) {
            /* 1) try current_dir/path */
            if (current_dir && strcmp(current_dir, ".") != 0) {
                size_t cur_len = strlen(current_dir);
                char* prefix = calloc(1, cur_len + 2);
                strcpy(prefix, current_dir);
                if (prefix[cur_len - 1] != '/') strcat(prefix, "/");
                included_raw = read_from_candidate_path(prefix, path);
                free(prefix);
                if (included_raw) read_ok = true;
            }
            /* 2) try path as given */
            if (!read_ok) {
                included_raw = read_from_candidate_path("", path);
                if (included_raw) read_ok = true;
            }
            /* 3) try ./path */
            if (!read_ok) {
            ...
            /* 4) try system dirs */
            size_t idx;
            for (idx = 0; !read_ok && idx < num_system_dirs; idx++) {
                included_raw = read_from_candidate_path(system_dirs[idx], path);
                if (included_raw) read_ok = true;
            }
        } else {
            /* angle includes: try as-given, then system dirs */
            included_raw = read_from_candidate_path("", path);
            if (included_raw) read_ok = true;
...
Node* unary() {
    ...
    while (consume(".")) {
        ...
        if (!node->type || node->type->ty != STRUCT) error_at_t(t, "member access on non-struct type");
...
int main(int argc, char** argv) {
    ...
    Function** arr = calloc(1, sizeof(Function*) * (n > 0 ? n : 1));
    int i = 0;
