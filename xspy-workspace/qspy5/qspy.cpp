// qspy.cpp
// Qt6 + libatspi - Single-file AT-SPI Inspector with hierarchy, live preview (real data only), overlay (1s) and QML export
// Build: qmake6 (QT += widgets) or CMake (Qt6 Widgets) and link with pkg-config atspi-2
//
// Änderungen:
// - Keine Qt WebEngine mehr.
// - Rechts: QTextBrowser zeigt eine HTML-Vorschau: zuerst das eingebettete Bild (data:image/png;base64,...),
//   darunter die Base64-Zeichenfolge als ASCII in einem <pre>-Block.
// - Für jedes generierte QML-Widget werden **relative** Koordinaten (x,y,width,height) relativ zur ausgewählten Root-Komponente eingefügt.
// - Vorschau- und QML-Textfenster zeigen ausschließlich reale Daten, keine Demo-Widgets.
// - Fix: Aufruf von itemSelectionChanged verbindet jetzt korrekt mit updatePreviewForSelection (kein Aufruf einer nicht vorhandenen Funktion).
// - MOC: Datei endet mit #include "qspy.moc"
...
public:
    MainWindow(QWidget *parent=nullptr) : QMainWindow(parent) {
        ...
        // Middle: preview area (real data only)
        ...
        // Single QLabel used to show captured image (decoded from base64) or textual info
        previewImage = new QLabel;
        previewImage->setAlignment(Qt::AlignCenter);
        previewImage->setMinimumSize(240,160);
        previewImage->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        pvLayout->addWidget(previewImage, 1);

        previewInfo = new QTextEdit;
        previewInfo->setReadOnly(true);
        previewInfo->setMinimumHeight(120);
        pvLayout->addWidget(previewInfo);

        previewRoot->setLayout(pvLayout);

        // Right: QML text output + Save button + HTML preview (QTextBrowser)
        qmlRoot = new QWidget;
        ...
        // QTextEdit for raw QML text
        qmlText = new QTextEdit;
        qmlText->setFont(QFontDatabase::systemFont(QFontDatabase::FixedFont));
        qmlLayout->addWidget(qmlText, 2);

        // QTextBrowser for rendered HTML preview: first image, then base64 ASCII
        qmlPreview = new QTextBrowser;
        qmlPreview->setOpenExternalLinks(true);
        qmlPreview->setMinimumHeight(200);
        qmlLayout->addWidget(qmlPreview, 1);
        ...
        splitMain->setStretchFactor(2, 3);
        ...
        // Selection: update preview and QML (real data only)
        // FIX: call only updatePreviewForSelection (it updates both preview and QML)
        connect(tree, &QTreeWidget::itemSelectionChanged, this, &MainWindow::updatePreviewForSelection);
...
private slots:
    ...
    // Preview update (real data only) — also updates QML output
    void updatePreviewForSelection() {
        ...
        AtspiAccessible *acc = mgr->getById(iid);
        if (!acc) {
            previewTitle->setText(QString("%1 — %2").arg(name, role));
            previewImage->clear();
            previewInfo->setPlainText("Accessible not available.");
            qmlPreview->clear();
            qmlText->clear();
            return;
        }

        previewTitle->setText(QString("%1 — %2").arg(name.isEmpty() ? role : name, role));
        ...
        QRect geom = mgr->getExtents(iid);

        // Try to capture image as base64 and show decoded image in previewImage
        QString b64 = captureImageAsBase64(acc);
        if (!b64.isEmpty()) {
            ...
            if (!img.isNull()) {
                ...
                previewImage->setPixmap(pm.scaled(previewImage->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
            } else {
                previewImage->clear();
            }
        } else {
        ...
        // Compose info text from real data only
        QString info;
        info += QString("Name: %1\n").arg(name.isEmpty() ? QString("(no name)") : name);
        info += QString("Role: %1\n").arg(role);
        if (!geom.isEmpty()) info += QString("Geom: %1,%2 %3x%4\n").arg(geom.x()).arg(geom.y()).arg(geom.width()).arg(geom.height());
        if (!text.isEmpty()) info += QString("Text: %1\n").arg(text);
        if (hasValue) info += QString("Value: %1 (min %2, max %3)\n").arg(curv).arg(minv).arg(maxv);
        if (!states.isEmpty()) info += QString("States: %1\n").arg(states.join(", "));
        QString attrs = mgr->getAttributes(iid);
        if (!attrs.isEmpty()) info += QString("Attributes: %1\n").arg(attrs);

        previewInfo->setPlainText(info);

        // Update QML text and HTML preview (QTextBrowser)
        // Use the selected accessible's extents as root for relative coordinates
        QRect rootGeom = mgr->getExtents(iid);
        QString qml = generateQmlForAccessible(acc, 0, rootGeom);

        // Build full QML document header/footer
        QString header = QString(
            ...
            "    width: 800\n"
            "    height: 600\n"
            "    title: \"qspy - exported accessible (relative coords)\"\n\n"
        );
        QString footer = "\n}\n";
        QString fullQml = header + "    // Generated component tree for selected accessible (coordinates relative to selected root)\n" + qml + footer;
        qmlText->setPlainText(fullQml);

        // For HTML preview: if image base64 exists, show <img> then <pre>base64</pre>
        if (!b64.isEmpty()) {
            QString html;
            html += "<div style='font-family: sans-serif;'>";
            html += QString("<div style='text-align:center;margin-bottom:8px;'><img src='data:image/png;base64,%1' style='max-width:100%;height:auto;border:1px solid #ccc;'/></div>").arg(b64);
            html += "<div style='font-size:12px;color:#333;margin-top:8px;'><strong>Base64 (ASCII)</strong></div>";
            html += "<pre style='white-space:pre-wrap;word-break:break-all;background:#f7f7f7;border:1px solid #e0e0e0;padding:8px;margin-top:6px;'>" + b64.toHtmlEscaped() + "</pre>";
            html += "</div>";
            qmlPreview->setHtml(html);
        } else {
            qmlPreview->setHtml("<div style='color:#666;padding:8px;'>No image available for preview.</div>");
        }
    }

    // Capture image as base64 (full-size, no scaling)
    QString captureImageAsBase64(AtspiAccessible *acc) {
    ...
    // Includes x, y, width, height for each generated widget when available.
    // Coordinates are **relative to rootGeom** passed in (usually the selected accessible).
    // For image widgets: embed base64 data URL.
    QString generateQmlForAccessible(AtspiAccessible *acc, int depth, const QRect &rootGeom) {
        if (!acc) return QString();
        ...
        // Compute relative coordinates to rootGeom (if rootGeom is empty, fall back to geom origin)
        QRect effectiveRoot = rootGeom;
        if (effectiveRoot.isEmpty()) {
            // if no root provided, use this node as root (so relative coords become 0,0)
            effectiveRoot = geom;
        }

        QString geomProps;
        if (hasGeom && !effectiveRoot.isEmpty()) {
            int rx = geom.x() - effectiveRoot.x();
            int ry = geom.y() - effectiveRoot.y();
            int rw = geom.width();
            int rh = geom.height();
            if (rx < 0) rx = 0;
            if (ry < 0) ry = 0;
            geomProps = QString("x: %1\n        y: %2\n        width: %3\n        height: %4")
                        .arg(rx).arg(ry).arg(rw).arg(rh);
        }
        ...
        if (type == "button") {
            ...
            QString b64 = captureImageAsBase64(acc);
            if (!b64.isEmpty()) {
                QString dataUrl = QString("data:image/png;base64,%1").arg(b64);
                props = QString("source: \"%1\"\n        fillMode: Image.PreserveAspectFit").arg(dataUrl);
            } else {
                props = QString("// image placeholder; accessible name: \"%1\"").arg(label);
            }
        } else if (type == "table") {
            element = "TableView";
        ...
        for (gint i=0;i<cnt;++i) {
            ...
            if (!ch) continue;
            QString childQ = generateQmlForAccessible(ch, depth+1, effectiveRoot);
            if (!childQ.isEmpty()) {
...
private:
    ...
    // preview (real data only)
    QWidget *previewRoot;
    QLabel *previewTitle;
    QLabel *previewImage;
    QTextEdit *previewInfo;

    // QML output
    QWidget *qmlRoot;
    QTextEdit *qmlText;
    QTextBrowser *qmlPreview;
...
int main(int argc, char **argv) {
    ...
    MainWindow w;
    w.setWindowTitle("qspy - Qt AT-SPI Inspector (live) - QML Export (relative coords)");
    w.show();
    return a.exec();
}
