...
// Raymarching tunnel visualizer driven by microphone input (ALSA).
// Fully patched and amalgamated. Ensures ALSA capture switch is enabled at startup
// and remains enabled while the program runs. Shader uses only uniforms and audio textures.
...
static double onset_factor = 1.0; // adjustable with arrow keys
...
// --------------------------- Creative Fragment Shader ---------------------------
// The shader below intentionally uses every shared uniform variable multiple times.
// It implements multiple concentric/offset tunnels, bubbles, and camera motion.
// All "detail" is derived deterministically from uniforms and the audio sample texture.
static const char *ray_fs = "#version 330 core\n"
"// Shared uniforms (from C)\n"
"uniform float iTime;\n"
...
"out vec4 fragColor;\n"
"\n"
"// Constants tuned for performance and visual richness\n"
"const float FOV = 0.45;\n"
"const float MarchDumping = 0.92;\n"
"const float FarBase = 18.0;\n"
...
"#define M_TUNNEL 1.0\n"
"\n"
// deterministic 'detail' function built from uniforms and samples (no randomness)\n"
"float detailField(vec3 p, float bass, float mid, float treb, float gain, float hue) {\n"
"    // sample audio at positions derived from world coords\n"
"    float sx = clamp((p.x * 0.5 + 0.5), 0.0, 0.999);\n"
"    float sy = clamp((p.y * 0.5 + 0.5), 0.0, 0.999);\n"
"    float sa = texture(u_samples, sx).r;\n"
"    float sb = texture(u_samples, sy).r;\n"
"    // combine band summaries and samples multiplicatively\n"
"    float a = (0.7 * bass + 0.2 * mid + 0.1 * treb) * (0.6 + 0.4 * sa);\n"
"    float b = (0.2 * bass + 0.6 * mid + 0.2 * treb) * (0.6 + 0.4 * sb);\n"
"    float c = (0.3 * gain + 0.2 * hue) * (0.5 + 0.5 * sa * sb);\n"
"    float osc = sin(iTime * (1.0 + 0.8 * bass) + p.z * (0.4 + 0.6 * mid));\n"
"    return clamp(a * 0.8 + b * 0.5 + c * 0.4 + 0.25 * osc, -1.0, 1.0);\n"
"}\n"
"\n"
// path for a tunnel layer; uses band summaries to modulate amplitude and frequency\n"
"vec2 pathLayer(float z, float layer, float mid, float treb, float hue) {\n"
"    float ampA = TunnelAmpA * (1.0 + 0.35 * mid) * (1.0 + 0.12 * layer);\n"
"    float ampB = TunnelAmpB * (1.0 + 0.35 * treb) * (1.0 + 0.10 * layer);\n"
"    float fA = TunnelFreqA * (1.0 + 0.06 * hue) * (1.0 + 0.01 * layer);\n"
"    float fB = TunnelFreqB * (1.0 + 0.06 * hue) * (1.0 + 0.01 * layer);\n"
"    return vec2(ampA * sin(z * fA + layer * 0.7), ampB * cos(z * fB + layer * 0.9));\n"
"}\n"
"\n"
// distance to a single tunnel layer with audio-driven micro-detail\n"
"float distLayer(vec3 p, float layer, float bass, float mid, float treb, float gain, float hue, float baseR) {\n"
"    vec2 c = pathLayer(p.z, layer, mid, treb, hue);\n"
"    float d2 = length(p.xy - c) - baseR * (1.0 + 0.12 * layer);\n"
"    float pat = detailField(p + vec3(layer*0.2, layer*0.2, layer*0.08), bass, mid, treb, gain, hue);\n"
"    float detail = 0.20 * pat * (0.5 + 0.5 * mid) * (1.0 + 0.6 * treb) * (1.0 + 0.3 * layer);\n"
"    return d2 + detail;\n"
"}\n"
"\n"
// scene map: multiple tunnel layers + audio-driven bubbles\n"
"vec2 mapScene(vec3 p, float bass, float mid, float treb, float gain, float hue) {\n"
"    float baseR = TunnelRadiusBase * (1.0 - 0.28 * bass);\n"
"    // combine several layers with smooth min\n"
"    float d = 1e6;\n"
"    for (int L = 0; L < 5; ++L) {\n"
"        float lo = float(L);\n"
"        float dl = distLayer(p, lo, bass, mid, treb, gain, hue, baseR);\n"
"        float k = 0.18 + 0.5 * (1.0 - mid); // smoothing depends on mid\n"
"        float h = clamp(0.5 + 0.5 * (d - dl) / k, 0.0, 1.0);\n"
"        d = mix(dl, d, h) - k * h * (1.0 - h);\n"
"    }\n"
"    // bubbles: spheres placed along the path; their size and frequency driven by audio\n"
"    float bubbleD = 1e6;\n"
"    for (int b = 0; b < 7; ++b) {\n"
"        float t = iTime * (0.5 + 0.6 * gain) + float(b) * (0.9 + 0.15 * u_band_bass);\n"
"        vec2 pc = pathLayer(t, float(b) * 0.25, mid, treb, hue);\n"
"        float zpos = t + 0.4 * (0.2 * bass + 0.1 * mid);\n"
"        vec3 bc = vec3(pc.x, pc.y, zpos);\n"
"        float samplePos = fract(0.07 * float(b) + 0.03 * u_band_bass);\n"
"        float s = texture(u_samples, samplePos).r;\n"
"        float br = 0.14 + 0.12 * (0.5 + 0.5 * s) + 0.08 * u_band_bass;\n"
"        float sd = length(p - bc) - br * (1.0 + 0.6 * u_band_mid);\n"
"        bubbleD = min(bubbleD, sd);\n"
"    }\n"
"    // carve bubbles into tunnels (bubbles appear as cavities/bright pockets)\n"
"    float sceneD = min(d, bubbleD);\n"
"    return vec2(sceneD, M_TUNNEL);\n"
"}\n"
"\n// soft shadow using mapScene and uniforms\n"
"float softShadow(vec3 ro, vec3 rd, float mint, float tmax, float bass, float mid, float treb, float gain, float hue) {\n"
"    float res = 1.0;\n"
"    float t = mint;\n"
"    for (int i = 0; i < 4; ++i) {\n"
"        vec3 pos = ro + rd * t;\n"
"        float h = mapScene(pos, bass, mid, treb, gain, hue).x;\n"
"        res = min(res, 5.0 * h / max(0.001, t));\n"
"        t += clamp(h, 0.02, 0.12) * (1.0 + 0.3 * mid);\n"
"        if (h < 0.002 || t > tmax) break;\n"
"    }\n"
"    return clamp(res, 0.0, 1.0);\n"
"}\n"
"\n// normal via finite differences of mapScene\n"
"vec3 calcNormal(vec3 pos, float bass, float mid, float treb, float gain, float hue) {\n"
"    float e = 0.004;\n"
"    float dx = mapScene(pos + vec3(e,0,0), bass, mid, treb, gain, hue).x - mapScene(pos - vec3(e,0,0), bass, mid, treb, gain, hue).x;\n"
"    float dy = mapScene(pos + vec3(0,e,0), bass, mid, treb, gain, hue).x - mapScene(pos - vec3(0,e,0), bass, mid, treb, gain, hue).x;\n"
"    float dz = mapScene(pos + vec3(0,0,e), bass, mid, treb, gain, hue).x - mapScene(pos - vec3(0,0,e), bass, mid, treb, gain, hue).x;\n"
"    return normalize(vec3(dx, dy, dz));\n"
"}\n"
"\n// castRay wrapper that uses mapScene\n"
...
"        float stepMul = MarchDumping * (1.0 + 0.6 * bass) * (1.0 + 0.45 * gain) * (1.0 + 0.2 * mid);\n"
...
"    return vec2(t, m);\n"
"}\n"
"\n// render the scene using only uniforms and textures\n"
"vec3 renderScene(vec3 ro, vec3 rd, float bass, float mid, float treb, float gain, float hue) {\n"
"    vec3 col = vec3(0.0);\n"
"    vec2 cres = castRay(ro, rd, bass, mid, treb, gain, hue);\n"
"    float t = cres.x;\n"
"    float m = cres.y;\n"
"    if (m > -0.5) {\n"
"        vec3 pos = ro + rd * t;\n"
"        vec3 nor = calcNormal(pos, bass, mid, treb, gain, hue);\n"
"        float bandIdx = fract(t * (0.12 + 0.06 * bass)) * float(u_nbands);\n"
...
"        vec3 base = mix(BaseColor * (1.0 + 0.6 * mid), bandcol * (0.6 + 0.9 * bass), 0.6);\n"
"        base *= (0.9 + 0.25 * hueShift);\n"
"        float depthOsc = sin(t * (0.6 + 0.35 * treb) + iTime * (0.25 + 0.6 * gain));\n"
"        col = base + depthOsc * (0.22 + 0.35 * mid);\n"
"        vec3 lig = -rd;\n"
"        float dif = clamp(dot(nor, lig), 0.0, 1.0);\n"
"        dif *= softShadow(pos, lig, 0.03, FarBase * (1.0 + 0.25 * treb), bass, mid, treb, gain, hue);\n"
"        vec3 brdf = vec3(0.0);\n"
"        brdf += 0.9 * dif * vec3(1.00, 0.92, 0.70);\n"
"        float spec = pow(clamp(dot(reflect(rd, nor), lig), 0.0, 1.0), 18.0) * (0.6 + 0.6 * gain) * (1.0 + 0.6 * bass);\n"
"        brdf += 0.7 * spec * vec3(1.0);\n"
"        col = col * brdf;\n"
"        col = mix(col, vec3(0.0), 1.0 - exp(-0.09 * t * (1.0 + 0.6 * treb)));\n"
"    }\n"
"    return clamp(col, 0.0, 1.0);\n"
"}\n"
"\nvoid main() {\n"
"    // flash overlay (highest priority) — uses flash uniforms and band summaries repeatedly\n"
"    if (u_flash_brightness > 0.001) {\n"
"        vec3 flash = u_flash_color * (1.0 + 0.9 * u_band_bass) * (0.6 + 0.6 * u_gain_display);\n"
"        float alpha = u_flash_brightness * (0.8 + 0.6 * u_band_mid);\n"
"        fragColor = vec4(flash, clamp(alpha, 0.0, 1.0));\n"
"        return;\n"
"    }\n"
"    // normalized coords\n"
...
"    // sample audio texture for detail\n"
...
"    float kick = float(u_kick);\n"
"    // camera speed and time modulation (multiple uses of uniforms)\n"
"    float cameraSpeed = CameraSpeedBase * (1.0 + 1.2 * bass * gain + 0.6 * mid * s_mid + 0.4 * s_bass);\n"
"    float timeMod = iTime * cameraSpeed + 4.0 * 60.0 * (1.0 + 0.12 * s_bass);\n"
"    // dynamic tunnel radius influenced by band averages and sample detail\n"
"    float tunnelRadius = TunnelRadiusBase * (1.0 - 0.30 * bass * (0.5 + 0.5 * s_bass)) * (1.0 - 0.08 * hue);\n"
"    // color tint derived from band averages and hue uniform\n"
"    float hueAngle = hue * 6.2831853;\n"
"    vec3 colorTint = vec3(1.0);\n"
"    colorTint.r += 0.28 * sin(hueAngle + 0.0) * bass * (0.5 + 0.5 * s_bass);\n"
"    colorTint.g += 0.28 * sin(hueAngle + 2.0) * mid * (0.5 + 0.5 * s_mid);\n"
"    colorTint.b += 0.28 * sin(hueAngle + 4.0) * treb * (0.5 + 0.5 * s_treb);\n"
"    colorTint = clamp(colorTint, 0.0, 2.0);\n"
"    // camera path and orientation (all uniform-driven)\n"
...
"    // render scene (uses uniforms many times internally)\n"
"    vec3 col = renderScene(ro, rd, bass, mid, treb, gain, hue);\n"
"    // subtle band color blend and extra modulation by gain and hue\n"
"    vec3 bandAvg = texture(u_bandcols, 0.5).rgb;\n"
"    col = mix(col, bandAvg * (0.8 + 0.5 * bass) * colorTint, 0.12 * gain * (0.5 + 0.5 * hue));\n"
"    // kick adds a short brightening (uses both CPU kick flag and sampled bass and band_bass)\n"
"    if (kick > 0.5) col += vec3(0.9 * s_bass * (1.0 + 1.0 * u_band_bass));\n"
"    // final gamma and small contrast boost influenced by gain and mid\n"
"    col = pow(col * (1.0 + 0.10 * gain + 0.08 * mid), vec3(0.4545));\n"
...
// --------------------------- ALSA Mixer (patched) ---------------------------
// This version ensures the capture switch (mic mute) is turned on at startup
// and remains enabled whenever we set capture volume. It avoids toggling the switch off.
...
static void alsa_set_capture_all(long v) {
    ...
    // Ensure capture switch remains enabled before setting volume.
    if (snd_mixer_selem_has_capture_switch(g_selem)) {
        int rc = snd_mixer_selem_set_capture_switch_all(g_selem, 1);
        if (rc < 0) {
            // Non-fatal: continue to set volume, but log for debugging.
            fprintf(stderr, "Warning: failed to re-enable capture switch: %s\n", snd_strerror(rc));
        }
    }

    // Set capture volume (safe clamped value)
    int rc = snd_mixer_selem_set_capture_volume_all(g_selem, v);
    if (rc < 0) {
        fprintf(stderr, "Warning: failed to set capture volume: %s\n", snd_strerror(rc));
    }
}
...
int main(void) {
	...
	SDL_Window *win = SDL_CreateWindow("BeatDetect Tunnel (Audio-driven)",
	...
	// enable blending for flash overlay
	glEnable(GL_BLEND);
	...
	// compile shaders (fullscreen VS + creative FS)
	...
	fprintf(stderr,
			"Start: Creative tunnel (audio-driven). All uniforms used to steer visuals.\n");
	...
	while (running) {
		...
		// Compute normalized per-band values and average them into bass/mid/treble summaries
		...
		for (int b = 0; b < N_BANDS; ++b) {
			...
			if (b < N_BANDS / 8) {
				bass_sum += norm;
				bass_count++;
			} else if (b < (N_BANDS * 5) / 8 && b >= (N_BANDS * 3) / 8) {
				mid_sum += norm;
				mid_count++;
			} else if (b >= (N_BANDS * 3) / 4) {
				treb_sum += norm;
				treb_count++;
			}
		}
		float band_bass = 0.0f, band_mid = 0.0f, band_treble = 0.0f;
		if (bass_count > 0) band_bass = (float)(bass_sum / (double)bass_count);
		if (mid_count > 0) band_mid = (float)(mid_sum / (double)mid_count);
		if (treb_count > 0) band_treble = (float)(treb_sum / (double)treb_count);

		// immediate kick detection on low bands (use raw bands)
		double low_sum = 0.0;
		int low_bands = 4;
		...
		// If detected -> advance contrast step and set background color
		if (detected) {
		...
		// set uniforms (use many of them; shader will reuse them heavily)
...
cleanup_early:
	...
	if (g_mixer) {
		// Do not forcibly toggle capture switch off on exit; leave system state unchanged.
		snd_mixer_close(g_mixer);
		g_mixer = NULL;
	}
	if (g_ring) {
