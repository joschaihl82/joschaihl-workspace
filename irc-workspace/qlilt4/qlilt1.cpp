...
// Scrollbar direction inverted so dragging up shows older output.
...
public:
          ...
          maxScrollbackLines(10000)
    {
        ...
        // invert appearance so dragging up moves to older content visually
        vscroll->setInvertedAppearance(true);
        vscroll->setInvertedControls(true);
...
private slots:
    ...
    void onScrollChanged(int value) {
        // vscroll is inverted visually; map its value to scrollOffset so that
        // dragging up (visually) increases scrollOffset (older content).
        if (!vscroll) return;
        int maxv = vscroll->maximum();
        int newOffset = qBound(0, maxv - value, (int)scrollbackRows.size());
...
private:
    ...
    void updateScrollbarRange() {
        ...
        // set visual scrollbar value so that inverted appearance maps correctly:
        int visualValue = vscroll->maximum() - scrollOffset;
        if (vscroll->value() != visualValue) vscroll->setValue(visualValue);
    }
