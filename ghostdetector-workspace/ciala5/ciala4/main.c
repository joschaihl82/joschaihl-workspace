// main.c — Lock-free with sched_yield inserted in heavy loops
...
// lock-free ring push/pop (single-producer allowed for pop)
...
void geiger_logic_process(uint32_t *src_rgb32, uint32_t *dst_rgb32, GeigerLogicState *state, double frame_sec, GeigerStats *stats) {
    ...
    // 12. Audio Request (best-effort, lock-free)
    if (rawVal > 0.0) {
        ClickEvent ev;
        ev.activity = rawVal;
        ev.timeMs = stats->currentTimeMs;
        click_queue_push(&ev); // drop if full
    }
}
...
static int audio_thread(void *data) {
    ...
    while (atomic_load_explicit(&g_running_atomic, memory_order_acquire)) {
        // Pop all queued click events (lock-free)
        ClickEvent ev;
        ...
        // Synthesize a block (yield periodically)
