// main.cpp
// Qt5 GUI proxy checker with progress, file watch, and CSV export
#include <QtWidgets>
#include <atomic>
#include <vector>
#include <queue>
...
protected:
    bool lessThan(const QModelIndex &left, const QModelIndex &right) const override {
        ...
        bool lOk = (lStatus.toString() == "OK");
        bool rOk = (rStatus.toString() == "OK");
...
private slots:
    ...
    void exportOkToCsv() {
        ...
        QTextStream out(&f);
        out << "Host,Port,Mode,TCP ms,PROTO ms,Status\n";
        ...
        for (int r = 0; r < model->rowCount(); ++r) {
            ...
            out << esc(host) << "," << esc(port) << "," << esc(mode) << "," << esc(tcp) << "," << esc(proto) << "," << esc(status) << "\n";
        }
        f.close();
