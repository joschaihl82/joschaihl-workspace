...
public slots:
    void startDownloads(const QJsonArray &queue, const QString &token, const QString &syncPath) {
        ...
        for (const QJsonValue &v : queue) {
            ...
            // Connect readyRead for this file only; capture per-file counters by reference
            // Use a QObject::connect that we can later rely on reply->finished to end the loop
            auto readyReadConn = connect(reply, &QNetworkReply::readyRead, this, [this, reply, &outFile, &downloadedBytes, &downloadedForFile, name, size, &timer, totalBytes]() mutable {
                ...
                if (!chunk.isEmpty()) {
                    ...
                    // Emit per-file progress (use size if known, otherwise use downloadedForFile as total)
                    qint64 reportedTotal = (size > 0) ? size : downloadedForFile;
                    emit fileProgress(name, downloadedForFile, reportedTotal);

                    // Update ETA based on cumulative rate
            ...
            // Wait synchronously for this file to finish
            QEventLoop loop;
            connect(reply, &QNetworkReply::finished, &loop, &QEventLoop::quit);
            loop.exec();

            // After finished: check for errors
            if (reply->error() != QNetworkReply::NoError) {
            ...
            // move .part to final name
            QFile::remove(outPath);
            QFile::rename(outPath + ".part", outPath);

            // If size was unknown, we already added bytes incrementally; no further action required.
            // If you want to ensure downloadedBytes matches actual file sizes, you could adjust here.
            ...
            // Clean up reply and its connections
            reply->deleteLater();
            disconnect(readyReadConn);
        }
...
// DriveSync (Hauptklasse) — unverändert, nur Schnittstellen genutzt
