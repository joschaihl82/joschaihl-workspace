/*
 * qgeister1 - Radionullon Ghost Filter Edition
 * Nutzt V4L2 und implementiert den komplexen Radionullon-Algorithmus (Hintergrundmodell, Flicker, Sobel)
 * zur Erzeugung eines "Gespenster-Heatmaps" als Ausgabebild.
 */
...
// --- Logik-Klasse (enthält den Radionullon-Filter) ---
...
public:
    explicit GeigerLogic(QObject *parent = nullptr) : QObject(parent) {
        ...
        // Wir nutzen QVectors als dynamische Arrays für einfache Verwaltung
    ...
    // Qt-konforme Version des Radionullon-Filters
    QImage processRadionullon(const QImage &img) {
        ...
        // 1. RGB32 zu Graustufen konvertieren
        for (int y = 0; y < IMG_H; ++y) {
            const QRgb* srcLine = (const QRgb*)img.constScanLine(y);
            for (int x = 0; x < IMG_W; ++x) {
                gray[y * IMG_W + x] = (quint8)qGray(srcLine[x]);
            }
        }
        ...
        // 5. Heatmap Overlay und Radionullon-Analyse (Geiger-Zählung)
        ...
        for (int y = 0; y < IMG_H; ++y) {
            ...
            for (int x = 0; x < IMG_W; ++x) {
                ...
                if (score < RADIONULLON_THRESH) {
                    // Optional: Bild leicht graustufen/entsättigen, wenn Score niedrig ist
        ...
        // 6. Geiger-Werte aus globalem Score ableiten
...
public:
    GeigerWidget(QWidget *parent = nullptr) : QWidget(parent) {
        setWindowTitle("qrad Radionullon Viewer");
        resize(IMG_W, IMG_H); // Passt zur Kameragröße
...
protected:
    void paintEvent(QPaintEvent *) override {
        ...
        // 1. Radionullon-Bild Hintergrund
        if (!m_lastFrame.isNull()) {
            p.drawImage(rect(), m_lastFrame);
        } else {
            p.fillRect(rect(), Qt::black);
            p.setPen(Qt::white);
            p.drawText(rect(), Qt::AlignCenter, "Warte auf V4L2-Stream...");
        }

        // 2. Hauptwerte (Oben Links)
        p.setFont(QFont("Monospace", 18, QFont::Bold));
        p.setPen(m_stats.alertState ? Qt::red : Qt::green);
        p.drawText(20, 40, QString("RADIONULLON: %1 Sv/h").arg(m_stats.currentRad, 0, 'f', 2));
        p.setPen(Qt::white);
        p.drawText(20, 70, QString("DOSE:        %1 Sv").arg(m_stats.totalDose, 0, 'f', 4));
...
public slots:
    void updateIcon(GeigerStats stats) {
        ...
        // Layering der Graphen für das Tray-Icon
...
int main(int argc, char *argv[]) {
    ...
    GeigerLogic logic;
    AudioEngine audio;
    GeigerWidget w; // Widget braucht keine Logic im Konstruktor mehr
    TrayGraphManager tray(&logic, &w);

    // WICHTIG: Verbindungen angepasst
    // 1. Kamera -> Logik
    QObject::connect(&capture, &V4L2Capture::imageCaptured, &logic, &GeigerLogic::processFrame);
    // 2. Logik -> Widget (Processed Image)
    QObject::connect(&logic, &GeigerLogic::processedImage, &w, &GeigerWidget::updateImage);
    // 3. Logik -> UI Updates
