...
// - NEW: Wraps neon-green "ghost" pixels in a semi-transparent, convex hull with EXTREME BRIGHTNESS.
...
private:
    ...
    Mat applyRadiationFilter(const Mat& inputFrame) {
        ...
        // --- NEW BRIGHTNESS LOGIC: Convex Hull and near 100% Alpha Blending ---
        ...
        // We simulate the "20x" brightness by using full intensity (255) and max alpha weight.
        Scalar neonGreenWithAlpha = Scalar(0, 255, 0, 255 * 0.95); // BGR + Alpha (95% opaque for blending)
        ...
        Mat blendedOutput = Mat::zeros(inputFrame.size(), CV_8UC3); // Final output is 3 channels
        ...
        for (int y = 0; y < outputFrame.rows; ++y) {
            for (int x = 0; x < outputFrame.cols; ++x) {
                ...
                double alpha_bg = 1.0 - alpha_fg;         // Alpha of the background

                // Blend: Foreground (Neon Green) gets maximum weight
                for (int c = 0; c < 3; ++c) { // For B, G, R channels
                    blendedOutput.at<Vec3b>(y, x)[c] =
                        static_cast<uchar>(backgroundPx[c] * alpha_bg + foregroundPx[c] * alpha_fg);
                }
            }
        }
        outputFrame = blendedOutput; // Update the outputFrame

        // 7. REMOVED: The subtle diffused glow (green wash) is removed to avoid darkening the extreme neon effect.
        // The ghosts will stand out much more dramatically against the pure grayscale background.
...
private slots:
    ...
    void loadGallery() {
        ...
        int row = 0, col = 0;
        const int cols = 1; // Only 1 column for maximum size
