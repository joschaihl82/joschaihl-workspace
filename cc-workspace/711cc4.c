...
static void patch_calls(Function *prog) {
    for (CallPatch *cp = call_patches; cp; cp = cp->next) {
        if (!strcmp(cp->name, "exit")) continue; // builtin
...
static void gen_addr(Node *node) {
    ...
    if (node->kind == ND_DEREF) {
        gen(node->lhs); // gen(lhs) should leave addr in rax, but since gen leaves in rax, and ND_DEREF gen is mov rax, [rax], but for addr, need to leave addr in rax
        // For ND_DEREF as lvalue, gen_addr should leave addr in rax
        // But since gen for ND_DEREF is mov rax, [rax], but for addr, it's already the addr.
        // No, for *p, the addr is p, so gen(p) leaves p in rax, then for lvalue, push rax
        return;
    }
...
static void gen(Node *node) {
    switch (node->kind) {
        ...
        case ND_IF: {
            ...
            emit8(0x0F); emit8(0x84); int else_pos = code_pos; emit32(0); // je rel
            gen(node->then);
            emit8(0xE9); int end_pos = code_pos; emit32(0); // jmp rel
            *(uint32_t*)(code + else_pos + 1) = code_pos - (else_pos + 5);
            if (node->els) gen(node->els);
            *(uint32_t*)(code + end_pos + 1) = code_pos - (end_pos + 5);
            break;
        }
        ...
        case ND_WHILE: {
            ...
            emit8(0x0F); emit8(0x84); int end_pos = code_pos; emit32(0);
            gen(node->then);
            emit8(0xE9); emit32(begin - (code_pos + 5));
            *(uint32_t*)(code + end_pos + 1) = code_pos - (end_pos + 5);
            break;
        }
        ...
        case ND_FOR: {
            ...
            if (node->cond) {
                gen(node->cond);
                emit8(0x48); emit8(0x85); emit8(0xC0);
                emit8(0x0F); emit8(0x84); end_pos = code_pos; emit32(0);
            }
            gen(node->then);
            if (node->inc) gen(node->inc);
            emit8(0xE9); emit32(begin - (code_pos + 5));
            if (node->cond) *(uint32_t*)(code + end_pos + 1) = code_pos - (end_pos + 5);
            break;
        }
        ...
        case ND_EXPR_STMT:
            gen(node->lhs);
            emit8(0x50); emit8(0x58); // discard value
            break;
        ...
        case ND_FUNCALL: {
            if (!strcmp(node->name, "exit")) {
                // builtin exit
                Node *arg = node->args;
                if (!arg || arg->next) error_at(current_input, "exit takes one arg");
                gen(arg);
                // now rax = code
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "o:")) != -1) {
        switch (opt) {
            case 'o':
                ...
                break;
            default:
                fprintf(stderr, "Usage: %s [-o output] input_files...\n", argv[0]);
                return 1;
        }
    }
    if (optind >= argc) {
        fprintf(stderr, "Expected input files\n");
        return 1;
    }
    ...
    ehdr.e_entry = 0x400000; // Entry point to start of code

    // Initialize the program header
    Elf64_Phdr phdr = {0};
    phdr.p_type = PT_LOAD;
    phdr.p_flags = PF_R | PF_W | PF_X; // Read, write, execute
    phdr.p_offset = sizeof(ehdr) + sizeof(phdr); // Offset in the file
    phdr.p_vaddr = 0x400000; // Virtual address
    phdr.p_paddr = 0; // Physical address (not used)
    phdr.p_filesz = code_pos; // Size of the segment in the file
    phdr.p_memsz = code_pos; // Size of the segment in memory
