...
public:
    ...
    Crawler(const QUrl &start, int maxDepth, int maxPages, bool followExternal, int politenessMs, QObject *parent = nullptr)
        : QObject(parent), maxDepth(maxDepth), maxPages(maxPages),
...
private:
    ...
    void enqueueResolved(const QUrl &base, int depth) {
        ...
        if (!visitedUrls.contains(s) && !enqueuedUrls.contains(s)) {
            ...
            // keep expected depth for reference
    ...
    QHash<QString,int> visitedDepths; // map url -> depth
    QUrl currentBase;
