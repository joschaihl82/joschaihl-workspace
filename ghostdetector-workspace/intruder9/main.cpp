// intruder.cpp
// Single-file Qt5 + OpenCV intruder detector (10x STRONGER GHOSTS, NO PHOTO SAVING)
...
public:
    ...
    {
        setWindowTitle("Intruder Detector: 10x GHOST POWER - Live Monitor");
        ...
        // QDir().mkpath("intruders"); // Photo directory creation removed
...
private:
    ...
    Mat applyRadiationFilter(const Mat& inputFrame) {
        ...
        // 4. Final Blend: 10x Ghost Power
        Mat finalBlendedOutput;
        // Background (outputFrame) 0.3 + Ghost Layer (transparentHullLayer) 1.0
        addWeighted(outputFrame, 0.3, transparentHullLayer, 1.0, 0.0, finalBlendedOutput);
...
private slots:
    void processFrame() {
        ...
        if (motionDetected || largeMotion) {
            if (!faceCascade.empty()) {
                // Aggressive detection only runs on motion
        ...
        for (const Rect &r : faces) {
            // Draw red box on the high-contrast display frame
            ...
            // Removed photo saving logic here
        }
    ...
    // Removed saveIntruderPhoto function entirely
...
private:
    ...
    int minMotionArea;
    bool motionDetected;
    int frameCounter = 0; // savingIntruder variable removed
};
