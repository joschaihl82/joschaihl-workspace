...
// NEUER FIX: Rekursive Suche nach allen VARDECLs innerhalb des Funktionskörpers (inkl. verschachtelter Blöcke)
    ...
    case ND_ARRAY_LITERAL: {
        ...
        if (n->kv_count > 0) {
            ...
            if (n->kind == ND_OBJ_LITERAL) {
                 ...
                 // Da wir nur top-level-Initialisierungen von Literalen unterstützen, sollte dieser Teil nie für namenlose Literale aufgerufen werden.
...
static void cg_emit_stmt(CodeGen *cg, Node *n) {
    ...
    case ND_PROGRAM: {
        ...
        // FIX: Stack Alignment für top-level Calls
        // %rbp wurde gepusht (8 Byte). Um 16-Byte-Alignment für Calls zu garantieren,
        // subtrahieren wir weitere 8 Byte.
