...
void init_v4l2(int fd, int width, int height, struct buffer **out_bufs, int *out_n) {
    ...
    if (!bufs) { perror("calloc bufs"); exit(1); }
...
/* Simple gradients (Sobel-like) */
void compute_grad(const uint8_t *gray, int w, int h, float *gx, float *gy) {
    for (int y = 1; y < h-1; ++y) {
        for (int x = 1; x < w-1; ++x) {
            int i = y*w + x;
            float dx = -gray[i-w-1] - 2*gray[i-1] - gray[i+w-1]
                       + gray[i-w+1] + 2*gray[i+1] + gray[i+w+1];
            float dy = -gray[i-w-1] - 2*gray[i-w] - gray[i-w+1]
...
/* Compute Shi–Tomasi corner score per pixel using gradients */
void compute_shi_tomasi_scores(const float *gx, const float *gy,
                               int w, int h, int win, float *score) {
    int half = win/2;
    int wh = w*h;
    for (int i = 0; i < wh; ++i) score[i] = 0.0f;
    ...
    for (int y = half; y < h-half; ++y) {
        for (int x = half; x < w-half; ++x) {
            ...
            float tr = a + c;
            float det = a*c - b*b;
            float tmp = tr*tr*0.25f - det;
            if (tmp < 0.0f) tmp = 0.0f;
            float s = sqrtf(tmp);
            float l1 = 0.5f*tr + s;
            float l2 = 0.5f*tr - s;
            float lambda_min = (l1 < l2) ? l1 : l2;
            score[y*w + x] = lambda_min;
        }
    }
}

/* Simple non-max suppression + quality threshold */
int select_good_features(const float *score, int w, int h,
                         float quality, int min_dist,
                         Pt *out_pts, int max_pts) {
    ...
    float thresh = quality * max_score;
    int count = 0;
        ...
        for (int x = 1; x < w-1; ++x) {
            ...
            for (int dy = -1; dy <= 1 && is_max; ++dy) {
                for (int dx = -1; dx <= 1; ++dx) {
                    ...
                    if (score[yy*w + xx] >= s) { is_max = 0; break; }
                }
            }
            if (!is_max) continue;

            /* distance to already chosen points */
            int ok = 1;
            for (int k = 0; k < count; ++k) {
                float dx = out_pts[k].x - (float)x;
                float dy = out_pts[k].y - (float)y;
                if (dx*dx + dy*dy < min_dist2) { ok = 0; break; }
            }
            if (!ok) continue;

            out_pts[count].x = (float)x;
            out_pts[count].y = (float)y;
            count++;
            if (count >= max_pts) return count;
        }
    }
    return count;
}
...
void draw_circle(uint8_t *rgb, int w, int h, int cx, int cy, int r,
...
int main(int argc, char **argv) {
    if (argc < 4) {
        fprintf(stderr, "Usage: %s /dev/video0 width height\n", argv[0]);
        return 1;
    }
    ...
    SDL_Window *win = SDL_CreateWindow("Simple Face Track",
    ...
    float *corner_score = calloc(width*height, sizeof(float));
    if (!rgb || !gray_prev || !gray_cur || !gx || !gy || !corner_score) {
    ...
    printf("Controls:\n");
    printf("  Left-click: place points manually (before init)\n");
    printf("  a: auto-detect good features (Shi–Tomasi) before init\n");
    printf("  i: initialize tracking with current points\n");
    ...
    while (1) {
        ...
        if (!initialized) {
            /* show frame and allow clicks / auto-detect */
            /* draw existing (manual or auto) points in red */
            for (int i = 0; i < placed; ++i) {
                draw_circle(rgb, width, height,
                            (int)pts[i].x, (int)pts[i].y, 4, 255,0,0);
            }
            ...
            while (SDL_PollEvent(&e)) {
                ...
                if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
                    ...
                    if (!pts || !pts_prev || !pts_next) {
                        fprintf(stderr, "realloc failed\n");
                        goto done;
                    }
                    pts[P-1].x = (float)e.button.x;
                    pts[P-1].y = (float)e.button.y;
                    placed = P;
                ...
                if (e.type == SDL_KEYDOWN) {
                    if (e.key.keysym.sym == SDLK_i) {
                        if (P > 0) {
                            ...
                            printf("Place or auto-detect at least one point before initializing.\n");
                        }
                    } else if (e.key.keysym.sym == SDLK_q) {
                        goto done;
                    } else if (e.key.keysym.sym == SDLK_r) {
                        free(pts); free(pts_prev); free(pts_next);
                        pts = pts_prev = pts_next = NULL;
                        P = 0; placed = 0; initialized = 0;
                        printf("Reset points.\n");
                    } else if (e.key.keysym.sym == SDLK_a) {
                        /* Auto-detect good features using Shi–Tomasi */
                        printf("Auto-detecting good features...\n");
                        compute_grad(gray_cur, width, height, gx, gy);
                        compute_shi_tomasi_scores(gx, gy, width, height, 7, corner_score);

                        Pt tmp_pts[MAX_POINTS];
                        int newP = select_good_features(corner_score, width, height,
                                                        0.01f, 8, tmp_pts, MAX_POINTS);
                        if (newP <= 0) {
                            printf("No good features found.\n");
                        } else {
                            free(pts); free(pts_prev); free(pts_next);
                            pts = malloc(sizeof(Pt)*newP);
                            pts_prev = malloc(sizeof(Pt)*newP);
                            pts_next = malloc(sizeof(Pt)*newP);
                            if (!pts || !pts_prev || !pts_next) {
        ...
        lk_track(gray_prev, gray_cur, gx, gy, width, height,
                 pts_prev, pts_next, P, 9);
        ...
        for (int i = 0; i < P; ++i) {
            ...
            printf(" (%.2f,%.2f)", dx, dy);
            draw_circle(rgb, width, height,
                        (int)pts_next[i].x, (int)pts_next[i].y, 3, 0,255,0);
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_KEYDOWN) {
                ...
                if (e.key.keysym.sym == SDLK_r) {
                    ...
                    pts = pts_prev = pts_next = NULL;
                    P = 0; placed = 0; initialized = 0;
                    printf("Reset points. Back to placement mode.\n");
                }
            }
        }
    }
    ...
    // NOTE: unmap of vbufs[i].start is omitted as in original
    close(fd);
