// editor_refactored_scroll.c
// gcc -O2 -o editor_refactored_scroll editor_refactored_scroll.c -lSDL2 -lSDL2_ttf
// Refaktorierte Version mit automatischem Runterscrollen bei Newline
...
static int create_ui(UI *ui, const char *font_path) {
    ui->win = SDL_CreateWindow("Editor Refactored Scroll on Newline", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WINW, WINH, SDL_WINDOW_RESIZABLE);
    if (!ui->win) return 0;
...
/* ---------------------------
   Sichtbarkeit des Carets sicherstellen
   --------------------------- */
static void ensure_caret_visible(Doc *doc, UI *ui, int cy, int *scroll_y) {
    ...
    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_QUIT) { running = 0; break; }
            ...
            if (k == SDLK_ESCAPE) { running = 0; break; }
                ...
                (*cy)++; *cx = 0;
                // **Neu:** nach Newline automatisch Caret sichtbar machen (runterscrollen)
                ensure_caret_visible(doc, ui, *cy, scroll_y);
            } else if (k == SDLK_LEFT) {
                if (*cx > 0) (*cx)--;
