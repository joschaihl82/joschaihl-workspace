// proxy_check.c
// Reads data.txt and checks protocol-level latency for proxies with prefixes:
// socks4://, socks5://, http://, https://, or host:port (defaults to http).
// Compile: gcc -O2 -Wall -pthread -o proxy_check proxy_check.c
...
static void print_result(const char *host, const char *port, const char *msg) {
...
static void test_proxy(job_t *j) {
    ...
    if (tcp_connect_to(j->proxy_host, j->proxy_port, timeout_ms, &sock, &t_tcp_start, &t_tcp_end) != 0) {
        snprintf(outbuf, sizeof(outbuf), "TCP_ERR %s", strerror(errno));
        print_result(j->proxy_host, j->proxy_port, outbuf);
        return;
    }
    ...
    if (j->mode == MODE_HTTP || j->mode == MODE_HTTPS) {
        if (do_http_connect(sock, t_host, t_port, timeout_ms) == 0) {
            ...
            snprintf(outbuf, sizeof(outbuf), "TCP %ldms\tPROTO_ERR http-connect", tcp_ms);
            //print_result(j->proxy_host, j->proxy_port, outbuf);
            close(sock);
            return;
        }
        ...
        if (do_socks4(sock, t_host, t_port, timeout_ms) == 0) {
            ...
            snprintf(outbuf, sizeof(outbuf), "TCP %ldms\tPROTO_ERR socks4", tcp_ms);
            //print_result(j->proxy_host, j->proxy_port, outbuf);
            close(sock);
            return;
        }
        ...
        if (do_socks5(sock, t_host, t_port, timeout_ms) == 0) {
            ...
            snprintf(outbuf, sizeof(outbuf), "TCP %ldms\tPROTO_ERR socks5", tcp_ms);
            //print_result(j->proxy_host, j->proxy_port, outbuf);
            close(sock);
            return;
        }
    ...
    if (proto_ok) {
        snprintf(outbuf, sizeof(outbuf), "TCP %ldms\tPROTO %ldms", tcp_ms, proto_ms);
        print_result(j->proxy_host, j->proxy_port, outbuf);
    } else {
        snprintf(outbuf, sizeof(outbuf), "TCP %ldms\tPROTO_ERR", tcp_ms);
        //print_result(j->proxy_host, j->proxy_port, outbuf);
    }
    close(sock);
}
...
static int parse_line_to_job(char *line, job_t *out) {
    ...
    // else no prefix -> default http

    // now p should be host:port
