...
static void mask_center_of_mass(uint8_t *mask_in, int *cx, int *cy){
    ...
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            if(mask_in[y*W + x]){ sx += x; sy += y; cnt++; }
        }
    }
    if(cnt < 50){ *cx = W/2; *cy = H/2; }
    else { *cx = (int)(sx / cnt); *cy = (int)(sy / cnt); }
}
...
///////////////////// Preprocessing (ROI) //////////////////////
static void box_blur_roi(const float *src, float *dst, int w, int h){
    // 3x3 Box-Blur
    for(int y=1;y<h-1;y++){
        for(int x=1;x<w-1;x++){
            float s=0;
    ...
    // Kopiere Ränder
    for(int x=0;x<w;x++){ dst[0*w+x]=src[0*w+x]; dst[(h-1)*w+x]=src[(h-1)*w+x]; }
    for(int y=0;y<h;y++){ dst[y*w+0]=src[y*w+0]; dst[y*w+(w-1)]=src[y*w+(w-1)]; }
}

static void sobel_mag_roi(const float *Y, float *G, int w, int h){
    for(int y=1;y<h-1;y++){
        for(int x=1;x<w-1;x++){
            float gx =
...
///////////////////// Pupil-focused eye detection //////////////////////
static void detect_pupils(uint8_t *rgb, int cx, int cy){
    // ROI bounds
    ...
    box_blur_roi(Y, Yb, Rw, Rh);
    sobel_mag_roi(Yb, G, Rw, Rh);

    // Eyes region (upper third of ROI)
    int eyes_y0 = 0;
    int eyes_y1 = ROI_SIZE/3;

    // Global stats in eyes region for adaptive thresholds
    double sumY=0, sumY2=0; int cnt=0;
    for(int y=eyes_y0; y<eyes_y1; y++){
        for(int x=0; x<Rw; x++){
            float v = Yb[y*Rw + x];
            sumY += v; sumY2 += v*v; cnt++;
        }
    }
    ...
    // Thresholds
    float core_thr = meanY - 1.0f*stdY;          // core must be darker than this
    float ring_thr = meanY + 0.6f*stdY;          // ring brighter than this
    float cont_gain = 0.9f;                      // gradient weight

    // Radii tuned for ROI=160 at 640x480; scale if needed
    int r_core = 3;
    int r_ring_in = 5;
    int r_ring_out = 9;
    int r_contour = 7;
    ...
    for(int y=eyes_y0 + r_ring_out; y<=eyes_y1 - r_ring_out; y++){
        for(int x=r_ring_out; x<=Rw - r_ring_out; x++){
            // Core mean (dark requirement)
            double core_sum=0; int core_cnt=0;
            for(int a=-r_core; a<=r_core; a++){
                for(int b=-r_core; b<=r_core; b++){
                    if(a*a + b*b <= r_core*r_core){
                        core_sum += Yb[(y+b)*Rw + (x+a)];
                        core_cnt++;
                    }
                }
            }
            float core_mean = (float)(core_sum / (float)core_cnt);
            if(core_mean > core_thr) continue; // not dark enough

            // Glint suppression: reject if any very bright pixel near center
            int glint = 0;
            for(int a=-2; a<=2 && !glint; a++){
                for(int b=-2; b<=2; b++){
                    float val = Yb[(y+b)*Rw + (x+a)];
                    if(val > meanY + 1.5f*stdY){ glint = 1; break; }
                }
            }
            if(glint) continue;

            // Ring mean (bright requirement)
            ...
            for(int a=-r_ring_out; a<=r_ring_out; a++){
                for(int b=-r_ring_out; b<=r_ring_out; b++){
                    ...
                    if(rr >= r_ring_in*r_ring_in && rr <= r_ring_out*r_ring_out){
                        ring_sum += Yb[(y+b)*Rw + (x+a)];
                        ring_cnt++;
                    }
                }
            }
            float ring_mean = (float)(ring_sum / (float)ring_cnt);
            if(ring_mean < ring_thr) continue; // not bright enough ring

            // Contour gradient along ring radius
            ...
            float cont_mean = (float)(cont_sum / (float)cont_cnt);

            // Score: darker core + strong contour + brighter ring -> lower score
            ...
            if(score < best1.score){
                ...
                if(best1.x<0 || (dx*dx + dy*dy) > (r_ring_out*r_ring_out*3)) {
    ...
    // Validate pair: similar y, plausible horizontal distance
    if(best1.x>=0 && best2.x>=0){
        ...
        if(dy <= r_ring_out*2 && dx >= min_dx && dx <= max_dx){
            // Draw: black core + white ring + thin blue contour
            int cx1 = rx + best1.x, cy1 = ry + best1.y;
            int cx2 = rx + best2.x, cy2 = ry + best2.y;

            draw_circle_rgb(rgb, cx1, cy1, r_core, 0, 0, 0);
            draw_ring_rgb(rgb, cx1, cy1, r_ring_in, r_ring_out, 255, 255, 255);
            draw_ring_rgb(rgb, cx1, cy1, r_ring_out+1, r_ring_out+2, 0, 128, 255);

            draw_circle_rgb(rgb, cx2, cy2, r_core, 0, 0, 0);
            draw_ring_rgb(rgb, cx2, cy2, r_ring_in, r_ring_out, 255, 255, 255);
            draw_ring_rgb(rgb, cx2, cy2, r_ring_out+1, r_ring_out+2, 0, 128, 255);
        }
    }
}

///////////////////// Mouth detection //////////////////////
static void rgb_to_hsv_pixel(uint8_t r, uint8_t g, uint8_t b, float *h, float *s, float *v); // forward
...
int v4l2_init(){
    ...
    fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV; // fallback: V4L2_PIX_FMT_MJPEG
...
int main(){
    ...
    while(running){
        ...
        // Pupillen + Mund erkennen und visualisieren
        detect_pupils(frame_rgb, cx, cy);
        ...
        if(stable_count >= STABLE_FRAMES_REQ){
            ...
            if(stable_update_counter % UPDATE_EVERY_N == 0){
                ...
                skin.mu_h = new_mu_h; skin.var_h = new_var_h;
                skin.mu_s = new_mu_s; skin.var_s = new_var_s;
