// crawler.cpp
// Amalgamated crawler core + Qt GUI (single file, logically separated)
// Updated: uses QTextBrowser for HTML preview (no Qt WebEngine dependency).
// This version avoids WebEngine so it links with standard Qt Widgets only.
...
private slots:
    ...
    void onHtmlFetched(QString url, QString html) {
        // Render HTML in the QTextBrowser. Note: QTextBrowser supports HTML/CSS subset (no JS).
