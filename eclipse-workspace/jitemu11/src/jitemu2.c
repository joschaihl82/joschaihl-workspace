/*
 ...
 - Filters heuristic false positives.
 - Minimal SystemTable/BootServices/ConOut wiring with OutputString and host service stubs.
 - FIX 1: Added EFI_TABLE_HEADER signatures to fix -1 termination issue.
 - FIX 2: Added more essential Boot Services stubs (ExitBootServices, GetMemoryMap) to prevent crash/hang during C runtime initialization.
...
/* (omitted read_file, alloc_executable, free_executable, validate_section, find_section_by_name, heuristic_discover) */
                                          ...
                                          uint64_t* out_entry_rva)
{
    ...
    if (named_count > 0) {
        ...
        if (nsec > 0 && nsec <= 1024 && (size_t)(sec_table - file) + need <= fsz) {
            ...
            int discovered = 0;
            /* use the optional header start as the start of the heuristic scan */
            sections = heuristic_discover(file, (size_t)(opt_hdr - file), fsz, &discovered, headers_limit);
    ...
    /* fix: use the correct type cast to match the signature of the entry_ptr argument */
...
static uint64_t EFIAPI host_ExitBootServices(void* image_handle, uint64_t map_key) {
    (void)image_handle; (void)map_key;
    fprintf(stderr, "\n[host_stub] called ExitBootServices. execution complete.\n");
    fflush(stderr);
    /* typically we would stop the emulation here, but we return EFI_SUCCESS */
    return EFI_SUCCESS;
}

/* GetMemoryMap stub */
static uint64_t EFIAPI host_GetMemoryMap(
    ...
    fprintf(stderr, "[host_stub] called GetMemoryMap. returning EFI_UNSUPPORTED.\n");
    fflush(stderr);
    *map_key = 0x1337;
    *descriptor_size = 0x30; /* common size */
    *descriptor_version = 1;
    if (*memory_map_size == 0) {
        /* image is querying for size; tell it we need space */
        *memory_map_size = 128 * 1024; /* return a large size estimate */
        return EFI_SUCCESS; /* should be EFI_BUFFER_TOO_SMALL in real EFI, but we try success */
    }
    /* if it's actually trying to write, we return unsupported */
...
static uint64_t EFIAPI host_OutputString(void* this, const uint16_t* str) {
    ...
    char buf[2048];
    size_t bi = 0;
    /* minimal utf-16 to ascii conversion */
    for (const uint16_t* p = str; *p != 0; ++p) {
        ...
        if (ch < 0x80) {
            ...
            if (bi + 1 >= sizeof(buf)) break;
            buf[bi++] = '?'; /* rplease implement an x64-jit-emulator which translates an .efi file into an runnable memory segment which can be calledeplace non-ascii with '?' */
        }
    }
    if (bi > 0) {
        /* efi output is typically written to stdout/stderr */
...
/* minimal placeholder stub for any unhandled service call */
static uint64_t EFIAPI host_unsupported_stub(void* ptr, ...) {
    (void)ptr;
    fprintf(stderr, "[host_stub] called unsupported boot service at %p. returning EFI_UNSUPPORTED (3).\n", __builtin_return_address(0));
    fflush(stderr);
    return EFI_UNSUPPORTED;
}
...
static int build_fake_system_table(void** out_st, void** out_bs, void** out_conout) {
    ...
    /* standard sizes for system table (0x70) and boot services (0x100), padded */
    size_t st_size = 0x200, bs_size = 0x400, conout_size = 0x100;
    ...
    /* 1. system table header (offset 0) */
    uint64_t st_sig = EFI_SYSTEM_TABLE_SIGNATURE;
    uint32_t st_hdr_sz = 0x70; /* typical efi system table size */
    memcpy(st + 0x00, &st_sig, sizeof(st_sig));
    memcpy(st + 0x0C, &st_hdr_sz, sizeof(st_hdr_sz));

    /* 2. boot services table header (offset 0) */
    uint64_t bs_sig = EFI_BOOT_SERVICES_SIGNATURE;
    uint32_t bs_hdr_sz = 0x100; /* typical boot services table size */
    memcpy(bs + 0x00, &bs_sig, sizeof(bs_sig));
    memcpy(bs + 0x0C, &bs_hdr_sz, sizeof(bs_hdr_sz));

    /* 3. systemtable->bootservices at +0x30 */
    uint64_t bs_ptr = (uint64_t)(uintptr_t)bs;
    memcpy(st + 0x30, &bs_ptr, sizeof(bs_ptr));

    /* 4. systemtable->conout variants (populate a few common offsets) */
    uint64_t conout_ptr = (uint64_t)(uintptr_t)conout;
    memcpy(st + 0x18, &conout_ptr, sizeof(conout_ptr)); /* consoleout handle */

    /* 5. bootservices table function pointers (starting at bs + 0x18)
       EFI_BOOT_SERVICES structure definition (approximate relevant entries):
       0x18: EFI_RAISE_TPL
       0x20: EFI_RESTORE_TPL
       0x28: EFI_ALLOCATE_PAGES (stub)
       0x30: EFI_FREE_PAGES (stub)
       0x38: EFI_GET_MEMORY_MAP (stub)
       ...
       0x98: EFI_HANDLE_PROTOCOL (stub)
       ...
       0x108: EFI_EXIT_BOOT_SERVICES (stub)
    */
    uint64_t stub_unsupported = (uint64_t)(uintptr_t)host_unsupported_stub;
    uint64_t stub_gmm = (uint64_t)(uintptr_t)host_GetMemoryMap;
    uint64_t stub_exit = (uint64_t)(uintptr_t)host_ExitBootServices;

    /* fill all services with the unsupported stub initially */
    for (size_t i = 0x18; i < bs_size - 8; i += 8) {
        memcpy(bs + i, &stub_unsupported, 8);
    }

    /* map specific critical stubs */
    memcpy(bs + 0x38, &stub_gmm, 8);   /* EFI_GET_MEMORY_MAP */
    memcpy(bs + 0x108, &stub_exit, 8); /* EFI_EXIT_BOOT_SERVICES */
    /* other stubs like AllocatePages (0x28), FreePages (0x30), HandleProtocol (0x98) are now stub_unsupported */

    /* 6. conout->outputstring = host_outputstring at offset 0x18 (after protocol header) */
    uint64_t outstr_ptr = (uint64_t)(uintptr_t)host_OutputString;
    /* protocol header is 0x18 bytes, OutputString is the first function pointer */
    memcpy(conout + 0x18, &outstr_ptr, sizeof(outstr_ptr));
