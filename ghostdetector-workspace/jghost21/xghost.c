...
typedef struct {
    // Basis-Gains (werden optimiert: nur sat_base und val_base adaptiert)
    double hue_base;
    double sat_base;
    double val_base;
    double tint_base;

    // Dither-Amplituden und -Frequenzen (sinusförmig, nur als Anregung beibehalten)
...
static void optshared_init(OptShared *os) {
    ...
    // Dither: kleine sinusförmige Anregung (bleibt für den visuellen Effekt)
    ...
    os->target_sat = 12000.0;
    os->target_val = 45000.0;
...
// OPTIMIERTE SAMPLING-FUNKTION (Nearest Neighbor, wenn Auflösungen gleich, sonst NN-Skalierung)
static HSV64 rgb_to_hsv64_scaled_optimized(const unsigned char *src, int srcW, int srcH, int dstW, int dstH, int dst_x, int dst_y) {
    int x0, y0;
    ...
    if (srcW == dstW && srcH == dstH) {
        ...
        x0 = (int)clampi(round((double)dst_x * srcW / dstW), 0, srcW - 1);
        y0 = (int)clampi(round((double)dst_y * srcH / dstH), 0, srcH - 1);
    }
    
    const unsigned char *p = src + (y0 * srcW + x0) * 3;
    return rgb8_to_hsv64(p[0], p[1], p[2]);
}
...
static void* filter_worker_thread(void *arg) {
    ...
    // Hier verwenden wir die Basiswerte und das Dither-Signal
    ...
    // Layer-spezifische Parameter (AUSSERHALB des inneren Loops berechnen)
    ...
    unsigned short alpha_v = (unsigned short)clampi((int)llround((18000.0 + 12000.0 * fabs(sin(0.3 * t + g))) * alpha_scale), 0, 65535);
    unsigned short alpha_v_40 = (unsigned short)llround(alpha_v * 0.4);
    ...
    for (int y = data->start_y; y < data->end_y; ++y) {
        ...
        // V-Modulation (Y-abhängig)
        double val_modulation = V_SHIFT + val_gain * sin(0.5 * t + ny * two_pi);
        
        for (int x = 0; x < dstW; ++x) {
            double nx = (double)x / (double)dstW;

            // Sinus-Berechnung der X-Komponente
            double s1 = sin(two_pi * (freqx * nx + phase));
            double s3 = sin(two_pi * (1.7 * freqx * nx + 1.0 * phase + 0.2 * g));
            double comb = s1 + 0.6 * s2 + 0.4 * s3;

            // Bild-Sample (OPTIMIERTE Funktion: Nearest Neighbor/Skalierung)
            HSV64 sample = rgb_to_hsv64_scaled_optimized(srcRGB, srcW, srcH, dstW, dstH, x, y);
            ...
            // Verwendung der Y-abhängigen V-Modulation
            int v_mod = clampi((int)sample.v + (int)llround(val_modulation), 0, 65535); 
            ...
            data->acc_chunk[acc_index] = hsv64_blend_add(data->acc_chunk[acc_index], mod, alpha_v);
            data->acc_chunk[acc_index] = hsv64_blend_add(data->acc_chunk[acc_index], tint, alpha_v_40);
...
static void* optimizer_thread(void *arg) {
    ...
    // NEU: Einfache Gradienten-Schätzung
    double gradient_sat = 0.0;
    double gradient_val = 0.0;
    ...
    const double sat_min = 500.0,  sat_max = 2000.0;
    const double val_min = 30000.0, val_max = 40000.0;
    const double tint_min= 0.1,    tint_max= 0.4;
    ...
    while (1) {
        ...
        // NEU: Vereinfachter, direkter Gradienten-Abstieg (Proportional zum Fehler)
        // Ziel: Verringere sat_base/val_base, wenn der Messwert zu hoch ist (e_sat/e_val > 0)
        gradient_sat = e_sat; 
        gradient_val = e_val;
        
        // Gradient-Abstieg auf Basis-Gains (Nur Saturation und Value)
        os->sat_base  -= lr_sat  * gradient_sat;
        os->val_base  -= lr_val  * gradient_val;
        
        // ANMERKUNG: Hue und Tint werden NICHT mehr dynamisch angepasst.
        // Ihre Werte bleiben auf den initialen Werten, es sei denn, man führt einen anderen
        // stabilen Optimierungsmechanismus ein.
...
int main(void) {
    ...
    XStoreName(dpy, win, "Live /dev/video0 + Ghost Sine + Neon Ghost Boxes (Optimized)");
    ...
    unsigned char *prev_rgb = malloc(V4L2_W*V4L2_H*3);
    unsigned char *gray = malloc(V4L2_W*V4L2_H);
    unsigned char *edges = malloc(V4L2_W*V4L2_H);
    unsigned char *prev_edges = calloc(V4L2_W*V4L2_H, 1);
    unsigned int *argb = (unsigned int*)xi->data;
    
    memset(prev_rgb, 0, V4L2_W*V4L2_H*3); 

    // Die Zeitstruktur ts wird nur für die V4L2-Timeout-Pause verwendet.
        ...
        if (ioctl(fd, VIDIOC_DQBUF, &buf) == 0) {
            ...
            for (int i = 0; i < NUM_THREADS; ++i) {
                ...
                thread_data[i] = (ThreadData){
                    ...
                    .start_y = current_y, .end_y = current_y + block_size,
                    .edge_image_chunk = edges,
                    .prev_edge_image_chunk = prev_edges,
                    .dst_argb_chunk = argb
                };
            ...
            double elapsed_sec = (end_time.tv_sec - start_time.tv_sec) + (end_time.tv_nsec - start_time.tv_nsec) / 1000000000.0;
            total_time += elapsed_sec;
            frame_count++;

            if (frame_count % 30 == 0) { // Ausgabe alle 30 Frames
                double current_fps = 1.0 / elapsed_sec;
                double avg_fps = frame_count / total_time;
                pthread_mutex_lock(&os.mtx);
                double current_J = os.J;
                pthread_mutex_unlock(&os.mtx);
                // Gibt die FPS-Zahlen aus und aktualisiert die Zeile
                printf("FPS (Current): %.1f | FPS (Average): %.1f | J: %.2f\r", current_fps, avg_fps, current_J);
                fflush(stdout);
            }
            
            t += elapsed_sec; 
        } else {
            // Kurze Pause, falls kein Frame von V4L2 verfügbar (CPU-Entlastung)
