/*
 * 711cc.c - Mini C compiler (extended)
 *
 * Features:
 * - Tokenizer with multi-char operators and string literals
 * - Parser: expressions, statements, function definitions, declarations
 * - Types: int, pointers, arrays, functions, struct (basic) and enums
 * - AST nodes including member access and ++/-- (pre/post)
 * - Codegen: x86-64 machine code emission (push/pop model)
 * - LEA/ModR-M/REX helpers, call placeholders and patching
 * - Simple ELF writer (writes code at file offset 0x1000, vaddr 0x401000)
 * - Improved error handling and debugging information
 * - Support for enum types and basic type checking
 *
 * Limitations:
 * - Parser and type system are minimal and simplified
 * - No external symbol linking; only internal functions supported
 * - Many C features not implemented (floating point, full declarations, etc.)
 *
 * Use at your own risk; intended as an educational toy.
 */
    ...
    switch (node->kind) {
        ...
        case ND_FUNCALL: {
            ...
            for (int i = 0; i < to_pop; i++) {
                switch (i) {
                    case 0: emit8(0x5F); break; // pop rdi
                    case 1: emit8(0x5E); break; // pop rsi
                    case 2: emit8(0x5A); break; // pop rdx
                    case 3: emit8(0x59); break; // pop rcx
                    case 4: emit8(0x41); emit8(0x58); break; // pop r8
                    case 5: emit8(0x41); emit8(0x59); break; // pop r9
                }
            }
        ...
        case ND_PREINC: {
            if (!(node->lhs->kind == ND_LVAR || node->lhs->kind == ND_DEREF || node->lhs->kind == ND_MEMBER))
                error_at(current_input, "lvalue required for ++");
            gen_addr(node->lhs);
            emit8(0x5F); // pop rdi
            emit_rex(1, 0, 0, 0);
            emit8(0x8B); emit_modrm(0x0, 0, 7); // mov rax, [rdi]
            emit8(0x48); emit8(0x83); emit8(0xC0); emit8(0x01); // add rax,1
            emit_rex(1, 0, 0, 0);
            emit8(0x89); emit_modrm(0x0, 0, 7); // mov [rdi], rax
            emit8(0x50); // push rax
            break;
        }
        ...
        case ND_PREDEC: {
            ...
            emit8(0x48); emit8(0x83); emit8(0xE8); emit8(0x01); // sub rax,1
            emit_rex(1, 0, 0, 0);
        ...
        case ND_POSTINC: {
            ...
            emit8(0x48); emit8(0x83); emit8(0xC2); emit8(0x01); // add rdx,1
            emit_rex(1, 0, 0, 0);
            emit8(0x89); emit_modrm(0x0, 2, 7); // mov [rdi], rdx
            emit8(0x50); // push rax (original)
            break;
        }
        ...
        case ND_POSTDEC: {
            ...
            emit8(0x8B); emit_modrm(0x0, 0, 7); // mov rax, [rdi]
            emit8(0x48); emit8(0x89); emit8(0xC2); // mov rdx, rax
            emit8(0x48); emit8(0x83); emit8(0xEA); emit8(0x01); // sub rdx,1
            emit_rex(1, 0, 0, 0);
            emit8(0x89); emit8(0xC0); // mov rax, rax (no-op)
            emit_rex(1, 0, 0, 0);
            emit8(0x89); emit_modrm(0x0, 2, 7); // mov [rdi], rdx
            emit8(0x50); // push rax (original)
            break;
...
static void write_elf(const char *filename, uint8_t *codebuf, size_t code_size) {
    ...
    Elf64_Ehdr ehdr = {
        ...
        .e_entry = 0x400000 + 0x1000, // entry at code segment start
