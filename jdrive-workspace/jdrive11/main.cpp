...
//
// NOTE:
// This file is a complete, self-contained patch of your original main.cpp.
// Key OAuth fix: use requestedScopeTokens property and ensure 'scope' is present
// in the authorization request by adding it in setModifyParametersFunction if Qt omits it.
// This avoids deprecated setScope() calls and the Google error "Missing required parameter: scope".
//
        ...
        for (const QJsonValue &v : queue) {
            ...
            connect(reply, &QNetworkReply::readyRead, this, [&]() {
                ...
                if (!chunk.isEmpty()) {
                    ...
                    emit fileProgress(name, downloadedBytes, totalBytes);
            ...
            if (!okSize) {
                QFile f(outPath);
                if (f.exists()) downloadedBytes += f.size();
            }
...
private:
    ...
    void setupOAuth() {
        ...
        // Preferred modern API: requestedScopeTokens property (Qt expects this)
        ...
        // Some Qt builds still omit 'scope' from the authorization URL.
        // Ensure 'scope' is present by adding it in modifyParametersFunction if missing.
        ...
        // Debug: show the authorization URL so you can inspect query parameters (including scope)
    ...
    QString buildPathForId(const QString &id) {
        if (!idIndex.contains(id)) return QString();
        QJsonObject meta = idIndex.value(id);
        return meta.value("name").toString();
    }
