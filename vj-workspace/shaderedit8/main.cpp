...
// - Audio input via Qt6 QAudioSource / QMediaDevices (no ALSA)
...
// Build: link Qt6::Widgets Qt6::OpenGLWidgets Qt6::Multimedia
...
public:
	explicit QtAudioInput(unsigned sr, QObject *parent = nullptr) :
			QObject(parent), rate(sr), audioSource(nullptr), audioIO(nullptr), runningAvg(
					0.0f), runningAvgInit(false), lastKickMs(0), glw(nullptr) {
	}
