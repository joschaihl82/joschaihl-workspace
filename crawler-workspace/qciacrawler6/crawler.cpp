// mega_crawler_async_ui_refresh.cpp
...
public:
    MegaCrawler(int workerCount = 8, QWidget *parent = nullptr)
        ...
        // UI-Refresh Timer: ruft regelmäßig updateStats, refreshContacts und dispatch auf
        m_uiTimer = new QTimer(this);
        connect(m_uiTimer, &QTimer::timeout, this, &MegaCrawler::onUiTimerTick);
        m_uiTimer->start(1000); // 1000 ms; anpassen nach Bedarf
    }
...
public slots:
    ...
    void onWorkerFinished(const QString &url) {
        Q_UNUSED(url);
        // leichte, sofortige UI-Aktualisierung
        updateStats();
        refreshContacts();
    }

    // Timer slot: sorgt dafür, dass UI regelmäßig aktualisiert und dispatch aufgerufen wird
    void onUiTimerTick() {
        // UI-Updates sollten kurz und nicht-blockierend sein
        updateStats();
        refreshContacts();
        dispatch();
    }
...
private:
    ...
    void setupUI() {
        ...
        resize(1280, 800);
        setWindowTitle("MegaCrawler - Async Workers + UI Refresh");
    }
