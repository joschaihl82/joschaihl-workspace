...
// - Overlay: Diagnostisches Neuro-Preset (maximale Hervorhebung)
...
// Neuro-Palette / Fokus-Parameter (werden vom diagnostischen Preset verwendet)
...
static int neuro_focus_radius = 150; // diagnostisch: größerer Fokusradius
...
// -----------------------------------------------------------------------------
// overlay_heatmap: diagnostisches Neuro-Preset (maximale Hervorhebung)
// -----------------------------------------------------------------------------

static void overlay_heatmap(uint8_t *rgb, const uint8_t *res, const uint8_t *edge, const uint8_t *flicker) {
  const float edge_weight = 2.0f; // diagnostisch: starke Kantenbetonung
  const int min_score_thresh = 14; // diagnostisch: empfindlicher Schwellenwert
  ...
  for (int i = 0; i < WIDTH*HEIGHT; i++) {
    ...
    if (t < 0.33f) {
      float u = t / 0.33f;
      r = (uint8_t)roundf(NEURO_BASE_R1 * (1.0f - u) + NEURO_BASE_R2 * u * 0.8f);
      g = (uint8_t)roundf(NEURO_BASE_G1 * (1.0f - u) + NEURO_BASE_G2 * u);
      b = (uint8_t)roundf(NEURO_BASE_B1 * (1.0f - u) + NEURO_BASE_B2 * u);
    } else if (t < 0.66f) {
      float u = (t - 0.33f) / 0.33f;
      r = (uint8_t)roundf(NEURO_BASE_R2 * (0.6f * (1.0f - u)) + 100.0f * u);
      g = (uint8_t)roundf(NEURO_BASE_G2 * (1.0f - u) + 220.0f * u);
      b = (uint8_t)roundf(NEURO_BASE_B2 * (1.0f - u) + 230.0f * u);
    } else {
      float u = (t - 0.66f) / 0.34f;
      r = (uint8_t)roundf(100.0f * (1.0f - u) + 255.0f * u);
      g = (uint8_t)roundf(220.0f * (1.0f - u) + 255.0f * u);
      b = (uint8_t)roundf(230.0f * (1.0f - u) + 255.0f * u);
    }

    // Diagnostisches Alpha: stärker, score-abhängig
    float alpha = 0.40f + 0.60f * ((float)s / 255.0f);

    // Fokus-Boost: wenn Pixel in Fokuskreis liegt, erhöhe Alpha deutlich
    int y = i / WIDTH;
    int x = i % WIDTH;
    ...
    if (dist2 < neuro_focus_radius * neuro_focus_radius) {
      ...
      float f = 1.0f - (dist / (float)neuro_focus_radius); // 1.0 in Mitte -> 0.0 am Rand
      alpha += 0.30f * f; // zusätzlicher Boost bis +0.30
...
int main(int argc, char *argv[]) {
  ...
  while (running) {
    ...
    // 6) Heatmap Overlay (diagnostisch)
