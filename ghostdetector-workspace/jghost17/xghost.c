...
static void optshared_init(OptShared *os) {
    ...
    os->hue_base  = 8.0;
    os->sat_base  = 2500.0;
    os->val_base  = 50000.0; // ANGEPASST: Deutlich höherer Basiswert für Helligkeit
    os->tint_base = 0.4;
    ...
    os->target_val = 60000.0; // ANGEPASST: Deutlich höherer Zielwert
...
static void ghost_sine_filter_hsv64(const unsigned char *srcRGB, int srcW, int srcH, unsigned int *dstARGB, int dstW, int dstH, double t, OptShared *os) {
    ...
    FilterThreadData thread_data[NUM_THREADS];
    ...
    for (int g = 0; g < layers; ++g) {
        ...
        // Initialisiere die Threads und teile die Arbeit auf
        int current_y = 0;
        for (int i = 0; i < NUM_THREADS; ++i) {
            int block_size = rows_per_thread + (i < remaining_rows ? 1 : 0);
            
            thread_data[i] = (FilterThreadData){
    ...
    // Wir müssen die Summen-Berechnung über alle Threads hinweg akkumulieren.
    // ACHTUNG: Die Summen im Worker sind aktuell über ALLE Layer akkumuliert,
    // da wir die thread_data-Struktur nicht zurücksetzen. Da wir die Layer-Schleife 
    // seriell ausführen, können wir die Summen der Threads akkumulieren und durch die 
    // Anzahl der Layer teilen.
    for (int i = 0; i < NUM_THREADS; ++i) {
    ...
    FilterThreadData convert_data[NUM_THREADS];
    
    current_y = 0;
    for (int i = 0; i < NUM_THREADS; ++i) {
        int block_size = rows_per_thread + (i < remaining_rows ? 1 : 0);
        
        convert_data[i] = (FilterThreadData){
            .dstARGB = dstARGB, .dstW = dstW, .dstH = dstH,
            .start_y = current_y,
            .end_y = current_y + block_size,
            .acc_chunk = acc,
    ...
    // Wir teilen durch layers, da die Summen pro Pixel in den Layern akkumuliert wurden.
    pthread_mutex_lock(&os->mtx);
...
int main(void) {
    ...
    XStoreName(dpy, win, "Live /dev/video0 + HSV64 + ESC-Optimierung (8 Threads)");
    ...
    while (running) {
        ...
        if (ioctl(fd, VIDIOC_DQBUF, &buf) == 0) {
            ...
            // Filter (Ghost Sine) mit 8 Threads
