...
static void ghost_sine_filter_hsv64(const unsigned char *srcRGB, int srcW, int srcH, unsigned int *dstARGB, int dstW, int dstH, double t, OptShared *os) {
    ...
    // current_y ist hier für den gesamten Funktions-Scope deklariert
    int current_y; 

    // 1. Layer-Schleife (seriell)
    for (int g = 0; g < layers; ++g) {
        
        // Initialisiere current_y für den Start der Thread-Blöcke
        current_y = 0; 
    ...
    // Setze current_y für die Konvertierungs-Threads zurück
    current_y = 0;
        ...
        while (XPending(dpy)) {
            ...
            if (e.type == KeyPress) {
                ...
                if (current_win_W != c_e->width || current_win_H != c_e->height) {
                    ...
                    if (xi && xi->data) {
                        free(xi->data); 
                        // FIX: Setze den Datenpointer auf NULL, um den Double Free durch XDestroyImage zu verhindern.
                        xi->data = NULL; 
                    }
                    if (xi) XDestroyImage(xi);
                    xi = NULL; // Setze xi zurück
