...
int main(void) {
    ...
    // open ALSA PCM
    // Use PulseAudio monitor source via ALSA plugin name "default.monitor"
    int err = snd_pcm_open(&g_pcm, "default.monitor", SND_PCM_STREAM_CAPTURE, 0);
    if (err < 0) {
    ...
    // Request stereo for monitor sources (most monitors are stereo)
    ...
    int16_t *read_buf = (int16_t*) malloc(sizeof(int16_t) * BUFFERSIZE * 2);
    if (!read_buf) {
    ...
    while (running) {
        ...
        // read ALSA (blocking) -- read interleaved stereo frames
        ...
        for (int i = 0; i < frames; ++i) {
            ...
            int32_t r = read_buf[channels * i + 1];
