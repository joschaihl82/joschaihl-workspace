...
static void cg_emit_lvalue_addr(CodeGen *cg, Node *n) {
    if (n->kind == ND_IDENT) {
        ...
        // 2. Basisadresse des Arrays in %rax holen (Adresse des globalen Arrays)
        if (n->target->kind != ND_IDENT) {
        ...
        cg_emit_expr(cg, n->target); // Lädt den Wert (Adresse des ersten Elements) des Arrays in %rax
...
static void cg_emit_expr(CodeGen *cg, Node *n) {
    ...
    case ND_CALL: {
        ...
        // --- PATCH FÜR PUSH-OPERATION ---
        // Muster: arr.push(value)
        if (call->call_target->kind == ND_MEMBER_ACCESS &&
            ...
            char *arr_name = array_ident->name;
            int lid = ++cg->label_id;

            // 1. Lese die aktuelle Größe (Index des neuen Elements) in %r10
            cg_emit(cg, "    mov Lglob_%d(%%rip), %%r10", size_gi); // %r10 = current size/new index

            // 2. Prüfe auf Kapazität
            cg_emit(cg, "    cmp $%d, %%r10", MAX_ARRAY_SIZE);
            cg_emit(cg, "    jge .Lpush_overflow_%d", lid);

            // 3. Berechne den Wert des Ausdrucks (value to push)
            cg_emit_expr(cg, call->args[0]);
            cg_emit(cg, "    push %%rax"); // Wert sichern

            // 4. Lade die Basisadresse des Arrays (%r8 = &arr_0)
            cg_emit_expr(cg, array_ident); // Lädt die Adresse von arr_0 in %rax
            cg_emit(cg, "    mov %%rax, %%r8"); // %r8 = Basisadresse

            // 5. Berechne die Adresse des neuen Slots: &arr_0 + index * 8
            cg_emit(cg, "    pop %%rax");        // %rax = Wert zum Pushen
            cg_emit(cg, "    mov %%r10, %%r9");  // %r9 = %r10 (Index)
            cg_emit(cg, "    shl $3, %%r9");     // index * 8
            cg_emit(cg, "    add %%r8, %%r9");     // %r9 = Adresse des neuen Slots

            // 6. Schreibe den Wert und inkrementiere die Größe
            cg_emit(cg, "    mov %%rax, (%%r9)"); // Wert in den neuen Slot schreiben

            // KORRIGIERTE Inkrementierung: %r10 (Index) ist bereits geladen
            cg_emit(cg, "    inc %%r10");
            // Speichere: Lglob_SIZE = %r10
            cg_emit(cg, "    mov Lglob_%d(%%rip), %%r11", size_gi);
            cg_emit(cg, "    mov %%r10, Lglob_%d(%%rip)", size_gi);

            cg_emit(cg, "    mov $0, %%rax"); // Rückgabewert 0 (void)
            cg_emit(cg, "    jmp .Lpush_end_%d", lid);

            cg_emit(cg, ".Lpush_overflow_%d:", lid);
            // Nutze den Standard-Exit-Call, um einen Fehler zu signalisieren.
...
static void cg_emit_stmt(CodeGen *cg, Node *n) {
    ...
    case ND_VARDECL: {
        if (g_current_func_scope) {
            ...
            if (n->init) {
                ...
                cg_emit(cg, "    movq $0, %d(%%rbp)", lv->offset); // Hier movq ist sicher, obwohl mov auch funktionieren sollte
            }
            return;
        } else {
            // GLOBALE Variable:
            ...
            if (init_kind == ND_OBJ_LITERAL || init_kind == ND_ARRAY_LITERAL) {
                ...
                // 1. Initialisiere geflattete Variablen
                ...
                // 2. *PATCH* Pre-registriere leere Slots für PUSH (nur für Arrays)
                if (init_kind == ND_ARRAY_LITERAL) {
                    for (int i = n->init->kv_count; i < MAX_ARRAY_SIZE; ++i) {
                        char buf[256];
                        snprintf(buf, sizeof(buf), "%s_%d", n->var_name, i);
                        add_global(buf); // Registriere arr_N (wird mit 0 initialisiert)
                    }
                }

                // 3. Setze die Basisvariable auf die Adresse des ersten geflatteten Elements
                ...
                if (n->init->kv_count > 0) {
                    ...
                    int gmain = add_global(n->var_name);
                    cg_emit(cg, "    movq $0, Lglob_%d(%%rip)", gmain); // KORRIGIERT: movq bei 0-Initialisierung
                }

                // 4. *PATCH* Größe registrieren und setzen (Nur für Arrays)
                if (init_kind == ND_ARRAY_LITERAL) {
                    char size_name[256];
                    snprintf(size_name, sizeof(size_name), "%s_SIZE", n->var_name);
                    int size_gi = add_global(size_name);
                    cg_emit(cg, "    movq $%d, Lglob_%d(%%rip)", n->init->kv_count, size_gi); // KORRIGIERT: movq für konstanten Wert
                }
            ...
            if (n->init) {
                ...
                cg_emit(cg, "    mov %%rax, Lglob_%d(%%rip)", gi);
                 ...
                 int gi = add_global(n->var_name);
                 cg_emit(cg, "    movq $0, Lglob_%d(%%rip)", gi); // KORRIGIERT: movq für konstanten Wert
            }
            return;
        }
    }
    ...
    case ND_IF: {
        ...
        if (n->else_branch) {
            ...
            cg_emit(cg, "    jmp .Lend_%d", lid); // Korrektur: Sprung am Ende des Else-Blocks
    ...
    case ND_PROGRAM: {
        ...
        // Stack Alignment (8 Bytes Padding, um 16-Byte Ausrichtung nach 'push %rbp' zu gewährleisten)
...
static void run_tests(void) {
    ...
    for (int i = 0; tests[i].name; ++i) {
        ...
        if (rc != 0) {
             ...
             // Special check for array overflow (exit code 255/0xFF)
             if (rc == 65280) { // 255 * 256 for a typical shell
                 printf("ERROR DETECTED: Array capacity exceeded (limit %d).\n", MAX_ARRAY_SIZE);
             }
        } else {
            printf("--- Output ---\n%s", buf);
            printf("--- Expect ---\n%s", tests[i].expect);
