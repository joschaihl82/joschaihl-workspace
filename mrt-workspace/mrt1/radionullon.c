...
// - Sinus-Frequenzwobbler + adaptive Mischung aus Hoch-/Niederfrequenz
//   basierend auf Residuum/Kante zur Sichtbarkeits-Maximierung.
...
#define BAR_HEIGHT 28            // Höhe eines Balkens (px)
#define BAR_SPACING 90           // Abstand zwischen parallelen Balken (px)
#define BAR_R 255                // Balkenfarbe (Rot)
#define BAR_G 0
#define BAR_B 0

// Frequenzen (ms pro Durchlauf)
// Niederfrequent: langsamer Sweep; Hochfrequent: schneller Sweep.
// Wir mischen zwischen beiden, zusätzlich sinuswobbeln wir die Mischung.
#define CYCLE_LOW_MS 1200.0f     // langsam
#define CYCLE_HIGH_MS 220.0f     // schnell
...
// Sichtbarkeit entlang des aktiven Balkenbandes schätzen
static float estimate_visibility_score(const uint8_t *res, const uint8_t *edge,
  ...
  // Downsampling in X und Y für Geschwindigkeit
...
// Aktualisiere die effektive Dauer aus Hoch/Niederfrequenz mit Sinus-Wobbler
static void update_cycle_duration(uint32_t now_ms, float vis_score) {
  // Sinuswobbler moduliert die Mischung (0..1) über die Zeit
  ...
  // Adaptive Beimischung:
  // Höherer Sichtbarkeits-Score -> leicht mehr Hochfrequenz (schnell)
  ...
  // Lineare Mischung der Zyklen
  cycle_duration_ms = (1.0f - mix_weight) * CYCLE_LOW_MS + mix_weight * CYCLE_HIGH_MS;
}

// -----------------------------------------------------------------------------
// 3b. Zeichnen: Mehrere horizontale Balken fallen nach unten
// -----------------------------------------------------------------------------
...
int main(int argc, char *argv[]) {
  ...
  while (running) {
    ...
    // 3) Balken zeichnen + Score messen
