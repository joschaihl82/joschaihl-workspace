...
static void kb_put(char c) {
    /* disable interrupts briefly to avoid race with kgetc */
...
/* Assembly stub for IRQ1 (keyboard). Preserve registers and call C handler.
   Fixed: pushfq/popfq added so the total pushed bytes are even and the stack
   is 16-byte aligned for the subsequent call to the C handler.
*/
__attribute__((naked)) void isr_keyboard_stub(void) {
    __asm__ volatile (
        "pushfq\n\t"            /* push RFLAGS to make pushes even (align stack) */
        "pushq %rax\n\t"
        "pushq %rbx\n\t"
        "pushq %rcx\n\t"
        "pushq %rdx\n\t"
...
/* Corrected exception stubs (vectors 0..31) — each sets RDI=vector, RSI=error (if any) */
