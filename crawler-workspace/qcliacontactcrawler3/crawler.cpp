// crawler.cpp
// Single-file Qt GUI crawler (no WebEngine). UI shows only the rendered crawled page
// and a progress bar estimating remaining time (based on average time per page).
//
...
/* ----------------------------- Frontier & DB (same as before, trimmed) ----------------------------- */
...
/* ----------------------------- Extractors & parsing (kept concise) ----------------------------- */
...
static void extract_emails(const string &text, const string &source, DB &db) {
    try {
        regex re(R"(([A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}))", regex::icase);
...
static void extract_phones(const string &text, const string &source, DB &db) {
    try {
        regex re(R"((\+?[0-9][0-9\-\.\s\(\)]{6,}[0-9]))", regex::icase);
        for (sregex_iterator it(text.begin(), text.end(), re), end; it != end; ++it) {
            string raw = (*it)[1].str();
    ...
    if (hrefObj && hrefObj->nodesetval) {
        ...
        for (int i = 0; i < nodes->nodeNr; ++i) {
            ...
            if (attr && attr->children && attr->children->content) {
                ...
                // reuse process_href logic inline (kept minimal)
                string s_href = href ? string(href) : string();
...
private:
    void worker_loop(int /*worker_id*/) {
        ...
        while (!stop_flag) {
            ...
            // record timestamp for estimation
...
/* ----------------------------- MainWindow: minimal UI per user request ----------------------------- */
/*
 UI shows:
  - A single large QTextBrowser that renders the crawled page (HTML subset).
  - A progress bar at the bottom showing fraction crawled / max_pages.
  - A small label next to the progress bar showing estimated remaining time (seconds).
  - A tiny top row with seed input and Start/Stop (kept minimal so user can start).
*/
...
private slots:
    ...
    // When a page is crawled (meta info)
    void onPageCrawled(QString /*url*/, QString /*norm*/, int /*depth*/, double /*score*/, QString /*title*/) {
        // we don't show a list; the rendered HTML will be updated when htmlFetched arrives
    }

    // When HTML is fetched, render it in the QTextBrowser (full page)
    void onHtmlFetched(QString url, QString html) {
        // QTextBrowser supports a subset of HTML/CSS (no JS). We render the full HTML here.
    ...
    void refreshProgress() {
        ...
        // optionally stop timer when done
        if (crawled >= maxp) {
            progressTimer->stop();
            uiStart->setEnabled(true);
            uiStop->setEnabled(false);
...
private:
    void setupUi() {
        ...
        // Minimal top controls (small)
        QHBoxLayout *top = new QHBoxLayout();
        seedEdit = new QLineEdit("https://example.com");
        seedEdit->setPlaceholderText("Seed URL");
        ...
        // Keep top controls small and unobtrusive
        mainLayout->addLayout(top);

        // Main rendered page view (fills most of the window)
        ...
        mainLayout->addWidget(htmlView, /*stretch*/ 1);

        // Bottom: progress bar + remaining label + current URL label
        ...
        bottom->addWidget(progressBar, /*stretch*/ 3);
        bottom->addWidget(remainingLabel, /*stretch*/ 1);
        bottom->addWidget(currentUrlLabel, /*stretch*/ 2);
