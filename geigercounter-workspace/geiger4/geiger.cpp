...
// =======================================================================
// KLASSENDEKLARATIONEN (Unverändert)
// =======================================================================
...
public slots:
    void playClick(double intensity) {
        ...
        double dynamicFreq = Config::MIN_PITCH_FREQ + (freqRange * normalizedIntensity);
        playAlsaClickSound(dynamicFreq);
    }
...
private:
    void playAlsaClickSound(double freq) {
...
public slots:
    void processCameraData(int hotPixelCount) {
        ...
        emit playClick(radValue); 

        // Alarm-Cooldown Logik (FIXED)
...
public:
    explicit GeigerMainWindow(QWidget *parent = nullptr) : QMainWindow(parent) {
        ...
        // Vollbild- und Transparenz-Einstellungen
...
private slots:
    void onStartStopClicked() {
        if (m_logic->isRunning()) {
            ...
            // Nur den Thread unterbrechen, wenn er tatsächlich läuft
...
private:
    ...
    QLabel *m_videoLabel; 
    QWidget *m_overlayWidget = nullptr; // Sicherstellen, dass es initial auf nullptr steht
    ...
    void setupUI() {
        ...
        // --- 2. Overlay-Widget für alle transparenten Steuerelemente ---
        // WICHTIGER FIX: Parent muss das MainWindow sein
        ...
        // Das Top-Level Overlay ist komplett transparent (Alpha 0)
        ...
        // --- TOP LAYOUT (LCD und Info) ---
        QHBoxLayout *topLayout = new QHBoxLayout;
        
        m_radValueDisplay = new QLCDNumber; m_radValueDisplay->setSegmentStyle(QLCDNumber::Flat);
        // LCD-Anzeige: Text Farbe Lime, Hintergrund ist komplett transparent (Alpha 0)
        ...
        // Linkes Panel (Semi-Transparent)
        QWidget *infoPanelWidget = new QWidget;
        // Hintergrund ist dunkles Grau mit Alpha 180 (70% opak)
        ...
        // Chart Hintergrund auf leicht transparent setzen (Alpha 100)
        m_chart->setBackgroundBrush(QBrush(QColor(0, 0, 0, 100))); 
        m_chart->setTitleBrush(QBrush(Qt::white));
        
        // Achsen FIXES: Formatierung und weiße Farbe
        ...
        // ChartView Widget selbst ist komplett transparent, damit nur der QChart-Hintergrund sichtbar ist
        m_chartView->setStyleSheet("background-color: transparent; border: none;");
        mainLayout->addWidget(m_chartView);

        // --- BOTTOM LAYOUT (Steuerung & Log) ---
        QWidget *bottomPanelWidget = new QWidget;
        // Halbtransparenter, dunkler Hintergrund (Alpha 180)
        bottomPanelWidget->setStyleSheet("background-color: rgba(30, 30, 30, 180); color: white; padding: 10px; border-radius: 5px;");
        QVBoxLayout *bottomVLayout = new QVBoxLayout(bottomPanelWidget);
        
        QHBoxLayout *bottomLayout = new QHBoxLayout;
        m_startStopButton = new QPushButton("Stoppen"); 
        m_startStopButton->setCheckable(true);
        m_startStopButton->setMinimumHeight(40);
        m_statusLabel = new QLabel("Status: Messung aktiv");
        m_statusLabel->setStyleSheet("color: lime;");
        
        bottomLayout->addWidget(m_startStopButton); 
        bottomLayout->addWidget(m_statusLabel);
        bottomVLayout->addLayout(bottomLayout);
        
        m_logConsole = new QTextEdit; 
        m_logConsole->setReadOnly(true); 
        m_logConsole->setMaximumHeight(100);
        // Log-Konsole hat einen minimal-transparenten Hintergrund (Alpha 10)
        m_logConsole->setStyleSheet("background-color: rgba(255, 255, 255, 10); color: white; border: 1px solid gray;");
        
        QLabel *logTitle = new QLabel("System Log:");
        logTitle->setStyleSheet("color: white;");
        bottomVLayout->addWidget(logTitle); 
        bottomVLayout->addWidget(m_logConsole);
        
        mainLayout->addWidget(bottomPanelWidget);

        // Setzt die Geometrie des Overlay-Widgets initial auf die Größe des MainWindows
        // Muss hier auf die volle Größe gesetzt werden
    ...
    void setupClassesAndConnections() {
        ...
        connect(m_logic, QOverload<double>::of(&DosimeterLogic::playClick), m_audioAlert, QOverload<double>::of(&AudioAlert::playClick));
    ...
    // Überschreibt resizeEvent, um das Overlay-Widget immer über das Vollbild-Video zu legen
    void resizeEvent(QResizeEvent* event) override {
        QMainWindow::resizeEvent(event);
        // WICHTIGER FIX: Prüfen auf nullptr, um den Start-Absturz zu verhindern
        if (m_overlayWidget) {
