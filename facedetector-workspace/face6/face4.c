...
#define W 800  // AUFLÖSUNG: 800
#define H 600  // AUFLÖSUNG: 600
...
static void compute_thresholds(float *h_low, float *h_high, float *s_min, float V_mean){
    ...
    *h_high = skin.mu_h + KH * sigma_h;
    // wrap-around handling: we'll handle in mask test
...
static int create_skin_mask_adaptive(uint8_t *rgbbuf, uint8_t *mask_out, float *out_Vmean, float *out_Hmean, float *out_Smean){
    ...
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            ...
            float h,s,v; rgb_to_hsv_pixel(rgbbuf[p], rgbbuf[p+1], rgbbuf[p+2], &h, &s, &v);
            int skinpix = 0;
            // handle wrap-around for H (e.g., near 0/360)
...
int main(){
    ...
    // ANPASSUNG: Maximiert und Resizable
    ...
    while(running){
        ...
        // Adaptive skin model update: only when stable_count >= STABLE_FRAMES_REQ and area >= AREA_MIN
        if(stable_count >= STABLE_FRAMES_REQ){
            ...
            if(stable_update_counter % UPDATE_EVERY_N == 0){
                ...
                float xh = Hmean_mask;
                float xs = Smean_mask;
                // wrap handling for H: choose shortest direction to update mu
        ...
        // ANPASSUNG: Skaliere Inhalt auf Fenstergröße
        int win_w, win_h;
        ...
        while(SDL_PollEvent(&e)){
            ...
            if(e.type==SDL_KEYDOWN){
                ...
                if(e.key.keysym.sym=='e'){
                    // enroll current projection as new class
                    project(xvec, z);
                    int id = num_classes++;
