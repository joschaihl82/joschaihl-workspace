...
static int decompress_file_from_archive(FILE *archive, const char *outroot, uint32_t comp_size, const char *name, uint64_t orig_size) {
    ...
    // patch: since 'name' might contain the full path including directories (e.g., /home/user/file.txt),
    // we must ensure the parent directory of outpath exists, regardless of whether outroot was used.
...
static int process_decompress_archive(FILE *archive, const char *outroot) {
    while (1) {
        ...
        // patch: decompress_file_from_archive now handles output path logic based on stored name
...
int main(int argc, char **argv) {
    ...
    if (do_compress) {
        ...
        if (S_ISDIR(st.st_mode) && (outroot == NULL || outroot[0] == '\0')) {
            ...
            // patch: use process_compress_to_archive which now uses realpath
