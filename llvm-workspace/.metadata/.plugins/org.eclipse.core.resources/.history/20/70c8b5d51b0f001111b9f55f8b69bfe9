...
public:
  ...
  // This version extends the end offset to include a trailing semicolon if present
  bool recordRangeForDecl(SourceLocation B, SourceLocation E, RangeInfo &out) {
    ...
    // If there's a semicolon immediately after optional whitespace, include it and following newline(s)
    if (i < buf.size() && buf[i] == ';') {
      unsigned j = i + 1;
      // include trailing spaces/tabs
...
public:
  ...
  void HandleTranslationUnit(ASTContext &Context) override {
    ...
    auto is_semicolon_only_line = [](llvm::StringRef line) -> bool {
      ...
      // If block comment markers present, treat as non-semicolon-only
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
      ...
      if (is_semicolon_only_line(lr)) {
        // Skip this line entirely (also skip its newline)
        continue;
      }
