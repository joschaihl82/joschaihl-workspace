/* play_ss2000_refactor.c
   Refactored live ALSA player for SS2000 sequence
   - Short main that calls setup_snd/close_snd, wail(), yelp()
   - Sweep range: 440 Hz -> 880 Hz (halved from 880->1760)
   - 4 ms linear fade in/out per block
   Compile: cc -O2 -std=c11 -lasound -lm -o play_ss2000_refactor play_ss2000_refactor.c
*/
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
static void apply_fade_in_out(double *buf, size_t frames, int sr, unsigned fade_ms) {
    ...
    size_t fade_samples = (size_t)((uint64_t)sr * fade_ms / 1000);
    if (fade_samples == 0) return;
    if (fade_samples * 2 > frames) fade_samples = frames / 2;
    for (size_t i = 0; i < fade_samples; ++i) {
        double g = (double)i / (double)fade_samples;
        buf[i] *= g;
    }
    for (size_t i = 0; i < fade_samples; ++i) {
...
static int setup_snd(snd_pcm_t **pcm, unsigned sr) {
    ...
    snd_pcm_hw_params_set_rate_near(p, hw, &sr_set, NULL);
...
static int alsa_write_frames(snd_pcm_t *pcm, const double *mono, size_t frames) {
    ...
    while (written < frames) {
        ...
        if (r < 0) { fprintf(stderr,"snd_pcm_writei: %s\n", snd_strerror((int)r)); return -1; }
        written += (size_t)r;
    }
    return 0;
}
...
/* ---------------- semantic functions ---------------- */
static int wail(snd_pcm_t *pcm, ctx_siren *ctx, unsigned fade_ms) {
...
static int yelp(snd_pcm_t *pcm, ctx_siren *ctx, unsigned fade_ms) {
    double *buf = NULL; size_t frames = 0;
...
int main(void) {
    ...
    /* Use sweep 440 Hz -> 880 Hz (halved) */
    ctx_siren wctx = {440.0, 880.0, 12.0/60.0};   /* slow wail */
    ctx_siren yctx = {440.0, 880.0, 180.0/60.0};  /* fast yelp */

    for (int i = 0; i < ITERATIONS; ++i) {
        if (wail(pcm, &wctx, FADE_MS) != 0) break;
        if (yelp(pcm, &yctx, FADE_MS) != 0) break;
    }

    close_snd(pcm);
    printf("Done: %d cycles of wail+ yelp at 440->880 Hz with %d ms fades\n", ITERATIONS, FADE_MS);
    return 0;
}
