/*
 * main.cpp - qgeister1 vollständiges Amalgamat
 ...
 * - UI: Full-window dual video, overlay graphs, tray minigraph
...
public slots:
    void playClick(double activity) {
        ...
        ev.timeMs = QDateTime::currentMSecsSinceEpoch();
        {
            ...
            if (clickQueue.size() < 2048) clickQueue.push_back(ev);
        }
        cv.notify_one();
    }
        ...
        while (running.load()) {
            {
                ...
                while (!clickQueue.empty()) {
                    ...
                    ac.pitchVar = 1.0 + (((double)((seed >> 8) & 0xFF) / 255.0) - 0.5) * 0.03;
                    ac.id = nextClickId++;
