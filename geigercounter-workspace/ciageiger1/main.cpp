 ...
 * CIA-ähnlicher Geiger-Click Sound via ALSA.
 *
 * Hinweis: beim Linken -lasound angeben.
 */
...
public:
    explicit AudioEngine(QObject *parent = nullptr) : QObject(parent) {
        ...
        if (rc < 0) {
            qWarning() << "ALSA open error:" << snd_strerror(rc);
            pcm = nullptr;
            return;
        }
        ...
        snd_pcm_hw_params_set_period_size_near(pcm, hwparams, &frames, 0);
        int err = snd_pcm_hw_params(pcm, hwparams);
        if (err < 0) {
            qWarning() << "ALSA hw params error:" << snd_strerror(err);
            snd_pcm_close(pcm); pcm = nullptr;
...
public slots:
    void playClick(double freq) {
        ...
        // Parameter (CIA-ähnlicher Klick)
        const int sampleRate = 44100;
        const double durationSec = 0.75;
        const int totalSamples = (int)(durationSec * sampleRate);
        const double activity = qBound(0.0, freq, 2000.0);
        double clicksPerSecond = qMin(200.0, 0.9 * activity + 0.5);
        clicksPerSecond = qMax(1.0, clicksPerSecond);
        ...
        QVector<int> clickPositions;
        {
            double t = 0.0;
            quint32 seed = (quint32)(QDateTime::currentMSecsSinceEpoch() & 0xffffffff);
            qsrand(seed);
            while (t < durationSec) {
                double meanDt = 1.0 / clicksPerSecond;
                double u = (double)qrand() / (double)RAND_MAX;
                if (u < 1e-6) u = 1e-6;
                double dt = -qLn(u) * meanDt;
                double jitter = (((double)qrand() / RAND_MAX) - 0.5) * meanDt * 0.18;
                t += qMax(0.00015, dt + jitter);
                int pos = (int)(t * sampleRate);
                if (pos >= 0 && pos < totalSamples) clickPositions.append(pos);
            }
        }

        const int clickLenSamples = qMax(1, (int)(clickLenMs * 0.001 * sampleRate));
        std::vector<int16_t> pcmBuf;
        pcmBuf.resize(totalSamples);
        std::fill(pcmBuf.begin(), pcmBuf.end(), 0);

        QVector<char> isClick(totalSamples);
        for (int p : clickPositions) if (p >= 0 && p < totalSamples) isClick[p] = 1;
        ...
        for (int n = 0; n < totalSamples; ++n) {
            double s = 0.0;
            int lookback = clickLenSamples;
            int start = qMax(0, n - lookback);
            for (int p = start; p <= n; ++p) {
                if (!isClick[p]) continue;
                int k = n - p;
                if (k >= clickLenSamples) continue;
                double tt = (double)k / sampleRate;
                uint seed = (uint)(p * 1664525u + 1013904223u);
                double ampVar = 0.85 + ((seed & 0xFF) / 255.0) * 0.4;
                double pitchVar = 1.0 + ((((seed >> 8) & 0xFF) / 255.0) - 0.5) * 0.03;
                double env = qExp(-tt / envTau);
                env *= (1.0 - qExp(-40.0 * tt));
                double c1 = qSin(2.0 * M_PI * carrierA * pitchVar * tt);
                double c2 = 0.55 * qSin(2.0 * M_PI * carrierB * (1.0 + (seed & 7) * 0.0018) * tt);
                double pseudoNoise = (((seed >> 16) & 0x7FFF) / (double)0x7FFF) * 2.0 - 1.0;
                s += baseAmp * ampVar * env * (0.88 * c1 + 0.48 * c2 + noiseLevel * pseudoNoise);
            }
            double bg = (((double)qrand() / RAND_MAX) * 2.0 - 1.0) * 0.0009 * (1.0 + activity / 360.0);
            s += bg;
            ...
            if (sample < -1.0) sample = -1.0;
            pcmBuf[n] = (int16_t)(sample * 30000.0);
        }
        ...
        while (written < frames) {
            ...
            int rc = snd_pcm_writei(pcm, pcmBuf.data() + written, toWrite);
            if (rc == -EPIPE) {
                snd_pcm_prepare(pcm);
                continue;
            } else if (rc < 0) {
...
protected:
    void paintEvent(QPaintEvent *) override {
        ...
        // Graphs: transparente Overlays unten im Fenster
