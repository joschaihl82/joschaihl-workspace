// main.cpp
// DriveSync - Ordnerstruktur rekonstrueren und alle Dateien herunterladen (Qt6)
...
// Bugfixes: Verhindere doppelte Starts, sicheres serielles Download-Verhalten, verbessertes Logging.
...
public:
    explicit DriveSync(QObject *parent = nullptr) : QObject(parent) {
        ...
        // Wenn Login erfolgreich, startet automatisch der Sync (nur wenn noch nicht automatisch gestartet)
        connect(google, &QOAuth2AuthorizationCodeFlow::granted, this, [this]() {
            ...
            // Nur starten, wenn nicht bereits automatisch gestartet durch vorhandene Tokens
            if (!autoStartedFromSavedTokens) {
                QTimer::singleShot(0, this, &DriveSync::startDropboxSync);
            }
        });

        // Wenn bereits Token vorhanden, sofort starten (einmalig)
        QString accessToken, refreshToken;
        qint64 expiresAt = 0;
        bool tokensLoaded = loadTokens(accessToken, refreshToken, expiresAt);
        if (tokensLoaded) {
            emit status(QStringLiteral("Gefundene Tokens geladen"));
...
#endif
                // Versuche Token zu erneuern; nach erfolgreichem refresh wird granted ausgelöst
                QMetaObject::invokeMethod(google, "refreshTokens", Qt::QueuedConnection);
                emit status(QStringLiteral("Erneuere Access-Token"));
                // markiere, dass wir bereits versucht haben automatisch zu starten (verhindert doppelten Start)
                autoStartedFromSavedTokens = true;
            ...
            // Kein Token: grant() öffnet Browser (einmalig)
            google->grant();
...
private:
    ...
    // Guards / flags to avoid reentrancy and duplicate starts
    bool syncInProgress = false;
    bool downloadActive = false;
    bool autoStartedFromSavedTokens = false;
    ...
    void setupWatcher() {
        ...
        // Debounce: single-shot timer to avoid repeated triggers
        QTimer *debounce = new QTimer(this);
        debounce->setSingleShot(true);
        debounce->setInterval(300);
        connect(watcher, &QFileSystemWatcher::directoryChanged, this, [this, debounce](const QString&) {
            emit status(QStringLiteral("Lokale Änderung erkannt"));
            debounce->start();
        });
        connect(debounce, &QTimer::timeout, this, [this]() {
            // Start reconciliation but respect syncInProgress guard
            QTimer::singleShot(0, this, &DriveSync::startDropboxSync);
        });
    }
    ...
    void startDropboxSync() {
        // Prevent re-entrancy / duplicate starts
        if (syncInProgress) {
            emit status(QStringLiteral("Sync bereits aktiv, Ignoriere Startaufruf"));
            return;
        }
        syncInProgress = true;
    ...
    void reconcileAndSync() {
        ...
        for (const QString &rel : allPaths) {
            ...
            if (hasLocal && !hasRemote) {
                ...
                // Filter out common build/temp files to avoid noisy uploads
                if (shouldIgnoreLocalPath(rel)) continue;
        ...
        if (!downloadQueue.isEmpty()) {
            ...
            QTimer::singleShot(0, this, &DriveSync::processNextDownload);
        } else {
    ...
    // Simple filter to ignore common build/temp files
    bool shouldIgnoreLocalPath(const QString &rel) const {
        static const QStringList ignoreSuffixes = {
            ".o", ".obj", ".class", ".pyc", ".swp", ".tmp", ".part", ".cache"
        };
        static const QStringList ignoreDirs = {
            ".git", ".svn", "build", "Debug", "Release", ".cache"
        };
        for (const QString &suf : ignoreSuffixes) {
            if (rel.endsWith(suf, Qt::CaseInsensitive)) return true;
        }
        for (const QString &d : ignoreDirs) {
            if (rel.startsWith(d + "/", Qt::CaseInsensitive) || rel.contains("/" + d + "/")) return true;
        }
        return false;
    }
    ...
    void processNextDownload() {
        // Ensure only one active download at a time
        if (downloadActive) {
            emit status(QStringLiteral("Download bereits aktiv, warte..."));
            return;
        }
        ...
        QString token = currentAccessToken();
        if (token.isEmpty()) {
            emit status(QStringLiteral("Kein Access-Token"));
            // mark sync finished if nothing can proceed
            syncInProgress = false;
            return;
        }
        ...
        // Mark active
        downloadActive = true;
        emit status(QStringLiteral("Starte Download: %1").arg(name));
        ...
        connect(reply, &QNetworkReply::finished, this, [this, reply, outFile, outPath, okSize, size]() mutable {
            ...
            if (reply->error() != QNetworkReply::NoError) {
                ...
                delete outFile;
                reply->deleteLater();
                // mark not active and continue with next
            ...
            // mark not active and start next file (serial behavior)
            downloadActive = false;
            QTimer::singleShot(50, this, &DriveSync::processNextDownload);
        });
    }
