...
// - No dummy values: preview widgets are populated only from live AT‑SPI queries (Value/Text/Children/States).
// - MOC: file ends with #include "qsp.moc"
...
public:
    ...
    QStringList readStates(quintptr id) {
        ...
        for (guint i=0;i<arr->len;++i) {
            // atspi_state_type_get_name exists in many libatspi versions; if not, fallback to numeric
            const char *sname = nullptr;
#if defined(HAVE_ATSPI_STATE_TYPE_GET_NAME) || 1
            // try to call function if available
            // some headers provide atspi_state_type_get_name; if not, this will be nullptr at link time
            // to be safe, use a switch fallback below
            // We'll attempt to map common enum values to names as a fallback
            switch (types[i]) {
            ...
            if (!sname) {
                ...
                sname = g_strdup_printf("state_%d", (int)types[i]);
...
private slots:
    ...
    void updatePreviewForSelection() {
        ...
        // Try Value
        double cur=0.0, minv=0.0, maxv=100.0;
        bool hasValue = mgr->readValue(iid, cur, minv, maxv);
        ...
        if (hasValue && (type == "progress" || type == "slider" || type == "spin")) {
            ...
            double pct = (span > 0.0) ? ((cur - minv) / span * 100.0) : 0.0;
        ...
        // Text-driven widgets
        if (!text.isEmpty() && (type == "edit" || type == "label" || type == "generic")) {
            if (type == "edit") {
        ...
        if (type == "combo" || type == "list" || type == "menu") {
            if (type == "combo") {
                demoCombo->clear();
...
int main(int argc, char **argv) {
    ...
    MainWindow w;
    w.setWindowTitle("qsp - Qt AT-SPI Inspector (live)");
    w.show();
    return a.exec();
}
