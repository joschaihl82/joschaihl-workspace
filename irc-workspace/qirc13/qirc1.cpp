...
private:
    ...
    void initLogDir() {
        ...
        QDir dir(base);
        if (!dir.exists("IRCClientLogs")) dir.mkpath("IRCClientLogs");
        logBaseDir = dir.filePath("IRCClientLogs");
    }
    ...
    void appendToFile(const QString &filename, const QString &line) {
        ...
        initLogDir();
        QFile f(QDir(logBaseDir).filePath(filename));
    ...
    void appendChannelLog(const QString &channel, const QString &line) {
        QString fname = QString("channel_%1.txt").arg(safeFileName(channel));
        appendToFile(fname, line);
    }

    void appendChatLog(const QString &user, const QString &line) {
        QString fname = QString("chat_%1.txt").arg(safeFileName(user));
        appendToFile(fname, line);
    }

    void appendStatusLog(const QString &line) {
        appendToFile("status.txt", line);
    }
