// semiconductormachine1_swarm.c
// Lattice Boltzmann D2Q9 demo mit SDL2-Visualisierung
// Inlet: Partikel-Schwarm-Algorithmus statt Sinus
// Compile: gcc -O2 -std=c11 -lm -lSDL2 -lSDL2_ttf semiconductormachine1_swarm.c -o laticeboltzmann_swarm
...
// Swarm parameters
#define SWARM_COUNT 60
#define SWARM_INLET_X 12    // region where agents are initialized (x)
#define SWARM_MIN_X 6
#define SWARM_MAX_X 40
#define SWARM_SIGMA 6.0     // kernel width for influence on inlet
#define SWARM_MAX_SPEED 0.06
#define SWARM_COHESION 0.002
#define SWARM_ALIGNMENT 0.05
#define SWARM_SEPARATION 0.08
#define SWARM_NOISE 0.01
...
void init_swarm(){
    ...
    for(int i=0;i<SWARM_COUNT;i++){
        ...
        swarm[i].x = rx;
        swarm[i].y = ry;
        // initial velocity roughly to the right with small variation
        swarm[i].vx = 0.02 + (rand()/(double)RAND_MAX)*0.02;
...
void update_swarm(double dt){
    // simple boids-like update
    for(int i=0;i<SWARM_COUNT;i++){
        double cx = 0.0, cy = 0.0; // cohesion center
        double avx = 0.0, avy = 0.0; // alignment
        double sepx = 0.0, sepy = 0.0; // separation
        int neighbors = 0;
        ...
        // small bias to the right to push flow into domain
        dvx += 0.002;
        // noise
        ...
        // keep agents inside inlet region and pipe: reflect or wrap
        ...
        // if agent inside obstacle, push it back into fluid region
        int ix = (int)floor(swarm[i].x);
        int iy = (int)floor(swarm[i].y);
        if (ix >= 0 && ix < NX && iy >= 0 && iy < NY && obstacle[idx(ix,iy)]){
            // move upward or downward until fluid found
            int found = 0;
...
// Initialize particles inside fluid region near inlet (visual tracer particles)
void init_particles(){
...
void lbm_step(double t, double dt){
    ...
    // apply inlet velocity based on swarm influence on leftmost fluid cells
    // For each y at x = 6 (near left boundary inside pipe), compute weighted sum of swarm vx
    for(int y=1;y<NY-1;y++){
        ...
        for(int s=0;s<SWARM_COUNT;s++){
            ...
            // optionally reduce influence if agent is far in x
            double dx = swarm[s].x - (double)x;
            double xfall = exp(- (dx*dx) / (2.0 * (SWARM_SIGMA*1.5) * (SWARM_SIGMA*1.5)));
        ...
        // scale baseline to ensure reasonable amplitude
        double inlet_base = 0.04;
        double factor = speed_factor[id];
        double u_final = inlet_base * (1.0 + u_in*10.0) * factor; // amplify swarm vx effect
        // clamp to safe range
        if (u_final < 0.0) u_final = 0.0;
        if (u_final > 0.2) u_final = 0.2;
        ux_field[id] = u_final;
        uy_field[id] = 0.0;
        rho_field[id] = 1.0;
        for(int k=0;k<NDIR;k++){
...
void render(SDL_Renderer *renderer, SDL_Texture *tex, Uint32 *pixels){
    ...
    // draw swarm agents as small yellow dots (overlay)
    for(int s=0;s<SWARM_COUNT;s++){
        int sx = (int)round(swarm[s].x);
        int sy = (int)round(swarm[s].y);
        if (sx>=0 && sx<NX && sy>=0 && sy<NY){
            int id = idx(sx,sy);
            pixels[id] = (255<<24) | (255<<16) | (200<<8) | 0;
        }
    }
...
int main(int argc, char **argv){
    ...
    SDL_Window *win = SDL_CreateWindow("LBM Schwarm-Simulator", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WINDOW_W, WINDOW_H, 0);
    ...
    while(running){
        ...
        for(int m=0;m<micro;m++){
            // update swarm first so inlet uses latest agent state
