...
// Nur noch wenige statische Konstanten
...
typedef struct {
    ...
    // DYNAMISCHE VARIABLEN (Hair-Learning Parameter)
    double hot_pixel_thresh;    // Dynamic Hot Pixel Threshold
    double bg_learn_rate_base;  // Basiswert für die Lernrate
    double bg_learn_rate;       // Modulierter Wert
    double val_boost;           // Dynamic Neon Value Boost
    double geiger_smooth_factor;// Dynamic Geiger Smoothing
    double geiger_alert_thresh; // Base Alert Threshold
    
    // STATISTIK FÜR FARBRAUM-ANALYSE
    int saturated_pixels; 
    int bright_pixels;    
    
    // SINUS-LEARNING-PARAMETER
    double sinus_cycle_counter; // Zählt die Frames/Zeit für den Sinus
    double SINUS_SPEED;         // Geschwindigkeit des Sinus-Zyklus
    double MIN_LEARN_RATE;      // Minimale/Basis Lernrate
} RadionullonState;
...
// Initialisierung der Dynamic Learning Parameter
void logic_init(RadionullonState *state) {
    ...
    // Initialisierung der dynamischen Werte
    state->hot_pixel_thresh = 15.0;      
    state->bg_learn_rate_base = 0.05;    
    state->val_boost = 0.8;              
    state->geiger_smooth_factor = 0.10;  
    state->geiger_alert_thresh = 5.0;    
    
    state->saturated_pixels = 0;
    state->bright_pixels = 0;
    
    // SINUS-LEARNING INITIALISIERUNG (NEU in V12.3)
    state->sinus_cycle_counter = 0.0;
    // 2 * PI = 6.283185 -> Ergibt einen Zyklus von 1 Sekunde
    state->SINUS_SPEED = 6.283185;         
    state->MIN_LEARN_RATE = 0.001;      
}
...
void radionullon_process_gtk(GdkPixbuf *original_pb, GdkPixbuf *processed_pb, GeigerStats *stats, RadionullonState *state, double delta_time) {
    ...
    // Zähler für Farbraum-Analyse zurücksetzen
    state->saturated_pixels = 0;
    state->bright_pixels = 0;

    // --- SINUS-LEARNING LOGIK (NEU in V12.2) ---
    // 1. Sinus-Zähler aktualisieren (delta_time für zeitbasierte Frequenz)
    state->sinus_cycle_counter += state->SINUS_SPEED * delta_time;
    // 2. Sinus-Modulation berechnen (Sinus im Bereich [-1.0, 1.0])
    double sin_modulator = my_sin(state->sinus_cycle_counter); 
    
    // 3. Lernrate anpassen (Range: 0.0 bis 2 * base_rate)
    state->bg_learn_rate = state->bg_learn_rate_base + sin_modulator * state->bg_learn_rate_base;

    double actual_rate;
    if (state->bg_learn_rate < state->MIN_LEARN_RATE) {
        // Negatives Learning (Schritt zurück) -> verwendet den vollen negativen Sinus
        actual_rate = sin_modulator * state->MIN_LEARN_RATE * 10.0; // Verstärkt den negativen Schritt
    } else {
        // Normales Learning (Pause oder volle Rate)
        actual_rate = state->bg_learn_rate;
    }

    // NEON-SEPIA FARBBEREICH: Konstanten verwenden die dynamischen Werte
    const double HUE_BASE = 40.0;     
    const double HUE_ACTIVE = 0.0;    
    const double SAT_BASE = 0.35;     
    const double SAT_ACTIVE = 1.0;    
    ...
    // Hair-Learning: Globale Helligkeit initial berechnen
    ...
    for (int y = 0; y < IMG_H; y++) {
        ...
        for (int x = 0; x < IMG_W; x++) {
            ...
            v = pow(v, GAMMA); 
            if (brightness > state->hot_pixel_thresh) hotPixels++; 

            // HINTERGRUND-ADAPTION (mit actual_rate, kann negativ sein!)
            state->bg[idx] = state->bg[idx] * (1.0f - my_fmax(0, actual_rate)) + (float)brightness * actual_rate; 
            ...
            // Neon-Sepia Logik (verwendet dynamischen val_boost)
            double h_new = HUE_BASE - (normalized_score * (HUE_BASE - HUE_ACTIVE));
            double s_new = SAT_BASE + (normalized_score * (SAT_ACTIVE - SAT_BASE)); 
            s_new = my_fmin(1.0, s_new); 

            double v_new = v * (1.0 + normalized_score * state->val_boost); // Dynamischer Boost
            v_new = my_fmin(1.0, v_new); 
            
            // STATISTIK: Überprüfung der Farbraum-Angemessenheit
            if (s_new >= 0.95) {
                state->saturated_pixels++;
            }
            if (v_new >= 0.95) {
                state->bright_pixels++;
            }
    ...
    // --- Hair-Learning Adaptive Anpassung ---
    double avg_global_brightness = (double)sum_brightness / (IMG_W * IMG_H);
    
    // 1. Hot Pixel Threshold: Abhängig von der globalen Helligkeit.
    state->hot_pixel_thresh = my_fmax(8.0, avg_global_brightness * 0.15); 
    
    // 2. Neon Value Boost (Statistisch angepasst)
    double activity_ratio = (double)hotPixels / (IMG_W * IMG_H * 0.05);
    
    const int total_pixels = IMG_W * IMG_H;
    double saturation_ratio = (double)state->saturated_pixels / total_pixels;
    double brightness_ratio = (double)state->bright_pixels / total_pixels;
    
    const double OVERFLOW_MAX_RATIO = 0.01; 

    double base_boost = my_fmax(0.6, 1.0 - activity_ratio); 

    if (saturation_ratio > OVERFLOW_MAX_RATIO || brightness_ratio > OVERFLOW_MAX_RATIO) {
        double max_overflow_ratio = my_fmax(saturation_ratio, brightness_ratio);
        double overflow_factor = max_overflow_ratio / OVERFLOW_MAX_RATIO;
        
        base_boost *= my_fmax(0.7, 1.0 - 0.3 * overflow_factor); 
    }
    
    state->val_boost = my_fmin(1.0, my_fmax(0.4, base_boost)); 
    
    
    // --- Geiger Logik (verwendet dynamische Glättung) ---
    double rawVal = (double)hotPixels / GEIGER_NORM_FACTOR;
    state->currentRadSmooth = (state->currentRadSmooth * (1.0 - state->geiger_smooth_factor)) + (rawVal * state->geiger_smooth_factor);
    
    stats->currentRad = rawVal;
    stats->avgRad = state->currentRadSmooth;
    stats->alertState = state->currentRadSmooth > state->geiger_alert_thresh;
...
void draw_overlay_manual(GdkPixbuf *overlay_pb, GeigerStats *stats, RadionullonState *state) {
        ...
        for (int y = 0; y < IMG_H; y++) {
            for (int x = 0; x < IMG_W; x++) {
                ...
                if (alpha > 0) {
                    ...
                    // NEON-SEPIA: Neon-Oranger Flicker-Effekt
                    pixels[pixel_idx + 0] = 255; 
                    pixels[pixel_idx + 1] = 100; 
    ...
    // Skalierung verwendet dynamischen Alert Threshold
    const double max_val = state->geiger_alert_thresh * 50.0; 
    ...
    int alert_y = GET_Y_POS(state->geiger_alert_thresh); 
    for(int x = 0; x < IMG_W; x++) {
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    gtk_window_set_title(GTK_WINDOW(g_window), "Radionullon Geiger (V12.3 - 1s Sinus-Learning)");
