...
// To capture the exact X11 cursor we use XFixesGetCursorImage when available.
...
#include <cmath>
  ...
  // X11 headers for exact cursor capture on X11
  #include <X11/Xlib.h>
  #include <X11/Xutil.h>
...
private slots:
    // Patched: capture screen and draw the exact X11 cursor into the frame before pushing
    void captureScreenFrame() {
...
#if defined(Q_OS_UNIX)
        // Attempt to get the exact X11 cursor via XFixes
        Display *dpy = XOpenDisplay(nullptr);
        if (dpy) {
            XFixesCursorImage *ci = XFixesGetCursorImage(dpy);
            if (ci) {
                // ci->width, ci->height, ci->xhot, ci->yhot, ci->pixels (unsigned long*)
                // Create a QImage from the cursor pixels. XFixes returns ARGB in server native endianness.
                int cw = ci->width;
                int ch = ci->height;
                // Build QImage with Format_ARGB32 (0xAARRGGBB)
                QImage cursorImg(cw, ch, QImage::Format_ARGB32);
                // Convert each pixel. XFixesCursorImage pixels are 32-bit CARD32 values in server byte order.
                // We'll normalize to 0xAARRGGBB for QImage.
                for (int y = 0; y < ch; ++y) {
                    QRgb *scan = reinterpret_cast<QRgb*>(cursorImg.scanLine(y));
                    for (int x = 0; x < cw; ++x) {
                        unsigned long px = ci->pixels[y * cw + x];
                        // px layout typically 0xAARRGGBB on little-endian X11; to be safe, extract components:
                ...
                // Compute image-space position for cursor top-left
                QPoint globalCursorPos = QCursor::pos();
                QRect screenGeom = screen->geometry();
                QPoint imageCursorPos = globalCursorPos - screenGeom.topLeft();
                QPoint topLeft = imageCursorPos - QPoint(hotX, hotY);

                // Draw cursor image onto the captured screen image
                QPainter painter(&img);
                painter.setCompositionMode(QPainter::CompositionMode_SourceOver);

                // If device pixel ratio differs, scale cursor accordingly
                qreal dpr = pix.devicePixelRatio();
                if (!qFuzzyCompare(dpr, 1.0)) {
                    QSize scaledSize = cursorImg.size() / dpr;
                    QImage scaled = cursorImg.scaled(scaledSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);
                    QPoint scaledTopLeft = QPoint(topLeft.x() - (hotX - scaled.width()/2), topLeft.y() - (hotY - scaled.height()/2));
                    painter.drawImage(scaledTopLeft, scaled);
                } else {
                    painter.drawImage(topLeft, cursorImg);
                }
        ...
        // Fallback: draw Qt cursor pixmap or simple cursor if X11 exact cursor not available
        QPoint globalCursorPos = QCursor::pos();
        QRect screenGeom = screen->geometry();
        QPoint imageCursorPos = globalCursorPos - screenGeom.topLeft();
        ...
        QCursor currentCursor = QCursor();
        QPixmap cursorPixmap = currentCursor.pixmap();
        QPoint hotspot = currentCursor.hotSpot();

        if (!cursorPixmap.isNull()) {
            qreal dpr = pix.devicePixelRatio();
            if (dpr != 1.0) {
                QSize scaledSize = cursorPixmap.size() / dpr;
                QPixmap scaled = cursorPixmap.scaled(scaledSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);
                QPoint topLeft = imageCursorPos - hotspot;
                painter.drawPixmap(topLeft, scaled);
            } else {
                QPoint topLeft = imageCursorPos - hotspot;
                painter.drawPixmap(topLeft, cursorPixmap);
            }
        } else {
            // Fallback: draw a simple arrow-like cursor (white with black outline)
            const int r = qMax(6, img.width() / 200); // scale fallback cursor with resolution
            QPoint center = imageCursorPos;
            painter.setPen(QPen(Qt::black, 2));
            painter.setBrush(Qt::white);
            painter.drawEllipse(center, r, r);
            painter.drawLine(center, center + QPoint(r * 2, r * 3));
        }
