// beatdetect_16band_flash.c
// gcc -O2 -o beatdetect_16band_flash beatdetect_16band_flash.c -lSDL2 -lasound -lGLEW -lGL -lm -lpthread
// ALSA capture (S16_LE mono) + ALSA mixer hardware gain
// 16-band FFT (internal) + fast kick detection on low bands -> immediate white flash
...
// GLSL shaders (same visual style)
...
"  if (u_kick == 1) { fragColor = vec4(1.0,1.0,1.0,0.9); }\n" // faint overlay if needed
...
"  }\n"
"  if (u_kick == 1) fragColor = vec4(col, final_alpha);\n"
"  else fragColor = vec4(col, final_alpha);\n"
"}\n";
...
static void map_bins_to_bands(const double *mag, double *bands_out, unsigned sample_rate) {
    // We'll create N_BANDS bands spaced logarithmically between 20Hz and Nyquist
    double f_min = 20.0;
...
int main(void) {
    ...
    SDL_Window *win = SDL_CreateWindow("BeatDetect 16-band Flash", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    ...
    // temporary arrays for FFT/bands
    ...
    fprintf(stderr, "16-band FFT enabled. Onset factor initial = %.2f (Arrow Up/Down)\n", onset_factor);
    ...
    while (running) {
        ...
        // focus on lower bands: sum first few bands (e.g., lowest 4 bands)
        double low_sum = 0.0;
        int low_bands = 4; // tune: how many lowest bands to consider
        for (int i = 0; i < low_bands; ++i) low_sum += bands[i];

        // normalize by band count to keep scale stable
        low_sum /= (double)low_bands;

        // running average on low energy
        running_avg_low = alpha_avg * low_sum + (1.0 - alpha_avg) * running_avg_low;

        // immediate kick detection and flash
        ...
        // set kick_flag: 2 = full white flash, 1 = overlay, 0 = none
