...
//
// UI layout changes requested:
//  1) Left and right panes fill the window vertically; progress bar reserved at bottom with 30px height.
//  2) Top dropdown lists device nodes that host ext4 filesystems (e.g., /dev/nvme0n1p3).
//  3) A spacer between grid and preview so preview can be made broader (three-pane splitter).
//
// Build with Qt6 (Widgets + WebEngineWidgets). Example CMakeLists.txt provided separately.
//
// Notes:
//  - Scanning raw block devices usually requires root privileges. Use a disk image file for testing.
//  - The scanner reads direct blocks only (no extents/indirect). It emits placeholder paths
//    of the form "/[ino:<pseudo>]/<name>" and the directory-entry inode number so the UI can
//    resolve attributes when the device is mounted.
//
// Author: Assistant
//
...
private:
    ...
    void populateFsCombo() {
        ...
        // Map device -> mountpoint (first mountpoint found)
        ...
        for (const QString &d : devList) {
            ...
            // store "device|mountpoint" (mountpoint may be empty)
            QString data = d + "|" + mnt;
