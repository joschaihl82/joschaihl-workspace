// kernel/kernel.c
// Merged, patched kernel with corrected ISR stubs and keyboard buffer + kgetc
#include <stdint.h>
#include <stddef.h>
#include <stdarg.h>
...
/* --- Forward declarations to avoid implicit declarations / conflicts --- */
...
static inline uint8_t serial_inb(uint16_t port) {
    return inb_port(port);
}

/* Initialize COM1: 115200 8N1, enable FIFO, clear interrupts */
void serial_init(void) {
    /* Disable interrupts */
...
static void serial_wait_tx(void) {
    while ((serial_inb(SERIAL_PORT_BASE + 5) & 0x20) == 0) {
...
static void pic_remap(void) {
    ...
    outb_port(PIC1_DATA, 0x20); /* PIC1 vector offset = 0x20 */
    outb_port(PIC2_DATA, 0x28); /* PIC2 vector offset = 0x28 */
...
/* NOTE: do NOT use cli/sti inside ISR; ISR already runs with interrupts disabled */
...
/* Blocking getchar for kernel: waits until a character is available and returns it.
   Implementation strategy:
   - If buffer has data, disable interrupts, pop and return.
   - Otherwise enable interrupts and execute HLT to wait for next interrupt.
*/
int kgetc(void) {
    char c;
    while (1) {
        ...
        /* no data: enable interrupts and halt until next interrupt (keyboard will wake us) */
        __asm__ volatile ("sti");
...
int kgets(char *buf, int maxlen) {
    ...
    while (i < maxlen - 1) {
        ...
        if (ch == '\n') {
            ...
            if (i > 0) {
                i--;
                /* move cursor back and erase visually */
...
void keyboard_handler_c(void) {
    ...
    if (sc < 128) {
        ...
        if (c) {
            /* push into keyboard buffer (do not call draw_char here) */
            kb_put(c);
        }
    }
    /* Acknowledge PIC */
...
__attribute__((naked)) void isr_keyboard_stub(void) {
    __asm__ volatile (
        ...
        /* Align stack to 16 bytes for the call (interrupt entry misaligns it) */
        "sub $8, %rsp\n\t"
...
/* Helper macro-like patterns implemented manually below */

/* Vectors without error code: 0,1,2,3,4,5,6,7,9,15,16,18,19,20,22,23,24,25,26,27,28,30,31 */
/* Vectors with error code: 8,10,11,12,13,14,17,21,29 */
...
/* vector 8 has error code (double fault uses IST but still handle) */
...
void init_pic_idt_keyboard(void) {
    ...
    outb_port(PIC1_DATA, 0xFD); /* 11111101 -> unmask IRQ1 only on master */
    outb_port(PIC2_DATA, 0xFF); /* mask all on slave */
...
void init_gdt_tss() {
    /* Zero entire GDT table first */
    memset(&gdt_table, 0, sizeof(gdt_table));
    memset(&kernel_tss, 0, sizeof(kernel_tss));

    /* Code descriptor (64-bit code segment) */
    gdt_table.code.access = 0x9A; /* present, ring0, code, exec/read */
    gdt_table.code.gran   = 0x20; /* L bit = 1 (64-bit) */

    /* Data descriptor (flat data) */
    gdt_table.data.access = 0x92; /* present, ring0, data, read/write */
    ...
    gdt_table.tss.low.access    = 0x89; /* present, type = 9 (available 64-bit TSS) */
    ...
    /* Reload segment registers: set DS/ES/SS to data selector (0x10) and reload CS via far return */
    __asm__ volatile (
        "mov $0x10, %%ax\n\t"
        "mov %%ax, %%ds\n\t"
        "mov %%ax, %%es\n\t"
...
/* --- draw_char now echoes to serial as well --- */
void draw_char(char c) {
    /* Echo to serial port */
    if (c == '\r') {
        serial_putc('\n');
...
void kmain(KernelGOPInfo *kgi) {
    ...
    /* init serial for debug/console output */
    serial_init();
    ...
    /* Example usage: echo characters read via kgetc() in main loop */
    while (1) {
        int ch = kgetc(); /* blocks until a key is pressed */
        if (ch >= 0) {
            if (ch == '\r') ch = '\n';
            draw_char((char)ch);
        }
