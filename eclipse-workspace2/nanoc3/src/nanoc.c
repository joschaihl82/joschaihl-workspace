...
static void codegen_lvalue_address(ASTNode *node, Function *f) {
	if (node->kind == AST_IDENT) {
		Var *v = node->var_def;
		if (!v)
			error_at(node->tok->loc, "Undefined variable in lvalue");
...
static void codegen_expr(ASTNode *node, Function *f) {
	...
	default:
		error_at(node->tok ? node->tok->loc : current_input,
				"Unhandled expression kind %d", node->kind);
	}
}
...
static void codegen_stmt(ASTNode *node, Function *f) {
	...
	case AST_COMPOUND:
		for (int i = 0; i < node->child_count; i++)
			codegen_stmt(node->children[i], f);
		break;
	case AST_VAR_DECL:
	...
	case AST_IF: {
		...
		printf(".Lelse%d:\n", lbl_else);
		if (node->child_count > 2 && node->children[2])
			codegen_stmt(node->children[2], f);
		printf(".Lend%d:\n", lbl_end);
		break;
	}
	case AST_WHILE: {
		...
		if (node->child_count > 0 && node->children[0]) {
			if (node->children[0]->kind == AST_VAR_DECL) {
				...
				if (node->children[0]->child_count > 0
					...
					ASTNode *ident = new_node(AST_IDENT,
							node->children[0]->tok);
	...
	case AST_BREAK:
		if (f->label_break < 0)
			error_at(node->tok->loc, "break not within loop");
		printf("  jmp .Lend%d\n", f->label_break);
		break;
	case AST_CONTINUE:
		if (f->label_continue < 0)
			error_at(node->tok->loc, "continue not within loop");
		printf("  jmp .Lbegin%d\n", f->label_continue);
		break;
	case AST_RETURN:
		if (node->child_count > 0 && node->children[0]) {
	...
	default:
		error_at(node->tok ? node->tok->loc : current_input,
				"Unhandled statement kind %d", node->kind);
	}
}
...
static void print_ast(ASTNode *n, int depth) {
	...
	if (n->tok && n->tok->str)
		printf(" (%s)", n->tok->str);
	if (n->kind == AST_NUM)
		printf(" num=%ld", n->num_val);
	if (n->kind == AST_STRING)
		printf(" str_id=%ld", n->num_val);
	printf("\n");
