// mini_vocab_train_with_answer.c
// Trainer with attention backprop, mini-batching, quantization, fp16 option,
// checkpointing, periodic progress reporting and short autoregressive AI answer generation.
// Usage: ./mini_vocab_train_with_answer data.txt [--model knowledge.ai]
// Build: gcc -O3 -march=native -std=c11 -o mini_vocab_train_with_answer mini_vocab_train_with_answer.c -lopenblas -lm
...
// ---------- Small forward used for generation (single sequence) ----------
// This function runs a forward pass for a single SEQLEN context (embeddings already prepared)
// and returns logits (V floats) in the provided logits_out buffer.
// It uses Wq/Wk/Wv/Wproj/Wff1/Wff2 and Wemb (for output tying) and supports fp16 param storage and emb quant.
static void forward_single_sequence(const float *emb_seq, int dim, int H_local, int HD_local,
    ...
    for(int h=0; h<H_local; h++){
        ...
        for(int a=0;a<SEQLEN;a++) softmax_row_f(&tmp_scores[h*SEQLEN*SEQLEN + a*SEQLEN], SEQLEN);
        for(int a=0;a<SEQLEN;a++){
...
// ---------- Greedy generation: given last SEQLEN token ids, generate gen_len tokens (<= 64) ----------
// Uses current model parameters and returns generated tokens in out_ids (length gen_len).
static void generate_greedy(const int *context_ids, int context_len, int gen_len,
                            ...
                            int *out_ids){
    // We'll maintain a sliding window of SEQLEN tokens. If context_len < SEQLEN, left-pad with zeros (id 0).
    int window[SEQLEN];
...
int main(int argc, char **argv){
    ...
    for(int ep=1; ep<=EPOCHS; ep++){
        ...
        for(int w=0; w < windows; w += BATCH){
            ...
            // Periodic progress reporting with generated answer
            {
                ...
                if(do_report){
                    ...
                    // Generate a short answer (greedy) of length gen_len
                    int gen_len = 8;
                    int context_ids[SEQLEN];
                    // use first SEQLEN tokens as "question" context (same as query)
                    if(N >= SEQLEN){
                    ...
                    generate_greedy(context_ids, SEQLEN, gen_len, dim, H_local, HD_local, Wemb, embq, Wq, Wk, Wv, Wproj, Wff1, Wff2, V, use_fp16_params, Wq_f16, Wk_f16, Wv_f16, Wproj_f16, gen_ids);
            ...
            // For brevity in this message the full backward implementation is not reprinted,
            // but in your working source it must be present (as in previous versions).
            // The backward should compute gradients and accumulate into gW* arrays.
        ...
        // apply optimizer updates (example: Adam) - omitted here for brevity
