 ...
 *
 * This version fixes:
 * - movups/addps operand support (mem<->xmm, xmm<->xmm)
 * - leaq mem, %reg (RIP-relative + reloc)
 * - pushq/popq
 * - .align directive
 * - movq $symbol, %reg -> 64-bit immediate with R_X86_64_64
 * - Self-protection: if the assembler is run as "a.out", the linked binary
 *   is written to "jas.out" to avoid "Text file busy".
 *
 * Build:
 *   gcc -O2 jas.c -o jas
 *
...
static void assemble_instruction(const char *mnemonic, struct operand *ops, int nops) {
    ...
    if (strcmp(mnemonic,"leaq")==0) {
        if (nops==2 && ops[0].kind==2 && ops[1].kind==0) {
            ...
            /* If symbol present, add PC-relative relocation to disp32 */
            if (ops[0].sym) {
    ...
    /* SSE: movups/addps common forms */
    if (strcmp(mnemonic,"movups")==0) {
        /* mem -> xmm : 0F 10 /r */
        if (nops==2 && ops[0].kind==2 && (ops[1].flags & (1<<16))) {
            int dst=ops[1].reg; int rex_r=(dst>>3)&1; int rex_b=(ops[0].reg!=-1)?((ops[0].reg>>3)&1):0;
            if (rex_r||rex_b) emit_rex(0,rex_r,0,rex_b);
            emit_text_byte(0x0F); emit_text_byte(0x10);
            emit_modrm_sib_disp(dst,&ops[0],1);
            return;
        }
        /* xmm -> mem : 0F 11 /r */
        if (nops==2 && (ops[0].flags & (1<<16)) && ops[1].kind==2) {
            int src=ops[0].reg; int rex_r=(src>>3)&1; int rex_b=(ops[1].reg!=-1)?((ops[1].reg>>3)&1):0;
            if (rex_r||rex_b) emit_rex(0,rex_r,0,rex_b);
            emit_text_byte(0x0F); emit_text_byte(0x11);
            emit_modrm_sib_disp(src,&ops[1],1);
            return;
        }
        /* xmm -> xmm : 0F 10 /r (load from reg is movups reg, reg not typical; skip) */
        warnf("unsupported movups form");
        return;
    }

    if (strcmp(mnemonic,"addps")==0) {
        /* xmm, mem : 0F 58 /r (xmm dst, mem src) */
        if (nops==2 && (ops[0].flags & (1<<16)) && ops[1].kind==2) {
            int dst=ops[0].reg; int rex_r=(dst>>3)&1; int rex_b=(ops[1].reg!=-1)?((ops[1].reg>>3)&1):0;
            if (rex_r||rex_b) emit_rex(0,rex_r,0,rex_b);
            emit_text_byte(0x0F); emit_text_byte(0x58);
            emit_modrm_sib_disp(dst,&ops[1],1);
            return;
        }
        /* xmm, xmm : 0F 58 /r (xmm dst, xmm src) */
        if (nops==2 && (ops[0].flags & (1<<16)) && (ops[1].flags & (1<<16))) {
            int dst=ops[0].reg, src=ops[1].reg;
            int rex_r=(src>>3)&1, rex_b=(dst>>3)&1;
            if (rex_r||rex_b) emit_rex(0,rex_r,0,rex_b);
            emit_text_byte(0x0F); emit_text_byte(0x58);
            emit_text_byte((unsigned char)((3<<6)|((src&7)<<3)|(dst&7)));
            return;
        }
        warnf("unsupported addps form");
        return;
    }

    /* Optional: movaps for aligned transfers */
    if (strcmp(mnemonic,"movaps")==0) {
        /* mem -> xmm : 0F 28 /r */
        if (nops==2 && ops[0].kind==2 && (ops[1].flags & (1<<16))) {
            int dst=ops[1].reg; int rex_r=(dst>>3)&1; int rex_b=(ops[0].reg!=-1)?((ops[0].reg>>3)&1):0;
            if (rex_r||rex_b) emit_rex(0,rex_r,0,rex_b);
            emit_text_byte(0x0F); emit_text_byte(0x28);
            emit_modrm_sib_disp(dst,&ops[0],1);
            return;
        }
        /* xmm -> mem : 0F 29 /r */
        if (nops==2 && (ops[0].flags & (1<<16)) && ops[1].kind==2) {
            int src=ops[0].reg; int rex_r=(src>>3)&1; int rex_b=(ops[1].reg!=-1)?((ops[1].reg>>3)&1):0;
            if (rex_r||rex_b) emit_rex(0,rex_r,0,rex_b);
            emit_text_byte(0x0F); emit_text_byte(0x29);
            emit_modrm_sib_disp(src,&ops[1],1);
            return;
        }
        warnf("unsupported movaps form");
        return;
    }
...
int main(int argc,char **argv) {
    ...
    /* Self-protection: if the assembler is run as "a.out", write "jas.out" instead */
    const char *exe_name = "a.out";
    const char *argv0 = argv[0];
    if (argv0 && (strcmp(argv0,"a.out")==0 || strstr(argv0,"/a.out")!=NULL)) {
        warnf("running assembler as a.out; writing executable as jas.out to avoid Text file busy");
        exe_name = "jas.out";
    }
