/* play_ss2000_grow_improved.c
   Play yelp first, then wail. Gain increases linearly each cycle.
   Compile: cc -O2 -std=c11 -lasound -lm -o play_ss2000_grow_improved play_ss2000_grow_improved.c
*/
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
/* synth a mono block into a float buffer */
static int synth_block(double (*freq_fn)(double, void*), void *ctx, double dur, int sr, float **outbuf, size_t *out_frames) {
    ...
    for (size_t i = 0; i < frames; ++i) {
        ...
        phase += incr;
        /* keep phase bounded but avoid expensive ops each sample */
...
static int alsa_write_frames(snd_pcm_t *pcm, const float *mono, size_t frames) {
    ...
    while (written < frames) {
        ...
        if (r == -EPIPE) {
            fprintf(stderr,"ALSA underrun, preparing\n");
            snd_pcm_prepare(pcm);
            continue;
        }
        if (r == -ESTRPIPE) {
            fprintf(stderr,"ALSA suspended, recovering\n");
            int rec = snd_pcm_recover(pcm, (int)r, 1);
            if (rec < 0) { fprintf(stderr,"snd_pcm_recover failed: %s\n", snd_strerror(rec)); return -1; }
            continue;
        }
        if (r < 0) {
            int rec = snd_pcm_recover(pcm, (int)r, 1);
            if (rec < 0) { fprintf(stderr,"snd_pcm_writei: %s\n", snd_strerror((int)r)); return -1; }
            continue;
        }
        /* r might be less than n; advance by r */
        written += (size_t)r;
    }
    return 0;
}
...
static int wail(snd_pcm_t *pcm, ctx_siren *ctx, unsigned fade_ms, double gain) {
    float *buf = NULL; size_t frames = 0;
    if (synth_block(freq_wail, ctx, WAIL_DUR, SR, &buf, &frames) != 0) return -1;
    if (!buf) return -1;
    for (size_t i = 0; i < frames; ++i) buf[i] *= (float)gain;
...
static int yelp(snd_pcm_t *pcm, ctx_siren *ctx, unsigned fade_ms, double gain) {
    float *buf = NULL; size_t frames = 0;
    if (synth_block(freq_yelp, ctx, YELP_DUR, SR, &buf, &frames) != 0) return -1;
    if (!buf) return -1;
    for (size_t i = 0; i < frames; ++i) buf[i] *= (float)gain;
    apply_fade_in_out(buf, frames, SR, fade_ms);
    int rc = alsa_write_frames(pcm, buf, frames);
