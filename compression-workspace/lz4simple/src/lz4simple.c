...
dec_read_ext_len(const uint8_t **ip, const uint8_t *end, size_t initial)
{
    ...
    if (initial == TOKEN_MAX_NIBBLE) {
        do {
            if (*ip >= end) {
                DEC_TRAP("ext length read past end");
                return 0;
            }
            s = *(*ip)++;
            len += s;
...
dec_copy_match(uint8_t *out, size_t opos, size_t off, size_t m_len, size_t cap)
{
    ...
    if (off == 0 || off > opos || opos + m_len > cap) {
        DEC_TRAP("invalid match copy bounds/offset");
        return -1;
    }
    for (i = 0; i < m_len; ++i)
...
lz4_dec(const uint8_t *in, size_t n, uint8_t *out, size_t cap)
{
    ...
    while (ip < end) {
        ...
        if (l_len == 0 && (tok >> 4) == TOKEN_MAX_NIBBLE && ip > end) {
            DEC_TRAP("literal ext truncated");
            return 0;
        }
        if (opos + l_len > cap) {
            DEC_TRAP("literal output overflow");
            return (0);
        }
        if ((size_t)(end - ip) < l_len) {
            DEC_TRAP("literal input underflow");
            return 0;
        }
        if (l_len > 0) {
        ...
        if ((size_t)(end - ip) < 2) {
            DEC_TRAP("missing match offset bytes");
            return 0;
        }
        ...
        /* read extension for match length if needed */
        {
            ...
            if (ext == 0 && (m_len - MIN_MAT) == TOKEN_MAX_NIBBLE && ip > end) {
                DEC_TRAP("match length ext truncated");
                return 0;
            }
            m_len = ext + MIN_MAT;
        }
...
blk_proc(io_ctx_t *c, FILE *out, int is_comp)
{
    ...
    if (res == 0) {
        if (!is_comp) {
            DEC_TRAP("block decode returned zero");
        }
        return (-1);
    }
    ...
    if (is_comp) {
        ...
        if (io_write(fileno(out), c->out, res)) {
            DEC_TRAP("write decoded block failed");
            return (-1);
        }
    }
    return (0);
...
file_dec_process_blocks(FILE *arc, const char *out, uint32_t c_sz)
{
    ...
    while (rem > 0) {
        if (io_rd_u32(arc, &b_sz)) {
            DEC_TRAP("failed to read block size");
            break;
        }
        if (b_sz > rem) {
            DEC_TRAP("block size exceeds remaining");
            break;
        }
        rem -= 4;
        c.c_len = b_sz;
        if (io_read(arc, c.in, b_sz) != b_sz) {
            DEC_TRAP("failed to read block payload");
            break;
        }
        rem -= b_sz;
