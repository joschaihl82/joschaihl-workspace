...
static void cg_emit_expr(CodeGen *cg, Node *n) {
    ...
    case ND_CALL: {
        ...
        if (call->call_target->kind == ND_MEMBER_ACCESS &&
            ...
            // 1. Lese die aktuelle Größe (Index des neuen Elements)
            cg_emit(cg, "    mov Lglob_%d(%%rip), %%rcx", size_gi); // %rcx = current size/new index

            // 2. Prüfe auf Kapazität
            cg_emit(cg, "    cmp $%d, %%rcx", MAX_ARRAY_SIZE);
            ...
            cg_emit(cg, "    mov %%rcx, %%r9");  // %r9 = index
            ...
            cg_emit(cg, "    mov (%%r9), %%r10"); // TEMP: Lade den Wert des Slots (optional)
            cg_emit(cg, "    mov %%rax, (%%r9)"); // Wert in den neuen Slot schreiben

            // KORREKTUR: Inkrementiere die Größe Lglob_SIZE (Load-Inc-Store)
            // Lade: %r10 = Lglob_SIZE
            cg_emit(cg, "    mov Lglob_%d(%%rip), %%r10", size_gi);
            // Inkrementiere: %r10 = %r10 + 1
            cg_emit(cg, "    inc %%r10");
        ...
        if (g_current_func_scope) {
            ...
            if (init_kind == ND_OBJ_LITERAL || init_kind == ND_ARRAY_LITERAL) {
                ...
                if (init_kind == ND_ARRAY_LITERAL) {
                    ...
                    int size_gi = add_global(size_name);
                    cg_emit(cg, "    mov $%d, Lglob_%d(%%rip)", n->init->kv_count, size_gi);
                }
