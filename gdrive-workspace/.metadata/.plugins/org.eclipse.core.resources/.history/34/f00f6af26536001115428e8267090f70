...
// --- DownloadWorker (seriell) mit Resume-Logik basierend auf Local/Remote-Größe ---
...
public slots:
    void startDownloads(const QJsonArray &queue, const QString &token, const QString &syncPath) {
        ...
        for (const QJsonValue &v : queue) {
            ...
            if (localFile.exists()) {
                localSize = QFileInfo(localFile).size();
            }

            // If remote size known and equal to local, skip
            ...
            // If local larger than remote (corrupt/changed), remove local and download fresh
            if (okSize && localSize > size) {
            ...
            // Check if server supports range requests via HEAD
            ...
            headReq.setRawHeader("Authorization", QString("Bearer %1").arg(token).toUtf8());
            headReq.setHeader(QNetworkRequest::UserAgentHeader, "DriveSync-Qt6");

            QNetworkReply *headReply = manager->head(headReq);
            QEventLoop headLoop;
            connect(headReply, &QNetworkReply::finished, &headLoop, &QEventLoop::quit);
            headLoop.exec();

            bool acceptRanges = false;
            if (headReply->error() == QNetworkReply::NoError) {
                QByteArray ar = headReply->rawHeader("Accept-Ranges");
                if (!ar.isEmpty() && ar.toLower().contains("bytes")) acceptRanges = true;
            }
            headReply->deleteLater();

            // Decide whether to resume or download full
            bool willResume = (localSize > 0 && acceptRanges && okSize && localSize < size);

            // If local exists but remote size unknown, try to resume only if acceptRanges true
            if (localSize > 0 && !okSize && !acceptRanges) {
                // cannot safely resume, re-download
                emit downloadStatus(QStringLiteral("Remote-Größe unbekannt und kein Resume möglich, lade komplett neu: %1").arg(name));
                QFile::remove(outPath);
                localSize = 0;
                willResume = false;
            }

            // If local exists and localSize < remote but server doesn't support ranges, re-download
            if (localSize > 0 && okSize && localSize < size && !acceptRanges) {
                emit downloadStatus(QStringLiteral("Server unterstützt kein Resume, lade komplett neu: %1").arg(name));
                QFile::remove(outPath);
                localSize = 0;
                willResume = false;
            }
            ...
            req.setHeader(QNetworkRequest::UserAgentHeader, "DriveSync-Qt6");
            if (willResume) {
                QByteArray range = QByteArray("bytes=") + QByteArray::number(localSize) + QByteArray("-");
                req.setRawHeader("Range", range);
                emit downloadStatus(QStringLiteral("Fortsetze: %1 (ab %2 Bytes)").arg(name).arg(localSize));
            } else {
            ...
            QFile outFile;
            if (willResume) {
                outFile.setFileName(outPath + ".part");
                if (!outFile.open(QIODevice::ReadWrite)) {
                    // fallback: remove and download fresh
                    emit downloadStatus(QStringLiteral("Fehler beim Öffnen zum Fortsetzen, lade neu: %1").arg(outPath));
                    QFile::remove(outPath);
                    outFile.setFileName(outPath + ".part");
                    if (!outFile.open(QIODevice::WriteOnly)) {
                        emit downloadStatus(QStringLiteral("Fehler beim Öffnen (neu): %1").arg(outPath));
                        continue;
                    }
                    localSize = 0;
                    willResume = false;
            ...
            // If resuming, we should account already downloaded bytes
            if (willResume) downloadedBytes += localSize;

            QNetworkReply *reply = manager->get(req);

            qint64 downloadedForFile = localSize;

            // readyRead handler writes directly to file
            ...
            // If server ignored Range and returned 200 while we attempted resume, restart full download
            if (willResume && statusCode == 200) {
                // close and remove partial, then re-download full
                outFile.close();
                QFile::remove(outPath + ".part");
                emit downloadStatus(QStringLiteral("Server lieferte kompletten Inhalt statt Partial; lade komplett neu: %1").arg(name));

                // perform full download (simple GET without Range)
                QNetworkRequest req2(url);
                req2.setRawHeader("Authorization", QString("Bearer %1").arg(token).toUtf8());
                req2.setHeader(QNetworkRequest::UserAgentHeader, "DriveSync-Qt6");

                QFile outFile2(outPath + ".part");
                ...
                qint64 downloadedForFile2 = 0;
                connect(reply2, &QNetworkReply::readyRead, this, [&outFile2, &downloadedForFile2, &downloadedBytes, name, size, &timer, totalBytes, this]() mutable {
                    QByteArray chunk = reply2->readAll();
                    if (!chunk.isEmpty()) {
                        outFile2.write(chunk);
                        downloadedForFile2 += chunk.size();
                        downloadedBytes += chunk.size();
                        qint64 reportedTotal = (size > 0) ? size : downloadedForFile2;
                        emit fileProgress(name, downloadedForFile2, reportedTotal);
                ...
                if (reply2->error() != QNetworkReply::NoError) {
                    emit downloadStatus(QStringLiteral("Fehler beim Herunterladen (neu): %1 - %2").arg(name, reply2->errorString()));
                    outFile2.close();
                    outFile2.remove();
                    reply2->deleteLater();
                    reply->deleteLater();
            ...
            // Normal completion for initial attempt (either resumed with 206 or full with 200)
            outFile.flush();
            outFile.close();

            // If we used a .part file, rename to final
            QFile::remove(outPath);
            QFile::rename(outPath + ".part", outPath);
