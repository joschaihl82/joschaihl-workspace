...
uint8_t heap[0x1000000] __attribute__((aligned(4096)));
...
void map_mmio_range(uint64_t phys_addr) {
    /* Map a 1GB PDPT entry for the given physical address.
       This is a minimal helper used by the simple kernel mapping approach here. */
    uint64_t idx = (phys_addr >> 30) & 0x1FF;
    pdpt[idx] = phys_addr | PAGE_PRESENT | PAGE_RW | PAGE_PCD | PAGE_PS;
}
...
void fb_scroll() {
    /* Scroll framebuffer up by one line_height and clear bottom lines.
       After scrolling, place cursor at the last visible line. */
    ...
    /* Place cursor at the last visible text line */
...
static void nvme_wait(nvme_queue_t *q) {
    /* Wait for completion entry with expected phase */
...
int nvme_init(nvme_ctrl_t *ctrl) {
    uint32_t bar0 = 0;
    /* Find NVMe PCI device (class/subclass/prog-if = 0x01/0x08/0x02) */
    for (int b = 0; b < 256; b++) {
        for (int s = 0; s < 32; s++) {
...
found_nvme:
    ...
    /* Zero controller structure before use */
    ...
    /* Doorbell base and offsets (use volatile pointers) */
    ...
    /* Create IO completion queue (admin command 1) */
    ...
    *ctrl->admin_q.sq_doorbell = 1;
    ...
    /* Create IO submission queue (admin command 1) */
...
void kmain(KernelGOPInfo *kgi) {
    ...
    if (nvme_init(&ctrl) == 0) {
        ...
        k_printf("NVMe initialization failed.\n");
    }
    while (1) { __asm__("hlt"); }
}
