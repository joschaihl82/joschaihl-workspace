...
/* ---------- NUCLEAR CONTRAST SETTINGS ---------- */
static const float SEPIA_HUE_DEG = 20.0f;
static const float SEPIA_SAT_MULT = 0.65f;
static const float SEPIA_VAL_MULT = 1.25f;
static const float GHOST_ALPHA = 0.25f;
static const float HSVA_EXPOSURE = 1.2f;

/* Contrast v3.0 - NUCLEAR MODE */
static const float CONTRAST_EMA_ALPHA = 0.15f;
static const float TARGET_STD_MULT = 2.5f;
static const float CONTRAST_GAIN_MIN = 1.0f;        /* Never reduce contrast */
static const float CONTRAST_GAIN_MAX = 5.0f;        /* Maximum contrast boost */
static const float CONTRAST_APPLY_CLAMP = 0.92f;    /* Very aggressive highlight clamping */
static const float HISTORY_DECAY = 0.40f;
...
/* YUYV -> RGB24 - FIXED: added bounds checking */
static void yuyv_to_rgb24(const unsigned char *yuyv, unsigned char *rgb, int w, int h) {
    ...
    for (int j = 0; j < h; ++j) {
        for (int i = 0; i < w; i += 2) {
            ...
            if ((p + 3 - yuyv) > (ssize_t)(w * h * 2) || 
                (q + 5 - rgb) > (ssize_t)rgb_size) {
...
static int mjpeg_to_rgb24_reuse(const unsigned char *mjpeg, size_t mjpeg_len, unsigned char *rgb, int expected_w, int expected_h, unsigned char *rowbuf, size_t rowbuf_sz, int *out_w, int *out_h) {
    ...
    size_t required_rgb_size = (size_t)w * (size_t)h * 3;
    if (mul_will_overflow_size_t((size_t)w, (size_t)h) || 
        mul_will_overflow_size_t((size_t)w * (size_t)h, 3)) {
    ...
    while (cinfo.output_scanline < cinfo.output_height) {
        ...
        if (dst_off + (size_t)w * 3 <= required_rgb_size) {
...
int main(int argc, char **argv) {
    ...
    st.fd = -1;
    st.started_stream = 0; // Initialize
    ...
    if (fd < 0) { 
        ...
        close(selfpipe_fds[0]); 
        close(selfpipe_fds[1]); 
        return 1; 
    }
    st.fd = fd;
    ...
    st.win = SDL_CreateWindow("CIA GhostViewer v25.0 - HSVA 4.0 Sepia + Contrast v3.0", 
    ...
    if (!st.ren) { 
        st.ren = SDL_CreateRenderer(st.win, -1, 0); 
        if (!st.ren) { 
    ...
    if (mul_will_overflow_size_t(pixels, (size_t)3)) { 
    ...
    if (!st.frame_rgb || !st.history_rgb) { 
        fprintf(stderr, "alloc frame/history failed\n"); 
        cleanup_state(&st, &req); 
        close(selfpipe_fds[0]); 
        close(selfpipe_fds[1]); 
        return 1; 
    ...
    if (!st.jpeg_rowbuf) { 
        fprintf(stderr, "alloc jpeg rowbuf failed\n"); 
        cleanup_state(&st, &req); 
        close(selfpipe_fds[0]); 
        close(selfpipe_fds[1]); 
        return 1; 
    ...
    while (!quit) {
        ...
        if (!skip_present) {
            ...
            if (st.tex_bpp == 3) {
                ...
                if (mul_will_overflow_size_t(pixels_count, 4)) {
                    fprintf(stderr, "texture buffer size overflow\n");
                } else {
                    ...
                    if (tmp) {
                        ...
                        free(tmp); // FIXED: Free the temporary buffer
                    } else {
