/*
 ...
 - All file writes are flushed with fflush() immediately after writing
...
void visited_save(visited_set_t *vs, const char *fname){
    ...
    pthread_mutex_lock(&vs->lock);
    for(size_t i=0;i<VISITED_BUCKETS;i++){
        visited_node_t *n = vs->buckets[i];
        while(n){
            fprintf(f, "%s\n", n->url);
            fflush(f);
            n = n->next;
...
void hashset_load(hash_set_t *hs, const char *fname){
    ...
    while(fgets(buf, sizeof(buf), f)){
        buf[strcspn(buf, "\r\n")] = 0;
        if(strlen(buf) == 64) hashset_insert_no_persist(hs, buf);
    }
    fclose(f);
}
