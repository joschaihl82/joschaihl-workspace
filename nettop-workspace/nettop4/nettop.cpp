// nettop.cpp
// Enhanced nettop with PID and ProcessName as first two columns,
// improved UI, details pane, aggregation, theming, and UX polish.
...
// ---------- SparklineWidget for details dialog ----------
class SparklineWidget : public QWidget {
    Q_OBJECT
public:
    SparklineWidget(QWidget *parent = nullptr) : QWidget(parent) {
        setMinimumHeight(48);
    }

    void addValue(double v) {
        m_values.push_back(v);
        if (m_values.size() > 100) m_values.erase(m_values.begin());
        update();
    }

    void setValues(const QVector<double> &vals) {
        m_values = vals.toStdVector();
        update();
    }
...
// ---------- Row delegate for nicer rendering ----------
...
public:
    ...
    void paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const override {
        ...
        // Input MB is column 4 in source model (PID=0, ProcessName=1, AppName=2, Path=3, InputMB=4)
...
public:
    ...
    void setField(const QString &label, const QString &value) {
        QLabel *l = new QLabel(value, this);
        m_form->addRow(label, l);
    }

    void setSparkValues(const QVector<double> &vals) {
        m_spark->setValues(vals);
    }
...
private slots:
    void onRecordReady(const QStringList &fields) {
        // Buffer incoming records and apply in batches to avoid UI thrash
        QMutexLocker locker(&m_bufferMutex);
        m_buffer.append(fields);
        if (!m_batchTimer->isActive()) m_batchTimer->start(200); // coalesce 200ms
    }
    ...
    void flushBuffer() {
        ...
        for (const QStringList &fields : batch) {
            ...
            // Derive ApplicationName from path/program string
            ...
            m_model->item(row, 0)->setText(pidStr);
            m_model->item(row, 1)->setText(procName);
    ...
    void onRowDoubleClicked(const QModelIndex &idx) {
        ...
        QString pid = m_model->item(src.row(), 0)->text();
        QString proc = m_model->item(src.row(), 1)->text();
...
private:
    void setupUi() {
        ...
        m_tableFilter->setPlaceholderText(QStringLiteral("Search table..."));
        layout->addWidget(m_tableFilter);

        // Table and model
        m_view = new QTableView(central);
        // Source model has 9 columns now (PID, ProcessName, ApplicationName, Path, Input MB, Output MB, Input KB/s, Output KB/s, LastSeen)
        m_model = new QStandardItemModel(0, 9, this);
        QStringList headers = {"PID", "ProcessName", "ApplicationName", "/path/programname", "Input MB", "Output MB", "Input KB/s", "Output KB/s", "LastSeen"};
        ...
        // Defaults
        setWindowTitle(QStringLiteral("nettop - enhanced"));
        resize(1200, 800);
